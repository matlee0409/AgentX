#!/usr/bin/env node
// set-exe-identity.cjs — stamp the AgentX icon + version metadata onto the
// built AgentX.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked AgentX.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.cjs <path-to-AgentX.exe>
//
