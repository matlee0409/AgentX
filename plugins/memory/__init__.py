...
_USER_NAMESPACE = "_agentx_user_memory"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_agentx_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_agentx_user_memory'`` — the
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _agentx_user_memory.<name>.cli, usually
