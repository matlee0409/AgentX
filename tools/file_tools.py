...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``agentx_constants.get_subprocess_home()`` so that ``~`` resolves
...
_agentx_config_resolved: str | None = None
_agentx_config_resolved_loaded = False


def _get_agentx_config_resolved() -> str | None:
    """Return the resolved absolute path of the AgentX config file (cached)."""
    global _agentx_config_resolved, _agentx_config_resolved_loaded
    if _agentx_config_resolved_loaded:
        return _agentx_config_resolved
    _agentx_config_resolved_loaded = True
    try:
        from agentx_cli.config import get_config_path
        _agentx_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _agentx_config_resolved = str(Path(_expand_tilde("~/.agentx/config.yaml")).resolve())
        except Exception:
            _agentx_config_resolved = None
    return _agentx_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    # this file.
    agentx_config = _get_agentx_config_resolved()
    if agentx_config and (resolved == agentx_config or normalized == agentx_config):
        return (
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with agentx_tools.write_file() for very "
            "large files."
        )
