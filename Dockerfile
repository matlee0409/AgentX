...
RUN set -eu; \
    ...
    # 'AgentX WebUI' catalog). The image has moved to s6-overlay /init\
...
#
# ui-tui/packages/agentx-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/agentx-ink/ ui-tui/packages/agentx-ink/
...
# Keep /opt/agentx immutable for the runtime agentx user. Hosted/container
...
# supervised AgentX processes drop to the non-root agentx user.
USER root
RUN mkdir -p /opt/agentx/bin && \
    ...
    chown -R root:root /opt/agentx && \
    chmod -R a+rX /opt/agentx && \
    chmod -R a-w /opt/agentx
...
# /opt/agentx/.agentx_build_sha at build time, and have
# agentx_cli/build_info.py read it at runtime.  Both `agentx dump` and
...
RUN if [ -n "${AGENTX_GIT_SHA}" ]; then \
        chmod u+w /opt/agentx && \
        printf '%s\n' "${AGENTX_GIT_SHA}" > /opt/agentx/.agentx_build_sha && \
        chmod a-w /opt/agentx /opt/agentx/.agentx_build_sha; \
    fi
...
# ---------- Runtime ----------
ENV AGENTX_WEB_DIST=/opt/agentx/agentx_cli/web_dist
...
ENTRYPOINT [ "/init", "/opt/agentx/docker/main-wrapper.sh" ]
CMD [ ]
