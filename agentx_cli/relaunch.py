...
from agentx_cli._parser import (
...
def resolve_agentx_bin() -> Optional[str]:
    ...
    ``agentx.exe`` on PATH or the ``sys.executable -m agentx_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
    ...
    *,
    ...
    Args:
        ...
        original_argv: The original argv to scan for flags (defaults to
            ``sys.argv[1:]``).
    """
    bin_path = resolve_agentx_bin()

    if bin_path:
        argv = [bin_path]
    else:
        argv = [sys.executable, "-m", "agentx_cli.main"]
...
def relaunch(
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    ``python -m agentx_cli.main``) or a ``.cmd`` batch file, and both
