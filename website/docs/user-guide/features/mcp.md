...
description: "Connect AgentX Agent to external tool servers via MCP — and control exactly which MCP tools AgentX loads"
---

# MCP (Model Context Protocol)

MCP lets AgentX Agent connect to external tool servers so the agent can use tools that live outside AgentX itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.
...
Manifests live at
[`optional-mcps/<name>/manifest.yaml`](https://github.com/NousResearch/agentx-agent/tree/main/optional-mcps)
...
[`optional-mcps/`](https://github.com/NousResearch/agentx-agent/tree/main/optional-mcps).
...
The MCP server reads conversation data directly from AgentX's session store (`~/.agentx/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the AgentX agent itself.
