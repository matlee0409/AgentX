...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._session_path: Path = Path(config.extra.get(
            "session_path",
            get_agentx_dir("platforms/whatsapp/session", "whatsapp/session")
        ))
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        # clear "run agentx whatsapp" message instead of the watcher
        ...
        if not creds_path.exists():
            logger.warning(
                ...
                "Run `agentx whatsapp` to pair, or remove WHATSAPP_ENABLED from "
                "your .env to disable.",
                self.name, creds_path,
            )
            self._set_fatal_error(
                "whatsapp_not_paired",
                "WhatsApp enabled but not paired — run `agentx whatsapp` to pair.",
                retryable=False,
            )
            return False
        ...
        try:
            ...
            bridge_env = with_agentx_node_path()
...
# dict in agentx_cli/gateway.py, and the _send_whatsapp dispatch in
...
def interactive_setup() -> None:
    ...
    Replaces the central _setup_whatsapp in agentx_cli/gateway.py and the
...
def _is_connected(config) -> bool:
    ...
    if config is not None and getattr(config, "enabled", False) and extra:
        ...
        return True
    # Read via agentx_cli.gateway.get_env_value (not os.getenv) so setup-status
