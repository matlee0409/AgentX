...
description: "Complete reference of all environment variables used by AgentX Agent"
---
...
| `AGENTX_LOCAL_STT_LANGUAGE` | Default language passed to `AGENTX_LOCAL_STT_COMMAND` or auto-detected local `whisper` CLI fallback (default: `en`) |
...
| `AGENTX_KANBAN_HOME` | Override the shared AgentX root that anchors the kanban board (db + workspaces + worker logs). Falls back to `get_default_agentx_root()` (the parent of any active profile). Useful for tests and unusual deployments |
...
| `AGENTX_GATEWAY_PLATFORM_CONNECT_TIMEOUT` | Per-platform connect timeout during gateway startup (seconds). |
...
| `AGENTX_CRON_SCRIPT_TIMEOUT` | Timeout for pre-run scripts attached to cron jobs in seconds (default: `120`). Override for scripts that need longer execution (e.g., randomized delays for anti-bot timing). Also configurable via `cron.script_timeout_seconds` in `config.yaml`. |
...
| Variable | Description |
|----------|-------------|
| `AGENTX_MAX_ITERATIONS` | Max tool-calling iterations per conversation (default: 90) |
...
| `AGENTX_SAFE_MODE` | Troubleshooting mode: disable ALL customizations — skips plugin discovery and MCP server loading. Set automatically by `--safe-mode` (which also sets the two flags above). |
| `AGENTX_MD_NAMES` | Comma-separated list of rules-file names to auto-inject (default: `AGENTS.md,CLAUDE.md,.cursorrules,SOUL.md`). |
| `AGENTX_TOOL_PROGRESS` | Deprecated compatibility variable for tool progress display. Prefer `display.tool_progress` in `config.yaml`. |
| `AGENTX_TOOL_PROGRESS_MODE` | Deprecated compatibility variable for tool progress mode. Prefer `display.tool_progress` in `config.yaml`. |
...
| `AGENTX_KANBAN_TASK` | Set by the kanban dispatcher when spawning a worker (task UUID). Workers and the spawned `agentx-tools` MCP subprocess inherit it so kanban tools gate correctly. Don't set manually. |
...
| `AGENTX_WRITE_SAFE_ROOT` | Optional directory prefix that restricts `write_file`/`patch` writes; paths outside require approval. |
...
| `AGENTX_CORE_TOOLS` | Comma-separated override for the canonical core tool list (advanced; rarely needed). |
...
| `AGENTX_OAUTH_FILE` | Override the path used for OAuth credential storage (default: `~/.agentx/auth.json`). |
