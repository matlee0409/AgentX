...
description: "如何为 AgentX Agent 构建 memory provider 插件"
---

# 构建 Memory Provider 插件

Memory provider 插件为 AgentX Agent 提供跨会话的持久化知识，超越内置的 MEMORY.md 和 USER.md。本指南介绍如何构建一个 memory provider 插件。
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs 始终包含：
          agentx_home (str): 当前活跃的 AGENTX_HOME 路径。用于存储数据。
        """
...
| `save_config(values, agentx_home)` | 将非敏感配置写入原生位置 | **是**（除非仅使用环境变量） |
...
所有存储路径**必须**使用 `initialize()` 中的 `agentx_home` kwarg，而不是硬编码的 `~/.agentx`：
