...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of AgentX's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple AgentX instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
agentx memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `agentx` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — AgentX syncs them back at session init.
...
The `agentx honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `agentx memory setup honcho` (or run `agentx memory setup` and pick it from the list); the `agentx honcho` subcommand then appears on the next invocation.

```bash
agentx memory setup honcho    # Configure Honcho directly (works before activation)
agentx honcho status          # Connection status, config, and key settings
agentx honcho setup           # Redirects to `agentx memory setup` (post-activation alias)
agentx honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
agentx honcho peer            # Show or update peer names + dialectic reasoning level
agentx honcho mode            # Show or set recall mode (hybrid/context/tools)
agentx honcho tokens          # Show or set token budget for context and dialectic
agentx honcho identity        # Seed or show the AI peer's Honcho identity
agentx honcho sync            # Sync Honcho config to all existing profiles
agentx honcho peers           # Show peer identities across all profiles
agentx honcho sessions        # List known Honcho session mappings
agentx honcho map             # Map current directory to a Honcho session name
agentx honcho enable          # Enable Honcho for the active profile
agentx honcho disable         # Disable Honcho for the active profile
agentx honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `agentx honcho`

If you previously used the standalone `agentx honcho setup`:
...
No re-login or re-setup needed. Run `agentx memory setup` and select "honcho" — the wizard detects your existing config.
