...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._dedup_state_path = get_agentx_home() / "feishu_seen_message_ids.json"
    ...
    async def send_exec_approval(
        ...
        The buttons carry ``agentx_action`` in their value dict so that
        ...
        try:
            ...
            def _btn(label: str, action_name: str, btn_type: str = "default") -> dict:
                return {
                    ...
                    "type": btn_type,
                    "value": {"agentx_action": action_name, "approval_id": approval_id},
                }
    ...
    def _build_update_prompt_card(*, prompt: str, default: str, prompt_id: int) -> Dict[str, Any]:
        ...
        def _btn(label: str, answer: str, btn_type: str) -> dict:
            return {
                ...
                "type": btn_type,
                "value": {
                    "agentx_update_prompt_action": answer,
    ...
    def _write_update_prompt_response(answer: str) -> None:
        response_path = get_agentx_home() / ".update_response"
    ...
    def _on_card_action_trigger(self, data: Any) -> Any:
        ...
        agentx_action = action_value.get("agentx_action") if isinstance(action_value, dict) else None
        update_prompt_action = (
            action_value.get("agentx_update_prompt_action")
    ...
    def _handle_approval_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        if not state:
            ...
            return P2CardActionTriggerResponse() if P2CardActionTriggerResponse else None
        choice = _APPROVAL_CHOICE_MAP.get(action_value.get("agentx_action"), "deny")
    ...
    def _handle_update_prompt_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        answer = str(action_value.get("agentx_update_prompt_action", "") or "").strip().lower()
        ...
        *,
        ...
        async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
            response = await client.get(
                file_url,
                headers={
                    "User-Agent": "Mozilla/5.0 (compatible; AgentXAgent/1.0)",
                    "Accept": "*/*",
                },
            )
...
# Called by `agentx gateway setup` via _setup_feishu() in agentx_cli/gateway.py.
# =============================================================================
...
# dict in agentx_cli/gateway.py, and the _send_feishu dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    """
    if not FEISHU_AVAILABLE:
        return {"error": "Feishu dependencies not installed. Run: pip install 'agentx-agent[feishu]'"}
...
def interactive_setup() -> None:
    ...
    Replaces the central _setup_feishu in agentx_cli/gateway.py and the static
    _PLATFORMS["feishu"] dict. CLI helpers are lazy-imported.
    """
    from agentx_cli.config import get_env_value, save_env_value
    from agentx_cli.setup import prompt_choice
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'agentx-agent[feishu]'",
