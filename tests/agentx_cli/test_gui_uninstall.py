"""Tests for agentx_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import agentx_cli.gui_uninstall as gu


def _make_agent(agentx_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = agentx_home / "agentx-agent"
    (agent_root / "agentx_cli").mkdir(parents=True)
    (agent_root / "agentx_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(agentx_home: Path) -> None:
    """Create the source-built GUI artifacts a `agentx desktop` run produces."""
    desktop = agentx_home / "agentx-agent" / "apps" / "desktop"
    ...
    (agentx_home / "agentx-agent" / "node_modules").mkdir(parents=True)
    (agentx_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(agentx_home: Path) -> None:
    (agentx_home / "config.yaml").write_text("x: 1\n")
    (agentx_home / ".env").write_text("KEY=secret\n")
    (agentx_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    agentx_home = tmp_path / ".agentx"
    agentx_home.mkdir()
    assert gu.agent_is_installed(agentx_home) is False
    _make_agent(agentx_home)
    assert gu.agent_is_installed(agentx_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    agentx_home = tmp_path / ".agentx"
    (agentx_home / "agentx-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(agentx_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    agentx_home = tmp_path / ".agentx"
    _make_gui_build(agentx_home)
    artifacts = gu.source_built_gui_artifacts(agentx_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(agentx_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(agentx_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    agentx_home = tmp_path / ".agentx"
    agent_root = _make_agent(agentx_home)
    _make_gui_build(agentx_home)
    _make_user_data(agentx_home)
    ...
    removed = gu.uninstall_gui(agentx_home)
    ...
    assert not (agentx_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "agentx_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (agentx_home / "config.yaml").exists()
    assert (agentx_home / ".env").exists()
    assert (agentx_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    agentx_home = tmp_path / ".agentx"
    _make_agent(agentx_home)
    userdata = tmp_path / "AgentX-userdata"
    userdata.mkdir()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(agentx_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(agentx_home)
    ...
    assert summary["agentx_home"] == str(agentx_home)
    assert summary["platform"] == sys.platform


def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "AgentX"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "AgentX"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "AgentX"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "AgentX"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    AGENTX_HOME with all the destructive externals stubbed out.
    """
    import agentx_cli.uninstall as uninstall
    ...
    monkeypatch.setattr(uninstall, "get_agentx_home", lambda: agentx_home)
    ...
    from agentx_cli import gui_uninstall as gu_mod
    ...
    assert not (desktop / "release").exists()
    assert (agentx_home / "config.yaml").exists()
    assert agentx_home.exists()


def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole AGENTX_HOME non-interactively."""
    import agentx_cli.uninstall as uninstall

    agentx_home = tmp_path / ".agentx"
    (agentx_home / "agentx-agent" / "agentx_cli").mkdir(parents=True)
    (agentx_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not agentx_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m agentx_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_agentx_home", lambda: agentx_home)
    ...
    assert not (desktop / "release").exists()
    assert not (agentx_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "agentx_cli").exists()
    assert (agentx_home / "config.yaml").exists()
