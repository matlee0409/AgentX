...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from agentx_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    agentx_home = tmp_path / "classic"
    agentx_home.mkdir()
    monkeypatch.setenv("AGENTX_HOME", str(agentx_home))

    _write(agentx_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (AGENTX_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.agentx as the "real home" — which is NOT equal
    # to the AGENTX_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    agentx_home = tmp_path / "classic"
    ...
    _write(agentx_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from agentx_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: AGENTX_HOME is set to a custom path that is NOT under
    # ~/.agentx/profiles/ — get_default_agentx_root() returns AGENTX_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from agentx_cli.auth import read_credential_pool, write_credential_pool
