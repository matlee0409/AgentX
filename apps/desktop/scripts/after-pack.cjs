 ...
 *
 * Stamps the AgentX icon + identity onto the packed Windows AgentX.exe via
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'AgentX')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'AgentX'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); AgentX.exe keeps the stock Electron icon`)
  }
}
