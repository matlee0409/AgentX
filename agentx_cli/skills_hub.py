...
def do_install(identifier: str, category: str = "", force: bool = False,
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                ...
                f"Files will be at: [cyan]{display_agentx_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
    ...
    # metadata.agentx.blueprint block, it is a runnable automation. Register it as
    ...
    if target == "github":
        ...
        if not auth.is_authenticated():
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    f"Set GITHUB_TOKEN in {display_agentx_home()}/.env or run 'gh auth login'.\n")
            return
...
def skills_command(args) -> None:
    """Router for `agentx skills <subcommand>` — called from agentx_cli/main.py."""
