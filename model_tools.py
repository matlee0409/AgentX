...
#   - cli.py, agentx_cli/*      -> inline on startup (no event loop)
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("AGENTX_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                if toolset_name.startswith("agentx-"):
                    # Platform bundles (agentx-*) include _AGENTX_CORE_TOOLS, so
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from agentx_cli.config import load_config as _load
        cfg = _load() or {}
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from agentx_cli.plugins import has_hook, invoke_hook
...
def handle_function_call(
    ...
    if not skip_tool_request_middleware:
        try:
            from agentx_cli.middleware import apply_tool_request_middleware
    ...
    try:
        ...
        try:
            ...
            from agentx_cli.middleware import run_tool_execution_middleware
