...
| `agentx secrets bitwarden status` | Show config + binary version + token presence |
...
| `cache_ttl_seconds` | `300` | How long an in-process fetch result is reused. Set to `0` to disable caching. Cache is per-process; new `agentx` invocations start fresh. |
