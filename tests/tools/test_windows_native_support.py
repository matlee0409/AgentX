...
class TestConfigureWindowsStdio:
    """``agentx_cli.stdio.configure_windows_stdio`` wiring.
    ...
    def _reset_configured(self, monkeypatch):
        ...
        sys.modules.pop("agentx_cli.stdio", None)
        # Fresh import now; tests import from agentx_cli.stdio themselves,
        # but this guarantees the module they get is a brand-new copy.
        import agentx_cli.stdio as _s
        _s._CONFIGURED = False
        yield
        sys.modules.pop("agentx_cli.stdio", None)

    def test_no_op_on_posix(self):
        from agentx_cli import stdio
    ...
    def test_windows_path_sets_env_and_reconfigures_streams(self, monkeypatch):
        ...
        monkeypatch.delenv("AGENTX_DISABLE_WINDOWS_UTF8", raising=False)
    ...
    def test_disable_flag_short_circuits(self, monkeypatch, optout):
        ...
        monkeypatch.setattr(stdio, "is_windows", lambda: True)
        monkeypatch.setenv("AGENTX_DISABLE_WINDOWS_UTF8", optout)
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("agentx_cli.kanban_db", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "agentx_cli" / "web_server.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    """cli.py, agentx_cli/main.py, gateway/run.py must call configure_windows_stdio."""

    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "agentx_cli/main.py", "gateway/run.py"],
    )
    def test_entry_point_calls_configure_stdio(self, relpath):
        root = Path(__file__).resolve().parents[2]
        source = (root / relpath).read_text(encoding="utf-8")
        assert "configure_windows_stdio" in source, (
            f"{relpath} must call agentx_cli.stdio.configure_windows_stdio() "
...
class TestSubprocessCompatHelpers:
    """agentx_cli/_subprocess_compat.py POSIX + Windows behaviour."""

    def test_is_windows_matches_sys_platform(self):
        from agentx_cli import _subprocess_compat as sc
    ...
    def test_resolve_node_command_returns_absolute_on_posix(self):
        ...
        from agentx_cli._subprocess_compat import resolve_node_command
...
# ---------------------------------------------------------------------------
# agentx_cli/kanban_db.py waitpid guard
# ---------------------------------------------------------------------------
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "agentx_cli" / "kanban_db.py").read_text(encoding="utf-8")
...
class TestNpmBareSpawnsResolved:
    ...
    shutil.which / agentx_cli._subprocess_compat.resolve_node_command
    so Windows can execute the .cmd batch shims."""

    @pytest.mark.parametrize(
        "relpath",
        [
            "agentx_cli/tools_config.py",
            "agentx_cli/doctor.py",
...
class TestLocalEnvironmentWindowsTempDir:
    ...
    def test_source_has_windows_branch_using_agentx_home(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "tools" / "environments" / "local.py").read_text(encoding="utf-8")
        assert "if _IS_WINDOWS:" in source
        assert "get_agentx_home" in source
        assert 'cache_dir = get_agentx_home() / "cache" / "terminal"' in source
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_agentx_cli_gateway_uses_compat_kwargs(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "agentx_cli" / "gateway.py").read_text(encoding="utf-8")
        assert "windows_detach_popen_kwargs" in source, (
            "agentx_cli/gateway.py must use the platform-aware detach helper"
        )
    ...
    def test_launch_detached_profile_gateway_restart_inlined_watcher_uses_breakaway(self):
        ...
        agentx_cli._subprocess_compat; the watcher must call that helper from
        ...
        """
        root = Path(__file__).resolve().parents[2]
        text = (root / "agentx_cli" / "gateway.py").read_text(encoding="utf-8")
        ...
        block = text[idx:end]
        assert "from agentx_cli._subprocess_compat import" in block
    ...
    def test_watcher_rewrites_console_python_to_windowless(self):
        ...
        assert '_popen_kwargs["env"]' in block, (
            ...
            "AGENTX_HOME) so the windowless base pythonw resolves agentx_cli."
        )
...
class TestWindowlessGatewayRestartSpec:
    ...
    def test_noop_on_non_windows(self):
        import agentx_cli.gateway_windows as gw

        argv = ["/path/venv/bin/python", "-m", "agentx_cli.main", "gateway", "run"]
    ...
    def test_windows_rewrites_to_pythonw_and_preserves_tail(self):
        ...
        # ``from agentx_cli.gateway import PROJECT_ROOT`` resolves from
        ...
        import agentx_cli.config  # noqa: F401
        ...
        # Mock get_agentx_home too: the real one calls Path.resolve(), which
        ...
        with mock.patch.object(gw.sys, "platform", "win32"), mock.patch.object(
            ...
            "agentx_cli.config.get_agentx_home", return_value="C:/agentx"
        ):
            new_argv, cwd, env = gw.windowless_gateway_restart_spec(list(argv))
