# nix/packages.nix — AgentX Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    { pkgs, lib, inputs', ... }:
    let
      agentxAgent = pkgs.callPackage ./agentx-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = agentxAgent.agentxTui;
        web = agentxAgent.agentxWeb;
        desktop = agentxAgent.agentxDesktop;
      };
    };
}
