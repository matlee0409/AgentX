...
const DEFAULT_REPLY_PREFIX = '⚕ *AgentX Agent*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['AgentX Agent', 'Chrome', '120.0'],
  ...
  try {
    ...
    switch (type) {
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `agentx_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execSync(
