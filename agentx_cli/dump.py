...
from agentx_cli.config import get_agentx_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.agentx_build_sha`` by the Dockerfile's
    ``AGENTX_GIT_SHA`` build-arg (see ``agentx_cli/build_info.py``).
...
def _gateway_status() -> str:
    ...
    try:
        from agentx_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(agentx_home: Path) -> int:
...
def _cron_summary(agentx_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = agentx_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"agentx_home:      {display_agentx_home()}")
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(agentx_home)}")
    lines.append(f"  skills:             {_count_skills(agentx_home)}")
