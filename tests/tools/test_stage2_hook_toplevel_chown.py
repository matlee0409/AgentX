"""Contract test: the s6-overlay stage2 hook resets ownership of agentx-owned
top-level state files in $AGENTX_HOME — but only those, never arbitrary
host-owned files.

Regression guard for the gateway restart loop reported in #35098: files such
as gateway.lock / state.db / auth.json live directly under $AGENTX_HOME (not in
a subdir), so the targeted subdir chown misses them. When created or rewritten
by `docker exec <container> agentx …` (root unless `-u` is passed) they land
root-owned and the unprivileged agentx runtime then hits PermissionError on next
startup.
...
bind-mounted $AGENTX_HOME may contain host-owned files AgentX does not manage,
and those must never be chowned.

The s6-overlay rework moved bootstrap from docker/entrypoint.sh (now a shim) to
docker/stage2-hook.sh, installed as /etc/cont-init.d/01-agentx-setup. This test
...
def _toplevel_chown_loop(text: str) -> str:
    """Extract the `for f in … chown agentx:agentx "$AGENTX_HOME/$f" … done`
    ...
    block = m.group(1)
    assert 'chown agentx:agentx "$AGENTX_HOME/$f"' in block, (
        "the top-level-file loop must chown each allowlisted file to agentx"
    )
    return block
...
def test_no_blanket_find_user_root_sweep(stage2_text: str) -> None:
    ...
    $AGENTX_HOME contents — that would clobber host-owned files in a bind mount
    (#19788 / PR #19795)."""
    assert not re.search(r"find\s+\"?\$\{?AGENTX_HOME\}?\"?[^\n]*-user\s+root", stage2_text), (
        "stage2-hook.sh must not blanket-chown root-owned files under "
        "$AGENTX_HOME via `find -user root`; use the targeted allowlist instead "
        "so host-owned bind-mounted files are preserved (#19788, #19795)."
    )


def _run_loop(text: str, present_files: list[str]) -> list[str]:
    """Run the extracted chown loop in a sandbox $AGENTX_HOME, with `chown`
...
def test_loop_skips_allowlisted_file_under_symlinked_home(
    ...
    """A symlinked $AGENTX_HOME must not let file chown reach its target."""
    bash = shutil.which("bash")
    if bash is None:
        pytest.skip("bash not available")
    block = _toplevel_chown_loop(stage2_text)
    ...
    proc = subprocess.run([bash, str(script_path)], capture_output=True, text=True)
    assert proc.returncode == 0, proc.stderr
    assert not log.exists(), "must not chown files through symlinked AGENTX_HOME"
    assert "refusing chown through symlinked path" in proc.stdout
