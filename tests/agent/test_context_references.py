...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_agentx_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("AGENTX_HOME", str(tmp_path / ".agentx"))

    agentx_env = tmp_path / ".agentx" / ".env"
    agentx_env.parent.mkdir(parents=True)
    agentx_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.agentx/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
