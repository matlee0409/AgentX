...
// no longer bundles the AgentX Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'AgentX.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'AgentX'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'AgentX.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'agentx'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default AGENTX_HOME for non-sandboxed runs -- matches main.cjs's
// resolveAgentXHome(). On Windows it's %LOCALAPPDATA%\agentx; elsewhere
...
function launchFresh() {
  ...
  const agentxHome = path.join(sandbox, 'agentx-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(agentxHome, { recursive: true })
  ...
  console.log(`  AGENTX_HOME: ${agentxHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(agentxHome, 'agentx-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The AgentX Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of agentx_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'agentx-agent', 'agentx_cli', 'main.py')
