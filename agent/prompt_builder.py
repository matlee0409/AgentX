...
from agentx_constants import get_agentx_home, get_skills_dir, is_wsl
...
def _find_agentx_md(cwd: Path) -> Optional[Path]:
...
DEFAULT_AGENT_IDENTITY = (
    "You are AgentX Agent, an intelligent AI assistant created by Nous Research. "
...
AGENTX_AGENT_HELP_GUIDANCE = (
    ...
    "the documentation at https://github.com/matlee0409/AgentX/wiki is your "
...
def build_environment_hints() -> str:
    ...
    # AgentX desktop GUI — any agent running under the desktop app should know
    # it. AGENTX_DESKTOP marks the backend powering the chat; AGENTX_DESKTOP_TERMINAL
    # marks a agentx launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    _in_desktop = (os.getenv("AGENTX_DESKTOP") or "").strip().lower() in _truthy
    _in_desktop_term = (os.getenv("AGENTX_DESKTOP_TERMINAL") or "").strip().lower() in _truthy
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the AgentX desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
...
def _skills_prompt_snapshot_path() -> Path:
    return get_agentx_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    _platform_hint = (
        os.environ.get("AGENTX_PLATFORM")
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot AgentX Agent itself — its CLI, config, models, providers, tools, "
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    ...
    """
    try:
        from agentx_cli.config import ensure_agentx_home
        ensure_agentx_home()
    except Exception as e:
        logger.debug("Could not ensure AGENTX_HOME before loading SOUL.md: %s", e)

    soul_path = get_agentx_home() / "SOUL.md"
...
def _load_agentx_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.agentx.md / AGENTX.md — walk to git root."""
    agentx_md_path = _find_agentx_md(cwd_path)
    if not agentx_md_path:
        return ""
    try:
        content = agentx_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = agentx_md_path.name
        try:
            rel = str(agentx_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        ...
        return _truncate_content(
            ...
            read_path=str(agentx_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", agentx_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    project_context = (
        _load_agentx_md(cwd_path, context_length)
