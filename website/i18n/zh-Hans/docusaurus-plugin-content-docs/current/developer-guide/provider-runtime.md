...
- `agentx_cli/runtime_provider.py` — 凭据解析，`_resolve_custom_runtime()`
- `agentx_cli/auth.py` — provider 注册表，`resolve_provider()`
- `agentx_cli/model_switch.py` — 共享 `/model` 切换流水线（CLI + gateway）
...
AgentX 包含相关逻辑，以避免在存在多个 provider 密钥时（例如同时存在 `OPENROUTER_API_KEY` 和 `OPENAI_API_KEY`）将错误的 API key 泄露给自定义端点。
