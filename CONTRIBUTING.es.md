# Contribuir a AgentX Agent

¡Gracias por contribuir a AgentX Agent! Esta guía cubre todo lo que necesitas: configurar tu entorno de desarrollo, entender la arquitectura, decidir qué construir y conseguir que tu PR sea aceptado.
...
- Implementan el mismo ABC `MemoryProvider` (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown` y opcionalmente `post_setup(agentx_home, config)` para integración con el asistente de configuración
...
├── cli.py                    # Clase AgentXCLI — TUI interactiva, integración prompt_toolkit
...
├── agentx_state.py           # Base de datos de sesiones SQLite con búsqueda de texto completo FTS5, títulos de sesión
...
│
├── agentx_cli/               # Implementaciones de comandos CLI
...
├── website/                  # Sitio de documentación (agentx-agent.nousresearch.com)
│
...
- **Persistencia de sesión**: Todas las conversaciones se almacenan en SQLite (`agentx_state.py`) con búsqueda de texto completo y títulos de sesión únicos.
...
Añade al dict `_BUILTIN_SKINS` en `agentx_cli/skin_engine.py`. Usa el mismo esquema que arriba pero como dict de Python.
...
- Usa [GitHub Issues](https://github.com/NousResearch/agentx-agent/issues)
