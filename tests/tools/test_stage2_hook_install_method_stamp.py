...
into the shared $AGENTX_HOME, and must heal a stale 'docker' stamp left there
by older images.

Background (shared-$AGENTX_HOME bug)
------------------------------------
$AGENTX_HOME (/opt/data) is a DATA volume that users commonly bind-mount from
the host (``~/.agentx:/opt/data``) and sometimes share with a host-side
Desktop/CLI install. Older images wrote ``printf 'docker' > $AGENTX_HOME/.install_method``
at boot, which clobbered the host install's own marker — so the host's in-app
updater read 'docker' and refused to run ``agentx update`` ("doesn't apply
inside the Docker container").

The fix scopes the stamp to the install tree (baked at
``/opt/agentx/.install_method`` in the Dockerfile, read first by
``detect_install_method``). stage2 must therefore:

  * NOT write the 'docker' stamp into $AGENTX_HOME any more, and
  * proactively remove a stale 'docker' stamp from $AGENTX_HOME so homes
...
def test_stage2_does_not_write_install_method_into_home(stage2_text: str) -> None:
    ...
    assert not re.search(
        r"(tee|>)\s*\"?\$AGENTX_HOME/\.install_method", stage2_text
    ), (
        "stage2 must not stamp $AGENTX_HOME/.install_method — that data dir "
...
def test_stage2_heals_stale_docker_home_stamp(stage2_text: str) -> None:
    # It must remove a stale 'docker' stamp from $AGENTX_HOME so already
    # poisoned shared homes recover.
    assert 'rm -f "$AGENTX_HOME/.install_method"' in stage2_text, (
        "stage2 must remove a stale 'docker' stamp from $AGENTX_HOME to heal "
