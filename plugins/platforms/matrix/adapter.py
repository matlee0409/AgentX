...
# Uses get_agentx_home() so each profile gets its own Matrix store.
from agentx_constants import get_agentx_dir as _get_agentx_dir

_STORE_DIR = _get_agentx_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "AgentX Agent" become "Agent".
        """
        if not body:
            return ""
...
# + _PLATFORMS["matrix"] static dict in agentx_cli/{setup,gateway}.py, and the
...
def interactive_setup() -> None:
    """Configure Matrix credentials. Replaces agentx_cli/setup.py::_setup_matrix
...
def _is_connected(config) -> bool:
    ...
    configured. Read via agentx_cli.gateway.get_env_value so setup-status
