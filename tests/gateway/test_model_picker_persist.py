...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(gateway_run, "_agentx_home", agentx_home)
    ...
    monkeypatch.setattr(
        "agentx_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_agentx_home() / config.yaml`` — point it here.
    monkeypatch.setattr("agentx_constants.get_agentx_home", lambda: agentx_home)
    monkeypatch.setattr("agentx_cli.config.get_agentx_home", lambda: agentx_home)
    return cfg_path
