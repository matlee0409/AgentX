...
agentx cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/agentx-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/agentx-agent --state open --json number,title,labels,author,createdAt --limit 30
...
agentx cron create "0 9 * * 1" \
  "Scan the NousResearch/agentx-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/agentx-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (agentx_cli/commands.py, agentx_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (agentx_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
agentx cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to AgentX Agent development." \
