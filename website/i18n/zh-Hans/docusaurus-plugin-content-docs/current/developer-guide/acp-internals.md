...
Zed ACP Registry 路径通过 `uvx --from 'agentx-agent[acp]==<version>' agentx-acp` 启动同一适配器，指向 `agentx-agent` PyPI 发布包。
...
因此 ACP 通告并使用当前配置的 AgentX provider/凭据。它还始终通告一个终端 setup 认证方法（`agentx-setup`，参数 `--setup`），以便首次运行的 registry 客户端在启动正常 ACP 会话前可以打开 AgentX 的交互式模型/provider 配置。
...
- `agentx_cli/main.py` — `agentx acp` CLI 子命令
