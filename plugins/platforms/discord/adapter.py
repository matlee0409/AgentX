...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_agentx_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="update", description="Update AgentX Agent to the latest version")
        ...
        # agentx_cli/commands.py automatically appear as Discord slash
        ...
        dropped_over_cap = 0
        try:
            from agentx_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from agentx_cli.commands import _iter_plugin_command_entries
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from agentx_cli.commands import discord_skill_commands_by_category
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by AgentX: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``agentx_cli.gateway.get_env_value`` at call time (not via
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'agentx-agent[messaging]'",
        # Interactive setup wizard — replaces the central
        # agentx_cli/setup.py::_setup_discord function.  Same shape as Teams.
