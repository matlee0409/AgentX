...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._agentx_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(agentx_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(agentx_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_agentx_bin(self):
        """After _INSTALL_FAILED, manual install in AGENTX_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        agentx_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(agentx_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(agentx_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._agentx_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == agentx_bin
            assert _tirith_mod._resolved_path == agentx_bin
            mock_clear.assert_called_once()
...
class TestAgentXHomeIsolation:
    def test_agentx_bin_dir_respects_agentx_home(self):
        """_agentx_bin_dir must use AGENTX_HOME, not hardcoded ~/.agentx."""
        from tools.tirith_security import _agentx_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"AGENTX_HOME": tmpdir}):
            result = _agentx_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_agentx_home(self):
        """_failure_marker_path must use AGENTX_HOME, not hardcoded ~/.agentx."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"AGENTX_HOME": "/custom/agentx"}):
            result = _failure_marker_path()
        assert result == "/custom/agentx/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets AGENTX_HOME; verify it's active."""
        agentx_home = os.getenv("AGENTX_HOME")
        assert agentx_home is not None, "AGENTX_HOME should be set by conftest"
        assert "agentx_test" in agentx_home, "Should point to test temp dir"

    def test_get_agentx_home_fallback(self):
        """Without AGENTX_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_agentx_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove AGENTX_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".agentx")
            result = _get_agentx_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._agentx_bin_dir",
