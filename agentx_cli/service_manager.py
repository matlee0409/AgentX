...
the existing module-level functions in agentx_cli.gateway and
agentx_cli.gateway_windows directly. This protocol is a thin facade
...
# in ``agentx_cli.gateway`` (systemd/launchd) and ``agentx_cli.gateway_windows``
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in agentx_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from agentx_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from agentx_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from agentx_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from agentx_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in agentx_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from agentx_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from agentx_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from agentx_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from agentx_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``agentx_cli.gateway_windows`` (Scheduled Task /
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    agentx_home = Path(os.environ.get("AGENTX_HOME", "/opt/data"))
    if agentx_home.parent.name == "profiles":
        root = agentx_home.parent.parent
    else:
        root = agentx_home
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        ``agentx_cli.gateway`` maps that empty string to the
        ...
        allocator in ``agentx_cli.profiles._allocate_gateway_port``).
    ...
    def _render_log_run(profile: str) -> str:
        ...
        return (
            ...
            # later then runs its log service as agentx and crash-loops on
            ...
            f'chown agentx:agentx "$AGENTX_HOME/logs/gateways" 2>/dev/null || true\n'
            f'chown -R agentx:agentx "$log_dir" 2>/dev/null || true\n'
            f'rm -f "$log_dir/lock"\n'
