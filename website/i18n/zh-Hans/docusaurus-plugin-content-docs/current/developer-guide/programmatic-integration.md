...
Pi-mono RPC 规范（[issue #360](https://github.com/NousResearch/agentx-agent/issues/360)）中的每条命令均有对应的 TUI gateway 等价项：
...
内置 provider 感知解析（相同的模型名称会根据当前 provider 自动选择正确格式）。参见 `agentx_cli/model_switch.py`。
