...
# and exposes ``window.__agentxMeetDrain()`` to pull new entries. This
# mirrors the OpenUtter caption scraping approach.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__agentxMeetInstalled) return;
  window.__agentxMeetInstalled = true;
  window.__agentxMeetQueue = [];
  ...
  window.__agentxMeetDrain = () => {
    const out = window.__agentxMeetQueue.slice();
    window.__agentxMeetQueue = [];
    return out;
  };
})();
...
def _start_realtime_speaker(
    *,
    ...
    if platform_tag == "linux":
        ...
        sink = (bridge_info or {}).get("write_target") or "agentx_meet_sink"
        try:
            proc = _sp.Popen(
                [
                    "paplay",
...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    ...
    guest_name = os.environ.get("AGENTX_MEET_GUEST_NAME", "AgentX Agent")
    ...
    try:
        with sync_playwright() as pw:
            ...
            while not stop_flag["stop"]:
                ...
                try:
                    queued = page.evaluate("window.__agentxMeetDrain && window.__agentxMeetDrain()")
...
def _detect_admission(page) -> bool:
    ...
    (() => {
      ...
      if (window.__agentxMeetInstalled) {
