...
The actual runtime resolution happens in agentx_cli.runtime_provider's
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        try:
            from agentx_cli.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the agentx-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "agentx-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            if "agentx-tools" in mig_report.migrated:
                msg_lines.append(
