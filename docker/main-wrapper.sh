...
# `#!/bin/sh` wrapper sees an empty environ and `ENV AGENTX_HOME=/opt/data`
# from the Dockerfile never reaches `agentx`. with-contenv repopulates
# the env from /run/s6/container_environment before exec'ing, which is
# what s6-supervised services use too (see main-agentx/run).
#
# Routing:
...
_agentx_orig_cwd="${AGENTX_ORIG_CWD:-$PWD}"
