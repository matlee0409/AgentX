...
Nous can attribute usage to AgentX Agent and bucket it by client release.
...
The version is sourced live from ``agentx_cli.__version__`` so it auto-aligns
...
``agentx_cli.__version__`` is the canonical source of truth.
"""
...
def _agentx_version() -> str:
    ...
    Falls back to ``"unknown"`` if ``agentx_cli`` cannot be imported (should
...
def agentx_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=agentx-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=agentx-client-v{_agentx_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=agentx-agent", agentx_client_tag()]
