---
title: "AgentX S6 Container Supervision"
sidebar_label: "AgentX S6 Container Supervision"
description: "Modify, debug, or extend the s6-overlay supervision tree inside the AgentX Agent Docker image — adding new services, debugging profile gateways, understandin..."
---
...
Modify, debug, or extend the s6-overlay supervision tree inside the AgentX Agent Docker image — adding new services, debugging profile gateways, understanding the Architecture B main-program pattern.
...
| Related skills | [`agentx-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-agentx-agent), `agentx-agent-dev` |
...
If you're just running the AgentX Agent and want to use Docker, see `website/docs/user-guide/docker.md` instead.
...
│   │   └── skills_sync.py
│   └── 02-reconcile-profiles          ← agentx_cli.container_boot
...
| Path | Role |
|---|---|
| `Dockerfile` | s6-overlay install + cont-init.d wiring + `ENTRYPOINT ["/init", "/opt/agentx/docker/main-wrapper.sh"]` |
| `docker/stage2-hook.sh` | The "old entrypoint logic" — UID remap, chown, seed, skills sync. Runs as cont-init.d/01-agentx-setup. |
| `docker/cont-init.d/02-reconcile-profiles` | Calls `agentx_cli.container_boot` on every boot to restore profile gateway slots from the persistent volume. |
...
| `agentx_cli/service_manager.py` | `S6ServiceManager`: `register_profile_gateway`, `unregister_profile_gateway`, `start/stop/restart/is_running`, `list_profile_gateways`. |
| `agentx_cli/container_boot.py` | `reconcile_profile_gateways()` — walks persistent profiles, regenerates s6 slots, emits `container-boot.log`. |
| `agentx_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `agentx gateway start/stop/restart` and routes to s6 when running in a container. |
...
So we use the s6-overlay-native CMD pattern: `ENTRYPOINT ["/init", "/opt/agentx/docker/main-wrapper.sh"]`. /init prepends the wrapper to user args automatically — so `docker run <image> --version` becomes `/init main-wrapper.sh --version`, and `--version` doesn't get intercepted by /init's POSIX shell. The wrapper drops to agentx via `s6-setuidgid`, then exec's the chosen program. The program's exit code becomes the container exit code, exactly matching the pre-s6 tini contract.
...
Edit `S6ServiceManager._render_run_script` in `agentx_cli/service_manager.py`. The function is also called by `agentx_cli/container_boot.py::_register_service` during boot reconciliation, so it's the single source of truth. Update the corresponding assertion in `tests/agentx_cli/test_service_manager.py::test_s6_register_creates_service_dir_and_triggers_scan`.
