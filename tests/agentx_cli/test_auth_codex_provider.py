...
def _setup_agentx_auth(agentx_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_agentx_auth(agentx_home)
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_agentx_auth(agentx_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
    ...
    monkeypatch.setattr("agentx_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    agentx_home = tmp_path / "agentx"
    _setup_agentx_auth(agentx_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((agentx_home / "auth.json").read_text())
...
def test_save_codex_tokens_still_refreshes_legacy_manual_alias(tmp_path, monkeypatch):
    ...
    ``agentx auth add openai-codex`` as a workaround when there was no
...
def test_resolve_returns_agentx_auth_store_source(tmp_path, monkeypatch):
    agentx_home = tmp_path / "agentx"
    _setup_agentx_auth(agentx_home)
    monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from agentx_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from agentx_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "agentx_cli.auth.resolve_codex_runtime_credentials",
        lambda: {"base_url": DEFAULT_CODEX_BASE_URL},
    )
    monkeypatch.setattr(
        "agentx_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("agentx_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("agentx_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch agentx_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("agentx_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
