"""Tests for agentx_cli.security_advisories.
...
import agentx_cli.security_advisories as adv
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full agentx_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "agentx_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "agentx_cli.config.save_config",
...
class TestRendering:
    def test_short_banner_lines_includes_id_and_version(self, fake_advisory):
        ...
        assert "agentx doctor" in joined
