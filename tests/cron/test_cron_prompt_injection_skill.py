...
def cron_env(tmp_path, monkeypatch):
    ...
    skills_dir.mkdir()
    (agentx_home / "cron").mkdir()
    (agentx_home / "cron" / "output").mkdir()
    monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
    monkeypatch.setenv("AGENTX_BUNDLES_DIR", str(agentx_home / "skill-bundles"))
    ...
    monkeypatch.setattr(_skills_tool, "AGENTX_HOME", agentx_home)
    ...
    return agentx_home, _scheduler


def _plant_skill(agentx_home: Path, name: str, body: str) -> None:
    """Drop a SKILL.md into ~/.agentx/skills/<name>/ bypassing skills_guard."""
    skill_dir = agentx_home / "skills" / name
...
def _plant_bundle(agentx_home: Path, name: str, skills: list[str], instruction: str = "") -> None:
    """Drop a bundle YAML into ~/.agentx/skill-bundles/ and refresh cache."""
    bundles_dir = agentx_home / "skill-bundles"
...
class TestBuildJobPromptScansSkillContent:
    def test_clean_skill_builds_normally(self, cron_env):
        agentx_home, scheduler = cron_env
        _plant_skill(agentx_home, "news-digest", "Fetch the top 5 headlines and summarize.")
    ...
    def test_skill_with_invisible_unicode_sanitized_not_blocked(self, cron_env):
        ...
        _plant_skill(agentx_home, "zwsp-skill", "clean looking\u200bskill content")
    ...
    def test_skill_bundle_in_job_skills_loads_referenced_skills(self, cron_env):
        agentx_home, scheduler = cron_env
        _plant_skill(agentx_home, "alpha-skill", "Alpha guidance for the cron task.")
        _plant_skill(agentx_home, "beta-skill", "Beta guidance for the cron task.")
        _plant_bundle(
            agentx_home,
            "article-pipeline",
    ...
    def test_bundle_name_shadows_skill_name_for_cron_jobs(self, cron_env):
        agentx_home, scheduler = cron_env
        _plant_skill(agentx_home, "article-pipeline", "Standalone skill should not win.")
        _plant_skill(agentx_home, "bundle-member", "Bundle member should win.")
        _plant_bundle(agentx_home, "article-pipeline", ["bundle-member"])
...
class TestScriptOutputNotStrictScanned:
    ...
    def test_command_shapes_in_context_from_output_not_blocked(self, cron_env, monkeypatch):
        ...
        output_root = agentx_home / "cron" / "output"
