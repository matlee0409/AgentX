...
def _load_plugin_init():
    ...
    spec = importlib.util.spec_from_file_location(
        "agentx_plugins.disk_cleanup",
        plugin_dir / "__init__.py",
        submodule_search_locations=[str(plugin_dir)],
    )
    # Ensure parent namespace package exists for the relative `. import disk_cleanup`
    import types
    if "agentx_plugins" not in sys.modules:
        ns = types.ModuleType("agentx_plugins")
        ns.__path__ = []
        sys.modules["agentx_plugins"] = ns
    mod = importlib.util.module_from_spec(spec)
    mod.__package__ = "agentx_plugins.disk_cleanup"
    mod.__path__ = [str(plugin_dir)]
    sys.modules["agentx_plugins.disk_cleanup"] = mod
    spec.loader.exec_module(mod)
    return mod
...
class TestIsSafePath:
    def test_accepts_path_under_agentx_home(self, _isolate_env):
        dg = _load_lib()
    ...
    def test_rejects_outside_agentx_home(self, _isolate_env):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/etc/passwd")) is False

    def test_accepts_tmp_agentx_prefix(self, _isolate_env, tmp_path):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/tmp/agentx-abc/x.log")) is True
...
class TestTrackForgetQuick:
    ...
    def test_quick_does_not_descend_into_protected_top_level_dirs(
        ...
        dg = _load_lib()
        protected_empty = (
            _isolate_env / "agentx-agent" / "node_modules" / "pkg" / "empty"
        )
        ...
        def guarded_iterdir(path):
            if path == _isolate_env / "agentx-agent":
                raise AssertionError("quick() descended into protected agentx-agent/")
...
class TestBundledDiscovery:
    def _write_enabled_config(self, agentx_home, names):
    ...
    def test_disk_cleanup_discovered_but_not_loaded_by_default(self, _isolate_env):
        ...
        from agentx_cli import plugins as pmod
