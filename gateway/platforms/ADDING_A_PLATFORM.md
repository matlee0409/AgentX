...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `agentx_cli/config.py` so the setup
...
## 12. Status Display (`agentx_cli/status.py`)
...
## 13. Gateway Setup Wizard (`agentx_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ agentx_cli/ toolsets.py \
  --include="*.py" -l | sort -u
