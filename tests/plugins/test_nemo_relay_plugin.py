...
def test_nemo_relay_plugin_is_discoverable_as_bundled_plugin(tmp_path, monkeypatch):
    monkeypatch.setenv("AGENTX_HOME", str(tmp_path / "agentx_test"))
...
def test_nemo_relay_plugin_emits_llm_tool_and_exports_atif(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("AGENTX_NEMO_RELAY_ATOF_ENABLED", "1")
    monkeypatch.setenv("AGENTX_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY", str(tmp_path / "atof"))
    monkeypatch.setenv("AGENTX_NEMO_RELAY_ATIF_ENABLED", "1")
    monkeypatch.setenv("AGENTX_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY", str(tmp_path / "atif"))
...
def test_nemo_relay_plugin_emits_approval_marks(monkeypatch):
    ...
    assert "agentx.approval.request" in mark_names
    assert "agentx.approval.response" in mark_names
...
def test_nemo_relay_plugin_emits_unmatched_fallback_marks(monkeypatch):
    ...
    mark_names = [event[1] for event in fake.events if event[0] == "scope.event"]
    assert "agentx.api.response.unmatched" in mark_names
    assert "agentx.api.error" in mark_names
    assert "agentx.tool.response.unmatched" in mark_names
...
def test_nemo_relay_plugin_metadata_promotes_trajectory_and_subagent_ids(monkeypatch):
    ...
    turn_mark = next(event for event in fake.events if event[0] == "scope.event" and event[1] == "agentx.turn.start")
    ...
    start_mark = next(event for event in fake.events if event[0] == "scope.event" and event[1] == "agentx.subagent.start")
    ...
    stop_mark = next(event for event in fake.events if event[0] == "scope.event" and event[1] == "agentx.subagent.stop")
...
def test_nemo_relay_plugin_reparents_child_session_scope_for_embedded_atif(monkeypatch):
    ...
    child_push = next(
        ...
        if event[0] == "scope.push" and event[1] == "agentx-session-child-session"
    )
    child_kwargs = child_push[3]
    assert child_kwargs["handle"] == ("scope", "agentx-session-parent-session")
...
def test_nemo_relay_plugin_skips_embedded_child_atif_file_by_default(tmp_path, monkeypatch):
    ...
    assert (tmp_path / "atif" / "agentx-atif-parent-session.json").exists()
    assert not (tmp_path / "atif" / "agentx-atif-child-session.json").exists()
...
def test_nemo_relay_plugin_can_write_embedded_child_atif_file_in_all_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("AGENTX_NEMO_RELAY_ATIF_ENABLED", "1")
    monkeypatch.setenv("AGENTX_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY", str(tmp_path / "atif"))
    monkeypatch.setenv("AGENTX_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE", "all")
    ...
    assert (tmp_path / "atif" / "agentx-atif-parent-session.json").exists()
    assert (tmp_path / "atif" / "agentx-atif-child-session.json").exists()
...
""",
    ...
    assert "agentx-session-s2" in scope_push_names
...
""",
    ...
    monkeypatch.setenv("AGENTX_NEMO_RELAY_PLUGINS_TOML", str(plugins_toml))
    monkeypatch.setenv("AGENTX_NEMO_RELAY_ATIF_ENABLED", "1")
    monkeypatch.setenv("AGENTX_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY", str(tmp_path / "direct-atif"))
    ...
    assert not (tmp_path / "direct-atif" / "agentx-atif-s1.json").exists()
...
""",
    ...
    assert "atif.register" in event_names
    assert (tmp_path / "direct-atif" / "agentx-atif-s1.json").exists()
...
""",
    ...
    monkeypatch.setenv("AGENTX_NEMO_RELAY_PLUGINS_TOML", str(plugins_toml))
    monkeypatch.setenv("AGENTX_NEMO_RELAY_ATOF_ENABLED", "1")
    monkeypatch.setenv("AGENTX_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY", str(tmp_path / "direct-atof"))
...
def test_nemo_relay_downstream_unwrap_matches_real_middleware_wrapper_shape(monkeypatch):
    ...
    # ``agentx_cli.middleware._run_execution_chain`` hands to a middleware
    ...
    from agentx_cli import middleware
