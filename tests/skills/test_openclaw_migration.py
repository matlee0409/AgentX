...
def load_module():
    spec = importlib.util.spec_from_file_location("openclaw_to_agentx", SCRIPT_PATH)
...
"""
    ...
    assert "Tyler Williams > Active Projects: AgentX Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `agentx cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # agentx_config_mod  — print statements in the post-migration summary that
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "agentx_config_mod"}
...
def test_rebrand_text_replaces_legacy_bot_names():
    ...
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "AgentX remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "agentx prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "AgentX was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "agentx uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "AgentX said to ask AgentX about AgentX settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.agentx`` (the real AgentX home), not the broken
    ``.AgentX``.
    ...
    rewritten to ``~/.AgentX/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.agentx/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .agentx directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.agentx'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "agentx config path: ~/.agentx/"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are AgentX" in result
