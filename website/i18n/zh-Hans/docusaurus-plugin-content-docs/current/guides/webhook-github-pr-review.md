...
本指南介绍如何将 AgentX Agent 连接到 GitHub，使其自动获取 pull request 的 diff、分析代码变更并发布评论——由 webhook 事件触发，无需手动 prompt（提示词）。
...
- AgentX Agent 已安装并运行（`agentx gateway`）
...
- **[构建 Plugin](/guides/build-a-agentx-plugin)** —— 将审查逻辑打包为可共享的 plugin
