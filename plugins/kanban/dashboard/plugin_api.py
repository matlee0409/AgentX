...
``agentx_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
the in-browser PTY bridge in ``agentx_cli/web_server.py``.
...
from agentx_cli import kanban_db
from agentx_cli import kanban_diagnostics as kd
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    WS auth gate (``agentx_cli.web_server._ws_auth_ok``).
    ...
    """
    try:
        from agentx_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``agentx_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from agentx_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from agentx_cli.kanban_diagnostics import SEVERITY_ORDER
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from agentx_cli.kanban import _check_dispatcher_presence
...
# spawn failures, stuck-blocked). See agentx_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def specify_task_endpoint(
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        ...
        from agentx_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from agentx_constants import get_agentx_home  # type: ignore
            from pathlib import Path as _Path
            profile_dir = _Path(get_agentx_home())
        else:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    """
    try:
        from agentx_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        from agentx_cli import kanban_decompose  # noqa: WPS433 (intentional)
