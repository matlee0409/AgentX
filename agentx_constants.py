"""Shared constants for AgentX Agent.
...
def set_agentx_home_override(path: str | Path | None) -> Token:
...
def _get_platform_default_agentx_home() -> Path:
    """Return the platform-native default AgentX home path."""
...
def get_agentx_home() -> Path:
    ...
    Reads AGENTX_HOME env var, falls back to the platform-native default.
    ...
    ``agentx_cli/kanban_db.py``).  See https://github.com/NousResearch/agentx-agent/issues/18594.
    """
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_agentx_home()
    ...
    return _get_platform_default_agentx_home()


def get_default_agentx_root() -> Path:
    ...
    """
    native_home = _get_platform_default_agentx_home()
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when AgentX is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_agentx_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_agentx_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        ...
        4. ``<AGENTX_HOME>/skills`` last-resort
    """
    ...
    if default is not None:
        return default
    return get_agentx_home() / "skills"


def get_agentx_dir(new_subpath: str, old_name: str) -> Path:
...
def iter_agentx_node_dirs(home: Path | None = None) -> list[Path]:
    ...
    """
    root = home or get_agentx_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with agentxManagedNodePathEntries() in
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    immediately throws ``MODULE_NOT_FOUND``. ``find_agentx_node_executable``
...
def agentx_managed_node_tree_present(home: Path | None = None) -> bool:
    ...
    for directory in iter_agentx_node_dirs(home):
        for name in names:
...
def heal_agentx_managed_node() -> bool:
    ...
    if _managed_node_heal_attempted:
        return False
    if not agentx_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "AGENTX_HOME": str(get_agentx_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def find_agentx_node_executable(command: str) -> str | None:
    ...
    for directory in iter_agentx_node_dirs():
        for name in names:
            candidate = directory / name
            if candidate.is_file() and (
                sys.platform == "win32" or os.access(candidate, os.X_OK)
                ...
                if node_tool_runnable(resolved):
                    return resolved
                broken_present = True
    if broken_present and heal_agentx_managed_node():
        for directory in iter_agentx_node_dirs():
            for name in names:
                candidate = directory / name
                if candidate.is_file() and (
...
def find_node_executable(command: str) -> str | None:
    ...
    """
    managed = find_agentx_node_executable(command)
    if managed:
        return managed
    if agentx_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_agentx_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with AgentX-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_agentx_node_dirs() if path.is_dir()]
...
def secure_parent_dir(path: Path) -> None:
    ...
    See https://github.com/NousResearch/agentx-agent/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    ...
    agentx_home = get_agentx_home_override() or (env or {}).get("AGENTX_HOME") or os.getenv("AGENTX_HOME")
    if not agentx_home:
        return None
    profile_home = os.path.join(agentx_home, "home")
...
def is_container() -> bool:
    ...
    See: NousResearch/agentx-agent#47111
    """
...
def get_config_path() -> Path:
    ...
    Replaces the ``get_agentx_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, agentx_logging.py, agentx_time.py, etc.).
    """
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_agentx_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._agentx_ipv4_patched = True  # type: ignore[attr-defined]
