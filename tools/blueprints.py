...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.agentx.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.agentx.blueprint`` is a mapping containing
    ...
    agentx = meta.get("agentx") if isinstance(meta, dict) else None
    blueprint = agentx.get("blueprint") if isinstance(agentx, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.agentx.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.agentx.blueprint`` block) they can hand
