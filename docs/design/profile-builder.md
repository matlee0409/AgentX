...
import time**. The context-local `set_agentx_home_override()` swap (which makes
...
The correct mechanism is the existing subprocess path: `_spawn_agentx_action`
runs `python -m agentx_cli.main <subcommand>`, and `_apply_profile_override()`
re-reads `sys.argv` at import in the fresh child. Prepend `-p <profile>`:

```python
_spawn_agentx_action(["-p", profile, "skills", "install", identifier], "skills-install")
```
...
spawned detached (`_spawn_agentx_action` returns a PID immediately). So the
create flow is:
