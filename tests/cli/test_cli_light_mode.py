...
class TestLightModeDetection:
    def test_agentx_light_env_true_forces_light(self, cli_mod, monkeypatch):
        monkeypatch.setenv("AGENTX_LIGHT", "1")
        assert cli_mod._detect_light_mode() is True

    def test_agentx_light_env_false_forces_dark(self, cli_mod, monkeypatch):
...
class TestSkinConfigHook:
    ...
    def test_hook_installed(self, cli_mod):
        from agentx_cli.skin_engine import SkinConfig

        assert getattr(SkinConfig, "_agentx_light_mode_hook_installed", False) is True
