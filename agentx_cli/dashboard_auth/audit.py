...
from ``agentx_constants`` or other agentx_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$AGENTX_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``agentx_constants.get_agentx_home`` semantics: env var wins,
    else ``~/.agentx``. A local copy avoids an import cycle with the
    middleware which lives below ``agentx_cli``.
    """
