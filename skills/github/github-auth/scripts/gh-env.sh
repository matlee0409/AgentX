#!/usr/bin/env bash
# GitHub environment detection helper for AgentX Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _agentx_env="${AGENTX_HOME:-$HOME/.agentx}/.env"; [ -f "$_agentx_env" ] && grep -q "^GITHUB_TOKEN=" "$_agentx_env" 2>/dev/null; then
