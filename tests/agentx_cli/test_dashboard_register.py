...
import agentx_cli.dashboard_register as dr
...
class TestFastFails:
    def test_not_logged_in_exits_1_with_setup_hint(self, capsys):
        ...
        with patch.object(dr, "cmd_dashboard_register", dr.cmd_dashboard_register):
            with patch(
                "agentx_cli.auth.resolve_nous_access_token", side_effect=err
            ), patch("agentx_cli.config.is_managed", return_value=False):
...
class TestHappyPath:
    def _run(self, *, args, account_token="tok_abc", portal="https://portal.nousresearch.com",
        ...
        with patch(
            "agentx_cli.auth.resolve_nous_access_token", return_value=account_token
        ), patch("agentx_cli.config.is_managed", return_value=False), patch.object(
            dr, "_resolve_portal_base_url", return_value=portal
        ), patch(
            "agentx_cli.config.get_env_value", side_effect=fake_get_env
        ), patch(
            "agentx_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
    ...
    def test_custom_redirect_uri_is_forwarded(self, capsys):
        captured: dict = {}
        self._run(
            args=_ns(redirect_uri="https://agentx.example.com/auth/callback"),
            captured=captured,
        )
        assert (
            captured["body"]["custom_redirect_uri"]
            == "https://agentx.example.com/auth/callback"
        )
    ...
    def test_non_default_portal_is_persisted(self, capsys):
        ...
        assert (
            saved["AGENTX_DASHBOARD_PORTAL_URL"]
...
class TestCustomPortalPersistence:
    ...
    def _run(self, *, args, portal, existing_portal):
        ...
        with patch(
            "agentx_cli.auth.resolve_nous_access_token", return_value="tok"
        ), patch("agentx_cli.config.is_managed", return_value=False), patch.dict(
            dr.os.environ, {}, clear=False
        ), patch.object(
            dr, "_resolve_portal_base_url", return_value=portal
        ), patch(
            "agentx_cli.config.get_env_value", side_effect=fake_get_env_value
        ), patch(
            "agentx_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
    ...
    def test_explicit_custom_url_updates_existing_in_place(self, capsys):
        ...
        assert (
            saved["AGENTX_DASHBOARD_PORTAL_URL"] == "https://new-preview.example.com"
        )
    ...
    def test_explicit_custom_url_persisted_even_when_equals_default(self, capsys):
        ...
        assert (
            saved["AGENTX_DASHBOARD_PORTAL_URL"] == "https://portal.nousresearch.com"
        )
...
class TestPublicUrlPersistence:
    ...
    def test_origin_derived_from_full_callback_path(self, capsys):
        ...
        assert "/auth/callback" not in saved["AGENTX_DASHBOARD_PUBLIC_URL"]

    def test_origin_preserves_port(self, capsys):
        saved = self._run(
            args=_ns(redirect_uri="https://agentx.example.com:8443/auth/callback"),
            existing_public=None,
        )
        assert saved["AGENTX_DASHBOARD_PUBLIC_URL"] == "https://agentx.example.com:8443"
    ...
    def test_public_url_persisted_alongside_portal_url(self, capsys):
        ...
        with patch(
            ...
            "agentx_cli.config.get_env_value", return_value=None
        ), patch(
            "agentx_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
            dr.urllib.request, "urlopen", return_value=_fake_http_ok(response)
