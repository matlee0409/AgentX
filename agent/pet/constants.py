...
row taxonomy is inferred from the atlas shape so AgentX can render both legacy
...
# rows (1536x1872), while older AgentX/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are AgentX' activity state names. They are not always identical to the
...
# Legacy AgentX/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical AgentX activity names -> accepted row-name aliases in descending
