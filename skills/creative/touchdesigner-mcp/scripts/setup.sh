...
# ── 3. Ensure AgentX config has twozero_td MCP entry ──
if [[ ! -f "$AGENTX_CFG" ]]; then
    echo -e " ${FAIL} AgentX config not found at ${AGENTX_CFG}"
    manual_steps+=("Create ${AGENTX_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$AGENTX_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in AgentX config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to AgentX config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
    ...
    manual_steps+=("Restart AgentX session to pick up config change")
fi
