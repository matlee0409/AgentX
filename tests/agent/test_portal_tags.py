...
def test_agentx_client_tag_includes_current_version():
    """The client tag must reflect agentx_cli.__version__ verbatim."""
    from agentx_cli import __version__
    from agent.portal_tags import agentx_client_tag

    assert agentx_client_tag() == f"client=agentx-client-v{__version__}"


def test_agentx_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import agentx_client_tag

    tag = agentx_client_tag()
    assert tag.startswith("client=agentx-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import agentx_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=agentx-agent" in tags
    assert agentx_client_tag() in tags
    assert len(tags) == 2
