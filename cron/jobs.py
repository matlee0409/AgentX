...
AGENTX_DIR = get_agentx_home().resolve()
...
# threshold in `agentx cron status` (agentx_cli/cron.py), so the two never
# drift apart.
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            # against `agentx_time.now()`, which uses the configured zone. If a
            ...
            if dt.tzinfo is None:
                agentx_tz = _agentx_now().tzinfo
                dt = dt.replace(tzinfo=agentx_tz)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
    ...
    try:
        ...
        run_at = _agentx_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    concurrent reader in another process (``agentx cron status``) never sees a
...
def _save_jobs_unlocked(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _agentx_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def _resolve_default_model_snapshot() -> Optional[str]:
    ...
    try:
        ...
        cfg_path = get_agentx_home() / "config.yaml"
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            "enabled": False,
            "state": "paused",
            "paused_at": _agentx_now().isoformat(),
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _agentx_now().isoformat(),
        },
    )
...
# execution. Unlike the quick-snapshot store (`agentx_cli.backup`, capped at 20)
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    Mirrors the quick-snapshot retention in ``agentx_cli.backup._prune_quick_snapshots``:
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _agentx_now().strftime("%Y-%m-%d_%H-%M-%S")
