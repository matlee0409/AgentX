...
try:
    ...
    agentx_bootstrap.harden_import_path()
...
def _load_env() -> None:
    ...
    loaded = load_agentx_dotenv(agentx_home=agentx_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", agentx_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="agentx-acp",
        description="Run AgentX Agent as an ACP stdio server.",
    )
...
def _print_version() -> None:
    ...
    print(agentx_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import AgentXACPAgent  # noqa: F401

    print("AgentX ACP check OK")


def _run_setup() -> None:
    from agentx_cli.main import main as agentx_main
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``agentx postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    import acp
    from .server import AgentXACPAgent
