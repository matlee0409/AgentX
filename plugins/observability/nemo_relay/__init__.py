...
class _Settings:
    ...
    atif_agent_name: str = "AgentX Agent"
...
class _Runtime:
    def __init__(self, nemo_relay: Any, settings: _Settings) -> None:
        ...
        self._atof_subscriber_name = "agentx.nemo_relay.atof"
    ...
    def ensure_session(self, kwargs: dict[str, Any]) -> _SessionState:
        ...
        if self.settings.atif_enabled and not self._plugins_toml_owns_exporter("atif"):
            state.atif_exporter = self.nemo_relay.AtifExporter(
                ...
                extra={"source": "agentx-agent", "plugin": "observability/nemo_relay"},
            )
            state.atif_subscriber_name = f"agentx.nemo_relay.atif.{session_id}"
    ...
    def _run_managed_with_downstream_preservation(
        ...
        # execution so AgentX retry classification still sees it. The LLM and tool
...
def on_post_api_request(**kwargs: Any) -> None:
    ...
    def _record() -> None:
        ...
        if span is None:
            runtime.mark("agentx.api.response.unmatched", kwargs)
            return
...
def on_api_request_error(**kwargs: Any) -> None:
    ...
    def _record() -> None:
        ...
        span = state.llm_spans.pop(_api_key(kwargs), None)
        if span is None:
            runtime.mark("agentx.api.error", kwargs)
            return
        runtime.nemo_relay.llm.call_end(
            span,
...
def on_post_tool_call(**kwargs: Any) -> None:
    ...
    def _record() -> None:
        ...
        if span is None:
            runtime.mark("agentx.tool.response.unmatched", kwargs)
            return
...
def _load_settings() -> _Settings:
    ...
    return _Settings(
        ...
        atif_agent_name=_env("AGENTX_NEMO_RELAY_ATIF_AGENT_NAME") or "AgentX Agent",
...
def _original_downstream_error(exc: Exception) -> BaseException:
    # AgentX wraps downstream execution failures in a local/private exception
...
def _is_relay_wrapped_callback_error(exc: Exception, callback_error: Exception | None) -> bool:
    ...
    # leading ``internal error:`` shape entirely, this returns False and AgentX
