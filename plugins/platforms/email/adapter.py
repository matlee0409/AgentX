...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "agentx-agent" "version" "{_agentx_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "AgentX Agent")
        ...
        msg_id = f"<agentx-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
...
# agentx_cli/gateway.py, and the _send_email dispatch in
