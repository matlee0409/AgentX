...
class TestResolveApiKey:
    ...
    def test_returns_api_key_from_host_block(self, monkeypatch):
        ...
        monkeypatch.setattr(honcho_cli, "_host_key", lambda: "agentx")
        monkeypatch.delenv("HONCHO_API_KEY", raising=False)
        cfg = {"hosts": {"agentx": {"apiKey": "host-key"}}, "apiKey": "root-key"}
...
class TestSetupWizardDeploymentShape:
    ...
    calls, asserting the resulting agentx_host block so the tree's routing
    ...
    def _run_setup(self, monkeypatch, tmp_path, *, answers, initial_cfg=None,
        ...
        monkeypatch.setattr(
            "agentx_cli.config.load_config", lambda: {"memory": {}}, raising=False,
        )
        monkeypatch.setattr(
            "agentx_cli.config.save_config", lambda c: None, raising=False,
        )
