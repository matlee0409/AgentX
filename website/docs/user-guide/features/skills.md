...
```bash
curl -fsSL https://raw.githubusercontent.com/matlee0409/AgentX/main/scripts/install.sh | bash -s -- --no-skills
```
...
[Skills Hub](https://github.com/matlee0409/AgentX/wiki) page — instead of a
...
AgentX supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

AgentX source id: `claude-marketplace`
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). AgentX fetches the URL, parses the YAML frontmatter, security-scans it, and installs.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.agentx.tags`, `version`, `author`, `platforms`, `metadata.agentx.config`).
...
If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.agentx/.hub/taps.json`:
...
Taps are stored in `~/.agentx/.hub/taps.json` (created on demand).
