"""ACP agent server — exposes AgentX Agent via the Agent Client Protocol."""
...
try:
    from agentx_cli import __version__ as AGENTX_VERSION
except Exception:
...
class AgentXACPAgent(acp.Agent):
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from agentx_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from agentx_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_agentx_session_id: str,
        previous_agentx_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.agentx.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_agentx_session_id,
                previous_agentx_session_id=previous_agentx_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_agentx_session_id: Optional[str] = None,
        previous_agentx_session_id: Optional[str] = None,
    ) -> None:
        ...
        split during a turn), pass ``previous_agentx_session_id`` so the
        attached ``_meta.agentx.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # agentx_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_agentx_session_id or session_id,
            previous_agentx_session_id,
        )
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        # not the executor's. Also set AGENTX_INTERACTIVE so approval.py
        ...
        def _run_agent() -> dict:
            ...
            previous_interactive = os.environ.get("AGENTX_INTERACTIVE")
            ...
            try:
                ...
                # Restore AGENTX_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("AGENTX_INTERACTIVE", None)
                else:
                    os.environ["AGENTX_INTERACTIVE"] = previous_interactive
        ...
        try:
            ...
            pre_turn_agentx_id = getattr(state.agent, "session_id", None)
        ...
        # _meta.agentx.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_agentx_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_agentx_id
            and pre_turn_agentx_id
            and post_turn_agentx_id != pre_turn_agentx_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_agentx_session_id=post_turn_agentx_id,
                    previous_agentx_session_id=pre_turn_agentx_id,
                )
            except Exception:
                logger.debug(
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"AgentX Agent v{AGENTX_VERSION}"
