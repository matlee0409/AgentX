"""
IRC Platform Adapter for AgentX Agent.
...
class IRCAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :AgentX Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :AgentX Agent shutting down")
...
def interactive_setup() -> None:
    ...
    Lazy-imports ``agentx_cli.setup`` helpers so the plugin stays importable
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :AgentX Agent (cron)")
