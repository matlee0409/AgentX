...
class TestClassifySandboxMirrorTarget:
    ...
    def test_path_outside_sandbox_returns_none(self, tmp_path):
        """A plain AgentX path is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target
    ...
    def test_sandboxes_segment_without_home_agentx_returns_none(self, tmp_path):
        """A ``sandboxes/`` directory unrelated to AgentX-state mirroring (e.g.
    ...
    def test_sandboxes_segment_with_home_but_no_agentx_returns_none(self, tmp_path):
        """``sandboxes/<backend>/<task>/home/anything-not-agentx`` is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target
    ...
    def test_truncated_sandbox_path_returns_none(self, tmp_path):
        """``…/sandboxes/<backend>/<task>`` without ``home/.agentx/<thing>`` is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target
...
class TestSandboxMirrorIsOrthogonalToCrossProfile:
    ...
    def test_same_profile_mirror_still_flagged(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(fs, "_agentx_root_path", lambda: tmp_path)
        monkeypatch.setattr(fs, "_agentx_home_path", lambda: tmp_path / "profiles" / "group1")
