...
This plugin connects AgentX Agent to iMessage (and other Spectrum
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-AgentX-Sidecar-Token`.
...
2. **Find or create** the `AgentX Agent` project on the Photon dashboard.
...
5. Verify end-to-end: `agentx photon status`, a DM and a group roundtrip,
