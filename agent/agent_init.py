...
from agentx_cli.timeouts import get_provider_request_timeout
...
def init_agent(
    ...
    agent._cache_ttl = "5m"
    try:
        from agentx_cli.config import load_config as _load_pc_cfg
    ...
    from agentx_logging import setup_logging, setup_verbose_logging
    setup_logging(agentx_home=_ra()._agentx_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that agentx_logging.setup_logging() does not
        ...
        # handler level inside agentx_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from agentx_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from agentx_cli.config import load_config as _load_br_cfg
    ...
    agent.logs_dir = agentx_home / "sessions"
    ...
    try:
        from agentx_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from agentx_cli.config import load_config as _load_agent_config
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from agentx_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from agentx_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by AgentX Agent.  Choose a model with at least "
