...
class TestInactivityTimeout:
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """AGENTX_CRON_TIMEOUT=0 yields None (unlimited)."""
    ...
    def test_timeout_invalid_value_falls_back_to_default(self, monkeypatch):
        """AGENTX_CRON_TIMEOUT=abc should fall back to 600s, not raise ValueError."""
        monkeypatch.setenv("AGENTX_CRON_TIMEOUT", "abc")
        raw = os.getenv("AGENTX_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
    ...
    def test_timeout_empty_string_uses_default(self, monkeypatch):
        """AGENTX_CRON_TIMEOUT='' (empty) should use the 600s default."""
        monkeypatch.setenv("AGENTX_CRON_TIMEOUT", "")
        raw = os.getenv("AGENTX_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
        assert _cron_timeout == 600.0
...
class TestSysPathOrdering:
    ...
    def test_agentx_time_importable(self):
        """agentx_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _agentx_now
        assert callable(_agentx_now)

    def test_agentx_constants_importable(self):
        """agentx_constants should be importable from cron context."""
        from agentx_constants import get_agentx_home
        assert callable(get_agentx_home)
