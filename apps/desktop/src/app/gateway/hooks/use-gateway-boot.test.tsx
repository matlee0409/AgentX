...
// bug that drives the REAL useGatewayBoot hook + REAL AgentXGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshAgentXConfig: async () => undefined,
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForAgentX) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startAgentX()'s remote branch awaits waitForAgentX() for 45s before it
    ...
    // After ~45s waitForAgentX gives up and getConnection rejects → boot()
