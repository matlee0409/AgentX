...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_agentx_home()}/.env manually."
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_agentx_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from agentx_cli.plugins import _get_disabled_plugins, get_plugin_manager
...
def skill_view(
    ...
    try:
        ...
        # Check metadata.agentx.* first (agentskills.io convention), fall back to top-level
        agentx_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            agentx_meta = metadata.get("agentx", {}) or {}

        tags = _parse_tags(agentx_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            agentx_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
