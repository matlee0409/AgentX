...
def has_xai_credentials() -> bool:
    ...
    try:
        ...
        auth_path = get_agentx_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    ...
    Wraps :func:`agentx_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from agentx_cli.config import get_env_value as _agentx_get_env_value

        value = _agentx_get_env_value(name)
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    to ``XAI_API_KEY`` resolved via ``agentx_cli.config.get_env_value`` so keys
