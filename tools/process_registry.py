...
CHECKPOINT_PATH = get_agentx_home() / "processes.json"
...
class ProcessRegistry:
    ...
    def _daemon_term_grace_seconds() -> float:
        ...
        """
        try:
            from agentx_cli.config import read_raw_config, cfg_get, DEFAULT_CONFIG
            cfg = read_raw_config()
    ...
    def spawn_via_env(
        ...
        log_path = f"{temp_dir}/agentx_bg_{session.id}.log"
        pid_path = f"{temp_dir}/agentx_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/agentx_bg_{session.id}.exit"
    ...
    def is_session_waiting(self, session_id: str) -> bool:
        ...
        Used by the goal-loop wait barrier (``agentx_cli.goals``) to support
