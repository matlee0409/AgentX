...
agentx_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_gateway_run_loader_honors_managed(homes, monkeypatch):
    ...
    monkeypatch.setattr(gr, "_agentx_home", home, raising=False)
...
def test_gateway_config_loader_honors_managed(homes, monkeypatch):
    ...
    # load_gateway_config resolves home via get_agentx_home() (AGENTX_HOME env).
...
def test_tui_loader_honors_managed(homes, monkeypatch):
    ...
    monkeypatch.setattr(ts, "_agentx_home", home, raising=False)
    monkeypatch.setattr(ts, "_cfg_cache", None, raising=False)
    monkeypatch.setattr(ts, "_cfg_mtime", None, raising=False)
    monkeypatch.setattr(ts, "get_agentx_home_override", lambda: None, raising=False)
    cfg = ts._load_cfg()
...
def test_logging_config_honors_managed(homes, monkeypatch):
    ...
    level, _max, _bk = agentx_logging._read_logging_config()
    assert level == "DEBUG"


def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # agentx_time checks an env override first; ensure it's unset so config wins.
    ...
    assert agentx_time._resolve_timezone_name() == "Asia/Tokyo"
