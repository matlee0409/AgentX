"""Tests for agentx_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from agentx_cli import relaunch as relaunch_mod


class TestResolveAgentXBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        ...
        assert relaunch_mod.resolve_agentx_bin() == fake
    ...
    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        ...
        assert relaunch_mod.resolve_agentx_bin() == str(fake)
    ...
    def test_falls_back_to_path_which(self, monkeypatch):
        ...
        assert relaunch_mod.resolve_agentx_bin() == "/usr/bin/agentx"

    def test_returns_none_when_unresolvable(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_agentx_bin() is None
...
class TestBuildRelaunchArgv:
    def test_uses_bin_when_available(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_agentx_bin", lambda: "/usr/bin/agentx")
    ...
    def test_falls_back_to_python_module(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_agentx_bin", lambda: None)
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv == [sys.executable, "-m", "agentx_cli.main", "--resume", "abc"]
...
class TestRelaunch:
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod, "resolve_agentx_bin", lambda: r"C:\Users\test\agentx.exe")
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_agentx_bin", lambda: r"C:\agentx.exe")
    ...
    def test_windows_surfaces_oserror_with_help(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_agentx_bin", lambda: r"C:\missing.exe")
...
class TestResolveAgentXBinWindowsPyGuard:
    """On Windows, resolve_agentx_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        bin_path = relaunch_mod.resolve_agentx_bin()
    ...
    def test_posix_still_accepts_py_argv0(self, monkeypatch, tmp_path):
        ...
        script.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(script), "chat"])
        assert relaunch_mod.resolve_agentx_bin() == str(script)

    def test_windows_py_argv0_with_no_agentx_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND agentx.exe
        isn't on PATH, return None so the caller falls back to
        python -m agentx_cli.main."""
        script = tmp_path / "main.py"
        script.write_text("# stub")
