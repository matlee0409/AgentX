...
def _load_supermemory_config(agentx_home: str) -> dict:
    config = _default_config()
    config_path = Path(agentx_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, agentx_home: str) -> None:
    config_path = Path(agentx_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, agentx_home):
        ...
        _save_supermemory_config(sanitized, agentx_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from agentx_constants import get_agentx_home
        self._agentx_home = kwargs.get("agentx_home") or str(get_agentx_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._agentx_home)
