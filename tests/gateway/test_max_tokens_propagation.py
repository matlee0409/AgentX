...
def isolated_home(tmp_path, monkeypatch):
    ...
    These tests deliberately re-import ``agentx_cli`` / ``gateway`` so each
    ...
    _saved = {
        ...
        if k.startswith(("agentx_cli", "gateway"))
    }

    def write_cfg(body: str) -> None:
        (agentx_home / "config.yaml").write_text(textwrap.dedent(body))

    def fresh_gateway():
        for mod in list(sys.modules.keys()):
            if mod.startswith(("agentx_cli", "gateway")):
                del sys.modules[mod]
...
def test_env_override_beats_everything(isolated_home, monkeypatch):
    """AGENTX_MAX_TOKENS is the internal override mechanism (highest priority)."""
    write_cfg, fresh_gateway = isolated_home
    monkeypatch.setenv("AGENTX_MAX_TOKENS", "2048")
    write_cfg(
        """
        model:
          default: glm-5.1
...
def test_lift_helper_accepts_alias_and_rejects_garbage(isolated_home):
    ...
    write_cfg("model:\n  provider: openrouter\n")
    for mod in list(sys.modules.keys()):
        if mod.startswith("agentx_cli"):
            del sys.modules[mod]
    rp = importlib.import_module("agentx_cli.runtime_provider")
