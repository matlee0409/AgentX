...
Used by agentx_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            agentx_meta = metadata.get("agentx", {})
            if isinstance(agentx_meta, dict):
                tags = agentx_meta.get("tags", [])
        if not tags:
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                agentx_meta = meta_block.get("agentx", {})
                if isinstance(agentx_meta, dict):
                    tags = agentx_meta.get("tags", [])
...
AGENTX_INDEX_URL = "https://agentx-agent.nousresearch.com/docs/api/skills-index.json"
AGENTX_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "agentx-index.json"
AGENTX_INDEX_TTL = 6 * 3600  # 6 hours


def _load_agentx_index() -> Optional[dict]:
    ...
    """
    # Check local cache
    if AGENTX_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - AGENTX_INDEX_CACHE_FILE.stat().st_mtime
            if age < AGENTX_INDEX_TTL:
                return json.loads(AGENTX_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(AGENTX_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
    ...
    # Cache locally
    try:
        AGENTX_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        AGENTX_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class AgentXIndexSource(SkillSource):
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_agentx_index()
            self._loaded = True
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        AgentXIndexSource(auth=auth), # Centralized index (search + resolved install paths)
