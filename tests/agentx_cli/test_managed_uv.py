"""Tests for agentx_cli.managed_uv — one path, no guessing."""
...
class TestManagedUvPath:
    def test_posix(self, tmp_path):
        with patch("agentx_cli.managed_uv.get_agentx_home", return_value=tmp_path), \
             patch("agentx_cli.managed_uv.platform.system", return_value="Linux"):
            from agentx_cli.managed_uv import managed_uv_path
            assert managed_uv_path() == tmp_path / "bin" / "uv"

    def test_windows(self, tmp_path):
        with patch("agentx_cli.managed_uv.get_agentx_home", return_value=tmp_path), \
             patch("agentx_cli.managed_uv.platform.system", return_value="Windows"):
            from agentx_cli.managed_uv import managed_uv_path
...
class TestEnsureUv:
    ...
    def test_installs_if_missing(self, tmp_path):
        with patch("agentx_cli.managed_uv.get_agentx_home", return_value=tmp_path), \
             patch("agentx_cli.managed_uv._install_uv") as mock_install:
    ...
    def test_install_failure_returns_falsy(self, tmp_path):
        with patch("agentx_cli.managed_uv.get_agentx_home", return_value=tmp_path), \
             patch("agentx_cli.managed_uv._install_uv", side_effect=RuntimeError("network down")):
            from agentx_cli.managed_uv import ensure_uv
            path = ensure_uv()
...
class TestEnsureUvUpdateBoundary:
    ...
    ``agentx_cli.main`` against the freshly pulled ``managed_uv``. A release
...
class TestEnsureUvWindowsSafe:
    ...
    def test_uvresult_would_break_windows_list2cmdline(self):
        ...
        import subprocess
        from agentx_cli.managed_uv import _UvResult
    ...
    def test_windows_returns_plain_str_safe_for_subprocess(self, tmp_path):
        ...
        with patch("agentx_cli.managed_uv.get_agentx_home", return_value=tmp_path), \
             patch("agentx_cli.managed_uv.platform.system", return_value="Windows"):
            from agentx_cli.managed_uv import _UvResult, ensure_uv
            uv_bin = ensure_uv()
...
class TestUpdateManagedUv:
    def test_no_uv_returns_none(self, tmp_path):
        with patch("agentx_cli.managed_uv.get_agentx_home", return_value=tmp_path):
            from agentx_cli.managed_uv import update_managed_uv
            assert update_managed_uv() is None

    def test_self_update_success(self, tmp_path):
        _make_executable(tmp_path / "bin" / "uv")
        with patch("agentx_cli.managed_uv.get_agentx_home", return_value=tmp_path), \
             patch("agentx_cli.managed_uv.subprocess.run") as mock_run:
...
class TestInstallUvInternals:
    def test_posix_sets_uv_unmanaged_install(self, tmp_path):
        ...
        with patch("agentx_cli.managed_uv._install_uv_posix") as mock_posix:
            from agentx_cli.managed_uv import _install_uv
            _install_uv(target)
    ...
    def test_windows_sets_uv_install_dir(self, tmp_path):
        ...
        with patch("agentx_cli.managed_uv.platform.system", return_value="Windows"), \
             patch("agentx_cli.managed_uv._install_uv_windows") as mock_windows:
            from agentx_cli.managed_uv import _install_uv
            _install_uv(target)
