...
# display_agentx_home is imported lazily at call sites to avoid ImportError
# when agentx_constants is cached from a pre-update version during `agentx update`.
...
from agentx_cli.colors import Colors, color
...
def _scan_gateway_pids(
    ...
    current_home = str(get_agentx_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"agentx_home={current_home_lc}" in command_lc
            )
        ...
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "agentx_home=" in command_lc
            and f"agentx_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_profile_gateway_processes(
    ...
    try:
        from gateway.status import get_running_pid
        from agentx_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "agentx_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m agentx_cli.main gateway
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    from agentx_cli._subprocess_compat import (
    ...
    if sys.platform == "win32":
        try:
            from agentx_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # base interpreter needs to import agentx_cli.  Empty on POSIX, where
...
def _sync_agentx_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``AGENTX_HOME``.

    Under ``sudo``, ``AGENTX_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_agentx_home` falls back to ``/root/.agentx`` — the wrong
    ...
    unit_home = env.get("AGENTX_HOME", "").strip()
    if not unit_home:
        return
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from agentx_cli.service_manager import detect_service_manager, get_service_manager
...
def _gateway_list() -> None:
    ...
    """
    try:
        from agentx_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans() -> bool:
    ...
    in-process under a ``gateway restart`` argv (agentx_cli/gateway.py restart
...
from agentx_constants import is_container, is_termux, is_wsl
...
SERVICE_DESCRIPTION = "AgentX Agent Gateway - Messaging Platform Integration"
...
def _profile_suffix() -> str:
    ...
    home = get_agentx_home().resolve()
    default = get_default_agentx_root().resolve()
    if home == default:
        return ""
...
def _profile_arg(agentx_home: str | None = None, default_root: str | Path | None = None) -> str:
    ...
    Args:
        agentx_home: Optional explicit AGENTX_HOME path. Defaults to the current
            ``get_agentx_home()`` value. Should be passed when generating a
        ...
        default_root: Optional AgentX root to compare against. Used when
            ...
            process, where ``Path.home()`` and ``get_default_agentx_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from agentx_constants import get_default_agentx_root

    home = Path(agentx_home or str(get_agentx_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_agentx_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(agentx_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".agentx"
    try:
        Path(agentx_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(agentx_home, default_root=target_root)
    except ValueError:
        return _profile_arg(agentx_home)
...
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("agentx.service",)
...
def _find_legacy_agentx_units() -> list[tuple[str, Path, bool]]:
    ...
    different service name (e.g. ``agentx.service`` before the rename to
    ...
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``agentx.service`` running an unrelated
...
def has_legacy_agentx_units() -> bool:
    """Return True when any legacy AgentX gateway unit files exist."""
    return bool(_find_legacy_agentx_units())
...
def print_legacy_unit_warning() -> None:
    ...
    """
    legacy = _find_legacy_agentx_units()
    if not legacy:
        return
...
def remove_legacy_agentx_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy AgentX gateway unit files.

    Iterates over whatever ``_find_legacy_agentx_units()`` returns — which is
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so AgentX can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo agentx gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo agentx gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo agentx gateway start --system")
            return scope, False
...
def _agentx_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current AGENTX_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_agentx_home() resolves to
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    agentx_node = agentx_home / "node" / "bin"
    if _is_dir(agentx_node):
        candidates.append(str(agentx_node))
    agentx_nm = agentx_home / "node_modules" / ".bin"
    if _is_dir(agentx_nm):
        candidates.append(str(agentx_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m agentx_cli.main``, so module
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        agentx_home = _agentx_home_for_target_user(home_dir)
        profile_arg = _profile_arg_for_target_user(agentx_home, home_dir)
        ...
        working_dir = str(agentx_home) if agentx_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m agentx_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="AGENTX_HOME={agentx_home}"
Restart=always
RestartSec=5
...
"""
    ...
    agentx_home = str(get_agentx_home().resolve())
    profile_arg = _profile_arg(agentx_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # generated unit bakes that ``/tmp/pytest-of-.../agentx_test`` path into
...
def systemd_install(
    ...
    # Offer to remove legacy units (agentx.service from pre-rename installs)
    ...
    if has_legacy_agentx_units():
        print()
        print_legacy_unit_warning()
        print()
        if prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_agentx_units(interactive=False)
            print()
...
def systemd_stop(system: bool = False):
    ...
    _sync_agentx_home_from_systemd_unit(system=system)
    try:
...
# (the documented `nohup agentx gateway run` workaround). See #23387.
...
def _launchd_unsupported_marker_path() -> Path:
    return get_agentx_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m agentx_cli.main [--profile X] gateway run --replace` argv.
    ...
    """
    cmd = [get_python_path(), "-m", "agentx_cli.main"]
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    """
    from agentx_constants import display_agentx_home as _dhh
...
def launchd_status(deep: bool = False):
    ...
    if deep:
        log_file = get_agentx_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    try:
        from agentx_constants import get_default_agentx_root
        default_root = get_default_agentx_root()
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_agentx_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from agentx_cli.telegram_managed_bot import (
...
def _setup_weixin():
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_agentx_home())))
    except KeyboardInterrupt:
        print()
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "AgentXAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``agentx_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from agentx_cli import setup as _s
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`agentx_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from agentx_cli.service_manager import (
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            from agentx_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
        ...
        # pre-rename installs (e.g. agentx.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_agentx_units(interactive=not yes, dry_run=dry_run)
