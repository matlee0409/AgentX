...
@pytest.fixture
def pty_client(monkeypatch, _isolate_agentx_home):
...
def test_resolve_chat_argv_sets_active_session_file_env(monkeypatch):
    ...
    _argv, _cwd, env = ws._resolve_chat_argv(
        active_session_file="/tmp/agentx-active-session.json"
    )

    assert env["AGENTX_TUI_ACTIVE_SESSION_FILE"] == "/tmp/agentx-active-session.json"
