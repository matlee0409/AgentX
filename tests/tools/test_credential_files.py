...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        agentx_home.mkdir()
        (agentx_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(agentx_home / "token.json")
    ...
    def test_dict_with_name_key_fallback(self, tmp_path):
        ...
        agentx_home = tmp_path / ".agentx"
        agentx_home.mkdir()
        (agentx_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        agentx_home = tmp_path / ".agentx"
        agentx_home.mkdir()
        (agentx_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        agentx_home = tmp_path / ".agentx"
        agentx_home.mkdir()
        (agentx_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    ...
    def test_custom_container_base(self, tmp_path):
        agentx_home = tmp_path / ".agentx"
        (agentx_home / "skills").mkdir(parents=True)
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a sensitive file one level above agentx_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside agentx_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside AGENTX_HOME must still be registered."""
        agentx_home = tmp_path / ".agentx"
        agentx_home.mkdir()
        monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
        (agentx_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_agentx_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of AGENTX_HOME must be allowed."""
        agentx_home = tmp_path / ".agentx"
        agentx_home.mkdir()
        subdir = agentx_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a sensitive file outside agentx_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside agentx_home pointing outside
        symlink = agentx_home / "evil_link.json"
        try:
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, agentx_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(agentx_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(agentx_home, [str(sensitive)])
    ...
    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        ...
        (agentx_home / "oauth.json").write_text("{}")
        self._write_config(agentx_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        agentx_home = tmp_path / ".agentx"
        agentx_home.mkdir()
        (agentx_home / "cache" / "documents").mkdir(parents=True)
        (agentx_home / "cache" / "audio").mkdir(parents=True)
        (agentx_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.agentx/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        agentx_home = tmp_path / ".agentx"
        agentx_home.mkdir()
        # Use legacy dir name — get_agentx_dir prefers old if it exists
        (agentx_home / "document_cache").mkdir()
        (agentx_home / "image_cache").mkdir()
        monkeypatch.setenv("AGENTX_HOME", str(agentx_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(agentx_home / "document_cache") in host_paths
        assert str(agentx_home / "image_cache") in host_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        agentx_home = tmp_path / ".agentx"
        img_dir = agentx_home / "cache" / "images"
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.agentx")
            == "/home/agent/.agentx/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        agentx_home = tmp_path / ".agentx"
        (agentx_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(agentx_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        agentx_home = tmp_path / ".agentx"
        doc_dir = agentx_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        agentx_home = tmp_path / ".agentx"
        ss_dir = agentx_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.agentx/cache/screenshots/session_abc/screen1.png"
