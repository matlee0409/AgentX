"""Tests for agentx_cli configuration management."""
...
class TestGetAgentXHome:
    ...
    def test_env_override(self):
        with patch.dict(os.environ, {"AGENTX_HOME": "/custom/path"}):
            home = get_agentx_home()
            assert home == Path("/custom/path")


class TestEnsureAgentXHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from agentx_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    def test_preserves_legacy_template_with_user_persona(self, tmp_path):
        ...
        from agentx_cli.default_soul import _LEGACY_TEMPLATE_SOULS
...
class TestLoadConfigParseFailure:
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from agentx_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"AGENTX_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="agentx_cli.config"):
                config = load_config()
            ...
            # stands out at agentx startup before logging is configured)
            captured = capsys.readouterr()
            assert "agentx config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"AGENTX_HOME": str(tmp_path)}):
            ...
            assert "agentx config:" in after_edit, "edited file should re-warn"
...
class TestRemoveEnvValue:
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"AGENTX_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"AGENTX_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from agentx_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
...
class TestEnvWriteDenylist:
    ...
    def _agentx_home(self, tmp_path, monkeypatch):
    ...
    def test_agentx_integration_keys_still_writable(self, allowed_key):
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``AGENTX_*``) keep working. The denylist
