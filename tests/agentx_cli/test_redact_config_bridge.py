...
`AGENTX_REDACT_SECRETS` at module-import time. `agentx_cli/main.py` at
...
env var in `agentx_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports agentx_cli.main and checks
    ...
    probe = textwrap.dedent(
        ...
        sys.path.insert(0, %r)
        import agentx_cli.main  # triggers the bridge + setup_logging
        import agent.redact
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    ...
    (agentx_home / ".env").write_text("AGENTX_REDACT_SECRETS=true\n")
