# Contributing to AgentX Agent

Thank you for contributing to AgentX Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo NousResearch/agentx-agent "<your terms>"
  gh search prs --repo NousResearch/agentx-agent --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/NousResearch/agentx-agent/issues?q=) · [PRs (all states)](https://github.com/NousResearch/agentx-agent/pulls?q=is%3Apr).
...
- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(agentx_home, config)` for setup-wizard integration
...
system `python3 -m agentx_cli.main` can pick up unrelated system Python
packages.
...
├── cli.py                    # AgentXCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (agentx-cli, agentx-telegram, etc.)
├── agentx_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── agentx_cli/               # CLI command implementations
...
├── tests/                    # Test suite
├── website/                  # Documentation site (agentx-agent.nousresearch.com)
│
...
- **Session persistence**: All conversations are stored in SQLite (`agentx_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.agentx/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
Four fields are supported under `metadata.agentx`:
...
Add to `_BUILTIN_SKINS` dict in `agentx_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `agentx_cli/skin_engine.py` for the full schema and existing skins as examples.
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `agentx_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `agentx_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `agentx_cli/gateway_windows.py::
...
- Use [GitHub Issues](https://github.com/NousResearch/agentx-agent/issues)
