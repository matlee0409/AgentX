...
_PROFILE_DIRS = [
    ...
    # See agentx_constants.get_subprocess_home().
    "home",
]
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    is_default_source = source_resolved == _get_default_agentx_home().resolve()
...
def _get_profiles_root() -> Path:
    ...
    """
    return _get_default_agentx_home() / "profiles"


def _get_default_agentx_home() -> Path:
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_agentx_home() / "active_profile"
...
def get_profile_dir(name: str) -> Path:
    ...
    if canon == "default":
        return _get_default_agentx_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, agentx subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            agentx_exe = shutil.which("agentx") or "agentx"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(agentx_exe)} -p {profile} "$@"\n')
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_agentx_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, agentx_home)`` pairs a gateway should serve.
    ...
    The returned ``agentx_home`` is the path to pass to
    ``set_agentx_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_agentx_home())]
...
def create_profile(
    ...
    if not soul_path.exists():
        try:
            from agentx_cli.default_soul import DEFAULT_SOUL_MD
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_agentx_home() / ".env"
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from agentx_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    try:
        ...
        from agentx_cli.gateway import get_service_name, get_launchd_plist_path
...
def get_active_profile_name() -> str:
    ...
    resolved = agentx_home.resolve()

    default_resolved = _get_default_agentx_home().resolve()
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="agentx_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    candidates = [
        new_dir / "honcho.json",
        _get_default_agentx_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("agentx_"):
