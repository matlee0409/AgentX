...
description: "How to build a model provider (inference backend) plugin for AgentX Agent"
---
...
1. **Bundled plugins** — `<repo>/plugins/model-providers/<name>/` — ship with AgentX
...
|---|---|---|
| Credential resolution | `agentx_cli/auth.py` | `PROVIDER_REGISTRY["acme-inference"]` populated from profile |
| `--provider` CLI flag | `agentx_cli/main.py` | Accepts `acme-inference` |
| `agentx model` picker | `agentx_cli/models.py` | Appears in `CANONICAL_PROVIDERS`, model list fetched from `{base_url}/models` |
| `agentx doctor` | `agentx_cli/doctor.py` | Health check for `ACME_API_KEY` + `{base_url}/models` probe |
| `agentx setup` | `agentx_cli/config.py` | `ACME_API_KEY` appears in `OPTIONAL_ENV_VARS` and the setup wizard |
...
| Runtime resolution | `agentx_cli/runtime_provider.py` | Returns correct `base_url`, `api_key`, `api_mode` |
...
```toml
[project.entry-points."agentx_agent.plugins"]
acme-inference = "acme_agentx_plugin:register"
```

…where `acme_agentx_plugin:register` is a function that calls `register_provider(profile)`. The general PluginManager picks up entry-point plugins during `discover_and_load()`. For `kind: model-provider` pip plugins, you still need to declare the kind in your manifest (or rely on the source-text heuristic).

See [Building a AgentX Plugin](/guides/build-a-agentx-plugin#distribute-via-pip) for the full entry-points setup.
...
- [Building a AgentX Plugin](/guides/build-a-agentx-plugin) — general plugin authoring
