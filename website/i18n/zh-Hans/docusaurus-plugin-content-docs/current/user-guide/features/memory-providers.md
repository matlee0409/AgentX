...
AgentX Agent 内置 8 个外部记忆提供者插件，为 Agent 提供跨会话的持久化知识，超越内置的 MEMORY.md 和 USER.md。同一时间只能激活**一个**外部提供者——内置记忆始终与其并行工作。
...
**配置：** `$AGENTX_HOME/honcho.json`（profile 本地）或 `~/.honcho/config.json`（全局）。解析顺序：`$AGENTX_HOME/honcho.json` > `~/.agentx/honcho.json` > `~/.honcho/config.json`。参见[配置参考](https://github.com/agentx-ai/agentx-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/agentx)。
...
参见[配置参考](https://github.com/agentx-ai/agentx-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/agentx)。
...
| `retain_context` | `conversation between AgentX Agent and the User` | 保留记忆的上下文标签 |
...
完整配置参考参见[插件 README](https://github.com/NousResearch/agentx-agent/blob/main/plugins/memory/hindsight/README.md)。
