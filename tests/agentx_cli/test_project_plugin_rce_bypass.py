...
1. ``agentx_cli.web_server._discover_dashboard_plugins`` opted into
...
2. ``agentx_cli.web_server._mount_plugin_api_routes`` then imported
...
class TestEndToEndPocBlocked:
    ...
    def test_full_chain_blocked(self, tmp_path, monkeypatch):
        ...
        for call in spec.call_args_list:
            ...
            assert module_name != "agentx_dashboard_plugin_evil"
            assert target != payload_py
            assert "evil-repo" not in target.parts
        assert "agentx_dashboard_plugin_evil" not in sys.modules
