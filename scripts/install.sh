...
# Configuration
REPO_URL_SSH="git@github.com:matlek0409/AgentX.git"
REPO_URL_HTTPS="https://github.com/matlek0409/AgentX.git"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   Installs optional deps + runs agentx setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/matlek0409/AgentX/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # The runtime update path (agentx_cli/managed_uv.py) looks in the same
...
# string clears that floor; anything below it is replaced with the AgentX-
...
check_node() {
    ...
    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing AgentX-managed Node $NODE_VERSION LTS..."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "AgentX Agent itself does not require or retain root access."
...
setup_path() {
    ...
    if [ "$USE_VENV" = true ]; then
        AGENTX_BIN="$INSTALL_DIR/venv/bin/agentx"
    else
    ...
    if [ ! -x "$AGENTX_BIN" ]; then
        log_warn "agentx entry point not found at $AGENTX_BIN"
...
EOF
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        PATH_COMMENT='# AgentX Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# AgentX Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# AgentX Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
...
copy_config_templates() {
    ...
    # This MUST match DEFAULT_SOUL_MD in agentx_cli/default_soul.py — the
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# AgentX Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# AgentX Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (agentx --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m agentx_cli.main setup < /dev/tty
    else
        python -m agentx_cli.main setup < /dev/tty
    fi
}
    ...
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                AGENTX_CMD="$(get_agentx_command_path)"
...
ensure_browser() {
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$AGENTX_HOME/node" --silent --ignore-scripts \
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up AgentX for pip install"
    ...
    AGENTX_CMD="$(command -v agentx 2>/dev/null || echo "")"
    if [ -n "$AGENTX_CMD" ]; then
        log_info "Running agentx setup..."
        "$AGENTX_CMD" setup
    else
        log_warn "agentx command not found on PATH"
        log_info "Try: python -m agentx_cli.main setup"
    fi
}
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the AgentX-managed Node to PATH, so
    ...
    if [ "$OS" = "linux" ]; then
        ...
        elif [ -x "$desktop_dir/release/linux-unpacked/agentx" ]; then
            app="$desktop_dir/release/linux-unpacked/agentx"
        fi
    else
        local cand
    ...
    # trips Gatekeeper's tamper check ("AgentX is damaged and can't be opened").
