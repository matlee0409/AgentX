...
def _get_sessions_dir() -> Path:
    ...
    try:
        ...
        return get_agentx_home() / "sessions"
    except ImportError:
...
def _load_channel_directory() -> dict:
    ...
    try:
        from agentx_constants import get_agentx_home
        directory_file = get_agentx_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
...
class EventBridge:
    ...
    def _poll_once(self, db):
        ...
        try:
            from agentx_constants import get_agentx_home
            db_file = get_agentx_home() / "state.db"
        except ImportError:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    ...
    mcp = FastMCP(
        "agentx",
        instructions=(
            "AgentX Agent messaging bridge. Use these tools to interact with "
