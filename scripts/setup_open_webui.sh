...
# Bootstrap Open WebUI against AgentX Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.agentx/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-agentx.sh
...
#   AGENTX_API_PORT=8642
#   AGENTX_API_HOST=127.0.0.1
#   AGENTX_API_MODEL_NAME='AgentX Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-AgentX Agent WebUI}"
...
AGENTX_ENV_FILE="${AGENTX_ENV_FILE:-$HOME/.agentx/.env}"
AGENTX_API_PORT="${AGENTX_API_PORT:-8642}"
AGENTX_API_HOST="${AGENTX_API_HOST:-127.0.0.1}"
AGENTX_API_CONNECT_HOST="${AGENTX_API_CONNECT_HOST:-127.0.0.1}"
AGENTX_API_MODEL_NAME="${AGENTX_API_MODEL_NAME:-AgentX Agent}"
AGENTX_API_BASE_URL="http://${AGENTX_API_CONNECT_HOST}:${AGENTX_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-agentx.sh"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$AGENTX_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$AGENTX_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.agentx.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.agentx</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.agentx"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.agentx"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-agentx.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to AgentX Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-agentx.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.agentx/logs/openwebui.log
StandardError=append:%h/.agentx/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-agentx.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$AGENTX_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring AgentX API server is configured...'
  upsert_env API_SERVER_ENABLED true "$AGENTX_ENV_FILE"
  upsert_env API_SERVER_HOST "$AGENTX_API_HOST" "$AGENTX_ENV_FILE"
  upsert_env API_SERVER_PORT "$AGENTX_API_PORT" "$AGENTX_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$AGENTX_API_MODEL_NAME" "$AGENTX_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$AGENTX_ENV_FILE"
  ensure_env_permissions

  log 'Restarting AgentX gateway so API server settings take effect...'
  agentx gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${AGENTX_API_CONNECT_HOST}:${AGENTX_API_PORT}/health" >/dev/null; then
    log 'AgentX API server did not answer on the first check. Trying to start gateway in the background...'
    nohup agentx gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${AGENTX_API_CONNECT_HOST}:${AGENTX_API_PORT}/health" >/dev/null
  ...
  log "AgentX API endpoint: ${AGENTX_API_BASE_URL}"
