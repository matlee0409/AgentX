"""Centralized logging setup for AgentX Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.agentx/logs/`` (profile-aware via ``get_agentx_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``agentx_cli.web_server``,
    ``agentx_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from agentx_constants import get_config_path, get_agentx_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_agentx_session_injector", False):
    ...
    _session_record_factory._agentx_session_injector = True  # type: ignore[attr-defined]
...
COMPONENT_PREFIXES = {
    ...
    "gateway": ("gateway", "agentx_plugins", "plugins.platforms"),
...
def setup_logging(
    *,
    agentx_home: Optional[Path] = None,
    ...
    *force* is ``True``.
    ...
    home = agentx_home or get_agentx_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_agentx_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._agentx_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from agentx_cli.config import is_managed
