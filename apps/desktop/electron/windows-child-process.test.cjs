...
test('desktop background child processes opt into hidden Windows consoles', () => {
  ...
  requireHiddenChildOptions(source, /agentxProcess = spawn\(\s*backend\.command,\s*backend\.args/)
  requireHiddenChildOptions(source, /spawn\(\s*py,\s*\['-m', 'agentx_cli\.main', 'uninstall', '--gui-summary'\]/)

  assert.match(source, /function unwrapWindowsVenvAgentXCommand\(command, dashboardArgs\)/)
  assert.match(source, /existing AgentX no-console Python at/)
  ...
  assert.match(source, /AGENTX_DESKTOP_READY_FILE/)
  ...
  assert.match(source, /args: \['-m', 'agentx_cli\.main', \.\.\.dashboardArgs\]/)
})
