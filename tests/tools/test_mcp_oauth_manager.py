...
def test_agentx_provider_subclass_exists():
    """AgentXMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
...
def test_manager_builds_agentx_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns AgentXMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _AGENTX_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("AGENTX_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _AGENTX_PROVIDER_CLS is not None
    assert isinstance(provider, _AGENTX_PROVIDER_CLS)
    assert provider._agentx_server_name == "srv"
