"""
Unified tool configuration for AgentX Agent.
...
from agentx_cli.nous_subscription import (
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from agentx_cli.nous_account import format_nous_portal_entitlement_message
...
from agentx_cli.cli_output import (  # noqa: E402 — late import block
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from agentx_cli.auth import _read_xai_oauth_tokens
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from agentx_cli.plugins import discover_plugins, get_plugin_toolsets
...
from agentx_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_agentx_home()}/agentx-agent && npm install --workspaces=false")
                if result.stderr:
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/agentx-agent:latest"
            )
            return
        ...
        try:
            from agentx_cli.auth import login_spotify_command
        except Exception as exc:
        ...
        try:
            from agentx_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    print(color("  Guide: https://agentx-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from agentx_constants import display_agentx_home
    print(color(f"  Tool configuration saved to {display_agentx_home()}/config.yaml", Colors.DIM))
