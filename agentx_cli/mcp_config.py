...
from agentx_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_agentx_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from agentx_cli.cli_output import prompt as _shared_prompt
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    try:
        ...
        return AgentXTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # agentx_cli/main.py for why the dest is renamed.
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_agentx_home()}/.env as {env_key}")
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_agentx_home()}/config.yaml ({tool_count}/{total} tools enabled)")
