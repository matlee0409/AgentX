"""disk_cleanup — ephemeral file cleanup for AgentX Agent.
...
try:
    ...
    def get_agentx_home() -> Path:  # type: ignore[no-redef]
...
def get_state_dir() -> Path:
    ...
    return get_agentx_home() / "disk-cleanup"
...
def is_safe_path(path: Path) -> bool:
    ...
    try:
        path.resolve().relative_to(agentx_home)
        return True
...
def quick() -> Dict[str, Any]:
    ...
    try:
        for top in agentx_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    agentx_home = get_agentx_home()
    try:
        rel = path.resolve().relative_to(agentx_home)
