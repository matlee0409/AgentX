...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        tokens_file = get_agentx_home() / "slack_tokens.json"
        ...
        try:
            ...
            # AgentX does not currently need to route them into the agent loop.
            ...
            from agentx_cli.commands import slack_native_slashes
            import re as _re
            ...
            async def handle_agentx_command(ack, command):
            ...
            for _action_id in (
                "agentx_approve_once",
                "agentx_approve_session",
                "agentx_approve_always",
                "agentx_deny",
            ):
            ...
            for _action_id in (
                "agentx_confirm_once",
                "agentx_confirm_always",
                "agentx_confirm_cancel",
            ):
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        if original_text.startswith("!"):
            try:
                from agentx_cli.commands import is_gateway_known_command

                first_token = original_text[1:].split(maxsplit=1)[0]
                # Strip "@suffix" the same way get_command() does, so
                # forms like ``!stop@agentx`` still resolve.
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "agentx_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "agentx_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "agentx_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "agentx_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "agentx_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "agentx_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "agentx_confirm_once": "once",
            "agentx_confirm_always": "always",
            "agentx_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "agentx_approve_once": "once",
            "agentx_approve_session": "session",
            "agentx_approve_always": "always",
            "agentx_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        if slash_name in {"agentx", ""}:
            ...
            from agentx_cli.commands import slack_subcommand_map
        ...
        # "Running /cmd…" ack shown by handle_agentx_command).
...
# ``agentx_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``agentx_cli/setup.py::_setup_slack``.
    """
    from pathlib import Path
    from agentx_cli.config import get_env_value, save_env_value
    from agentx_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_header,
        print_info,
    ...
    def _write_slack_manifest_and_instruct() -> None:
        ...
        try:
            from agentx_cli.slack_cli import _build_full_manifest
            from agentx_constants import get_agentx_home
            import json as _json
            ...
            target = Path(get_agentx_home()) / "slack-manifest.json"
    ...
    print()
    print_info("   Full guide: https://agentx-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``agentx_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'agentx-agent[slack]'",
        # Interactive setup wizard — replaces agentx_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in agentx_cli/gateway.py.
