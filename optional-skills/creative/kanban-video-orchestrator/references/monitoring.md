...
agentx kanban watch --tenant <project-slug>

# Snapshot of the board
agentx kanban list --tenant <project-slug>
agentx kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
agentx kanban stats --tenant <project-slug>

# Visual dashboard (browser)
agentx dashboard

# Inspect a specific task (includes comments + events)
agentx kanban show <task-id>

# Follow a single task's event stream
agentx kanban tail <task-id>
```

Verify available subcommands with `agentx kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `agentx kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `agentx kanban block <id>` then `agentx kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `agentx kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `agentx kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `agentx profile list` (profile exists?), `agentx status` (gateway/dispatcher up?), and verify tenant. |
...
agentx kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
agentx kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(agentx kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
agentx kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
agentx kanban block <task-id>
agentx kanban archive <task-id>

# Diagnose what happened
agentx kanban show <task-id>      # task body, comments, recent events
agentx kanban tail <task-id>      # follow the live event stream
agentx kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `agentx kanban create` for the director
...
while true; do
    clear
    agentx kanban list --tenant <slug>
    echo "---"
    agentx kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `agentx kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `agentx profile list` and start
