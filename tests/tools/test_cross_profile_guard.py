...
def fake_agentx(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(fs, "_agentx_home_path", lambda: sec_home)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_agentx):
        from tools.file_tools import write_file_tool
        target = fake_agentx["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_agentx):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_agentx["root"] / "skills" / "shared-skill" / "SKILL.md"
    ...
    def test_cross_profile_True_bypass(self, fake_agentx):
    ...
    def test_non_agentx_path_unaffected(self, fake_agentx, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_agentx):
    ...
    def test_cross_profile_patch_bypass(self, fake_agentx):
        from tools.file_tools import patch_tool
        target = fake_agentx["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_agentx):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_multiple_profiles(self, fake_agentx, monkeypatch):
        """When the skill exists in TWO other profiles, both should be named."""
        self._make_skill_in_profile(fake_agentx["root"], "everywhere-skill")
        self._make_skill_in_profile(fake_agentx["coder_home"], "everywhere-skill")
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_agentx_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_agentx_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_agentx):
