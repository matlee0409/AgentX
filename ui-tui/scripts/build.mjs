...
await build({
  ...
  // Skip the prebuilt @agentx/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@agentx/ink': resolve(root, 'packages/agentx-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The agentx_cli launcher
