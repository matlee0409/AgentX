...
Managed scope is read from a system-level directory, default `/etc/agentx`:
...
files. It is **never persisted** to any `.env` by AgentX.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export AGENTX_MANAGED_DIR=/opt/org/agentx-policy
```
...
left user-settable. `agentx doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/agentx/config.yaml` (managed) | `/etc/agentx/.env` (managed) |
...
```bash
agentx config        # shows a header naming the managed source + the pinned keys
agentx doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, AgentX refuses and names the source:

```bash
$ agentx config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/agentx/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `agentx config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/agentx

# Pin some config values for every user on this machine
sudo tee /etc/agentx/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/agentx/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/agentx
sudo chmod 0644 /etc/agentx/config.yaml /etc/agentx/.env
```

Changes take effect on the next AgentX start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`agentx doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs AgentX as `root`), managed scope is advisory.
