...
When the model wants one of these, codex spawns the `agentx_tools_mcp_server` subprocess via stdio MCP, the call is dispatched through `model_tools.handle_function_call()` (same code path as AgentX' default runtime), and the result is returned to codex like any other MCP response.
...
2. **Codex OAuth login.** The codex subprocess reads `~/.codex/auth.json`. Two ways to populate it:
   ...
   ```
   AgentX' own `agentx auth login codex` writes to `~/.agentx/auth.json` — that's a separate session. **Run `codex login` separately** if you haven't.
...
```toml
[mcp_servers.agentx-tools]
command = "/path/to/python"
args = ["-m", "agent.transports.agentx_tools_mcp_server"]
env = { AGENTX_HOME = "/your/.agentx", PYTHONPATH = "...", AGENTX_QUIET = "1" }
startup_timeout_sec = 30.0
tool_timeout_sec = 600.0
```

When the model calls `web_search` (or another exposed AgentX tool), codex spawns the `agentx_tools_mcp_server` subprocess via stdio, the request is dispatched through `model_tools.handle_function_call()`, and the result is projected back to codex like any other MCP response.
...
This runtime is **opt-in beta**. Working as of AgentX Agent 2026.5 + Codex CLI 0.130.0:

- Multi-turn conversations
- `commandExecution` and `fileChange` (apply_patch) approvals via AgentX UI
- MCP tool calls (verified against `@modelcontextprotocol/server-filesystem` and the new `agentx-tools` callback)
...
- **AgentX auth and codex auth are separate sessions.** You need both `codex login` AND `agentx auth login codex` for the cleanest UX (the runtime uses codex's session for the LLM call). This is a deliberate design choice in AgentX' `_import_codex_cli_tokens` — AgentX won't share OAuth state with codex CLI to avoid clobbering each other on token refresh.
...
If you find a bug, [open an issue](https://github.com/NousResearch/agentx-agent/issues) with the output of `agentx logs --since 5m`. Mention `codex-runtime` in the title so it's easy to triage.
...
```
        ...
        └──────────────────────────────────┘           │
                                                        ...
                                                        ▼
        ┌──────────────────────────────────────────────────────────┐
        │  agentx_tools_mcp_server.py (subprocess on demand)        │
        │   web_search, web_extract, browser_*, vision_analyze,    │
        │   image_generate, skill_view, skills_list, text_to_speech│
        └──────────────────────────────────────────────────────────┘
```

For implementation details, see [PR #24182](https://github.com/NousResearch/agentx-agent/pull/24182) and the [Codex app-server protocol README](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md).
