...
`mcp_tool.py`, and `agentx_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# AgentXMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_agentx_provider_class() -> Optional[type]:
    ...
    class AgentXMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._agentx_server_name = server_name
            ...
            self._agentx_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`AgentXTokenStorage` persisting an absolute
            ...
            from tools.mcp_oauth import AgentXTokenStorage
            if (
                isinstance(storage, AgentXTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._agentx_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import AgentXTokenStorage
                        if isinstance(storage, AgentXTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._agentx_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import AgentXTokenStorage
            if not isinstance(storage, AgentXTokenStorage):
                return
    ...
    return AgentXMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_AGENTX_PROVIDER_CLS: Optional[type] = _make_agentx_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`AgentXMCPOAuthProvider` directly using the helpers
        ...
        storage = AgentXTokenStorage(server_name)
