...
import type { AgentXReadDirEntry, AgentXReadDirResult } from '@/global'
...
export type ProjectTreeEntry = AgentXReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: AgentXReadDirEntry) {
...
async function filterIgnored(entries: AgentXReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<AgentXReadDirResult> {
