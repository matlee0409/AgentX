...
from agentx_cli.dashboard_auth import clear_providers, register_provider
from tests.agentx_cli.conftest_dashboard_auth import StubAuthProvider
...
class TestGateRedirectsCarryPrefix:
    def test_html_redirect_to_login_carries_prefix(self, gated_app_proxied):
        ...
        assert r.headers["location"].startswith("/agentx/login"), (
...
class TestPublicUrlOverride:
    ...
    def patch_config(self, monkeypatch):
        """Replace ``agentx_cli.config.load_config`` with a stub
        ...
        def _set(public_url) -> None:
            ...
            monkeypatch.setattr(
                "agentx_cli.config.load_config", lambda: cfg
            )
    ...
    def test_public_url_config_yaml_used_when_env_unset(
        ...
        monkeypatch.delenv("AGENTX_DASHBOARD_PUBLIC_URL", raising=False)
    ...
    def test_public_url_with_path_prefix_baked_in(
        ...
        (``https://example.com/agentx``), the OAuth callback URL is
        the path appended verbatim. The operator is declaring the
        whole authority; we trust them."""
        patch_config(None)
        monkeypatch.setenv(
            "AGENTX_DASHBOARD_PUBLIC_URL", "https://example.com/agentx",
        )
        redirect_uri = self._redirect_uri(gated_app_direct)
        assert redirect_uri == "https://example.com/agentx/auth/callback"
    ...
    def test_public_url_ignores_x_forwarded_prefix(
        ...
        patch_config(None)
        monkeypatch.setenv(
            "AGENTX_DASHBOARD_PUBLIC_URL", "https://example.com/already-prefixed",
        )
    ...
    def test_public_url_strips_trailing_slash(
        ...
        patch_config(None)
        monkeypatch.setenv(
            "AGENTX_DASHBOARD_PUBLIC_URL", "https://example.com/",
        )
        redirect_uri = self._redirect_uri(gated_app_direct)
    ...
    def test_empty_public_url_env_treated_as_unset(
        ...
        entry."""
        monkeypatch.setenv("AGENTX_DASHBOARD_PUBLIC_URL", "")
        patch_config("https://from-config.example")
        redirect_uri = self._redirect_uri(gated_app_direct)
        assert redirect_uri == "https://from-config.example/auth/callback"
    ...
    def test_scheme_less_public_url_env_warns_operator(
        ...
        from agentx_cli.dashboard_auth import prefix as prefix_mod
    ...
    def test_scheme_less_public_url_warning_is_deduplicated(
        ...
        scheme_warnings = [
            ...
            for r in caplog.records
            if r.levelno == logging.WARNING
            and "agentx.domain.com" in r.getMessage()
        ]
    ...
    def test_valid_public_url_emits_no_warning(
        ...
        patch_config(None)
        monkeypatch.setenv(
            "AGENTX_DASHBOARD_PUBLIC_URL", "https://agentx.domain.com"
        )

        with caplog.at_level(logging.WARNING, logger=prefix_mod.__name__):
            result = prefix_mod.resolve_public_url()

        assert result == "https://agentx.domain.com"
        assert not [
...
class TestCookiePathRespectsPrefix:
    ...
    def test_pkce_cookie_uses_prefix_path(self, gated_app_proxied):
        ...
        pkce = next(c for c in cookies if "agentx_session_pkce" in c)
        # Browser only sends cookie back if the request path is under
        # the cookie's Path attribute, so we need /agentx here. Bare
        # /-rooted cookies would still be sent but would also be sent
        # to /billing/... etc.
        assert "Path=/agentx" in pkce, (
    ...
    def test_pkce_cookie_uses_secure_prefix_when_proxied(
        ...
        pkce_candidates = [
            c for c in cookies
            if c.startswith("__Secure-agentx_session_pkce=")
        ]
    ...
    def test_pkce_cookie_uses_host_prefix_when_direct(
        ...
        pkce_candidates = [
            c for c in cookies
            if c.startswith("__Host-agentx_session_pkce=")
        ]
        assert pkce_candidates, (
    ...
    def test_loopback_cookies_unprefixed(self):
        ...
        from agentx_cli.dashboard_auth.cookies import set_pkce_cookie
        ...
        assert any(c.startswith("agentx_session_pkce=") for c in cookies), (
    ...
    def test_cookies_read_back_round_trip_through_prefix(
        ...
        pkce_set = next(
            ...
            if "agentx_session_pkce" in c
        )
        # Parse "__Secure-agentx_session_pkce=...; HttpOnly; ...".
        pkce_kv = pkce_set.split(";", 1)[0]  # "__Secure-agentx_session_pkce=value"
        ...
        at_cookies = [
            c for c in cookies
            if c.startswith("__Secure-agentx_session_at=")
        ]
        assert at_cookies, (
