...
class TestWriteDenyExactPaths:
    ...
    def test_agentx_env(self):
        # ``.env`` under the active AGENTX_HOME (profile-aware, not just
        # ``~/.agentx``) must be write-denied. The hermetic test conftest
        # points AGENTX_HOME at a tempdir — resolve via get_agentx_home()
        # to match the denylist.
        from agentx_constants import get_agentx_home
        path = str(get_agentx_home() / ".env")
        assert _is_write_denied(path) is True

    def test_agentx_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        from agentx_constants import get_agentx_home, get_default_agentx_root
        assert get_agentx_home() == profile_home
        assert get_default_agentx_root() == root
...
class TestWriteAllowed:
    ...
    def test_agentx_control_files_requested_writable(self):
        from agentx_constants import get_agentx_home
