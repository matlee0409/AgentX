...
Module-level run.py helpers a handler needs (``_agentx_home``,
...
from agentx_cli.config import cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
    atomic_yaml_write,
...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_kanban_command(self, event: MessageEvent) -> str:
        ...
        import re
        import shlex
        from agentx_cli.kanban import run_slash
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _agentx_home
        ...
        try:
            ...
            atomic_json_write(
                _agentx_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            atomic_json_write(
                _agentx_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running AgentX Agent version."""
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from agentx_cli.commands import gateway_help_lines
        lines = [
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from gateway.run import _agentx_home, _load_gateway_config
        import yaml
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected(
                        ...
                        try:
                            from agentx_cli.context_switch_guard import (
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return t("gateway.personality.none_configured", path=display_agentx_home())
    ...
    async def _handle_goal_command(self, event: "MessageEvent") -> str:
        ...
        if lower.startswith("draft"):
            ...
            try:
                import asyncio
                from agentx_cli.goals import draft_contract
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _agentx_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _agentx_home, _platform_config_key
        import yaml
    ...
    async def _handle_memory_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _agentx_home
        from agentx_cli.write_approval_commands import handle_pending_subcommand
    ...
    async def _handle_fast_command(self, event: MessageEvent) -> str:
        ...
        from gateway.run import _agentx_home, _load_gateway_config, _resolve_gateway_model
        import yaml
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _agentx_home, _load_gateway_config, _platform_config_key
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _agentx_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from agentx_cli.session_listing import (
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update AgentX Agent to the latest version.
        ...
        """
        from gateway.run import _agentx_home, _resolve_agentx_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        from agentx_cli.config import is_managed, format_managed_message
        ...
        if is_managed():
            return f"✗ {format_managed_message('update AgentX Agent')}"
        ...
        if not agentx_cmd:
            return t("gateway.update.agentx_cmd_not_found")
        ...
        try:
            if sys.platform == "win32":
                ...
                # agentx_cmd is a list of argv parts we can pass directly
                ...
                agentx_cmd_str = " ".join(shlex.quote(part) for part in agentx_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {agentx_cmd_str} update --gateway"
