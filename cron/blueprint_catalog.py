...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``agentx://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``agentx://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"agentx://blueprint/{quote(blueprint.key)}{qs}"
