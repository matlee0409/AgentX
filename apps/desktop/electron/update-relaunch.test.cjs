...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  // AppImage mount
  assert.equal(resolveUnpackedRelease('/tmp/.mount_AgentX12345/AppRun', ROOT, 'linux'), null)
  // .deb / .rpm system install
  assert.equal(resolveUnpackedRelease('/usr/lib/agentx/agentx', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease('/opt/AgentX/agentx', ROOT, 'linux'), null)
  // dev electron
  assert.equal(
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  const script = buildRelaunchScript({
    pid: 4242,
    execPath: '/home/u/.agentx/agentx-agent/apps/desktop/release/linux-unpacked/AgentX',
  ...
  assert.match(script, /exec '.*\/linux-unpacked\/AgentX' 'agentx:\/\/open\/agent\/42' '--note=it'\\''s fine'/)
...
test('buildRelaunchScript with no args/env still lints clean', () => {
  ...
  assert.match(script, /exec '\/opt\/AgentX\/AgentX'\n/)
})
