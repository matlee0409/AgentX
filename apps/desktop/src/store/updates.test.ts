...
const checkAgentXUpdateSpy = vi.fn()
const updateAgentXSpy = vi.fn()
const getActionStatusSpy = vi.fn()

vi.mock('@/agentx', () => ({
  checkAgentXUpdate: (...args: unknown[]) => checkAgentXUpdateSpy(...args),
  updateAgentX: (...args: unknown[]) => updateAgentXSpy(...args),
...
describe('checkBackendUpdates', () => {
  beforeEach(() => {
    storage.clear()
    notifySpy.mockClear()
    checkAgentXUpdateSpy.mockReset()
  ...
  it('maps the backend /update/check onto the backend status, including commits', async () => {
    setRemote(true)
    checkAgentXUpdateSpy.mockResolvedValue({
      install_method: 'git',
    ...
    expect(checkAgentXUpdateSpy).toHaveBeenCalled()
  ...
  it('is a no-op in local mode (backend check only runs when remote)', async () => {
    ...
    expect(checkAgentXUpdateSpy).not.toHaveBeenCalled()
  })
})
...
describe('applyBackendUpdate recovery', () => {
  ...
  it('waits for the backend to return after the restart drops the connection, then clears the overlay', async () => {
    updateAgentXSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
  ...
  it('surfaces an error when the backend never comes back after the restart', async () => {
    updateAgentXSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkAgentXUpdateSpy.mockRejectedValue(new Error('ECONNREFUSED'))
