...
Optional AgentX observability plugin that maps AgentX observer hooks to
...
not replace AgentX Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets AgentX emit NeMo Relay lifecycle events for
the work AgentX already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, AgentX Agent can:

- Preserve AgentX execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/agentx_agent-*.whl
...
exposes `llm.execute(...)` / `tools.execute(...)`, AgentX routes LLM and tool
...
export AGENTX_NEMO_RELAY_ATIF_AGENT_NAME='AgentX Agent E2E'
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "AgentX Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
- AgentX API request hooks map to `llm` scope start/end events.
- AgentX tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses AgentX execution middleware to hand
...
When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, AgentX routes execution
...
AgentX provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> AgentX provider adapter next_call(...)

AgentX tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> AgentX tool dispatcher next_call(...)
```
