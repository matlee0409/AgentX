...
sidebar_position: 5
title: "添加 Provider"
description: "如何向 AgentX Agent 添加新的推理 provider——认证、运行时解析、CLI 流程、适配器、测试与文档"
---
...
1. `agentx_cli/auth.py` 决定如何查找凭据。
2. `agentx_cli/runtime_provider.py` 将其转换为运行时数据：
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `agentx_cli/models.py` 和 `agentx_cli/main.py` 使 provider 在 CLI 中可见。（`agentx_cli/setup.py` 自动委托给 `main.py`——无需在此处做任何修改。）
...
:::tip
`agentx_cli/setup.py` **无需**修改。setup 向导将 provider/model 选择委托给 `main.py` 中的 `select_provider_and_model()`——在那里添加的任何 provider 都会自动出现在 `agentx setup` 中。
:::
...
- `agentx_cli/auth.py` 中的 `PROVIDER_REGISTRY`
- `agentx_cli/models.py` 中的 `_PROVIDER_LABELS`
- `agentx_cli/auth.py` 和 `agentx_cli/models.py` 中的 `_PROVIDER_ALIASES`
- `agentx_cli/main.py` 中的 CLI `--provider` 选项
- setup / 模型选择分支
- 辅助模型默认值
- 测试
...
## 第 2 步：在 `agentx_cli/auth.py` 中添加认证元数据
...
## 第 3 步：在 `agentx_cli/models.py` 中添加模型目录和别名
...
## 第 4 步：在 `agentx_cli/runtime_provider.py` 中解析运行时数据
...
## 第 5 步：在 `agentx_cli/main.py` 中接线 CLI

在交互式 `agentx model` 流程中出现之前，provider 是不可发现的。

在 `agentx_cli/main.py` 中更新以下内容：
...
:::tip
`agentx_cli/setup.py` 无需修改——它调用 `main.py` 中的 `select_provider_and_model()`，因此你的新 provider 会自动出现在 `agentx model` 和 `agentx setup` 中。
:::
...
- [ ] 在 `agentx_cli/auth.py` 中添加 `ProviderConfig`
- [ ] 在 `agentx_cli/auth.py` 和 `agentx_cli/models.py` 中添加别名
- [ ] 在 `agentx_cli/models.py` 中添加模型目录
- [ ] 在 `agentx_cli/runtime_provider.py` 中添加运行时分支
- [ ] 在 `agentx_cli/main.py` 中添加 CLI 接线（setup.py 自动继承）
