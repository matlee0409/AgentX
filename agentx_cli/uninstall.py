    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# AgentX Agent" comment and following line
                if '# AgentX Agent' in line or '# agentx-agent' in line:
                    skip_next = True
                    continue
...
def remove_wrapper_script():
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains agentx_cli reference)
                content = wrapper.read_text()
                if 'agentx_cli' in content or 'agentx-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(agentx_home: Path) -> list:
    ...
    """
    node_dir = (agentx_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    try:
        from agentx_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from agentx_cli.gateway import get_launchd_plist_path
...
def _agentx_path_markers(agentx_home: Path) -> list[str]:
    """Path-entry substrings that identify AgentX-owned User-PATH entries."""
    root = str(agentx_home).rstrip("\\/")
    ...
    # unrelated entries like "cagentx-foo" or "ephermeral", so we look for
    # backslash-agentx as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(agentx_home: Path) -> list[str]:
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _agentx_path_markers(agentx_home)
            kept: list[str] = []
...
def remove_agentx_env_vars_windows() -> list[str]:
...
def remove_portable_tooling_windows(agentx_home: Path) -> list[Path]:
...
def _is_default_agentx_home(agentx_home: Path) -> bool:
    """Return True when ``agentx_home`` points at the default (non-profile) root."""
    try:
        from agentx_constants import get_default_agentx_root
        return agentx_home.resolve() == get_default_agentx_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    #    Use `python -m agentx_cli.main` so we don't depend on a `agentx`
    #    wrapper that may be half-removed mid-uninstall.
    agentx_invocation = [_sys.executable, "-m", "agentx_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                agentx_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
...
def run_gui_uninstall(args):
    ...
    """
    from agentx_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(agentx_home)
    ...
    if not summary["gui_installed"]:
        ...
        print(f"  Checked: {agentx_home}, and the standard app locations for this OS.")
        return
    ...
    print()
    if agent_is_installed(agentx_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The AgentX agent at {agentx_home / 'agentx-agent'}")
        print(f"  • Your config, sessions, and secrets under {agentx_home}")
        print()
...
def run_uninstall(args):
    ...
    is_default_profile = _is_default_agentx_home(agentx_home)
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ AgentX Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {agentx_home / 'config.yaml'}")
    print(f"  Secrets: {agentx_home / '.env'}")
    print(f"  Data:    {agentx_home / 'cron/'}, {agentx_home / 'sessions/'}, {agentx_home / 'logs/'}")
    print()
...
def _perform_uninstall(
    *,
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in agentx_home so the marker matching is
        ...
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(agentx_home))))
        ...
        removed_env = remove_agentx_env_vars_windows()
        if removed_env:
    ...
    removed_node_links = remove_node_symlinks(agentx_home)
    ...
    #     step-5 rmtree(agentx_home) would sweep the in-tree artifacts anyway,
    ...
    try:
        from agentx_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(agentx_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            ...
            if agentx_home in project_root.parents or project_root.parent == agentx_home:
    ...
    #     the step-5 rmtree(agentx_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(agentx_home)
    ...
    if full_uninstall:
        ...
        try:
            if agentx_home.exists():
                shutil.rmtree(agentx_home)
                log_success(f"Removed {agentx_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {agentx_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {agentx_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/matlek0409/AgentX/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/matlek0409/AgentX/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using AgentX Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m agentx_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import agentx_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``agentx_constants`` + ``agentx_cli.colors``
    (and lazily ``agentx_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m agentx_cli.uninstall")
    parser.add_argument(
        "--mode",
