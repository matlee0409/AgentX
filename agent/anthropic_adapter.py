"""Anthropic Messages API adapter for AgentX Agent.
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      ...
      4. Anthropic credential_pool OAuth entry (~/.agentx/auth.json)
    ...
    # 4. AgentX credential_pool OAuth entry.
    resolved_pool_token = _resolve_anthropic_pool_token()
    if resolved_pool_token:
        return resolved_pool_token

    # 5. Regular API key, or a legacy OAuth token saved in ANTHROPIC_API_KEY.
    # This remains as a compatibility fallback for pre-migration AgentX configs.
...
_AGENTX_OAUTH_FILE = get_agentx_home() / ".anthropic_oauth.json"
...
def run_agentx_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    try:
        from agentx_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_agentx_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read AgentX-managed OAuth credentials from ~/.agentx/.anthropic_oauth.json."""
    if _AGENTX_OAUTH_FILE.exists():
        try:
            data = json.loads(_AGENTX_OAUTH_FILE.read_text(encoding="utf-8"))
