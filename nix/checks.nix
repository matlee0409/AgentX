...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      ...
      agentxVenv = agentx-agent.agentxVenv;
    ...
    in {
      ...
      checks = {
        ...
        bundled-skills = pkgs.runCommand "agentx-bundled-skills" { } ''
          ...
          SKILL_COUNT=$(find ${agentx-agent}/share/agentx-agent/skills -name "SKILL.md" | wc -l)
        ...
        bundled-locales = pkgs.runCommand "agentx-bundled-locales" { } ''
          ...
          LOC_COUNT=$(find ${agentx-agent}/share/agentx-agent/locales -name "*.yaml" | wc -l)
          ...
          echo "=== Rendering via the wrapper override (AGENTX_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && AGENTX_BUNDLED_LOCALES=${agentx-agent}/share/agentx-agent/locales \
            ${agentxVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          ...
          BARE_DIR=$(cd "$HOME" && ${agentxVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${agentxVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without AGENTX_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        # Verify AGENTX_NODE is set in wrapper and points to Node 20+
        ...
        extra-python-packages = let
          ...
          agentxWithExtra = agentx-agent.override {
        ...
        in pkgs.runCommand "agentx-extra-python-packages" { } ''
          ...
          grep -q "PYTHONPATH" ${agentxWithExtra}/bin/agentx || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${agentxWithExtra}/bin/agentx || \
        ...
        extra-dependency-groups = let
          agentxWithGroups = agentx-agent.override {
        ...
        in pkgs.runCommand "agentx-extra-dependency-groups" { } ''
          ...
          echo "derivation: ${agentxWithGroups}"
          echo "venv: ${agentxWithGroups.agentxVenv}"
        ...
        messaging-variant = pkgs.runCommand "agentx-messaging-variant" { } ''
          ...
          ${self'.packages.messaging.agentxVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "agentx-config-roundtrip" {
          ...
          merge_and_load() {
            local agentx_home="$1"
            export AGENTX_HOME="$agentx_home"
            ${configMergeScript} ${nixSettings} "$agentx_home/config.yaml"
            ${agentxVenv}/bin/python3 -c '
import json, sys
