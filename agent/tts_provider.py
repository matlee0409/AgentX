...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into AgentX with shell-template placeholders. **Wins over a
...
``{success: False, error: …}`` JSON envelope the rest of AgentX
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``agentx tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``agentx setup`` for
