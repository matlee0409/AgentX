"""Tests for agentx_bootstrap — Windows UTF-8 stdio shim.
...
Key invariants covered by these tests:
  ...
  5. Load order: every AgentX entry point imports agentx_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported agentx_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("agentx_bootstrap", None)
    import agentx_bootstrap  # noqa: WPS433
    return agentx_bootstrap
...
class TestEntryPointsImportBootstrap:
    """Every AgentX entry point must import agentx_bootstrap as its
    ...
    # import agentx_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "agentx_cli/main.py",   # agentx CLI (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import agentx_bootstrap' and that
        ...
        ``agentx_bootstrap`` as a top-level module) leaves agentx
        ...
        # at tests/test_agentx_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # agentx start even when agentx_bootstrap hasn't been
        ...
        assert first_import_name == "agentx_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'agentx_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import agentx_bootstrap' line to be the first import."
        )
...
class TestHardenImportPath:
    ...
    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/agentx", "/opt/agentx", ""])
        assert result.count("/opt/agentx") == 1
