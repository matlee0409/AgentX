...
- `agentx_cli/runtime_provider.py` — credential resolution, `_resolve_custom_runtime()`
- `agentx_cli/auth.py` — provider registry, `resolve_provider()`
- `agentx_cli/model_switch.py` — shared `/model` switch pipeline (CLI + gateway)
...
AgentX contains logic to avoid leaking the wrong API key to a custom endpoint when multiple provider keys exist (e.g. `OPENROUTER_API_KEY` and `OPENAI_API_KEY`).
...
- `tests/agentx_cli/test_fallback_cmd.py` — the `/fallback` CLI command
