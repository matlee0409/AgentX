...
Pure display functions with no AgentXCLI state dependency.
"""
...
from agentx_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
_UPSTREAM_REPO_URL = "https://github.com/NousResearch/agentx-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/agentx-agent"
...
def _fetch_pypi_latest(package: str = "agentx-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    ...
    cache_file = agentx_home / ".update_check"
    embedded_rev = os.environ.get("AGENTX_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # AGENTX_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `agentx update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/agentx/update/check`
    ...
    if embedded_rev:
        ...
        if not (repo_dir / ".git").exists():
            repo_dir = agentx_home / "agentx-agent"
        if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``agentx_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from agentx_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/agentx-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical NousResearch/agentx-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"AgentX Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        from agentx_cli.codex_runtime_switch import get_current_runtime
        from agentx_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from agentx_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
    ...
    # Pip-install warning — `pip install agentx-agent` is not the supported
