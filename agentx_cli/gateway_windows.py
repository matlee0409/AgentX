...
_TASK_DESCRIPTION = "AgentX Agent Gateway - Messaging Platform Integration"
...
def _preserve_agentx_home_path(path: str | Path) -> str:
...
def _current_profile_cli_args() -> list[str]:
    ...
    from agentx_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "agentx_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    """
    _assert_windows()
    # Local import to avoid circular module initialization during agentx_cli boot.
    from agentx_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    script_dir = Path(get_agentx_home()) / "gateway-service"
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - invokes ``pythonw -m agentx_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "AGENTX_HOME={agentx_home}"')
    ...
    # if someone imports agentx_constants-based logic during startup.
    lines.append(f'set "VIRTUAL_ENV={_preserve_agentx_home_path(venv_dir)}"')
    pythonpath_entries = [
        _preserve_agentx_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_agentx_home_path(entry) for entry in extra_pythonpath],
    ]
    lines.append(f'set "PYTHONPATH={";".join([*pythonpath_entries, "%PYTHONPATH%"])}"')

    prog_args = [pythonw_path, "-m", "agentx_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    repo_root = _preserve_agentx_home_path(Path(__file__).resolve().parent.parent)
    static_pythonpath = os.pathsep.join(
        [repo_root, *[_preserve_agentx_home_path(entry) for entry in extra_pythonpath]]
    )
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('AGENTX_HOME')}) = {_quote_vbs_string(agentx_home)}",
        ...
        f"env.Item({_quote_vbs_string('VIRTUAL_ENV')}) = {_quote_vbs_string(_preserve_agentx_home_path(venv_dir))}",
...
def _write_task_script() -> Path:
    ...
    python_path = _preserve_agentx_home_path(get_python_path())
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    agentx_home = str(Path(get_agentx_home()))
    profile_arg = _profile_arg(agentx_home)

    content = _build_gateway_cmd_script(python_path, working_dir, agentx_home, profile_arg)
    ...
    vbs_content = _build_gateway_vbs_script(python_path, working_dir, agentx_home, profile_arg)
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(
        _preserve_agentx_home_path(get_python_path())
    )
    project_root = _preserve_agentx_home_path(PROJECT_ROOT)
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    agentx_home = str(Path(get_agentx_home()))
    profile_arg = _profile_arg(agentx_home)

    argv = [python_exe, "-m", "agentx_cli.main"]
    if profile_arg:
    ...
    env_overlay = {
        ...
        "VIRTUAL_ENV": _preserve_agentx_home_path(venv_dir),
    }
    _prepend_pythonpath(
        env_overlay,
        [project_root, *[_preserve_agentx_home_path(entry) for entry in extra_pythonpath]]
        if extra_pythonpath
        else [project_root],
    )
...
def windowless_gateway_restart_spec(
    ...
    ``agentx_cli`` package without the venv launcher's site config.

    Returns ``(new_argv, working_dir, env_overlay)``.  ``new_argv``
    preserves every argument after the interpreter (``-m agentx_cli.main
    ...
    from agentx_cli.config import get_agentx_home
    from agentx_cli.gateway import PROJECT_ROOT
    ...
    try:
        agentx_home = str(Path(get_agentx_home()).resolve())
    except Exception:
        agentx_home = ""
    ...
    if agentx_home:
        env_overlay["AGENTX_HOME"] = agentx_home
    _prepend_pythonpath(
        env_overlay,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m agentx_cli.main gateway run``
    ...
    log_dir = Path(get_agentx_home()) / "logs"
...
def _prompt_install_choices(
    ...
    from agentx_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    from agentx_cli.gateway import find_gateway_pids, _profile_arg
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from agentx_cli.config import get_agentx_home
        print(f"    type {Path(get_agentx_home())}\\logs\\gateway.log")
        print(f"    type {Path(get_agentx_home())}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from agentx_cli.config import get_agentx_home

    agentx_home = Path(get_agentx_home())
    print()
    print("Next steps:")
    print("  agentx gateway status                      # Check status")
    print(f"  type {agentx_home}\\logs\\gateway.log       # View logs")
