...
def _gateway_command_subcommand(command: str | None) -> str | None:
    ...
    test also matched ``agentx_cli.main gateway status`` and even unrelated
    ...
    joined = " ".join(tokens)
    has_gateway_entry = (
        "agentx_cli.main" in joined
        or "agentx_cli/main.py" in joined
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``agentx_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"agentx_home={home_lc}" in command_lc
        )
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "agentx_home=" in command_lc and f"agentx_home={home_lc}" not in command_lc:
        return False
    return True
...
# enabled in the user's systemd (e.g. ``agentx.service`` + ``agentx-
# gateway.service``).
#
