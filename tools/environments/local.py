...
def _inject_context_agentx_home(env: dict) -> None:
    """Bridge the context-local AgentX home override into subprocess env."""
    try:
        from agentx_constants import get_agentx_home_override

        value = get_agentx_home_override()
        if value:
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_AGENTX_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _AGENTX_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value
    ...
    for key, value in (extra_env or {}).items():
        ...
        elif key not in _AGENTX_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    _inject_context_agentx_home(sanitized)
...
def _find_bash() -> str:
    ...
    _agentx_portable_git = os.path.join(_local_appdata, "agentx", "git") if _local_appdata else ""
    if _agentx_portable_git:
        for candidate in (
            os.path.join(_agentx_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_agentx_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    raise RuntimeError(
        "Git Bash not found. AgentX Agent requires Git for Windows on Windows.\n"
...
def _resolve_agentx_bin_dir() -> str | None:
...
def _prepend_agentx_bin_dir(existing_path: str) -> str:
    ...
    """
    bin_dir = _resolve_agentx_bin_dir()
    if not bin_dir:
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_AGENTX_PROVIDER_ENV_FORCE_PREFIX):
            ...
            run_env[real_key] = v
        elif k not in _AGENTX_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    ...
    if path_key is not None:
        ...
        run_env[path_key] = _prepend_agentx_bin_dir(new_path)

    _inject_context_agentx_home(run_env)
...
class LocalEnvironment(BaseEnvironment):
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        if _IS_WINDOWS:
            ...
            try:
                ...
                cache_dir = get_agentx_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "agentx_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._agentx_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_agentx_pgid", None)
                    if pgid is None:
                        raise
