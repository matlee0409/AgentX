...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            agentx_home = os.path.expanduser(
                os.getenv(
                    "AGENTX_HOME", os.path.join(os.path.expanduser("~"), ".agentx")
                )
            )
            candidates = [
                os.path.join(agentx_home, "node", "bin", resolved_command),
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'agentx-agent[mcp]'\n"
                "or (full install):\n"
                "  pip install 'agentx-agent[all]'"
            )
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    ...
    try:
        ...
        home_override = get_agentx_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_agentx_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # vanish here: OAuth token stores and any other get_agentx_home()
