...
def provider(monkeypatch, tmp_path):
    ...
    p.initialize("session-1", agentx_home=str(tmp_path), platform="cli")
    return p
...
def test_identity_template_resolved_in_container_tag(monkeypatch, tmp_path):
    ...
    p = SupermemoryMemoryProvider()
    p.initialize("s1", agentx_home=str(tmp_path), platform="cli", agent_identity="coder")
...
def test_identity_template_default_profile(monkeypatch, tmp_path):
    ...
    _save_supermemory_config({"container_tag": "agentx-{identity}"}, str(tmp_path))
    p = SupermemoryMemoryProvider()
    p.initialize("s1", agentx_home=str(tmp_path), platform="cli")
    assert p._container_tag == "agentx_default"
...
def test_multi_container_enabled_adds_schema_param(monkeypatch, tmp_path):
    ...
    assert p._allowed_containers == ["agentx", "project_alpha", "shared"]
