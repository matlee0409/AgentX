...
def tail_log(
    ...
    *,
    ...
    log_path = get_agentx_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when AgentX runs — try 'agentx chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_agentx_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_agentx_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_agentx_home()}/logs/")
        return

    print(f"Log files in {display_agentx_home()}/logs/:\n")
    found = False
