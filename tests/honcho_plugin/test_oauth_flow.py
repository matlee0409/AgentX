...
def test_client_id_defaults_to_agentx_agent(monkeypatch):
...
def test_launcher_reports_error_on_flow_failure(monkeypatch, reset_flow):
    ...
    oauth_flow.start_loopback_flow_background(config_path=Path("/t/honcho.json"), host="agentx")
...
def test_launcher_is_idempotent_while_pending(monkeypatch, reset_flow):
    ...
    s1 = oauth_flow.start_loopback_flow_background(config_path=Path("/t/h.json"), host="agentx")
    assert _wait_until(lambda: len(calls) == 1)  # first flow is running
    s2 = oauth_flow.start_loopback_flow_background(config_path=Path("/t/h.json"), host="agentx")
    block.set()
...
def test_memory_oauth_router_dispatches_by_provider_convention():
    ...
    from agentx_cli.memory_oauth import _resolve_flow
