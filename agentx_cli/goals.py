...
- When a real user message arrives mid-loop it preempts the continuation
  ...
  will say ``done``).
- This module has zero hard dependency on ``cli.AgentXCLI`` or the gateway
...
def _get_session_db() -> Optional[Any]:
    ...
    ``agentx_home`` path so profile switches still pick up the right DB.
