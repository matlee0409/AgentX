...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - agentx_home (str): The active AGENTX_HOME directory path. Use this
    ...
    def save_config(self, values: Dict[str, Any], agentx_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'agentx memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``agentx_home`` is the active AGENTX_HOME directory path.
