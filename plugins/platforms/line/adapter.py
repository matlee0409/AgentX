"""
LINE Messaging API platform adapter for AgentX Agent.
...
to AgentX Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        try:
            ...
            agentx_home = Path(get_agentx_home()).resolve()
        except Exception:
            agentx_home = Path.home().joinpath(".agentx").resolve()
...
def interactive_setup() -> None:
    ...
    one optional public URL. Writes to ``~/.agentx/.env`` via ``agentx_cli.config``.
    """
    print()
    ...
    try:
        from agentx_cli.config import get_env_var, set_env_var
    except ImportError:
        print("agentx_cli.config not available; set LINE_* vars manually in ~/.agentx/.env")
        return
