...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'agentx:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.agentxDesktop?.saveImageFromUrl) {
        if (await window.agentxDesktop.saveImageFromUrl(src)) {
