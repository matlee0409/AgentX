...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *AgentX Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    ...
    agentx_home_bridge = agentx_home / "scripts" / "whatsapp-bridge"
    ...
    if agentx_home_bridge.exists():
        return agentx_home_bridge

    # Mirror the bridge source to AGENTX_HOME
    try:
        agentx_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            agentx_home_bridge,
            dirs_exist_ok=False,
