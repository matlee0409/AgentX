...
def isolated_kanban_home(monkeypatch):
    ...
    for mod in list(sys.modules.keys()):
        if mod.startswith("agentx_cli") or mod.startswith("agentx_state") or mod == "agentx_constants":
            del sys.modules[mod]
    yield test_home
...
def test_cli_dispatch_passes_max_in_progress_from_config(isolated_kanban_home, monkeypatch):
    ...
    monkeypatch.setattr(
        "agentx_cli.config.load_config", lambda: fake_config
    )
...
def test_cli_max_flag_overrides_config_max_spawn(isolated_kanban_home, monkeypatch):
    ...
    monkeypatch.setattr("agentx_cli.config.load_config", lambda: fake_config)
