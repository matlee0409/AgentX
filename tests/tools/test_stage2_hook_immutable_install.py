...
Hosted/container AgentX keeps user-writable state under AGENTX_HOME
(/opt/data). The installed source, venv, TUI bundle, and node_modules under
/opt/agentx must remain root-owned/non-writable by the runtime agentx user so
...
def test_stage2_does_not_chown_install_tree_to_agentx(stage2_text: str) -> None:
    ...
    for install_tree in (
        ...
        assert install_tree not in stage2_text, (
            f"stage2 must not chown {install_tree} back to agentx; "
            "the Dockerfile keeps /opt/agentx immutable and writable state "
            "belongs under AGENTX_HOME"
        )
...
def test_stage2_documents_immutable_install_contract(stage2_text: str) -> None:
    ...
    assert "AGENTX_DISABLE_LAZY_INSTALLS=1" in stage2_text
    assert "/opt/agentx" in stage2_text
