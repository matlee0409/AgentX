...
description: "How to add a new inference provider to AgentX Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---
...
1. `agentx_cli/auth.py` decides how credentials are found.
2. `agentx_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `agentx_cli/models.py` and `agentx_cli/main.py` make the provider show up in the CLI. (`agentx_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `agentx_cli/auth.py`
2. `agentx_cli/models.py`
3. `agentx_cli/runtime_provider.py`
4. `agentx_cli/main.py`
...
:::tip
`agentx_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `agentx setup`.
:::
...
- `PROVIDER_REGISTRY` in `agentx_cli/auth.py`
- `_PROVIDER_LABELS` in `agentx_cli/models.py`
- `_PROVIDER_ALIASES` in both `agentx_cli/auth.py` and `agentx_cli/models.py`
- CLI `--provider` choices in `agentx_cli/main.py`
...
## Step 2: Add auth metadata in `agentx_cli/auth.py`
...
## Step 3: Add model catalog and aliases in `agentx_cli/models.py`
...
## Step 4: Resolve runtime data in `agentx_cli/runtime_provider.py`
...
:::tip
`agentx_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `agentx model` and `agentx setup` automatically.
:::
...
- `tests/agentx_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/agentx_cli/test_model_switch_custom_providers.py` (and adjacent `tests/agentx_cli/test_model_switch_*.py`)
- `tests/agentx_cli/test_setup_model_provider.py`
...
python -m pytest tests/agentx_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/agentx_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -n0 -q
```
...
```bash
source venv/bin/activate
python -m agentx_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m agentx_cli.main model
python -m agentx_cli.main setup
```
...
- [ ] `ProviderConfig` added in `agentx_cli/auth.py`
- [ ] aliases added in `agentx_cli/auth.py` and `agentx_cli/models.py`
- [ ] model catalog added in `agentx_cli/models.py`
- [ ] runtime branch added in `agentx_cli/runtime_provider.py`
- [ ] CLI wiring added in `agentx_cli/main.py` (setup.py inherits automatically)
