...
Shows the status of all AgentX Agent components.
"""
...
from agentx_cli.auth import AuthError, resolve_provider
from agentx_cli.colors import Colors, color
from agentx_cli.config import get_env_path, get_env_value, get_agentx_home, load_config
...
from agentx_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    """Show status of all AgentX Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ AgentX Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if _effective_provider_label() == "LM Studio":
        from agentx_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from agentx_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_agentx_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_agentx_home() / "sessions" / "sessions.json"
