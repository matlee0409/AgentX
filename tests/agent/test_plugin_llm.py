...
""",
        ...
        from agentx_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        agentx_home = tmp_path / ".agentx"
        agentx_home.mkdir()
        (agentx_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
        from agentx_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from agentx_cli.plugins import PluginContext, PluginManifest, PluginManager
