...
``agentx_cli.main`` skips eager plugin discovery at argparse-setup time
...
def _live_subcommand_names() -> set[str]:
    ...
    built-in-only set.
    """
    from agentx_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["agentx", "--tui", "chat"], "chat"),
        (["agentx", "-w", "logs"], "logs"),
        (["agentx", "chat", "hello world"], "chat"),
        (["agentx", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["agentx", "-m", "gpt5", "chat"], "chat"),
        (["agentx", "--model", "gpt5", "chat", "hi"], "chat"),
        (["agentx", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["agentx", "-z", "hello world"], None),
        (["agentx", "-z", "hello", "chat"], "chat"),
        (["agentx", "--model=gpt5", "chat"], "chat"),     # inline form
        (["agentx", "--", "chat"], "chat"),               # -- terminator
        (["agentx", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["agentx", "some-plugin-cmd"], "some-plugin-cmd"),
        (["agentx", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["agentx"],                          # bare → chat
        ["agentx", "--help"],                # top-level help
        ["agentx", "-h"],
        ["agentx", "version"],               # known built-in
        ["agentx", "logs"],
        ["agentx", "gateway", "run"],
        ["agentx", "--tui"],
        ["agentx", "-w", "--tui"],
        ["agentx", "chat", "hi"],
        ["agentx", "help"],                  # accepted built-in-ish
        ["agentx", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["agentx", "meet", "join"],          # potential google_meet plugin
        ["agentx", "honcho", "status"],      # potential memory plugin
        ["agentx", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``agentx help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"agentx_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
