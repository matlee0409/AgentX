...
``cli.py``'s ``AgentXCLI`` class. ``AgentXCLI`` inherits ``CLICommandsMixin`` so
...
from agentx_constants import display_agentx_home, is_termux as _is_termux_environment
...
class CLICommandsMixin:
    ...
    def _handle_snapshot_command(self, command: str):
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_agentx_home()}/state-snapshots/):\n")
    ...
    def _handle_paste_command(self):
        ...
        from agentx_cli.clipboard import has_clipboard_image
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in agentx_cli/colors.py
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if target.isdigit():
            ...
            from agentx_cli.main import _resolve_session_by_name_or_id
    ...
    def _handle_pet_command(self, cmd: str):
        ...
        from agentx_cli.pets import _set_active, _set_enabled, print_pet_gallery, set_pet_scale, toggle_pet_display
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to agentx_cli.curator so the CLI and the `agentx curator`
        ...
        try:
            from agentx_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to agentx_cli.skills_hub."""
        ...
        if args and args[0].lower() in {"pending", "approve", "apply", "reject",
            ...
            if out is not None:
                print(out)
                return
        from agentx_cli.skills_hub import handle_skills_slash
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from agentx_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_agentx_home()}/skins/\n")
            return
    ...
    def _compose_in_editor(self, initial_text: str = "") -> str:
        ...
        fd, path = tempfile.mkstemp(suffix=".md", prefix="agentx_prompt_")
        try:
    ...
    def _handle_busy_command(self, cmd: str):
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                ...
                behavior = "Enter will interrupt the current run while AgentX is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from agentx_cli.models import _is_anthropic_fast_model
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update AgentX Agent to the latest version.
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update AgentX Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update AgentX Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update AgentX Agent",
