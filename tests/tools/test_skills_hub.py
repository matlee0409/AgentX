...
class TestParseFrontmatterQuick:
    ...
    def test_nested_yaml(self):
        content = "---\nname: test\nmetadata:\n  agentx:\n    tags: [a, b]\n---\n\nBody.\n"
        fm = GitHubSource._parse_frontmatter_quick(content)
        assert fm["metadata"]["agentx"]["tags"] == ["a", "b"]
...
class TestTrustLevelFor:
    ...
    def test_nvidia_skills_tap_is_registered_and_trusted(self):
        ...
        # browseable/searchable through `agentx skills browse` must also
...
def _make_index_source(skills):
    """Build a AgentXIndexSource pre-loaded with a fixed skill list."""
    from tools.skills_hub import AgentXIndexSource
    src = AgentXIndexSource(auth=GitHubAuth())
    src._index = {"skills": skills}
    src._loaded = True
    return src


class TestAgentXIndexSearch:
