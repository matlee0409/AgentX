...
def _setup_agentx_auth(
    ...
    *,
    ...
    auth_file = agentx_home / "auth.json"
...
def _patch_httpx_client(monkeypatch, response):
    ...
    monkeypatch.setattr("agentx_cli.auth.httpx.Client", _factory)
    return holder
...
def test_xai_oauth_authorize_url_includes_referrer_agentx_agent():
    """Attribution: xAI's OAuth server can identify AgentX-originated logins
    ...
    params = _parse_authorize_url(url)
    assert params["referrer"] == "agentx-agent"
...
def test_xai_callback_handler_returns_400_when_callback_url_lacks_code_and_error():
    ...
    try:
        ...
        assert "agentx auth add xai-oauth" in body
...
def test_save_and_read_xai_oauth_tokens_roundtrip(tmp_path, monkeypatch):
    ...
    agentx_home.mkdir(parents=True, exist_ok=True)
    (agentx_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
...
def test_read_xai_oauth_tokens_missing_access_token(tmp_path, monkeypatch):
    agentx_home = tmp_path / "agentx"
    _setup_agentx_auth(agentx_home, access_token="")
    monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
...
def test_read_xai_oauth_tokens_missing_refresh_token(tmp_path, monkeypatch):
    agentx_home = tmp_path / "agentx"
    _setup_agentx_auth(agentx_home, refresh_token="")
    monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
...
def test_resolve_xai_runtime_credentials_returns_singleton_state(tmp_path, monkeypatch):
    agentx_home = tmp_path / "agentx"
    fresh = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_agentx_auth(agentx_home, access_token=fresh)
    monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
...
def test_resolve_xai_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("agentx_cli.auth._refresh_xai_oauth_tokens", _fake_refresh)
...
def test_resolve_xai_runtime_credentials_honours_env_base_url(tmp_path, monkeypatch):
    ...
    _setup_agentx_auth(agentx_home, access_token=fresh)
    monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
    monkeypatch.setenv("AGENTX_XAI_BASE_URL", "https://custom.x.ai/v1/")
...
def _seed_xai_oauth_state(
    agentx_home: Path, state: dict, *, active_provider: str = "xai-oauth"
) -> None:
    agentx_home.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
    ...
    (agentx_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    """
    agentx_home = tmp_path / "agentx"
    _seed_xai_oauth_state(agentx_home, dict(_STALE_XAI_OAUTH_STATE), active_provider="nous")
    monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
    ...
    monkeypatch.setattr("agentx_cli.auth._refresh_xai_oauth_tokens", _terminal_refresh)
    ...
    raw = json.loads((agentx_home / "auth.json").read_text())
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure(
    ...
    """
    agentx_home = tmp_path / "agentx"
    _seed_xai_oauth_state(agentx_home, dict(_STALE_XAI_OAUTH_STATE))
    monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
    ...
    monkeypatch.setattr("agentx_cli.auth._refresh_xai_oauth_tokens", _transient_refresh)
...
def test_xai_oauth_discovery_raises_typed_error_on_malformed_json(monkeypatch):
    ...
    from agentx_cli.auth import _xai_oauth_discovery
...
def test_xai_oauth_discovery_validates_endpoints(monkeypatch):
    ...
    monkeypatch.setattr("agentx_cli.auth.httpx.get", _fake_get)
    with pytest.raises(AuthError) as exc:
        _xai_oauth_discovery()
    assert exc.value.code == "xai_discovery_invalid"
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    ...
    agentx_home = tmp_path / "agentx"
    fresh = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_agentx_auth(agentx_home, access_token=fresh, refresh_token="rt-1")
    monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
...
def test_credential_pool_seed_respects_suppression(tmp_path, monkeypatch):
    """`agentx auth remove xai-oauth <N>` for the seeded entry suppresses
    ...
    # Suppress the source — mimic `agentx auth remove`.
    from agentx_cli.auth import suppress_credential_source
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    ...
    # nothing to import.
    raw_after = json.loads((agentx_home / "auth.json").read_text())
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    _setup_agentx_auth(agentx_home, access_token=expired, refresh_token="rt-old")
    monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
    ...
    monkeypatch.setattr("agentx_cli.auth.refresh_xai_oauth_pure", _fake_refresh)
    ...
    auth_path = agentx_home / "auth.json"
...
def test_pool_entry_needs_refresh_when_jwt_within_skew(tmp_path, monkeypatch):
    ...
    from agent.credential_pool import load_pool, AUTH_TYPE_OAUTH, PooledCredential
    from agentx_cli.auth import XAI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS
    import uuid
...
def test_pool_refresh_marks_entry_exhausted_on_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("agentx_cli.auth.refresh_xai_oauth_pure", _fake_refresh_fail)
...
def test_pool_seeded_entry_sync_back_after_refresh(tmp_path, monkeypatch):
    ...
    agentx_home = tmp_path / "agentx"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_agentx_auth(agentx_home, access_token=near_expiry, refresh_token="rt-singleton")
    monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    """Multi-process race: another AgentX process refreshed the singleton
    ...
    profiles + AgentX processes."""
    from agent.credential_pool import load_pool

    agentx_home = tmp_path / "agentx"
    in_memory_at = _jwt_with_exp(int(time.time()) + 30)  # near-expiry
    _setup_agentx_auth(agentx_home, access_token=in_memory_at, refresh_token="rt-stale")
    monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
    ...
    (agentx_home / "auth.json").write_text(json.dumps(raw))
...
def test_pool_refresh_recovers_when_other_process_already_refreshed(tmp_path, monkeypatch):
    ...
    _setup_agentx_auth(agentx_home, access_token=in_memory_at, refresh_token="rt-shared")
    monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    user runs ``agentx model`` -> xAI Grok OAuth (or another process
    ...
    _setup_agentx_auth(agentx_home, access_token=stale_at, refresh_token="rt-stale")
    monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
    ...
    # Simulate the user re-running `agentx model` -> xAI Grok OAuth: the
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    ``agentx auth add xai-oauth``) own their own refresh-token lifecycle
    ...
    _setup_agentx_auth(agentx_home, access_token=singleton_at, refresh_token="rt-singleton")
    monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    agentx_home = tmp_path / "agentx"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_agentx_auth(agentx_home, access_token=near_expiry, refresh_token="rt-xai")
    monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
