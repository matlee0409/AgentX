...
def _chown_agentx_tree_function(text: str) -> str:
...
def _run_helper(
    ...
    log_path: Path,
    *,
    agentx_home: Path | None = None,
) -> subprocess.CompletedProcess[str]:
    shell = shutil.which("sh")
    if shell is None:
        pytest.skip("sh not available")
    agentx_home = target if agentx_home is None else agentx_home
    script = (
        "set -eu\n"
        f'AGENTX_HOME="{agentx_home}"\n'
        f"{_chown_agentx_tree_function(text)}\n"
        f'chown() {{ printf "%s\\n" "$*" >> "{log_path}"; }}\n'
        f'chown_agentx_tree "{target}"\n'
    )
...
def test_stage2_uses_symlink_safe_helper_for_agentx_home_trees(stage2_text: str) -> None:
    assert 'chown_agentx_tree "$AGENTX_HOME/$sub"' in stage2_text
    assert 'chown_agentx_tree "$AGENTX_HOME/profiles"' in stage2_text
    assert 'chown_agentx_tree "$AGENTX_HOME/cron"' in stage2_text
...
def test_stage2_skips_top_level_chown_for_symlinked_agentx_home(
    stage2_text: str,
) -> None:
