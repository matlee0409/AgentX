...
load_agentx_dotenv(
    agentx_home=_agentx_home, project_env=Path(__file__).parent.parent / ".env"
)
...
_CRASH_LOG = os.path.join(_agentx_home, "logs", "tui_gateway_crash.log")
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("AGENTX_TUI_RPC_POOL_WORKERS") or "4")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
class _SlashWorker:
    """Persistent AgentXCLI subprocess for slash commands."""
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from agentx_cli.active_sessions import transfer_active_session
...
def _max_live_sessions() -> int:
    try:
        from agentx_cli.active_sessions import coerce_max_concurrent_sessions
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_agentx_home).resolve():
        return None
...
def _profile_scoped(handler):
    ...
    sprites install under its ``pets/`` dir (both resolve via ``get_agentx_home``).
    ...
    def wrapper(rid, params):
        ...
        token = set_agentx_home_override(home)
        try:
            return handler(rid, params)
        finally:
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    def _build() -> None:
        ...
        try:
            ...
            session_db = None
            if profile_home:
                home_token = set_agentx_home_override(profile_home)
                try:
                    from agentx_state import SessionDB
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from agentx_cli.runtime_provider import canonical_custom_identity
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _agentx_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_agentx_home_override()
        home = override if isinstance(override, str) and override else _agentx_home
...
def _apply_managed(cfg: dict) -> dict:
    ...
    agentx_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def resolve_skin() -> dict:
    try:
        from agentx_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _config_model_target() -> tuple[str, str]:
    ...
    config.yaml wins over AGENTX_MODEL / AGENTX_INFERENCE_MODEL here, the
    reverse of `_resolve_model()`'s startup order. Those env vars are a
    provision-time seed (hosted instances set AGENTX_INFERENCE_MODEL in the
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    try:
        from agentx_cli.models import detect_static_provider_for_model
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    if explicit and validate_toolset is not None:
        ...
        try:
            ...
            from agentx_cli.tools_config import _parse_enabled_flag
...
def _apply_model_switch(
    ...
    *,
    ...
    custom_provs = None
    try:
        from agentx_cli.config import get_compatible_custom_providers, load_config
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        from agentx_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from agentx_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from agentx_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from agentx_cli.config import load_config as _load_full_cfg
...
def _load_fallback_model():
    ...
    stays in parity with ``AgentXCLI.__init__`` and ``gateway/run.py``:
...
def _resolve_runtime_with_fallback(
    ...
    ``agentx_cli/cli_agent_setup_mixin.py``: when the primary provider
    raises ``AuthError``, walk the configured ``fallback_providers`` /
    ``fallback_model`` chain before giving up.
    """
    from agentx_cli.auth import AuthError
    from agentx_cli.runtime_provider import resolve_runtime_provider
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses agentx_cli.mcp_startup; TUI stdio keeps
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_agentx_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
        db.reopen_session(target)
        history = db.get_messages_as_conversation(target)
    ...
    with _session_resume_lock:
        ...
        try:
            init_home_token = (
                set_agentx_home_override(str(profile_home))
                if profile_home is not None
                else None
            )
            try:
                ...
                if init_home_token is not None:
                    reset_agentx_home_override(init_home_token)
            if sid in _sessions:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from agentx_cli.pets import _set_active
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agent.pet import store
        from agentx_cli.pets import _clear_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if new_slug != slug:
            try:
                from agentx_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agentx_cli.pets import _set_enabled
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from agentx_cli.pets import set_pet_scale
...
def _pet_gen_root():
    ...
    root = get_agentx_home() / "cache" / "pet-gen"
...
# (agent/billing_view.py + agentx_cli/nous_billing.py) — same as /credits.
...
def _(rid, params: dict) -> dict:
    ...
    """
    from agentx_cli.nous_billing import BillingError, post_charge
...
def _(rid, params: dict) -> dict:
    ...
    """
    from agentx_cli.nous_billing import BillingError, get_charge_status
...
def _(rid, params: dict) -> dict:
    ...
    """
    from agentx_cli.nous_billing import BillingError, patch_auto_top_up
...
def _spawn_trees_root():
    ...
    root = get_agentx_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
    ...
    def run():
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_agentx_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from agentx_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from agentx_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: agentx_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from agentx_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _agentx_home / "images"
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not AgentX Desktop itself.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not AgentX Desktop. If it serves AgentX/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from agentx_cli.model_switch import parse_model_flags
...
def _projects_payload(conn) -> dict:
    from agentx_cli import projects_db as pdb
...
def _is_repo_junk(root: str) -> bool:
    ...
    agentx_home = os.path.realpath(str(get_agentx_home()))

    return real == home or real == agentx_home or real.startswith(agentx_home + os.sep)
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from agentx_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_agentx_home), "display": display_agentx_home()})
    if key == "project":
...
def _(rid, params: dict) -> dict:
    try:
        from agentx_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agentx_cli.runtime_provider import resolve_runtime_provider
        from agentx_cli.auth import has_usable_secret
        from agentx_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    ``agentx_cli.config.reload_env``, matching classic CLI's ``/reload``
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m agentx_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "agentx_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agentx_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from agentx_cli.config import is_managed, save_env_value
        from agentx_cli.inventory import build_models_payload, load_picker_context
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from agentx_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from agentx_cli.config import remove_env_value
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        if not enabled:
            ...
            try:
                from agentx_cli.voice import stop_continuous
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from agentx_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from agentx_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from agentx_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from agentx_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("AGENTX_API_KEY", "") or cfg.get("api_key", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_agentx_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from agentx_cli.skills_hub import do_install
        ...
        if action == "browse":
            from agentx_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from agentx_cli.skills_hub import inspect_skill
