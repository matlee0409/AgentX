...
def reconcile_profile_gateways(
    *,
    ...
    not under ``profiles/``). The dispatcher in ``agentx_cli.gateway``
    ...
    Args:
        agentx_home: The container's AGENTX_HOME (typically /opt/data).
            Profiles live under ``<agentx_home>/profiles/<name>/``;
            the default profile lives at ``<agentx_home>`` itself.
    ...
    default_prior_state = legacy_default_state or _read_desired_state(agentx_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(agentx_home)
    ...
    profiles_root = agentx_home / "profiles"
    ...
    if not dry_run:
        _write_reconcile_log(agentx_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = agentx_home / "gateway_state.json"
...
def _write_reconcile_log(
    agentx_home: Path, actions: list[ReconcileAction],
) -> None:
...
def main() -> int:
    ...
    actions = reconcile_profile_gateways(
        agentx_home=agentx_home, scandir=scandir,
    )
    for a in actions:
        print(
