...
into AgentX Agent, archives selected unmapped docs for manual review, and
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-agentx/apply.ts "blocked by earlier
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-agentx/plan.ts warnings[].
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
        ...
        for oc_key, agentx_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and agentx_key not in secret_additions:
                secret_additions[agentx_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, agentx_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and agentx_key not in secret_additions:
                        secret_additions[agentx_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        agentx_config = load_yaml_file(destination)
        current_model = agentx_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = agentx_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                agentx_config["model"] = {"default": model_str}
            dump_yaml_file(destination, agentx_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        agentx_config = load_yaml_file(destination)
        existing_tts = agentx_config.get("tts", {})
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        agentx_cfg_path = self.target_root / "config.yaml"
        agentx_cfg = load_yaml_file(agentx_cfg_path)
        existing_mcp = agentx_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            agentx_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                agentx_srv["command"] = srv["command"]
                if srv.get("args"):
                    agentx_srv["args"] = srv["args"]
                if srv.get("env"):
                    agentx_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    agentx_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                agentx_srv["url"] = srv["url"]
                if srv.get("headers"):
                    agentx_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    agentx_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                agentx_srv["enabled"] = False
            if srv.get("timeout"):
                agentx_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                agentx_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                agentx_srv["tools"] = {}
                if tools_cfg.get("include"):
                    agentx_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    agentx_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(agentx_cfg_path)
            agentx_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(agentx_cfg_path, agentx_cfg)
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = agentx_cfg.get("agent") or {}
        ...
        if compaction:
            compression = agentx_cfg.get("compression") or {}
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        agentx_cfg_path = self.target_root / "config.yaml"
        agentx_cfg = load_yaml_file(agentx_cfg_path)
        sr = agentx_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        agentx_cfg_path = self.target_root / "config.yaml"
        agentx_cfg = load_yaml_file(agentx_cfg_path)
        custom_providers = agentx_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(agentx_cfg_path)
            agentx_cfg["custom_providers"] = custom_providers
            dump_yaml_file(agentx_cfg_path, agentx_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if discord_cfg:
            agentx_cfg_path = self.target_root / "config.yaml"
            agentx_cfg = load_yaml_file(agentx_cfg_path)
            discord_agentx = agentx_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_agentx["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_agentx["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                agentx_cfg["discord"] = discord_agentx
                dump_yaml_file(agentx_cfg_path, agentx_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        agentx_cfg_path = self.target_root / "config.yaml"
        agentx_cfg = load_yaml_file(agentx_cfg_path)
        browser_agentx = agentx_cfg.get("browser") or {}
        changed = False

        # Map fields that have AgentX equivalents
        if browser.get("cdpUrl"):
            browser_agentx["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_agentx["headless"] = browser["headless"]
            changed = True

        if changed:
            agentx_cfg["browser"] = browser_agentx
            if self.execute:
                self.maybe_backup(agentx_cfg_path)
                dump_yaml_file(agentx_cfg_path, agentx_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            agentx_mode = mode_map.get(mode, "manual")
            agentx_cfg.setdefault("approvals", {})["mode"] = agentx_mode
            if self.execute:
                self.maybe_backup(agentx_cfg_path)
                dump_yaml_file(agentx_cfg_path, agentx_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{agentx_mode}'")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into AgentX Agent.")
