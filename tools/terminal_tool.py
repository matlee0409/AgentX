...
# display_agentx_home imported lazily at call site (stale-module safety during agentx update)
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so AgentX can track lifecycle and output.
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, AgentXSweEnv, ...)
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so AgentX can track the process, then run "
...
def terminal_tool(
    ...
    try:
        ...
        # agentx_cli/gateway.py and the cron-path guard in agentx_cli/cron.py,
        # but applies unconditionally (force=True cannot help here).
        if os.environ.get("_AGENTX_GATEWAY") == "1":
            from agentx_cli.cron import _contains_gateway_lifecycle_command
            if _contains_gateway_lifecycle_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Run `agentx gateway restart` from a separate shell outside "
                        "the running gateway."
                    ),
                    "status": "error",
                }, ensure_ascii=False)
