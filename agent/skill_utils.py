...
from agentx_constants import get_config_path, get_skills_dir, is_termux
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``agentx_cli.config`` on the
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    agentx = metadata.get("agentx") or {}
    if not isinstance(agentx, dict):
        agentx = {}
    return {
        "fallback_for_toolsets": agentx.get("fallback_for_toolsets", []),
        "requires_toolsets": agentx.get("requires_toolsets", []),
        "fallback_for_tools": agentx.get("fallback_for_tools", []),
        "requires_tools": agentx.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    agentx = metadata.get("agentx")
    if not isinstance(agentx, dict):
        return []
    raw = agentx.get("config")
    if not raw:
        return []
