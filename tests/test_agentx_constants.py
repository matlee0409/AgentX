"""Tests for agentx_constants module."""
...
from agentx_constants import (
    ...
    find_agentx_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_agentx_root,
    get_agentx_home,
    heal_agentx_managed_node,
    agentx_managed_node_tree_present,
    iter_agentx_node_dirs,
    is_container,
    node_tool_runnable,
    parse_reasoning_effort,
    secure_parent_dir,
    with_agentx_node_path,
)


class TestGetDefaultAgentXRoot:
    """Tests for get_default_agentx_root() — Docker/custom deployment awareness."""

    def test_no_agentx_home_returns_native(self, tmp_path, monkeypatch):
        """When AGENTX_HOME is not set, returns ~/.agentx."""
        monkeypatch.delenv("AGENTX_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_agentx_root() == tmp_path / ".agentx"

    def test_agentx_home_is_native(self, tmp_path, monkeypatch):
        """When AGENTX_HOME = ~/.agentx, returns ~/.agentx."""
        native = tmp_path / ".agentx"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AGENTX_HOME", str(native))
        assert get_default_agentx_root() == native

    def test_agentx_home_is_profile(self, tmp_path, monkeypatch):
        """When AGENTX_HOME is a profile under ~/.agentx, returns ~/.agentx."""
        native = tmp_path / ".agentx"
    ...
    def test_agentx_home_is_docker(self, tmp_path, monkeypatch):
        """When AGENTX_HOME points outside ~/.agentx (Docker), returns AGENTX_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AGENTX_HOME", str(docker_home))
        assert get_default_agentx_root() == docker_home

    def test_agentx_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any AGENTX_HOME outside ~/.agentx is treated as the root."""
        custom = tmp_path / "my-agentx-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AGENTX_HOME", str(custom))
        assert get_default_agentx_root() == custom
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AGENTX_HOME", str(profile))
        assert get_default_agentx_root() == docker_root

    def test_no_agentx_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\agentx, not ~/.agentx."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("AGENTX_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(agentx_constants.sys, "platform", "win32")

        assert get_default_agentx_root() == local_appdata / "agentx"

    def test_no_agentx_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/agentx without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("AGENTX_HOME", raising=False)
        ...
        assert get_default_agentx_root() == home / "AppData" / "Local" / "agentx"


class TestGetAgentXHome:
    """Tests for get_agentx_home() platform-aware fallback."""
    ...
    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(agentx_constants.sys, "platform", "win32")
        monkeypatch.setattr(agentx_constants, "_profile_fallback_warned", False)

        assert get_agentx_home() == local_appdata / "agentx"


class TestAgentXManagedNode:
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_agentx_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(agentx_constants.sys, "platform", "win32")
        monkeypatch.setenv("AGENTX_HOME", str(home))
        monkeypatch.setattr(agentx_constants, "node_tool_runnable", lambda path: True)

        assert find_agentx_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AGENTX_HOME", str(home))
        monkeypatch.setenv("PATH", str(bin_dir))
        monkeypatch.setattr(agentx_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(agentx_constants, "heal_agentx_managed_node", lambda: False)
        monkeypatch.setattr(
            agentx_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert agentx_managed_node_tree_present() is True
        assert find_node_executable("npm") is None
        assert find_node_executable("npm") != str(path_npm)

    def test_with_agentx_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        home = tmp_path / "agentx"
        node_dir = home / "node"
        bin_dir = node_dir / "bin"
        node_dir.mkdir(parents=True)
        ...
        env = with_agentx_node_path({"PATH": "system-node"})
...
class TestNodeToolRunnable:
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(agentx_constants, "heal_agentx_managed_node", _heal)
    ...
    def test_broken_managed_npm_heals_instead_of_path_fallback(self, tmp_path, monkeypatch):
        ...
        assert find_agentx_node_executable("npm") == str(healed_npm)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(agentx_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(agentx_constants, "_container_detected", True)
        assert is_container() is True
