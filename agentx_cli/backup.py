...
from agentx_constants import get_default_agentx_root, get_agentx_home, display_agentx_home
...
def run_backup(args) -> None:
    ...
    agentx_root = get_default_agentx_root()

    if not agentx_root.is_dir():
        print(f"Error: AgentX home directory not found at {agentx_root}")
        sys.exit(1)
    ...
    # Collect files
    print(f"Scanning {display_agentx_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(agentx_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(agentx_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(agentx_root)
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_agentx_home()}."
        )
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_agentx_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (agentx_root / "config.yaml").exists()
        has_env = (agentx_root / ".env").exists()
        ...
        for member in members:
            ...
            target = agentx_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(agentx_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_agentx_home()}."
            )
        ...
        profiles_dir = agentx_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # agentx_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: agentx profile list  (after installing agentx)")

        # Guidance
        print()
        if not (agentx_root / "agentx-agent").is_dir():
...
def _quick_snapshot_root(agentx_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(agentx_home)
    if not root.exists():
        return []
...
def restore_cron_jobs_if_emptied(
    ...
    Args:
        ...
        agentx_home: Override for the AgentX home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(agentx_home), keep=keep)
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        ...
        print(f"  {len(snaps)} snapshot(s) stored in {display_agentx_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, agentx_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``agentx_root`` to ``out_path``.
...
def _pre_update_backup_dir(agentx_home: Optional[Path] = None) -> Path:
    home = agentx_home or get_agentx_home()
...
def create_pre_update_backup(
    ...
    """
    agentx_root = agentx_home or get_default_agentx_root()
    if not agentx_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(agentx_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, agentx_root)
    if result is None:
        return None
