...
from agentx_constants import reset_agentx_home_override, set_agentx_home_override
from agentx_cli.active_sessions import active_session_registry_snapshot
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("AGENTX_VOICE", "1")
    monkeypatch.delenv("AGENTX_VOICE_TTS", raising=False)
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    ...
    import agentx_cli.config as config_mod
...
def test_session_resume_profile_uses_profile_db_cwd(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("agentx_state.SessionDB", lambda db_path=None: profile_db)
...
def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    ...
    monkeypatch.setattr(
        "agentx_cli.models.detect_static_provider_for_model",
...
def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    ...
    monkeypatch.setattr(
        "agentx_cli.models.detect_static_provider_for_model", fake_detect
    )
...
def test_load_fallback_model_merges_chain_providers_first(monkeypatch):
    # Parity with AgentXCLI / gateway: fallback_providers stays first and keeps
...
def test_make_agent_passes_configured_fallback_chain(monkeypatch):
    ...
    monkeypatch.setattr(
        "agentx_cli.runtime_provider.resolve_runtime_provider",
...
def test_startup_runtime_resolves_short_alias_without_network(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "agentx_cli.models.fetch_openrouter_models",
...
def test_startup_runtime_does_not_call_network_detector(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "agentx_cli.models.detect_provider_for_model",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
...
def test_config_set_yolo_global_scope_writes_approvals_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_agentx_home", tmp_path)
...
def test_config_set_fast_updates_live_agent_and_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "agentx_cli.models.resolve_fast_mode_overrides",
...
def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("agentx_cli.main._has_any_provider_configured", lambda: False)
...
def test_setup_runtime_check_rejects_empty_runtime_key(monkeypatch):
    monkeypatch.setattr("agentx_cli.main._has_any_provider_configured", lambda: True)
    monkeypatch.setattr(
        "agentx_cli.runtime_provider.resolve_runtime_provider",
...
def test_config_set_model_requires_confirmation_for_expensive_model(monkeypatch):
    ...
    server._sessions["sid"] = _session(agent=agent)
    monkeypatch.setattr(
        "agentx_cli.model_switch.switch_model", lambda **_kwargs: result
    )
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    monkeypatch.setattr("agentx_cli.model_switch.switch_model", _switch_model)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
...
def test_config_set_model_explicit_provider_skips_broken_default_init(monkeypatch):
    ...
    monkeypatch.setattr("agentx_cli.runtime_provider.resolve_runtime_provider", fake_runtime_provider)
...
def test_config_set_model_switches_agent_without_touching_env(monkeypatch):
    ...
    monkeypatch.setattr("agentx_cli.model_switch.switch_model", fake_switch_model)
...
def test_session_compress_syncs_session_key_after_rotation(monkeypatch):
    ...
    continuation session — mirrors AgentXCLI._manual_compress's
...
def test_plugins_list_surfaces_loader_error(monkeypatch):
    with patch("agentx_cli.plugins.get_plugin_manager", side_effect=Exception("boom")):
        resp = server.handle_request(
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "agentx_cli.commands.SlashCommandCompleter",
...
def test_get_db_degrades_cleanly_when_sessiondb_init_fails(monkeypatch):
    fake_mod = types.ModuleType("agentx_state")
    ...
    monkeypatch.setitem(sys.modules, "agentx_state", fake_mod)
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    ...
    # too — not just the SQLite row.  The autouse _isolate_agentx_home
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    with patch(
        "agentx_cli.model_switch.list_authenticated_providers",
        ...
        # guarding against.
        with patch("agentx_cli.models.provider_model_ids") as live_fetch:
...
def test_browser_manage_status_falls_back_to_config_cdp_url(monkeypatch):
    ...
    with patch.dict(sys.modules, {"agentx_cli.config": fake_cfg}):
        resp = server.handle_request(
            {"id": "1", "method": "browser.manage", "params": {"action": "status"}}
        )
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        ...
        with (
            patch(
                "agentx_cli.browser_connect.try_launch_chrome_debug", return_value=False
            ),
            patch(
                "agentx_cli.browser_connect.get_chrome_debug_candidates",
                return_value=[],
            ),
        ):
            resp = server.handle_request(
...
def test_browser_manage_connect_default_local_retries_after_launch(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with patch(
            "agentx_cli.browser_connect.try_launch_chrome_debug", return_value=True
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_reload_env_rpc_calls_agentx_cli_reload_env(monkeypatch):
    ...
    with patch.dict(sys.modules, {"agentx_cli.config": fake}):
...
def test_make_agent_waits_for_shared_mcp_discovery(monkeypatch):
    ...
    from agentx_cli import mcp_startup
...
def test_session_save_writes_under_agentx_home_with_system_prompt(monkeypatch, tmp_path):
    ...
    Regression: the gateway handler wrote ``agentx_conversation_*.json`` to
    ...
    assert not list(work.glob("agentx_conversation_*.json"))
...
def test_persist_model_switch_preserves_sibling_model_keys(tmp_path, monkeypatch):
    ...
    # save_config_value() resolves the config path from cli._agentx_home, which
    # is captured at import time — patch it directly (set_agentx_home_override
    # does NOT affect this snapshot).
    monkeypatch.setattr(cli, "_agentx_home", tmp_path)
