...
Architecture (two transports):
  ...
  **Local backend (UDS):**
  1. Parent generates a `agentx_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `agentx_tools.py` with file-based RPC stubs
...
# OS-essential name.
#
# NB: the broad "AGENTX_" prefix was deliberately removed (#27303) — it leaked
# AGENTX_*-named config that lacks a secret substring (e.g. AGENTX_BASE_URL,
...
# are non-secret runtime-location flags (the same set agentx_cli treats as the
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    # broad "AGENTX_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like AGENTX_KANBAN_DB / AGENTX_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("AGENTX_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped AGENTX_* var.
            _dropped_agentx.append(k)
    if _dropped_agentx:
        logger.debug(
            "execute_code: dropped %d non-allowlisted AGENTX_* var(s) from the "
...
# ---------------------------------------------------------------------------
# agentx_tools.py code generator
# ---------------------------------------------------------------------------
...
def generate_agentx_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the agentx_tools.py stub module.
...
_RPC_DIR = os.environ.get("AGENTX_RPC_DIR") or os.path.join(tempfile.gettempdir(), "agentx_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated agentx_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/agentx_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_agentx_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/agentx_tools.py", tools_src)
...
def execute_code(
    ...
    # --- Set up temp directory with agentx_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="agentx_sandbox_")
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"agentx_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated agentx_tools module.
        ...
        tools_src = generate_agentx_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "agentx_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        # the staging tmpdir so ``from agentx_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _agentx_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _agentx_root]
        if _existing_pp:
        ...
        from agentx_constants import apply_subprocess_home_env
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    description = (
        "Run a Python script that can call AgentX tools programmatically. "
        ...
        f"Available via `from agentx_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into agentx_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from agentx_tools import {import_str}` "
