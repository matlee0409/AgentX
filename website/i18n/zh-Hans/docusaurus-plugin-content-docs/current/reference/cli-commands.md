...
| `agentx plugins` | 管理 AgentX Agent plugin（安装、启用、禁用、删除）。 |
...
agentx skills install https://sharethis.chat/SKILL.md                     # 直接 URL（单文件 SKILL.md）
...
参见 [Plugins](../user-guide/features/plugins.md) 和 [构建 AgentX Plugin](../guides/build-a-agentx-plugin.md)。
...
- **旧版 `agentx.service` 警告。** 如果 AgentX 检测到预重命名的 `agentx.service` systemd 单元（而非当前的 `agentx-gateway.service`），会打印一次性迁移提示，帮助你避免循环重启问题。
