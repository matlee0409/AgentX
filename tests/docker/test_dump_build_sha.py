...
``$AGENTX_GIT_SHA`` build-arg to ``/opt/agentx/.agentx_build_sha`` and
``agentx_cli/build_info.py`` reads it as a fallback.
...
* If ``/opt/agentx/.agentx_build_sha`` exists in the image, assert that
...
def _read_baked_sha_from_image(image: str) -> str | None:
    """Return the ``/opt/agentx/.agentx_build_sha`` content, or None if absent."""
    r = subprocess.run(
        [
            "docker", "run", "--rm", "--entrypoint", "cat", image,
            "/opt/agentx/.agentx_build_sha",
        ],
