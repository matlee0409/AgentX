...
import {
  getElevenLabsVoices,
  getAgentXConfigDefaults,
  getAgentXConfigRecord,
  getAgentXConfigSchema,
  saveAgentXConfig
} from '@/agentx'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, AgentXConfigRecord } from '@/types/agentx'
...
export function voiceFieldVisible(key: string, config: AgentXConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const { t } = useI18n()
  const c = t.settings.config
  const [config, setConfig] = useState<AgentXConfigRecord | null>(null)
  const [_defaults, setDefaults] = useState<AgentXConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getAgentXConfigRecord(), getAgentXConfigDefaults(), getAgentXConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  const updateConfig = (next: AgentXConfigRecord) => {
