...
const getAgentXConfigRecord = vi.fn()
const saveAgentXConfig = vi.fn()
...
vi.mock('@/agentx', () => ({
  ...
  getAgentXConfigRecord: () => getAgentXConfigRecord(),
  saveAgentXConfig: (config: unknown) => saveAgentXConfig(config)
}))
...
beforeEach(() => {
  getGlobalModelInfo.mockResolvedValue({ provider: 'nous', model: 'agentx-4' })
  getGlobalModelOptions.mockResolvedValue({
    providers: [
      {
        name: 'Nous',
        slug: 'nous',
        models: ['agentx-4', 'agentx-4-mini'],
        authenticated: true,
        capabilities: { 'agentx-4': { reasoning: true, fast: true } }
      },
  ...
  getAuxiliaryModels.mockResolvedValue({
    main: { provider: 'nous', model: 'agentx-4' },
    tasks: [{ task: 'vision', provider: 'auto', model: '', base_url: '' }]
  })
  setModelAssignment.mockResolvedValue({ provider: 'nous', model: 'agentx-4', gateway_tools: [] })
  getRecommendedDefaultModel.mockResolvedValue({ provider: 'deepseek', model: 'deepseek-chat', free_tier: null })
  setEnvVar.mockResolvedValue({ ok: true })
  getAgentXConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
  saveAgentXConfig.mockResolvedValue({ ok: true })
})
...
describe('ModelSettings', () => {
  ...
  it('writes the profile default speed (service_tier) when the fast switch is toggled', async () => {
    await renderModelSettings()
    await waitFor(() => expect(getAgentXConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() =>
      expect(saveAgentXConfig).toHaveBeenCalledWith(
      ...
      providers: [
        {
          ...
          models: ['agentx-4'],
          authenticated: true,
          capabilities: { 'agentx-4': { reasoning: false, fast: false } }
        }
      ]
    })
  ...
  it('warns when a main switch leaves auxiliary tasks pinned to another provider', async () => {
    setModelAssignment.mockResolvedValueOnce({
      ...
      gateway_tools: [],
      stale_aux: [{ task: 'compression', provider: 'nous', model: 'agentx-4' }]
    })
