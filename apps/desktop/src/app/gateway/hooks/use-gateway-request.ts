...
export function useGatewayRequest() {
  ...
  const gatewayRef = useRef<AgentXGateway | null>(null)
  ...
  const reconnectingRef = useRef<Promise<AgentXGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as AgentXGateway | null
      }),
    []
  )
