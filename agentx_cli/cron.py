...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(agentx\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*agentx)"
    r"|(systemctl\s+(-\S+\s+)*(restart|stop|start)\s+.*agentx)"
    r"|(p?kill\s+.*agentx.*gateway)"
)
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `agentx gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: agentx cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
