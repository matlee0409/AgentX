"""``agentx debug`` debug tools for AgentX Agent.
...
def _pending_file() -> Path:
    ...
    """
    return get_agentx_home() / "pastes" / "pending.json"
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, AgentX version, provider, which API keys
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from agentx_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_agentx_home() / "logs" / filename) if filename else None
...
def run_debug_share(args):
    ...
    print(f"To delete now:  agentx debug delete <url>")
