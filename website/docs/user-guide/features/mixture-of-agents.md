...
If the text does not exactly match a preset name, AgentX treats it as a one-shot prompt. It temporarily switches to the default MoA preset for that turn, sends the prompt, then restores the previous model afterward.

Preset matching is exact on purpose. AgentX does not fuzzy-match preset names, so normal prompts cannot accidentally become model switches.
...
On AgentXBench, a two-model MoA preset — `claude-opus-4.8` aggregating over a `gpt-5.5` reference — outscores either model run on its own:

| Model | AgentXBench score |
|---|---|
