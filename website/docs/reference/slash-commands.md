...
AgentX has two slash-command surfaces, both driven by a central `COMMAND_REGISTRY` in `agentx_cli/commands.py`:
...
| `/version` | Show AgentX Agent version, build, and environment info. |
...
| `/update` | Update AgentX Agent to the latest version. |
...
**Inline skip:** append `now`, `--yes`, or `-y` to bypass the modal for a single invocation — e.g. `/reset now`, `/new --yes my-session`, `/clear -y`, `/undo -y`. Useful when the modal doesn't render correctly on your terminal (see [issue #30768](https://github.com/NousResearch/agentx-agent/issues/30768) for native Windows PowerShell) or when scripting against the CLI.
