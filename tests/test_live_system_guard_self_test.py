...
def test_subprocess_pkill_agentx_blocked():
    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["pkill", "-f", "agentx"])


def test_subprocess_pkill_agentx_gateway_blocked():
    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["pkill", "-f", "agentx-gateway"])


def test_subprocess_pkill_python_dash_f_blocked():
    """``pkill -f python`` matches the gateway's "python -m agentx_cli.main"."""
    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["pkill", "-f", "python"])


def test_subprocess_killall_agentx_blocked():
    with pytest.raises(RuntimeError, match="live-system guard"):
...
def test_systemctl_status_passes_through():
    ...
    r = subprocess.run(
        ["systemctl", "--user", "status", "agentx-gateway", "--no-pager"],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_systemctl_show_passes_through():
    r = subprocess.run(
        ["systemctl", "--user", "show", "agentx-gateway", "--no-pager"],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_systemctl_unrelated_unit_passes_through():
    """systemctl restart of a non-agentx unit is allowed (we only protect agentx)."""
...
def test_subprocess_pkill_with_unrelated_pattern_passes_through():
    """``pkill -f some-unrelated-pattern`` (no agentx/python) is fine."""
