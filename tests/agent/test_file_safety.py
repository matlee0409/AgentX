...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_agentx_env(self):
        """AgentX' own .env inside AGENTX_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: agentx internal .env is in ~/.agentx/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.agentx/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.agentx/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    ...
    def test_hub_index_cache_blocked(self, tmp_path):
        ...
        cache = agentx_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._agentx_home_path", return_value=agentx_home):
    ...
    def test_hub_directory_blocked(self, tmp_path):
        ...
        agentx_home = tmp_path / ".agentx"
        hub = agentx_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_agentx_home(self, tmp_path):
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        agentx_home = tmp_path / ".agentx"
        cache = agentx_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
