...
def agentx_home(tmp_path, monkeypatch):
    ...
    # Some modules cache get_agentx_home; clear if needed.
    import agentx_constants
    if hasattr(agentx_constants, "_AGENTX_HOME_CACHE"):
        agentx_constants._AGENTX_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(agentx_home, monkeypatch):
...
def test_install_bws_happy_path(agentx_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(agentx_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(agentx_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from agentx_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under agentx_home/cache/."""
    home = tmp_path / ".agentx"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
