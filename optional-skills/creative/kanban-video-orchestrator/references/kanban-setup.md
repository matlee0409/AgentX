...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _agentx_env="${AGENTX_HOME:-$HOME/.agentx}/.env"
    if grep -q "^${var}=" "$_agentx_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_agentx_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_agentx_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}
