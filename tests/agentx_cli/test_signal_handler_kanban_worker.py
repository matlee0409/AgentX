...
def _synthetic_worker_script() -> str:
    ...
    shape without needing the full agentx_cli boot path (config, providers,
...
def _is_alive_like_dispatcher(pid: int) -> bool:
    """Mirrors agentx_cli/kanban_db.py:_pid_alive on Linux.
...
def test_sigterm_with_kanban_task_env_terminates_quickly():
    ...
    try:
        ...
        while time.time() < deadline:
            ...
            time.sleep(0.02)
        pytest.fail(
            f"process still alive 2s after SIGTERM with AGENTX_KANBAN_TASK set "
...
def test_real_handler_uses_os_exit_for_kanban_workers():
    ...
    os._exit(0) when AGENTX_KANBAN_TASK is set.
    ...
    assert "AGENTX_KANBAN_TASK" in body, (
        "_signal_handler_q must gate its kanban-worker exit path on "
        "AGENTX_KANBAN_TASK — see #28181"
    )
