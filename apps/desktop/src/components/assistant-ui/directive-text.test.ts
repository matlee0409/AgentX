...
import { formatRefValue, agentxDirectiveFormatter } from './directive-text'
...
describe('agentxDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = agentxDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = agentxDirectiveFormatter.parse('@file:src/main.tsx the entry point')
