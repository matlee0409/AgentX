---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into AgentX Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into AgentX Agent"
---
...
Migrate a user's OpenClaw customization footprint into AgentX Agent. Imports AgentX-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
...
| Version | `1.0.0` |
| Author | AgentX Agent (Nous Research) |
| License | MIT |
...
Use this skill when a user wants to move their OpenClaw setup into AgentX Agent with minimal manual cleanup.
...
It uses `scripts/openclaw_to_agentx.py` to:
...
- `scripts/openclaw_to_agentx.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.agentx/skills/migration/openclaw-migration/scripts/openclaw_to_agentx.py`
...
```bash
python3 ~/.agentx/skills/migration/openclaw-migration/scripts/openclaw_to_agentx.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.agentx/skills/migration/openclaw-migration/scripts/openclaw_to_agentx.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.agentx/skills/migration/openclaw-migration/scripts/openclaw_to_agentx.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.agentx/skills/migration/openclaw-migration/scripts/openclaw_to_agentx.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.agentx/skills/migration/openclaw-migration/scripts/openclaw_to_agentx.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.agentx/skills/migration/openclaw-migration/scripts/openclaw_to_agentx.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
