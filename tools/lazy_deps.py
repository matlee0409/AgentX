"""
Lazy dependency installer for opt-in AgentX Agent backends.
...
(``agentx-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``agentx_cli.tools_config._pip_install`` but
