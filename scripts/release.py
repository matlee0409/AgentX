...
VERSION_FILE = REPO_ROOT / "agentx_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "agentx@marian.local": "Schrotti77",
    ...
    "agentx@nousresearch.com": "NousResearch",
    ...
    "agentx@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "github@nadyaagentx.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: agentx-eval community link)
    ...
    "agentxagent26@gmail.com": "agentxagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+agentxagent26@users.noreply.github.com": "agentxagent26",
    ...
    "vigo@agentx": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.agentx@agentmail.to": "oseftg",
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"agentx-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "agentx@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/agentx-agent",
    ...
    lines.append(f"# AgentX Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for AgentX Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="AgentX Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  AgentX Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"AgentX Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"AgentX Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'AgentX Agent v{new_version} ({calver_date})' "
