...
class TestApply:
    ...
    def test_enable_triggers_mcp_migration(self):
        ...
        with patch.object(crs, "check_codex_binary_ok",
                          return_value=(True, "0.130.0")), \
             patch("agentx_cli.codex_runtime_plugin_migration.migrate") as mig:
            mig.return_value.migrated = ["filesystem", "agentx-tools"]
        ...
        # User MCP servers are reported (excluding internal agentx-tools)
    ...
    def test_disable_does_not_trigger_migration(self):
        ...
        with patch("agentx_cli.codex_runtime_plugin_migration.migrate") as mig:
            r = crs.apply(cfg, "auto")
        assert r.success
