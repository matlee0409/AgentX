...
argparse → cmd_chat → cli.main() → AgentXCLI → AIAgent call chain.
"""
...
class TestIgnoreUserConfigEnvGate:
    ...
    When the env var is set, user config at ``<agentx_home>/config.yaml`` is
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._agentx_home at tmp_path and return a fresh load_cli_config."""
        import cli
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``AgentXCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting AGENTX_IGNORE_RULES=1 flips AgentXCLI.ignore_rules True."""
        monkeypatch.setenv("AGENTX_IGNORE_RULES", "1")

        # Import AgentXCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of AgentXCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.AgentXCLI)
        # Replicate the exact logic from cli.py AgentXCLI.__init__:
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``agentx_cli/main.py`` must set
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real agentx parser accepts both flags."""
        ...
        import inspect
        import agentx_cli.main as hm
        src = inspect.getsource(hm)
        assert "AGENTX_IGNORE_USER_CONFIG" in src
        assert "AGENTX_IGNORE_RULES" in src
