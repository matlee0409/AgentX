...
# the module) fail with ModuleNotFoundError for agentx_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from agentx_constants import get_agentx_home
from agentx_cli._subprocess_compat import windows_hide_flags
from agentx_cli.config import load_config, _expand_env_vars
from agentx_time import now as _agentx_now
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy agentx_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from agentx_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    try:
        from agentx_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
_agentx_home: Path | None = None


def _get_agentx_home() -> Path:
    """Resolve AgentX home dynamically while preserving test monkeypatch hooks."""
    return _agentx_home or get_agentx_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    agentx_home = _get_agentx_home()
    lock_dir = agentx_home / "cron"
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    * anything else — run with the current Python interpreter
    ...
    """
    scripts_dir = _get_agentx_home() / "scripts"
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    if job.get("no_agent"):
        ...
        now_iso = _agentx_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_agentx_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_agentx_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    os.environ["AGENTX_CRON_SESSION"] = "1"
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_agentx_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_agentx_home() / ".env"), override=True, encoding="latin-1")
        ...
        # Model resolution precedence: per-job override > AGENTX_MODEL env >
        ...
        try:
            import yaml
            _cfg_path = str(_get_agentx_home() / "config.yaml")
        ...
        if prefill_file:
            ...
            if not pfpath.is_absolute():
                pfpath = _get_agentx_home() / pfpath
            if pfpath.exists():
                try:
...
**Job ID:** {job_id}
**Run Time:** {_agentx_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_agentx_now().strftime('%b %d %H:%M')}"
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _agentx_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _agentx_now().strftime('%H:%M:%S'), len(due_jobs))
