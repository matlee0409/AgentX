...
export const $agentxHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, agentxHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_agentx_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $agentxHome.set(agentxHome)
    $mode.set(mode)
  } catch (err) {
...
export async function launchAgentXDesktop(): Promise<void> {
  ...
  await invoke('launch_agentx_desktop', { installRoot })
}
