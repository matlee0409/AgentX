...
# ``sessions_dir / f"{session_id}.json"`` in agentx_state, request-dump
...
def build_session_context_prompt(
    ...
    *,
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_agentx_home()}/cron/output/)"
    )
    ...
    def _save(self) -> None:
        ...
        data = {
            "_README": (
                ...
                "~/.agentx/state.db and are shown by `agentx sessions list` and "
