...
def test_writable_install_returns_install_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agentx_constants.get_agentx_home", lambda: agentx_home
    )

    resolved = whatsapp_common.resolve_whatsapp_bridge_dir()
    assert resolved == install_bridge
    # Nothing mirrored into AGENTX_HOME.
    assert not (agentx_home / "scripts" / "whatsapp-bridge").exists()


def test_readonly_install_mirrors_to_agentx_home(tmp_path, monkeypatch):
    ...
    expected = agentx_home / "scripts" / "whatsapp-bridge"
...
def test_readonly_install_reuses_existing_mirror(tmp_path, monkeypatch):
    """If the AGENTX_HOME mirror already exists, return it without re-copying."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)

    agentx_home = tmp_path / "agentx_home"
    mirror = agentx_home / "scripts" / "whatsapp-bridge"
