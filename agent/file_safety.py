...
def _agentx_home_path() -> Path:
    """Resolve the active AGENTX_HOME (profile-aware) without circular imports."""
    try:
        from agentx_constants import get_agentx_home  # local import to avoid cycles
        return get_agentx_home()
    except Exception:
        return Path(os.path.expanduser("~/.agentx"))


def _agentx_root_path() -> Path:
    """Resolve the AgentX root dir (always the parent of any profile, never per-profile)."""
    try:
        from agentx_constants import get_default_agentx_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    agentx_home = _agentx_home_path()
    agentx_root = _agentx_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(agentx_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(agentx_root / ".anthropic_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved AGENTX_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("AGENTX_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    agentx_dirs = []
    for base in (_agentx_home_path(), _agentx_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in agentx_dirs:
                agentx_dirs.append(real)
        except Exception:
            continue

    for base_real in agentx_dirs:
        try:
...
def _resolve_active_profile_name() -> str:
    ...
    """
    try:
        home_real = _agentx_home_path().resolve()
        root_real = _agentx_root_path().resolve()
