...
def _load_agentx_env_vars() -> dict[str, str]:
...
class DockerEnvironment(BaseEnvironment):
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _AGENTX_PROVIDER_ENV_BLOCKLIST)
        agentx_env = _load_agentx_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = agentx_env.get(key)
            if value:
                exec_env[key] = value
