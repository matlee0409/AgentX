...
Usage:
    ...
    agentx uninstall           Uninstall AgentX Agent
...
# IMPORTANT: agentx_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``agentx_bootstrap`` is a
...
# new code references ``agentx_bootstrap`` but the editable install's
...
try:
    import agentx_bootstrap  # noqa: F401
...
# that run *before* agentx_cli.config is importable. Mirrors the explicit
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``AGENTX_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
...
def _print_fast_version_info() -> None:
    from agentx_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"AgentX Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``agentx --version`` before config/logging imports on Termux."""
...
from agentx_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from agentx_cli.subcommands.cron import build_cron_parser
from agentx_cli.subcommands.gateway import build_gateway_parser
from agentx_cli.subcommands.profile import build_profile_parser
from agentx_cli.subcommands.model import build_model_parser
from agentx_cli.subcommands.setup import build_setup_parser
from agentx_cli.subcommands.postinstall import build_postinstall_parser
from agentx_cli.subcommands.whatsapp import build_whatsapp_parser
from agentx_cli.subcommands.slack import build_slack_parser
from agentx_cli.subcommands.login import build_login_parser
from agentx_cli.subcommands.logout import build_logout_parser
from agentx_cli.subcommands.auth import build_auth_parser
from agentx_cli.subcommands.status import build_status_parser
from agentx_cli.subcommands.webhook import build_webhook_parser
from agentx_cli.subcommands.hooks import build_hooks_parser
from agentx_cli.subcommands.doctor import build_doctor_parser
from agentx_cli.subcommands.security import build_security_parser
from agentx_cli.subcommands.dump import build_dump_parser
from agentx_cli.subcommands.debug import build_debug_parser
from agentx_cli.subcommands.backup import build_backup_parser
from agentx_cli.subcommands.import_cmd import build_import_cmd_parser
from agentx_cli.subcommands.config import build_config_parser
from agentx_cli.subcommands.version import build_version_parser
from agentx_cli.subcommands.update import build_update_parser
from agentx_cli.subcommands.uninstall import build_uninstall_parser
from agentx_cli.subcommands.dashboard import build_dashboard_parser
from agentx_cli.subcommands.gui import build_gui_parser
from agentx_cli.subcommands.logs import build_logs_parser
from agentx_cli.subcommands.prompt_size import build_prompt_size_parser
from agentx_cli.subcommands.memory import build_memory_parser
from agentx_cli.subcommands.acp import build_acp_parser
from agentx_cli.subcommands.tools import build_tools_parser
from agentx_cli.subcommands.insights import build_insights_parser
from agentx_cli.subcommands.skills import build_skills_parser
from agentx_cli.subcommands.pairing import build_pairing_parser
from agentx_cli.subcommands.plugins import build_plugins_parser
from agentx_cli.subcommands.mcp import build_mcp_parser
from agentx_cli.subcommands.claw import build_claw_parser
...
def _apply_profile_override() -> None:
    ...
    # Mirrors agentx_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # See issue #22502.
    agentx_home_env = os.environ.get("AGENTX_HOME", "")
    if profile_name is None and agentx_home_env:
        if Path(agentx_home_env).parent.name == "profiles":
            return
    ...
    if profile_name is None and not os.environ.get("AGENTX_S6_SUPERVISED_CHILD"):
        try:
            from agentx_constants import get_default_agentx_root

            active_path = get_default_agentx_root() / "active_profile"
    ...
    if profile_name is not None:
        try:
            from agentx_cli.profiles import resolve_profile_env

            agentx_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            agentx_home = _resolve_sudo_user_profile_env(profile_name)
            if not agentx_home:
            ...
            return
        os.environ["AGENTX_HOME"] = agentx_home
...
from agentx_cli.config import get_agentx_home
from agentx_cli.env_loader import load_agentx_dotenv

load_agentx_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → AGENTX_REDACT_SECRETS env
# var BEFORE agentx_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_agentx_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            from agentx_cli import managed_scope
...
try:
    from agentx_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from agentx_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if agentx_constants not importable yet
...
from agentx_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to agentx_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (agentx_cli.main._model_flow_*) keep resolving unchanged.
from agentx_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_agentx_home() / "skills" / ".termux_bundled_sync_stamp"
...
def _has_any_provider_configured() -> bool:
    ...
    from agentx_cli.config import get_env_path, get_agentx_home, load_config
    from agentx_cli.auth import get_auth_status
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_agentx_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from agentx_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_agentx_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from agentx_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, agentx_bin
    ...
    agentx_bin = container_info["agentx_bin"]
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/agentx-ink/package.json",
    "packages/agentx-ink/index.js",
    "packages/agentx-ink/text-input.js",
)
...
def _ensure_tui_node() -> None:
    ...
    agentx_home = os.environ.get("AGENTX_HOME") or str(Path.home() / ".agentx")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "AGENTX_HOME": agentx_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(agentx_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(agentx_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if agentx_cli_dir is None:
        agentx_cli_dir = Path(__file__).parent
    bundled = agentx_cli_dir / "tui_dist" / "entry.js"
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        if not path and bin == "node":
            try:
                from agentx_cli.dep_ensure import ensure_dependency
    ...
    if tui_dev:
        ...
        # packages/agentx-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "agentx-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from agentx_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from agentx_cli.config import apply_terminal_config_to_env
    ...
    env["AGENTX_PYTHON_SRC_ROOT"] = os.environ.get(
        "AGENTX_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("AGENTX_PYTHON", sys.executable)
    env.setdefault("AGENTX_CWD", os.getcwd())
    ...
    if code == 42:
        from agentx_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    ...
    if os.environ.get("AGENTX_KANBAN_BOARD"):
        return
    try:
        from agentx_cli.kanban_db import get_current_board
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``AGENTX_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("AGENTX_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    try:
        from agentx_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from agentx_cli.config import load_config as _load_config_for_xai_check
    ...
    if _termux_should_prefetch_update_check():
        try:
            from agentx_cli.banner import prefetch_update_check
    ...
    # setup (config, rules files, plugins, MCP servers) or from AgentX itself.
    # Implemented as a superset of --ignore-user-config + --ignore-rules plus
    # plugin/MCP discovery suppression (AGENTX_SAFE_MODE is checked by
    # agentx_cli/plugins.py and tools/mcp_tool.py).
...
def cmd_gateway(args):
    ...
    from agentx_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from agentx_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from agentx_cli.config import get_env_value, save_env_value
    from agentx_constants import find_node_executable, with_agentx_node_path
    ...
    session_dir = get_agentx_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ AgentX Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  agentx gateway")
...
def cmd_whatsapp_cloud(args):
    ...
    ``agentx_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from agentx_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from agentx_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from agentx_cli.config import stamp_install_method
    from agentx_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ AgentX post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from agentx_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    persistence.
    """
    from agentx_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from agentx_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from agentx_cli.providers import resolve_provider_full
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from agentx_cli.config import read_raw_config
    ...
    # PROVIDER_GROUPS in agentx_cli/models.py). A multi-member group shows one
...
def _clear_stale_openai_base_url():
    ...
    """
    from agentx_cli.config import get_env_value, save_env_value, load_config
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`agentx_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from agentx_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``agentx model`` flow, then route aux tasks to them here.
    """
    from agentx_cli.config import load_config
    from agentx_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from agentx_cli.auth import _prompt_model_selection
    from agentx_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from agentx_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from agentx_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from agentx_cli.runtime_provider import _detect_api_mode_for_url
...
def _remove_custom_provider(config):
    ...
    try:
        from agentx_cli.curses_ui import curses_radiolist
...
# downstream call sites read `agentx_cli.main._PROVIDER_MODELS` directly,
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from agentx_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    """
    from agentx_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from agentx_cli.config import save_env_value
    from agentx_cli.secret_prompt import masked_secret_prompt
    ...
    from agentx_cli.env_loader import format_secret_source_suffix
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from agentx_constants import display_agentx_home as _dhh_fn
...
def cmd_login(args):
    ...
    from agentx_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from agentx_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from agentx_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from agentx_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from agentx_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from agentx_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from agentx_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from agentx_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from agentx_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from agentx_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from agentx_cli.doctor import run_doctor
...
def cmd_security(args):
    ...
    if sub in ("audit", None):
        from agentx_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from agentx_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from agentx_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from agentx_cli.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from agentx_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from agentx_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from agentx_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from agentx_cli.banner import format_banner_version_label
    ...
    try:
        from agentx_cli.banner import check_for_updates
        from agentx_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall AgentX Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from agentx_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from agentx_cli.uninstall import run_gui_uninstall
    ...
    from agentx_cli.uninstall import run_uninstall
...
# Critical files that every ``agentx`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``agentx update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "agentx_cli/main.py",
    "agentx_cli/config.py",
    "agentx_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``agentx_cli/web_dist/`` (per vite.config.ts
    outDir: "../agentx_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "agentx_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``agentx`` launcher present but ``agentx_cli`` not importable.
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m agentx_cli.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = with_agentx_node_path()
...
def _desktop_stamp_path() -> Path:
    ...
    from agentx_constants import get_agentx_home
    return get_agentx_home() / "desktop-build-stamp.json"
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_agentx_node_path(env)
    if mirror:
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "AgentX is damaged and can't be opened." The
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ AgentX Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ AgentX Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from agentx_logging import setup_logging as _setup_logging_gui
    ...
    # with_agentx_node_path() copies os.environ when called with no arg.
    env = with_agentx_node_path()
    ...
    if source_mode:
        print("→ Launching AgentX Desktop from source build...")
    ...
    print(f"→ Launching packaged AgentX Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``agentx dashboard`` processes other than ourselves.
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the AgentX Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``agentx dashboard`` as
    ...
    patterns = [
        "agentx dashboard",
        "agentx_cli.main dashboard",
        "agentx_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # with `agentx_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    ...
    print(
        "  Docs:         https://agentx-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``agentx dashboard`` processes.
    ...
    """
    # When the AgentX Desktop Electron app spawns this dashboard as a
...
def _update_via_zip(args):
    """Update AgentX Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        sys.exit(1)
    zip_url = (
        f"https://github.com/NousResearch/agentx-agent/archive/refs/heads/{branch}.zip"
    )
    ...
    from agentx_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        from agentx_cli.model_catalog import seed_cache_from_checkout
...
OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/agentx-agent.git",
    "git@github.com:NousResearch/agentx-agent.git",
    "https://github.com/NousResearch/agentx-agent",
    "git@github.com:NousResearch/agentx-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/agentx-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_agentx_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_agentx_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from NousResearch/agentx-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/agentx-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/agentx-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    default_home = get_default_agentx_root()
...
# Install-scoped breadcrumb dropped right before ``agentx update`` mutates the
...
# survives and the next ``agentx`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $AGENTX_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``agentx update`` left half-done.
    ...
    try:
        ...
        try:
            from agentx_cli.managed_uv import ensure_uv
...
def _agentx_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "agentx.exe",
        scripts_dir / "agentx-gateway.exe",
    ]


def _detect_concurrent_agentx_instances(
    ...
    The AgentX Desktop Electron app spawns ``agentx.EXE`` as a backend child,
    ...
    for shim in _agentx_exe_shims(scripts_dir):
        try:
...
def _format_concurrent_instances_message(
    ...
    lines.append("")
    lines.append("  Close AgentX Desktop, exit any open `agentx` REPLs, and")
...
def _quarantine_running_agentx_exe(
    ...
    handles (recovers in <1s), or the AgentX Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       AgentX Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _agentx_exe_shims(scripts_dir):
        ...
        print(
            "    Close AgentX Desktop, exit other `agentx` REPLs, stop the "
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_agentx_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_agentx_exe(scripts_dir)
    try:
...
def _refresh_active_lazy_features() -> None:
    ...
    if failed:
        ...
        print("  `agentx update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    ``_quarantine_running_agentx_exe`` for the rationale.
    """
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from agentx_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    """
    from agentx_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    nixos_env = with_agentx_node_path(_nixos_build_env())
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # agentx_cli.config.get_agentx_home to simulate setup failure.
        from agentx_cli.config import get_agentx_home as _get_agentx_home

        logs_dir = _get_agentx_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    dropping the flag.
    """
    from agentx_cli.config import detect_install_method
    ...
    if method == "docker":
        ...
        from agentx_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from agentx_cli.config import recommended_update_command
        from agentx_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# AgentX Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of AGENTX_HOME before running the update.
    ...
    AGENTX_HOME directories.  The ``--backup`` flag on ``agentx update``
    ...
    # ``agentx_cli/config.py``). Fall back to false if the key is missing
    # so the default behaviour matches the shipped config: zipping a large
    # AGENTX_HOME can add minutes to every update. Users who want the
    ...
    try:
        from agentx_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_agentx_home so the user sees ~/.agentx/...
    try:
        from agentx_constants import get_agentx_home, display_agentx_home

        home = get_agentx_home()
        try:
            display_path = f"{display_agentx_home()}/{out_path.relative_to(home)}"
        except ValueError:
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    try:
        ...
        from agentx_cli.gateway import (
    ...
    if not running_pids:
        ...
        try:
            from agentx_cli import gateway_windows
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m agentx_cli.main
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    try:
        from agentx_cli import gateway_windows
        from agentx_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def cmd_update(args):
    """Update AgentX Agent to the latest version.
    ...
    if is_managed():
        managed_error("update AgentX Agent")
        return
...
def _cmd_update_pip(args):
    """Update AgentX via pip (for PyPI installs)."""
    from agentx_cli import __version__
    from agentx_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "agentx-agent"]
        ...
        cmd = [pipx, "upgrade", "agentx-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "agentx-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "agentx-agent"]
    ...
    print("✓ Update complete! Restart agentx to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_agentx_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # though `git pull` can't touch $AGENTX_HOME, this is cheap
        ...
        try:
            from agentx_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in agentx_cli/config.py bricked
        ...
        # the old bytecode (e.g. get_agentx_home added to agentx_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``agentx`` launch finishes the
        ...
        from agentx_constants import find_node_executable

        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "agentx_cli.main", "desktop", "--build-only"]
        ...
        # modules in this process.  Reload agentx_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_agentx_home() added since the last release.
        try:
            import importlib
            import agentx_constants as _hc
        ...
        try:
            from agentx_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from agentx_cli.profiles import backfill_profile_envs
        ...
        # it and warn loudly.
        try:
            from agentx_cli.backup import restore_cron_jobs_if_emptied
        ...
        try:
            if sys.platform in ("darwin", "win32", "linux") and shutil.which("cua-driver"):
                from agentx_cli.tools_config import install_cua_driver
        ...
        if gateway_mode:
            _exit_code_path = get_agentx_home() / ".update_exit_code"
            try:
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. agentx-gateway.service or agentx-gateway-coder.service
        ...
        # When both agentx.service (from a pre-rename install) and the
        ...
        try:
            from agentx_cli.gateway import (
                has_legacy_agentx_units,
                _find_legacy_agentx_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_agentx_units():
                print()
                print("⚠ Legacy AgentX gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_agentx_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (agentx.service) fight the current")
...
def cmd_profile(args):
    ...
    from agentx_constants import display_agentx_home
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.agentx)")
            else:
        ...
        from agentx_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from agentx_constants import get_agentx_home as _hh
        ...
        from agentx_cli import profile_describer as _pd
        ...
        from agentx_cli.profiles import profile_exists
        ...
        from agentx_cli.profiles import rename_profile
        ...
        from agentx_cli.profiles import export_profile
        ...
        from agentx_cli.profiles import import_profile
        ...
        import tempfile
        from agentx_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="agentx_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
        ...
        from agentx_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from agentx_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("agentx_requires"):
            print(f"Requires:     AgentX {data['agentx_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from agentx_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.agentx_requires:
        print(f"  Requires: AgentX {mf.agentx_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``agentx dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``agentx dashboard --status``
    ...
    print(f"{len(pids)} agentx dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from agentx_cli.web_server import should_require_auth
    ...
    try:
        from agentx_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    if choice == "2":
        ...
        print(
            ...
            "  Docs: https://agentx-agent.nousresearch.com/docs/"
    ...
    try:
        from agentx_cli.plugins import discover_plugins
...
def cmd_dashboard(args):
    ...
    try:
        from agentx_cli.profiles import get_active_profile_name
    ...
    if (
        ...
        reexec_argv = [
            sys.executable, "-m", "agentx_cli.main",
            "-p", "default",
            "dashboard",
            "--port", str(args.port),
            "--host", args.host,
        ...
        # update-button guard also misfires).  get_default_agentx_root()
        ...
        try:
            from agentx_constants import get_default_agentx_root
            env["AGENTX_HOME"] = str(get_default_agentx_root())
        except Exception:
    ...
    if "AGENTX_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "agentx_cli" / "web_dist"
        )
    ...
    try:
        from agentx_cli.mcp_startup import start_background_mcp_discovery
    ...
    from agentx_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from agentx_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from agentx_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from agentx_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from agentx_cli.prompt_size import cmd_prompt_size as _impl
...
def cmd_logs(args):
    ...
    from agentx_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from agentx_cli.models import CANONICAL_PROVIDERS as _cp
...
# the top-level flags declared in ``agentx_cli/_parser.py``.
#
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from agentx_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from agentx_cli.oneshot import run_oneshot
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_agentx_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_agentx_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_agentx_home()}/memories/\n")
    else:
        from agentx_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch AgentX Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from agentx_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from agentx_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from agentx_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from agentx_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from agentx_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from agentx_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from agentx_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from agentx_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from agentx_cli.claw import claw_command
...
def main():
    ...
    try:
        from agentx_cli.stdio import configure_windows_stdio
    ...
    # there's nothing to clean. See ``_quarantine_running_agentx_exe``.
    try:
    ...
    # =========================================================================
    # model command  (parser built in agentx_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from agentx_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from agentx_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://agentx-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://agentx-agent.nousresearch.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from agentx_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from agentx_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in agentx_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in agentx_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in agentx_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in agentx_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    # =========================================================================
    # slack command  (parser built in agentx_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from agentx_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in agentx_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in agentx_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in agentx_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in agentx_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in agentx_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in agentx_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from agentx_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from agentx_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from agentx_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in agentx_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in agentx_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in agentx_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in agentx_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in agentx_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in agentx_cli/subcommands/backup.py)
    # =========================================================================
    ...
    from agentx_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in agentx_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in agentx_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # pairing command  (parser built in agentx_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in agentx_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from agentx_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in agentx_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from agentx_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from agentx_cli.curator import register_cli as _register_curator_cli
    ...
    try:
        from agentx_cli.pets import register_cli as _register_pets_cli
    ...
    # =========================================================================
    # memory command  (parser built in agentx_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in agentx_cli/subcommands/tools.py)
    # =========================================================================
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from agentx_cli.tools_config import _cua_driver_cmd
            # Honor AGENTX_CUA_DRIVER_CMD for local-build testing — same
            ...
            if path:
                version = ""
                try:
                    from agentx_cli.tools_config import _cua_driver_env
    ...
    # =========================================================================
    # mcp command  (parser built in agentx_cli/subcommands/mcp.py)
    # =========================================================================
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_agentx_home() / "sessions"
    ...
    # =========================================================================
    # insights command  (parser built in agentx_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in agentx_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in agentx_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in agentx_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in agentx_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in agentx_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in agentx_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in agentx_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # =========================================================================
    # gui command  (parser built in agentx_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in agentx_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in agentx_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    from agentx_cli.config import get_container_exec_info
