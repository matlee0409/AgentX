...
def _build_full_manifest(
    ...
    ``/new``, etc. work inline.
    """
    from agentx_cli.commands import slack_app_manifest
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``agentx_cli/main.py``):
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("AGENTX_HOME") or str(Path.home() / ".agentx")) / "slack-manifest.json"
        else:
