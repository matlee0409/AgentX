"""Tests for agentx_cli/completion.py — shell completion script generation."""
...
from agentx_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_agentx_completion()" in out
        assert "complete -F _agentx_completion agentx" in out
...
class TestGenerateZsh:
    ...
    def test_registers_compdef_instead_of_invoking_completion_function(self):
        out = generate_zsh(_make_parser())
        assert 'compdef _agentx agentx' in out
        assert '_agentx "$@"' not in out
    ...
    def test_preserves_valid_zsh_arguments_alias_syntax(self):
        ...
        assert "'(-)'{-p,--profile}'[Profile name]:profile:_agentx_profiles'" in out
        ...
        try:
            result = subprocess.run(
                [
                    "zsh",
                    "-fc",
                    f"autoload -Uz compinit && compinit -D; source {path}; [[ ${{_comps[agentx]}} == _agentx ]]",
                ],
                capture_output=True,
                text=True,
            )
...
class TestGenerateFish:
    def test_disables_file_completion(self):
        ...
        assert "complete -c agentx -f" in out
...
class TestSubcommandDrift:
    def test_SUBCOMMANDS_covers_required_commands(self):
        ...
        """
        import inspect
        from agentx_cli.main import _coalesce_session_name_args
...
class TestProfileCompletion:
    ...
    def test_bash_has_profiles_helper(self):
        out = generate_bash(_make_parser())
        assert "_agentx_profiles()" in out
        assert 'profiles_dir="$HOME/.agentx/profiles"' in out
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        ...
        out = generate_bash(_make_parser())
        # The profile case should have _agentx_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_agentx_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _agentx_profiles"

    def test_zsh_has_profiles_helper(self):
        out = generate_zsh(_make_parser())
        assert "_agentx_profiles()" in out
        assert "$HOME/.agentx/profiles" in out
    ...
    def test_fish_has_profiles_helper(self):
        out = generate_fish(_make_parser())
        assert "__agentx_profiles" in out
        assert "$HOME/.agentx/profiles" in out
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__agentx_profiles" in out
        count = out.count("(__agentx_profiles)")
