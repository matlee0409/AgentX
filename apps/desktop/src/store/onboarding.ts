...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/agentx'
...
// mirroring agentx_cli/nous_subscription._GATEWAY_TOOL_LABELS so the GUI and
...
export async function refreshOnboarding(ctx: OnboardingContext) {
  ...
  if (shouldPreserveConfiguredOnFallback(runtime, state)) {
    ...
    notify({
      ...
      message:
        'AgentX Desktop could not verify the running backend on startup. Some features may be unavailable until the gateway is reachable.'
    })
