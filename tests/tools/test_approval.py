...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("agentx_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("agentx_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestTeePattern:
    ...
    def test_tee_agentx_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.agentx/.env")
        assert dangerous is True
        assert key is not None
    ...
    def test_tee_custom_agentx_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $AGENTX_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_agentx_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$AGENTX_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestAgentXConfigWriteProtection:
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.agentx/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.agentx/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.agentx/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.agentx/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.agentx/config.yaml")
        assert dangerous is True
        assert "agentx config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.agentx/config.yaml")
        assert dangerous is True

    def test_sed_in_place_absolute_agentx_home_config(self):
    ...
    def test_sed_in_place_absolute_agentx_home_env(self):
        env_path = get_agentx_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_custom_agentx_home(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $AGENTX_HOME/config.yaml")
        assert dangerous is True
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.agentx/config.yaml"
        )
        assert dangerous is True
        assert "in-place" in desc.lower() or "perl" in desc.lower()

    def test_perl_in_place_absolute_agentx_home_config(self):
        config_path = get_agentx_home() / "config.yaml"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.agentx/config.yaml"
        )
        assert dangerous is True

    def test_ruby_in_place_absolute_agentx_home_env(self):
        env_path = get_agentx_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.agentx/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.agentx/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.agentx/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.agentx/config.yaml"
        )
        assert dangerous is False

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.agentx/config.yaml")
        assert dangerous is False

    def test_normal_yaml_write_safe(self):
        # A non-AgentX config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_agentx_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $AGENTX_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestSensitiveCopyMovePattern:
    ...
    def test_cp_to_agentx_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.agentx/config.yaml")
        assert dangerous is True
...
class TestWindowsAbsolutePathFolding:
    ...
    The OS-specific ``Path.home()`` / ``get_agentx_home()`` tests above only
    ...
    def test_windows_agentx_home_config_folds(self, monkeypatch):
        # AgentX home nests under the user home on Windows; it must fold before
        # the user-home rewrite eats its prefix.
        monkeypatch.setenv("HOME", r"C:\Users\tester")
        monkeypatch.setenv("AGENTX_HOME", r"C:\Users\tester\.agentx")
        dangerous, key, _ = detect_dangerous_command(
            r"sed -i 's/manual/off/' C:\Users\tester\.agentx\config.yaml"
        )
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.agentx/agentx-agent && source venv/bin/activate && python -m agentx_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m agentx_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m agentx_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "agentx_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_pkill_agentx_detected(self):
        """pkill targeting agentx/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_agentx_detected(self):
        cmd = "killall agentx"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestPgrepKillExpansion:
    ...
    def test_pkill_agentx_still_detected(self):
...
class TestLaunchctlGatewayLifecycle:
    ...
    def test_launchctl_stop_agentx_detected(self):
        cmd = "launchctl stop ai.agentx.gateway"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "launchd" in desc.lower() or "agentx" in desc.lower()

    def test_launchctl_kickstart_agentx_detected(self):
        cmd = "launchctl kickstart -k system/ai.agentx.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_bootout_agentx_detected(self):
        cmd = "launchctl bootout system/ai.agentx.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_unload_agentx_detected(self):
        cmd = "launchctl unload ~/Library/LaunchAgents/ai.agentx.gateway.plist"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_launchctl_stop_unrelated_not_flagged(self):
        """`launchctl stop` on a non-AgentX label is out of scope for the
