...
def _plugins_dir() -> Path:
    ...
    plugins = get_agentx_home() / "plugins"
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from agentx_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_agentx_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_agentx_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_agentx_home()}/.env later)[/dim]")
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    try:
        from agentx_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def _run_composite_ui(curses, plugin_names, plugin_labels, plugin_selected,
    ...
    from agentx_cli.curses_ui import flush_stdin
