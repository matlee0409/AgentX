...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``agentx_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from agentx_cli.config import get_env_value as _get_env_value
    except ImportError:
...
from tools.xai_http import agentx_xai_user_agent
...
def _get_default_output_dir() -> str:
    ...
    return str(get_agentx_dir("cache/audio", "audio_cache"))
...
def _load_tts_config() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug("agentx_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": agentx_xai_user_agent(),
        },
        json=payload,
        timeout=60,
    )
...
def _get_piper_voices_dir() -> Path:
    ...
    """
    from agentx_constants import get_agentx_dir
    root = Path(get_agentx_dir("cache/piper-voices", "piper_voices_cache"))
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'agentx-agent[mistral]'"
                }, ensure_ascii=False)
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_agentx_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
