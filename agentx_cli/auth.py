"""
Multi-provider authentication system for AgentX Agent.
...
from agentx_cli.config import get_agentx_home, get_config_path, read_raw_config
from agentx_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
SPOTIFY_DOCS_URL = "https://agentx-agent.nousresearch.com/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

XAI_OAUTH_DOCS_URL = "https://agentx-agent.nousresearch.com/docs/guides/xai-grok-oauth"
OAUTH_OVER_SSH_DOCS_URL = "https://agentx-agent.nousresearch.com/docs/guides/oauth-over-ssh"
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from agentx_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.agentx/.env file
...
def _auth_file_path() -> Path:
    path = get_agentx_home() / "auth.json"
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    try:
        ...
        global_root = get_default_agentx_root()
    except Exception:
        return None
    profile_home = get_agentx_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``get_default_agentx_root()`` for a profile-shaped AGENTX_HOME can
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # (#27385 — "callback received but AgentX times out") can produce
            # actionable evidence without a code change.  Logged values are
            # fingerprints / booleans only; no actual code/state strings leak
            # into the log file.  Run with ``AGENTX_LOG_LEVEL=INFO`` (or check
            # ``~/.agentx/logs/agent.log`` which captures INFO+ unconditionally).
            try:
                logger.info(
            ...
            if incoming["code"] is None and incoming["error"] is None:
                ...
                body = (
                    ...
                    "<code>agentx auth add xai-oauth</code> to retry.</p>"
                    "</body></html>"
                )
                self.wfile.write(body.encode("utf-8"))
                return
...
# ``get_default_agentx_root()`` returns — ``~/.agentx`` on Linux/macOS,
...
def _nous_shared_auth_dir() -> Path:
    ...
    :func:`agentx_constants.get_default_agentx_root` returns — so
    ...
    from agentx_constants import get_default_agentx_root
    return get_default_agentx_root() / "shared"
...
def _nous_shared_store_path() -> Path:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from agentx_constants import get_default_agentx_root
        real_home_shared = (
            get_default_agentx_root() / "shared" / NOUS_SHARED_STORE_FILENAME
...
def _prompt_model_selection(
    ...
    """
    from agentx_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from agentx_cli.config import save_config, load_config
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=agentx-agent` lets xAI attribute AgentX-originated logins
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "This is a bug in AgentX — please report at "
            "https://github.com/NousResearch/agentx-agent/issues/26990.",
            provider="xai-oauth",
...
def _xai_oauth_loopback_login(
    *,
    ...
    if manual_paste:
        ...
        print("Open this URL to authorize AgentX with xAI:")
        print(authorize_url)
