"""
AgentX Desktop (Chat GUI) uninstaller.
...
def _agent_root(agentx_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return agentx_home / "agentx-agent"
...
def source_built_gui_artifacts(agentx_home: Path) -> "list[Path]":
    ...
    """
    agent_root = _agent_root(agentx_home)
    ...
    return [
        ...
        agentx_home / "desktop-build-stamp.json",
    ]
...
def agent_is_installed(agentx_home: Path) -> bool:
    ...
    if (agent_root / "agentx_cli").is_dir():
        return True
...
def gui_is_installed(agentx_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(agentx_home):
        if p.exists():
            return True
...
def gui_install_summary(agentx_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = agentx_home if agentx_home is not None else get_agentx_home()
...
def uninstall_gui(agentx_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``agentx-agent/agentx_cli`` (agent source), ``venv/``, or any
