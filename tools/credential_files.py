...
def register_credential_file(
    ...
    *relative_path* is relative to ``AGENTX_HOME`` (e.g. ``google_token.json``).
    ...
    """
    agentx_home = _resolve_agentx_home()
    ...
    host_path = agentx_home / relative_path
    ...
    containment_error = validate_within_dir(host_path, agentx_home)
...
# Each tuple is (new_subpath, old_name) matching agentx_constants.get_agentx_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_agentx_dir()`` for backward compatibility with old directory layouts.
    """
    from agentx_constants import get_agentx_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_agentx_dir(new_subpath, old_name)
        if host_dir.is_dir():
