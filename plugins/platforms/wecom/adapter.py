...
def qr_scan_for_bot_info(
    *,
    ...
    try:
        req = urllib.request.Request(generate_url, headers={"User-Agent": "AgentXAgent/1.0"})
    ...
    while time.monotonic() < deadline:
        try:
            req = urllib.request.Request(query_url, headers={"User-Agent": "AgentXAgent/1.0"})
...
# wizard + _PLATFORMS["wecom"] static dict in agentx_cli/gateway.py, and the
...
def interactive_setup() -> None:
    ...
    Replaces agentx_cli/gateway.py::_setup_wecom and the static
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'agentx-agent[wecom]'",
