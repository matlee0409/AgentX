...
export const ja = defineLocale({
  ...
  boot: {
    ready: 'AgentX Desktop の準備ができました',
    ...
    steps: {
      ...
      startingAgentXDesktop: 'AgentX Desktop を起動中…'
    },
    errors: {
  ...
  notifications: {
    ...
    errors: {
      ...
      methodNotAllowed:
        'デスクトップバックエンドがそのリクエストを拒否しました (405 Method Not Allowed)。AgentX Desktop を再起動してください。',
  ...
  settings: {
    ...
    gateway: {
      ...
      intro:
        'AgentX Desktop はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している AgentX バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。以下でプロファイルを選択して、それぞれのリモートホストを設定します。',
      appliesTo: '適用対象',
      ...
      restartingMessage: 'AgentX Desktop は保存された設定を使用して再接続します。',
  ...
  commandCenter: {
    ...
    agentxActiveSessions: (version, count) => `AgentX ${version} · アクティブセッション ${count}`,
  ...
  install: {
    ...
    settingUpTitle: 'AgentX Agent を設定中',
    finishingTitle: '仕上げ中',
    failedDesc:
  ...
  onboarding: {
    headerTitle: 'AgentX Agent のセットアップをしましょう',
    ...
    flowSubtitles: {
      ...
      loopback: 'サインインのためブラウザーを開きます — AgentX が自動接続します',
  ...
  shell: {
    ...
    statusbar: {
      ...
      gatewayTitle: 'AgentX 推論ゲートウェイのステータス',
      agents: 'エージェント',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '画像を保存するには AgentX Desktop を再起動してください。',
    restartToSaveImages: '画像を保存するには AgentX Desktop を再起動してください',
