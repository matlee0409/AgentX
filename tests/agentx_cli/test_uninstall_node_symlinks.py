"""Tests for agentx_cli.uninstall.remove_node_symlinks.
...
def _make_agentx_node(agentx_home: Path) -> Path:
    """Create a fake $AGENTX_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = agentx_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_agentx_node(fake_home):
    agentx_home = fake_home / ".agentx"
    node_bin = _make_agentx_node(agentx_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(agentx_home)
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(agentx_home) == []


def test_removes_dangling_symlink_into_agentx_node(fake_home):
    """A link into the AgentX node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the AgentX links that exist; ignores the ones that don't."""
    agentx_home = fake_home / ".agentx"
    node_bin = _make_agentx_node(agentx_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are AgentX-managed; node is a real user binary.
