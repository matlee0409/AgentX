"""Tests for the per-profile Projects store (agentx_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="AgentX Agent", folders=["/tmp/agentx"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "agentx-agent"
    assert proj.name == "AgentX Agent"
    ...
    # Lookup by slug too.
    assert pdb.get_project(conn, "agentx-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="AgentX Agent")
    pdb.create_project(conn, name="AgentX Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["agentx-agent", "agentx-agent-2"]
...
def test_db_path_under_agentx_home():
    # Resolves under AGENTX_HOME (set by the autouse isolation fixture).
