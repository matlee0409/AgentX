...
def _load_config() -> dict:
    ...
    profile_path = get_agentx_home() / "hindsight" / "config.json"
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between AgentX Agent and the User"
    ...
    def save_config(self, values, agentx_home):
        """Write config to $AGENTX_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(agentx_home) / "hindsight"
    ...
    def post_setup(self, agentx_home: str, config: dict) -> None:
        ...
        from agentx_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        self.save_config(provider_config, agentx_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(agentx_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(agentx_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between AgentX Agent and the User"},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._retain_context = self._config.get("retain_context", "conversation between AgentX Agent and the User")
