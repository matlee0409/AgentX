...
/**
 * Map an uninstall mode to the `python -m agentx_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `agentx_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'agentx_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/AgentX.app/Contents/MacOS/AgentX  → …/AgentX.app
 *   Windows: …\AgentX\AgentX.exe                 → …\AgentX  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env = {}) {
  ...
  if (platform === 'darwin') {
    // …/AgentX.app/Contents/MacOS/AgentX → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/AgentX.app
    if (appBundle.endsWith('.app')) return appBundle
    return null
  }

  if (platform === 'win32') {
    // NSIS per-user installs AgentX.exe directly in the install dir.
    const dir = p.dirname(exe)
    if (/[\\/]AgentX$/i.test(dir) || /[\\/]agentx-desktop$/i.test(dir)) return dir
    return null
  }

  // Linux: an AppImage exposes its own path via the APPIMAGE env var.
  if (env.APPIMAGE) return env.APPIMAGE
  // Unpacked electron-builder tree: …/linux-unpacked/agentx
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import agentx_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, agentxHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export AGENTX_HOME=${q(agentxHome)}`
  ]
  if (pythonPath) {
 ...
 * modes + `pythonPath`=agentRoot so `import agentx_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  const lines = [
    ...
    `set "AGENTX_HOME=${String(agentxHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
  if (pythonPath) {
