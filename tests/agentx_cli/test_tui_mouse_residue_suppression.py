...
from agentx_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    def test_writes_disable_sequence_when_tui_flag_in_argv(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["agentx", "--tui", "-c", "abc"])
    ...
    def test_writes_disable_sequence_when_agentx_tui_env_set(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["agentx"])
        monkeypatch.setenv("AGENTX_TUI", "1")
        monkeypatch.delenv("AGENTX_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_skips_when_stdout_is_not_a_tty(self, monkeypatch):
        # `agentx --tui … >log` or CI capture: pipe is fd 1, not a TTY. The
