...
sidebar_position: 10
title: "模型提供商插件"
description: "如何为 AgentX Agent 构建模型提供商（推理后端）插件"
---
...
| 集成点 | 位置 | 获得的能力 |
|---|---|---|
| 凭据解析 | `agentx_cli/auth.py` | `PROVIDER_REGISTRY["acme-inference"]` 从 profile 填充 |
| `--provider` CLI 标志 | `agentx_cli/main.py` | 接受 `acme-inference` |
| `agentx model` 选择器 | `agentx_cli/models.py` | 出现在 `CANONICAL_PROVIDERS` 中，从 `{base_url}/models` 获取模型列表 |
| `agentx doctor` | `agentx_cli/doctor.py` | 对 `ACME_API_KEY` 及 `{base_url}/models` 进行健康检查 |
| `agentx setup` | `agentx_cli/config.py` | `ACME_API_KEY` 出现在 `OPTIONAL_ENV_VARS` 和设置向导中 |
| URL 反向映射 | `agent/model_metadata.py` | 主机名 → 提供商名称，用于自动检测 |
| 辅助模型 | `agent/auxiliary_client.py` | 使用 `default_aux_model` 进行压缩/摘要 |
| 运行时解析 | `agentx_cli/runtime_provider.py` | 返回正确的 `base_url`、`api_key`、`api_mode` |
...
……其中 `acme_agentx_plugin:register` 是一个调用 `register_provider(profile)` 的函数。通用 PluginManager 在 `discover_and_load()` 期间会拾取入口点插件。对于 `kind: model-provider` 的 pip 插件，你仍需在 manifest 中声明 kind（或依赖源码文本启发式检测）。

完整的入口点设置请参阅 [构建 AgentX 插件](/guides/build-a-agentx-plugin#distribute-via-pip)。
...
- [构建 AgentX 插件](/guides/build-a-agentx-plugin) — 通用插件编写指南