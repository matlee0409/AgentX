"""langfuse — AgentX plugin for Langfuse observability.

Traces AgentX conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the AgentX plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``agentx plugins enable
observability/langfuse`` or ``agentx tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``agentx tools`` or ~/.agentx/.env):
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the AgentX plugin system —
...
def _trace_key(
    ...
    *,
    ...
    Older AgentX paths only expose ``task_id``/``session_id``. Newer paths
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="AgentX turn",
                tags=["agentx", "langfuse"],
            ):
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older AgentX branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current AgentX also has a turn-scoped
    ...
    # messages is a list only for legacy AgentX branches that fired
    # pre_llm_call with API messages directly. Current AgentX fires
...
def register(ctx) -> None:
    ...
    # AgentX versions.  pre_api_request / post_api_request fire per API
