...
See: https://github.com/NousResearch/agentx-agent/issues/1002
See: https://github.com/NousResearch/agentx-agent/issues/1264
"""
...
class TestAgentXBinDirOnPath:
    ...
    _resolve_agentx_bin_dir / _prepend_agentx_bin_dir.
    """
    ...
    def test_resolves_via_which(self, monkeypatch):
        ...
        assert local_mod._resolve_agentx_bin_dir() == "/opt/agentx/bin"
    ...
    def test_resolves_via_sys_executable_dir(self, monkeypatch, tmp_path):
        ...
        (venv_bin / "agentx").write_text("#!/bin/sh\n")
        ...
        assert local_mod._resolve_agentx_bin_dir() == str(venv_bin)
    ...
    def test_returns_none_when_unresolvable(self, monkeypatch):
        ...
        assert local_mod._resolve_agentx_bin_dir() is None

    def test_prepend_adds_missing_dir_at_front(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._AGENTX_BIN_DIR = "/opt/agentx/bin"
        out = local_mod._prepend_agentx_bin_dir("/usr/bin:/bin")
        assert out.split(os.pathsep)[0] == "/opt/agentx/bin"
    ...
    def test_prepend_is_idempotent(self, monkeypatch):
        ...
        self._reset_cache()
        local_mod._AGENTX_BIN_DIR = "/opt/agentx/bin"
        once = local_mod._prepend_agentx_bin_dir("/usr/bin:/bin")
        twice = local_mod._prepend_agentx_bin_dir(once)
        assert twice == once
        assert once.split(os.pathsep).count("/opt/agentx/bin") == 1

    def test_prepend_noop_when_unresolved(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._AGENTX_BIN_DIR = None
        assert local_mod._prepend_agentx_bin_dir("/usr/bin:/bin") == "/usr/bin:/bin"

    def test_make_run_env_injects_agentx_bin_dir(self, monkeypatch):
