"""
AgentX Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m agentx_cli.main web          # Start on http://127.0.0.1:9119
    python -m agentx_cli.main web --port 8080
"""
...
from agentx_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
from agentx_cli.memory_providers import (
    MemoryProvider,
    ProviderField,
    get_memory_provider,
)
...
def _warm_gateway_module() -> None:
    try:
        import agentx_cli.gateway  # noqa: F401
    except Exception:
        pass
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from agentx_cli.gateway import _get_restart_drain_timeout
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire agentx_cli.gateway import into a background thread so the event
    # loop is not blocked and AGENTX_DASHBOARD_READY fires without delay.
...
app = FastAPI(title="AgentX Agent", version=__version__, lifespan=_lifespan)

# Memory-provider OAuth connect routes live in the memory layer, not here.
from agentx_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("AGENTX_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
...
#
# This list is defined in ``agentx_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from agentx_cli.dashboard_auth.public_paths import (
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``agentx-0day``
...
async def _dashboard_auth_gate(request: Request, call_next):
    from agentx_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from agentx_cli.dashboard_auth.token_auth import token_auth_middleware
...
class ModelAssignment(BaseModel):
    ...
    # ``agentx model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    """
    from agentx_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from agentx_cli.model_normalize import normalize_model_for_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from agentx_cli.models import _AGGREGATOR_PROVIDERS
...
def _default_agentx_root_is_opt_data() -> bool:
    ...
    try:
        ...
        root = get_default_agentx_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    ...
    """
    if _default_agentx_root_is_opt_data():
        return True
    try:
        from agentx_constants import is_container
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_agentx_home() at call time
    ...
    try:
        ...
        # Windows install the first import of agentx_cli.gateway blocks the
        ...
        try:
            from agentx_cli.dashboard_auth import list_providers as _list_providers
        ...
        status = {
            ...
            "can_update_agentx": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
...
async def get_system_stats():
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_agentx_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_agentx_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from agentx_cli.auth import get_nous_auth_status
    ...
    features = []
    try:
        from agentx_cli.nous_subscription import get_nous_subscription_features
...
async def run_prompt_size():
    try:
        proc = _spawn_agentx_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_agentx_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_agentx_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from agentx_cli.debug import build_debug_share
...
_ACTION_LOG_DIR: Path = get_agentx_home() / "logs"
...
def _spawn_agentx_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``agentx <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``agentx_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "agentx_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "AGENTX_NONINTERACTIVE": "1"},
    }
    if sys.platform == "win32":
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    ...
    return _spawn_agentx_action(subcommand, "gateway-restart"), False
...
async def update_agentx():
    ...
    try:
        proc = _spawn_agentx_action(["update"], "agentx-update")
    except Exception as exc:
...
async def check_agentx_update(force: bool = False):
    ...
    try:
        ...
        if force:
            try:
                (get_agentx_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def get_profiles_sessions(
    ...
    from agentx_state import SessionDB
    from agentx_cli import profiles as profiles_mod
...
def _memory_provider_config_path(provider: MemoryProvider) -> Path:
    return get_agentx_home() / provider.name / "config.json"
...
# in agentx_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(profile: Optional[str] = None, refresh: bool = False):
    ...
    """
    try:
        from agentx_cli.inventory import build_models_payload, load_picker_context
        ...
        # onboarding picker) sees the SAME full provider universe `agentx model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    if slug == "nous":
        try:
            ...
            from agentx_cli.auth import get_provider_auth_state
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from agentx_cli.moa_config import normalize_moa_config
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from agentx_cli.model_cost_guard import expensive_model_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (agentx_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from agentx_cli.nous_subscription import apply_nous_managed_defaults
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from agentx_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    """
    try:
        from agentx_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from agentx_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "email": {
        ...
        "docs_url": "https://agentx-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "weixin": {
        ...
        "docs_url": "https://agentx-agent.nousresearch.com/docs/user-guide/messaging/weixin/",
    ...
    "webhook": {
        ...
        "docs_url": "https://agentx-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.agentx-agent.nousresearch.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"AgentXDashboard/{__version__}"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "AgentX Agent").strip() or "AgentX Agent"
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's AGENTX_HOME.
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_agentx_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_agentx_oauth_credentials,
            _AGENTX_OAUTH_FILE,
        )
    except ImportError:
        read_agentx_oauth_credentials = None  # type: ignore
        _AGENTX_OAUTH_FILE = None  # type: ignore

    agentx_creds = None
    if read_agentx_oauth_credentials:
        try:
            agentx_creds = read_agentx_oauth_credentials()
        except Exception:
            agentx_creds = None
    if agentx_creds and agentx_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "agentx_pkce",
            "source_label": f"AgentX PKCE ({_AGENTX_OAUTH_FILE})",
            "token_preview": _truncate_token(agentx_creds.get("accessToken")),
            "expires_at": agentx_creds.get("expiresAt"),
            "has_refresh_token": bool(agentx_creds.get("refreshToken")),
        }
    ...
    try:
        ...
        pass
    try:
        from agentx_cli.config import get_env_value
    except ImportError:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "docs_url": "https://agentx-agent.nousresearch.com/docs/guides/xai-grok-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from agentx_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("agentx_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _AGENTX_OAUTH_FILE
                if _AGENTX_OAUTH_FILE.exists():
                    _AGENTX_OAUTH_FILE.unlink()
                    cleared = True
            except Exception:
                pass
            # Also clear the credential pool entry if present.
            try:
                from agentx_cli.auth import clear_provider_auth
        ...
        try:
            from agentx_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    _AGENTX_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _AGENTX_OAUTH_FILE.with_name(
        f"{_AGENTX_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _AGENTX_OAUTH_FILE)
        try:
            _AGENTX_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
# the tokens exactly like `agentx auth add xai-oauth`.
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `agentx auth add xai-oauth`'s credential-pool insert.
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from agentx_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from agentx_cli.auth import _save_codex_tokens
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `agentx sessions prune`)."""
    if body.older_than_days < 1:
        raise HTTPException(status_code=400, detail="older_than_days must be >= 1")
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_agentx_home()
    db = _open_session_db_for_profile(body.profile)
    try:
...
async def get_logs(
    ...
    from agentx_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_agentx_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from agentx_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["agentx_home"] = str(home)
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process AGENTX_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (agentx_cli.mcp_config), so
...
async def list_mcp_servers(profile: Optional[str] = None):
    from agentx_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from agentx_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from agentx_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from agentx_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    def _probe_scoped():
        ...
        # AGENTX_HOME override (see mcp_tool._wrap_with_home_override), so
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    """
    try:
        from agentx_cli import mcp_catalog
    except Exception as exc:
...
#
# Wraps the same JSON store the CLI uses (agentx_cli.webhook); the webhook
...
async def list_webhooks():
    import agentx_cli.webhook as wh
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_agentx_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_agentx_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from agentx_cli.auth import read_credential_pool
...
# `agentx memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `agentx memory setup` to configure a new one.",
            )
...
async def run_doctor():
    try:
        proc = _spawn_agentx_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_agentx_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_agentx_action(args, "backup")
    except Exception as exc:
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_agentx_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from agentx_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from agentx_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <agentx_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_agentx_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_agentx_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from agentx_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_agentx_home()
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    token = set_agentx_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
        ...
        reset_agentx_home_override(token)
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    """
    from agentx_constants import set_agentx_home_override, reset_agentx_home_override
    from agentx_cli.mcp_security import validate_mcp_server_entry
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    """
    from agentx_constants import set_agentx_home_override, reset_agentx_home_override
    from agentx_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    _resolve_profile_dir(name)
    try:
        from agentx_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_agentx_home()`` at call
       time — the context-local override from ``set_agentx_home_override``
    ...
    retargeted to the *current* ``get_agentx_home()`` so writes land in the
    ...
    from agentx_constants import (
        get_agentx_home,
        set_agentx_home_override,
        reset_agentx_home_override,
    )
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_agentx_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_agentx_home()`` at call time
...
async def get_skills(profile: Optional[str] = None):
    ...
    from agentx_cli.skills_config import get_disabled_skills
    with _profile_scope(profile):
        config = load_config()
...
async def get_toolsets(profile: Optional[str] = None):
    from agentx_cli.tools_config import (
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``agentx tools`` picker uses, so the GUI and CLI stay in
...
if sys.platform.startswith("win"):
    try:
        from agentx_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from agentx_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from agentx_cli.dashboard_auth.audit import AuditEvent, audit_log
        from agentx_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    matching what ``agentx_cli.main._launch_tui`` does for the CLI path.
    ...
    ``get_agentx_home()`` from that env var at its own import, so the child
    ...
    """
    from agentx_cli.main import PROJECT_ROOT, _make_tui_argv
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from agentx_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`agentx_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from agentx_cli.dashboard_auth.prefix import normalise_prefix
...
def _discover_user_themes() -> list:
    ...
    """
    themes_dir = get_agentx_home() / "dashboard-themes"
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as agentx_cli.plugins):
    ...
    from agentx_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_agentx_home() / "plugins", "user"),
    ...
    # ``agentx_cli/plugins.py`` and the documented user contract.
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from agentx_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_agentx_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from agentx_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from agentx_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from agentx_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from agentx_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"agentx_dashboard_plugin_{plugin['name']}"
...
from agentx_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    # the agentx-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # Each provider plugin that ships with AgentX Agent exposes a
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"AGENTX_DASHBOARD_READY port={actual_port}", flush=True)
