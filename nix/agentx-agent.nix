# nix/agentx-agent.nix — Overridable AgentX Agent package
#
...
let
  nodejs = nodejs_22;
  agentxVenv = callPackage ./python.nix {
    inherit uv2nix pyproject-nix pyproject-build-systems;
    dependency-groups = [ "all" ] ++ extraDependencyGroups;
  };

  agentxNpmLib = callPackage ./lib.nix {
    inherit npm-lockfile-fix nodejs;
  };

  agentxTui = callPackage ./tui.nix {
    inherit agentxNpmLib;
  };

  agentxWeb = callPackage ./web.nix {
    inherit agentxNpmLib;
  };
  ...
  checkPackageCollisions = ''
    ...
    core = set()
    venv_sp = pathlib.Path('${agentxVenv}/${sitePackagesPath}')
...
stdenv.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    cp -r ${bundledSkills} $out/share/agentx-agent/skills
    cp -r ${bundledPlugins} $out/share/agentx-agent/plugins
    cp -r ${bundledLocales} $out/share/agentx-agent/locales
    cp -r ${agentxWeb} $out/share/agentx-agent/web_dist

    mkdir -p $out/ui-tui
    cp -r ${agentxTui}/lib/agentx-tui/* $out/ui-tui/

    ${lib.concatMapStringsSep "\n"
      (name: ''
        makeWrapper ${agentxVenv}/bin/${name} $out/bin/${name} \
          ...
          --set AGENTX_PYTHON ${agentxVenv}/bin/python3 \
          --set AGENTX_NODE ${lib.getExe nodejs} \
          ${lib.optionalString (rev != null) ''--set AGENTX_REVISION ${rev} \''}
    ...
    ${lib.optionalString (extraPythonPackages != [ ]) ''
      ...
      ${agentxVenv}/bin/python3 -c "${checkPackageCollisions}"
      echo "=== No collisions ==="
    ''}
  ...
  passthru = {
    ...
    agentxDesktop = callPackage ./desktop.nix {
      inherit agentxNpmLib electron;
      agentxAgent = finalAttrs.finalPackage;
    };

    devShellHook = ''
      STAMP=".nix-stamps/agentx-agent"
      STAMP_VALUE="${pyprojectHash}:${uvLockHash}"
      if [ ! -f "$STAMP" ] || [ "$(cat "$STAMP")" != "$STAMP_VALUE" ]; then
        echo "agentx-agent: installing Python dependencies..."
        ...
        source .venv/bin/activate
        export AGENTX_PYTHON=${agentxVenv}/bin/python3
      fi
    '';
  };
