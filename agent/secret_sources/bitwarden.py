...
* The ``bws`` binary is auto-installed into ``<agentx_home>/bin/bws`` on
...
# In-process cache so repeated load_agentx_dotenv() calls (CLI startup,
...
# <agentx_home>/cache/bws_cache.json. The file holds only the secret VALUES,
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under agentx_home/cache/.

    `home_path` is what `load_agentx_dotenv()` already resolved; falling back
    to `$AGENTX_HOME` / `~/.agentx` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("AGENTX_HOME", Path.home() / ".agentx"))
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<agentx_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _agentx_bin_dir() / _platform_binary_name()
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<agentx_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
# ---------------------------------------------------------------------------
# Public entry point — called from agentx_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_agentx_dotenv()`` calls after the .env
