...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://agentx-agent.nousresearch.com",
...
def _apply_user_default_headers(headers: dict | None) -> dict | None:
    ...
    """
    try:
        from agentx_cli.config import cfg_get, load_config
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "AgentXAgent",
}
...
# in lockstep with agentx_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``agentx_cli.__version__`` is
...
_AUTH_JSON_PATH = get_agentx_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (AgentX Agent)",
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``agentx_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _nous_api_key(provider: dict) -> str:
    ...
    from agentx_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from agentx_cli.auth import _agent_key_is_usable
...
def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from agentx_cli.auth import resolve_nous_runtime_credentials
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    Falls back to ``agentx_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from agentx_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    ...
    try:
        from agentx_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from agentx_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from agentx_cli.models import copilot_default_headers
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    model = _NOUS_MODEL
    try:
        from agentx_cli.models import get_nous_recommended_aux_model
...
def _try_azure_foundry(
    *,
    ...
    :func:`agentx_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from agentx_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    ...
    try:
        from agentx_cli.nous_account import get_nous_portal_account_info
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from agentx_cli.copilot_auth import copilot_request_headers
...
def resolve_provider_client(
    ...
    try:
        from agentx_cli.runtime_provider import _get_named_custom_provider
    ...
    try:
        ...
        logger.debug("agentx_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from agentx_cli.models import _should_use_copilot_responses_api
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`agentx_cli.plugins.PluginContext.register_auxiliary_task`) the
