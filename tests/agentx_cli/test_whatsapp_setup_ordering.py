...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    agentx = home / ".agentx"
    agentx.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("AGENTX_HOME", str(agentx))
...
def _env_value(agentx_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from agentx_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("agentx_cli.main._require_tty", lambda *_a, **_kw: None)
