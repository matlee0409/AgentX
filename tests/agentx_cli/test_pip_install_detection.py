...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("agentx_cli.config.get_managed_system", return_value=None), \
         patch("agentx_cli.config.get_agentx_home", return_value=tmp_path):
...
def test_managed_install_takes_precedence(tmp_path):
    """When AGENTX_MANAGED is set, that takes precedence over git detection."""
    (tmp_path / ".git").mkdir()
    with patch("agentx_cli.config.get_managed_system", return_value="NixOS"), \
         patch("agentx_cli.config.get_agentx_home", return_value=tmp_path):
        from agentx_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_recommended_update_command_pip():
    ...
    from agentx_cli.config import recommended_update_command_for_method
...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    ...
    with patch("agentx_cli.config.get_managed_system", return_value=None), \
         patch("agentx_cli.config.get_agentx_home", return_value=home):
        from agentx_cli.config import detect_install_method
...
def test_home_docker_stamp_ignored_when_not_containerized(tmp_path):
    ...
    'docker' into the shared $AGENTX_HOME. With no code-scoped stamp, a host
    ...
    with patch("agentx_cli.config.get_managed_system", return_value=None), \
         patch("agentx_cli.config.get_agentx_home", return_value=home), \
         patch("agentx_cli.config._running_in_container", return_value=False):
        from agentx_cli.config import detect_install_method
        assert detect_install_method(project_root=code) == "git"
...
def test_home_docker_stamp_honored_inside_container(tmp_path):
    ...
    with patch("agentx_cli.config.get_managed_system", return_value=None), \
         patch("agentx_cli.config.get_agentx_home", return_value=home), \
         patch("agentx_cli.config._running_in_container", return_value=True):
        from agentx_cli.config import detect_install_method
...
def test_home_non_docker_stamp_still_honored_for_backcompat(tmp_path):
    ...
    install that historically stamped 'git'/'pip' into $AGENTX_HOME keeps
...
def test_stamp_install_method_writes_code_scoped(tmp_path):
    ...
    code.mkdir()
    home.mkdir()
    with patch("agentx_cli.config.get_agentx_home", return_value=home):
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    with patch("agentx_cli.config.get_managed_system", return_value=None), \
         patch("agentx_cli.config.get_agentx_home", return_value=tmp_path), \
         patch("agentx_constants.is_container", return_value=True):
        from agentx_cli.config import detect_install_method
...
def test_banner_warns_on_pip_install(tmp_path):
    ...
    from agentx_cli import banner

    hh = tmp_path / ".agentx"
    hh.mkdir()
    (hh / ".install_method").write_text("pip\n")

    with patch("agentx_cli.config.get_agentx_home", return_value=hh), \
         patch("agentx_constants.get_agentx_home", return_value=hh):
        buf = io.StringIO()
