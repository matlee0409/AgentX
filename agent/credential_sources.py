...
AgentX seeds its credential pool from many places:
    ...
    agentx_pkce   — ~/.agentx/.anthropic_oauth.json
...
qwen-cli, nous device_code (partial), agentx_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    """
    from agentx_cli.config import get_env_path, remove_env_value
...
def _remove_agentx_pkce(provider: str, removed) -> RemovalResult:
    """~/.agentx/.anthropic_oauth.json is ours — delete it outright."""
    from agentx_constants import get_agentx_home

    result = RemovalResult()
    oauth_file = get_agentx_home() / ".anthropic_oauth.json"
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Belt-and-braces against the manual entry path: ``agentx auth add
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="agentx_pkce",
        remove_fn=_remove_agentx_pkce,
