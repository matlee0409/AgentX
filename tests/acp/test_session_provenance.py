...
def test_root_session_no_compression(db):
    ...
    assert prov["currentAgentXSessionId"] == "root1"
    assert prov["rootAgentXSessionId"] == "root1"
    assert prov["parentAgentXSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_agentx_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentAgentXSessionId"] == "old"
    assert prov["rootAgentXSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousAgentXSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootAgentXSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootAgentXSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_agentx_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousAgentXSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"agentx"}
    assert "sessionProvenance" in meta["agentx"]
    assert meta["agentx"]["sessionProvenance"]["currentAgentXSessionId"] == "root1"
