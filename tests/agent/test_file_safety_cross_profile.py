...
@pytest.fixture
def fake_agentx(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(agentx_constants, "get_default_agentx_root", lambda: root)

    # The reloads below ensure get_cross_profile_warning/classify see the patched root.
    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_agentx_root_path", lambda: root)
...
def _set_active_home(monkeypatch, agentx_home: Path):
    """Point file_safety._agentx_home_path at a specific profile dir."""
    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_agentx_home_path", lambda: agentx_home)
...
class TestResolveActiveProfileName:
    def test_default_when_home_is_root(self, fake_agentx, monkeypatch):
        _set_active_home(monkeypatch, fake_agentx["default_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "default"

    def test_named_profile(self, fake_agentx, monkeypatch):
        _set_active_home(monkeypatch, fake_agentx["security_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "agentx-security"

    def test_falls_back_to_default_on_resolution_failure(self, fake_agentx, monkeypatch):
        ...
        monkeypatch.setattr(fs, "_agentx_home_path", _boom)
...
class TestClassifyCrossProfileTarget:
    def test_same_profile_write_returns_none(self, fake_agentx, monkeypatch):
        _set_active_home(monkeypatch, fake_agentx["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_agentx["security_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is None

    def test_security_writing_default_skill(self, fake_agentx, monkeypatch):
        """The exact incident from May 2026."""
        _set_active_home(monkeypatch, fake_agentx["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_agentx["default_home"] / "skills" / "foo" / "SKILL.md")
        )
    ...
    def test_default_writing_security_skill(self, fake_agentx, monkeypatch):
    ...
    def test_named_to_named_cross_profile(self, fake_agentx, monkeypatch):
        _set_active_home(monkeypatch, fake_agentx["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_agentx["coder_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["target_profile"] == "coder"

    @pytest.mark.parametrize("area", ["skills", "plugins", "cron", "memories"])
    def test_all_profile_scoped_areas_classified(self, fake_agentx, monkeypatch, area):
        _set_active_home(monkeypatch, fake_agentx["security_home"])
        from agent.file_safety import classify_cross_profile_target
        target = fake_agentx["default_home"] / area / "foo.txt"
        result = classify_cross_profile_target(str(target))
        assert result is not None
        assert result["area"] == area

    def test_non_agentx_path_returns_none(self, fake_agentx, monkeypatch, tmp_path):
        _set_active_home(monkeypatch, fake_agentx["security_home"])
        from agent.file_safety import classify_cross_profile_target
        # Path outside any AgentX root
        assert classify_cross_profile_target(str(tmp_path / "random.txt")) is None

    def test_agentx_config_not_classified_as_cross_profile(self, fake_agentx, monkeypatch):
        ...
        result = classify_cross_profile_target(
            str(fake_agentx["default_home"] / "config.yaml")
        )
        assert result is None
...
class TestGetCrossProfileWarning:
    def test_in_profile_returns_none(self, fake_agentx, monkeypatch):
        _set_active_home(monkeypatch, fake_agentx["security_home"])
    ...
    def test_cross_profile_warning_names_both_profiles(self, fake_agentx, monkeypatch):
        _set_active_home(monkeypatch, fake_agentx["security_home"])
        from agent.file_safety import get_cross_profile_warning
    ...
    def test_warning_is_defense_in_depth_not_boundary(self, fake_agentx, monkeypatch):
        _set_active_home(monkeypatch, fake_agentx["security_home"])
        from agent.file_safety import get_cross_profile_warning
        warn = get_cross_profile_warning(
            str(fake_agentx["default_home"] / "skills" / "foo" / "SKILL.md")
