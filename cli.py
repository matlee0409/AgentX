#!/usr/bin/env python3
"""
AgentX Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the AgentX Agent, inspired by Claude Code.
...
from agentx_cli.cli_agent_setup_mixin import CLIAgentSetupMixin
from agentx_cli.cli_commands_mixin import CLICommandsMixin
...
try:
    from agentx_cli.pt_input_extras import (
...
from agentx_cli.banner import _format_context_length, format_banner_version_label
...
from agentx_cli.browser_connect import (
...
load_agentx_dotenv(agentx_home=_agentx_home, project_env=_project_env)
...
def load_cli_config() -> Dict[str, Any]:
    ...
    user_config_path = _agentx_home / 'config.yaml'
    ...
    defaults = {
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see agentx_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from agentx_cli.config import _normalize_root_model_keys
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and AgentXCLI.__init__ checks "default" first.
    ...
    # agentx_cli.config._load_config_impl (which has its own managed merge), so
    ...
    # Normalize config key: the new config system (agentx_cli/config.py) and all
...
try:
    from agentx_logging import setup_logging
...
try:
    from agentx_cli.skin_engine import init_skin_from_config
...
def get_tool_definitions(*args, **kwargs):
    from agentx_cli.mcp_startup import wait_for_mcp_discovery
...
from agentx_cli.banner import build_welcome_banner
from agentx_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from agentx_cli.callbacks import prompt_for_secret
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`agentx_cli.config.load_config` (the authoritative loader that
    ...
    try:
        ...
        from agentx_constants import get_agentx_home as _get_agentx_home
        _agentx_home_maint = _get_agentx_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_agentx_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`agentx_cli.config.load_config`. Honours ``auto_prune`` /
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from agentx_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_agentx_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._agentx_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from agentx_cli.skin_engine import get_active_skin
...
from agentx_constants import is_termux as _is_termux_environment
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_agentx_bp_timeout_patched`` sentinel on the module.
    """
    try:
        ...
        if getattr(_vt100_mod, "_agentx_bp_timeout_patched", False):
            return
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._agentx_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_agentx_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._agentx_bp_start = now
        ...
        _vt100_mod._agentx_bp_timeout_patched = True
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``AgentXCLI._busy_command()``.
        """
        yield self

# ASCII Art - AGENTX-AGENT logo (full width, single line - requires ~95 char terminal)
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "⚕ NOUS AGENTX - AI Agent Framework"
        tiny_line = "⚕ NOUS AGENTX"
    else:
        agent_name = _skin.get_branding("agent_name", "AgentX Agent") if _skin else "AgentX Agent"
        line1 = f"{agent_name} - AI Agent Framework"
        tiny_line = agent_name

    if os.environ.get("AGENTX_FAST_STARTUP_BANNER") == "1":
        from agentx_cli import __release_date__ as _release_date
        from agentx_cli import __version__ as _version

        version_line = f"AgentX Agent v{_version} ({_release_date})"
    else:
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from agentx_cli.plugins import get_plugin_commands
...
class AgentXCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    Interactive CLI for the AgentX Agent.
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from agentx_cli.runtime_provider import _auto_detect_local_model
        ...
        # by `agentx chat --ignore-rules` in agentx_cli/main.py. When true we
        ...
        self._history_file = _agentx_home / ".agentx_history"
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from agentx_cli.voice import format_voice_record_key_for_status
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from agentx_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from agentx_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from agentx_cli.models import normalize_opencode_model_id, opencode_model_api_mode
        ...
        if self._model_is_default:
            ...
            try:
                from agentx_cli.codex_models import get_codex_model_ids
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - AgentXCLI._status_bar_display_width(label)
    ...
    def show_banner(self):
        ...
        # Warn if the configured model is a Nous AgentX LLM (not agentic)
        from agentx_cli.model_switch import is_nous_agentx_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_agentx_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research AgentX 3 & 4 models are NOT agentic and are not "
                "designed for use with AgentX Agent.[/]"
            )
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        """
        from agentx_cli.clipboard import save_clipboard_image

        img_dir = get_agentx_home() / "images"
    ...
    def _fast_command_available(self) -> bool:
        try:
            from agentx_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from agentx_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from agentx_cli.skin_engine import get_active_help_header
    ...
    def _list_recent_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if not self._session_db:
            return []
        try:
            from agentx_cli.session_listing import query_session_listing
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from agentx_cli.main import _relative_time
    ...
    def _discard_session_if_empty(self, session_id: Optional[str]) -> bool:
        ...
        if getattr(self, "conversation_history", None):
            return False
        try:
            from agentx_constants import get_agentx_home as _ghh
    ...
    def save_conversation(self):
        ...
        saved_dir = get_agentx_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"agentx_conversation_{timestamp}.json"
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from agentx_cli.curses_ui import curses_single_select
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        if self.agent is not None:
            try:
                from agentx_cli.context_switch_guard import merge_preflight_compression_warning
        ...
        try:
            from agentx_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            if not model_list:
                try:
                    from agentx_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from agentx_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        from agentx_cli.providers import get_label
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        """
        from agentx_cli import codex_runtime_switch as crs
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from agentx_cli.commands import resolve_command
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_agentx_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_agentx_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in agentx_cli/commands.py instead of touching every dispatch site.
        from agentx_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from agentx_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from agentx_cli.skin_engine import get_active_skin
            ...
            from agentx_cli.main import _print_version_info
            ...
            from agentx_cli.config import reload_env
            count = reload_env()
            ...
            try:
                ...
                if not user_entries:
                    ...
                    print(f"  Or drop a plugin directory into {display_agentx_home()}/plugins/")
                    if bundled_count:
                    ...
                    loaded: dict = {}
                    try:
                        from agentx_cli.plugins import get_plugin_manager
            ...
            # Check for plugin-registered slash commands
                ...
                from agentx_cli.plugins import (
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from agentx_cli.commands import COMMANDS
    ...
    def _toggle_verbose(self):
        ...
        from agentx_cli.colors import Colors as _Colors
        labels = {
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from agentx_cli.partial_compress import (
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by agentx_logging not
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        wall-clock-bounded inside the helper; also honors AGENTX_DEV_CREDITS_FIXTURE
    ...
    def _show_credits(self):
        ...
        if not view.logged_in:
            ...
            print("  Run `agentx portal` to log in, then /credits.")
            return
    ...
    def _show_billing(self, command: str = "/billing"):
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Nous Portal.')}")
                print("  Run `agentx portal` to log in, then /billing.")
            return
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        from agentx_cli.nous_billing import (
            BillingError,
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        from agentx_cli.nous_billing import BillingRateLimited
    ...
    def _billing_handle_scope_required(self, state):
        ...
        if not getattr(self, "_app", None):
            print("  Run `agentx portal` and approve terminal billing, then retry.")
            return
        confirm_choices = [
        ...
        if choice != "yes":
            ...
            return
        try:
            from agentx_cli.auth import step_up_nous_billing_scope
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from agentx_cli.config import get_config_path as _get_config_path
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "agentx_voice"), exist_ok=True)
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _agentx_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            try:
                from agentx_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from agentx_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from agentx_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to AgentX Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to AgentX Agent! Type your message or /help for commands."
        ...
        try:
            from agentx_cli.model_switch import prewarm_picker_cache_async
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from agentx_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in agentx_cli.pt_input_extras
        ...
        def handle_enter(event):
            ...
            if self._model_picker_state:
                try:
                    ...
                    from agentx_cli.model_switch import resolve_persist_behavior
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from agentx_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "AgentX Agent")
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = AgentXCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_agentx_osd_patched", False):
                ...
                _pt_renderer._agentx_osd_patched = True
        except Exception:
            pass
...
def _run_kanban_goal_loop_q(cli: "AgentXCLI", first_response: str) -> None:
    ...
    from agentx_cli import kanban_db as _kb
    from agentx_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    """
    AgentX Agent CLI - Interactive AI Assistant
    ...
    # mode registers its own inside AgentXCLI.run(), but `-q` runs
                    ...
                    if cli._init_agent(
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            ...
                            if os.environ.get("AGENTX_KANBAN_TASK") and result.get(
                                ...
                                try:
                                    from agentx_cli.kanban_db import (
