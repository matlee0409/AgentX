...
from agentx_cli import __version__ as _AGENTX_VERSION
...
def _codex_curated_models() -> list[str]:
    ...
    """
    from agentx_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_agentx_home() / "cache" / "nous_recommended_cache.json"
...
# path. The *interactive* default (GUI onboarding / ``agentx model``) uses the
# richer free/paid-tier-aware resolver — see ``get_recommended_default_model``
# in agentx_cli/web_server.py and ``partition_nous_models_by_tier`` — which can
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from agentx_cli.model_catalog import get_curated_openrouter_models
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from agentx_cli.model_catalog import get_curated_nous_models
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from agentx_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from agentx_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``agentx_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    """
    try:
        from agentx_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        ...
        from agentx_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from agentx_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``agentx model`` shows.
...
def _provider_models_cache_path() -> Path:
    from agentx_constants import get_agentx_home
    return get_agentx_home() / "provider_models_cache.json"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from agentx_constants import get_agentx_home
    return get_agentx_home() / "ollama_cloud_models_cache.json"
