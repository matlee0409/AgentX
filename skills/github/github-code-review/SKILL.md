...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _agentx_env="${AGENTX_HOME:-$HOME/.agentx}/.env"; [ -f "$_agentx_env" ] && grep -q "^GITHUB_TOKEN=" "$_agentx_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_agentx_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from AgentX Agent\",
    \"comments\": [
...
gh pr review $PR_NUMBER --approve --body "Reviewed by AgentX Agent. Code looks clean — good test coverage, no security concerns."
