...
Extracted from ``agentx_cli/main.py`` as part of the god-file decomposition
...
in main.py re-imports them (``from agentx_cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``agentx_cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from agentx_cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``agentx_cli.main`` at import time -> no import cycle.
"""
...
from agentx_cli.config import clear_model_endpoint_credentials
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from agentx_cli.main import _prompt_api_key
    from agentx_constants import OPENROUTER_BASE_URL
    ...
    from agentx_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from agentx_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier:
        try:
            from agentx_cli.nous_account import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from agentx_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    if not _codex_token:
        try:
            from agentx_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    if status.get("logged_in"):
        ...
        if choice == "reauth":
            ...
            print()
            try:
                # Forward CLI flags from ``agentx model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``agentx model``.
                mock_args = argparse.Namespace(
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from agentx_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from agentx_cli.auth import (
...
def _model_flow_custom(config):
    ...
    """
    from agentx_cli.main import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
    from agentx_cli.auth import _save_model_choice, deactivate_provider
    from agentx_cli.config import get_env_value, load_config, save_config
    ...
    from agentx_cli.models import probe_api_models
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from agentx_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from agentx_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from agentx_cli import azure_detect
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from agentx_cli.main import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from agentx_cli.auth import _save_model_choice, deactivate_provider
    from agentx_cli.config import load_config, save_config
    from agentx_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    from agentx_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from agentx_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from agentx_cli.config import save_env_value, load_config, save_config
    from agentx_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from agentx_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from agentx_cli.copilot_auth import validate_copilot_token
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from agentx_cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from agentx_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from agentx_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _select_zai_endpoint(current_base: str) -> str:
    ...
    sourced from ``ZAI_ENDPOINTS`` in ``agentx_cli.auth`` so it stays in
    sync with the probe list.

    Returns the selected base URL.  Falls back to *current_base* on cancel
    or error.
    """
    from agentx_cli.main import _prompt_provider_choice
    from agentx_cli.auth import ZAI_ENDPOINTS
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from agentx_cli.main import _run_anthropic_oauth_flow
    from agentx_cli.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
