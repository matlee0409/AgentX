...
def test_env_scrub_agentx_allowlist_and_secret_blocks():
    ...
    for kept in ("AGENTX_HOME", "AGENTX_PROFILE", "AGENTX_CONFIG", "AGENTX_ENV", "PATH"):
...
def test_env_scrub_logs_dropped_agentx_vars(caplog):
    """Dropping a non-allowlisted, non-secret AGENTX_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset AGENTX_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "AGENTX_HOME": "/h",          # allowlisted → kept, not logged
        "AGENTX_BASE_URL": "https://x",   # dropped → logged
        "AGENTX_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "AGENTX_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "AGENTX_HOME" in out and "PATH" in out
    assert "AGENTX_BASE_URL" not in out and "AGENTX_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "AGENTX_BASE_URL" in msgs and "AGENTX_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "AGENTX_API_KEY" not in msgs
