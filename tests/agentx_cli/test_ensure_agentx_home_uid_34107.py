"""Regression tests for #34107 — Docker UID/GID handling in ensure_agentx_home.
...
The fix is a ``_chown_to_agentx_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_agentx_uid_gid
# ---------------------------------------------------------------------------


class TestResolveAgentXUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        ...
        from agentx_cli.config import _resolve_agentx_uid_gid
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("AGENTX_UID", "not-a-number")
        monkeypatch.setenv("AGENTX_GID", "911")
        from agentx_cli.config import _resolve_agentx_uid_gid
        uid, gid = _resolve_agentx_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("AGENTX_UID", "")
        monkeypatch.setenv("AGENTX_GID", "")
        from agentx_cli.config import _resolve_agentx_uid_gid
        uid, gid = _resolve_agentx_uid_gid()
        assert uid is None
        assert gid is None
...
# ---------------------------------------------------------------------------
# _chown_to_agentx_uid
# ---------------------------------------------------------------------------


class TestChownToAgentXUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("AGENTX_UID", "1000")
        monkeypatch.setenv("AGENTX_GID", "911")
        from agentx_cli import config as cfg

        d = tmp_path / "subdir"
        d.mkdir()

        with patch.object(cfg.os, "chown") as mock_chown:
            cfg._chown_to_agentx_uid(d)
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_agentx_uid(d)  # must not raise
