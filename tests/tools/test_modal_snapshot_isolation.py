...
def _restore_tool_modules():
    original_agentx_home = os.environ.get("AGENTX_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_agentx_home is None:
            os.environ.pop("AGENTX_HOME", None)
        else:
            os.environ["AGENTX_HOME"] = original_agentx_home
        _reset_modules(("tools", "agentx_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    agentx_cli = types.ModuleType("agentx_cli")
    agentx_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["agentx_cli"] = agentx_cli
    agentx_home = tmp_path / "agentx-home"
    os.environ["AGENTX_HOME"] = str(agentx_home)
    sys.modules["agentx_cli.config"] = types.SimpleNamespace(
        get_agentx_home=lambda: agentx_home,
    )
    ...
    return {
        "snapshot_store": agentx_home / "modal_snapshots.json",
