...
//!
//! Mirrors `agentx_constants.get_agentx_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\agentx
//!   macOS:   ~/.agentx
//!   Linux:   ~/.agentx  (override via $AGENTX_HOME)
//!
//! NOTE (macOS): Python's get_agentx_home(), scripts/install.sh, and the
//! Electron desktop's resolveAgentXHome() ALL use ~/.agentx on macOS — there
...
pub fn agentx_home() -> PathBuf {
    ...
    // macOS + Linux + fallback: ~/.agentx (matches Python get_agentx_home(),
    // install.sh, and the Electron desktop's resolveAgentXHome()).
...
pub fn bootstrap_cache_dir() -> PathBuf {
    agentx_home().join("bootstrap-cache")
}
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_agentx`,
...
pub fn update_in_progress_marker() -> PathBuf {
    agentx_home().join(".agentx-update-in-progress")
}
...
pub fn copy_self_to_agentx_home() -> std::io::Result<()> {
...
#[tauri::command]
pub fn get_agentx_home() -> String {
    agentx_home().to_string_lossy().into_owned()
}
