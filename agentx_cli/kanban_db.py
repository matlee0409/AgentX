...
from agentx_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "agentx_kanban_current_board_override",
    default=None,
)
...
def kanban_home() -> Path:
    ...
    2. ``get_default_agentx_root()``, which already returns ``<root>``
    ...
    return get_default_agentx_root()
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (agentx_cli/projects_db). When set,
    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    if resolved in _INITIALIZED_PATHS:
        ...
        try:
            ...
            with _INIT_LOCK:
                from agentx_state import apply_wal_with_fallback
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See agentx_state._WAL_INCOMPAT_MARKERS for detection logic.
                from agentx_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from agentx_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    if project_id:
        try:
            from agentx_cli import projects_db as _pdb
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from agentx_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from agentx_cli.profiles import profile_exists as _pe
...
def _agentx_path_argv(path: str) -> list[str]:
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_agentx_argv()
    return [_absolute_agentx_path(path)]


def _resolve_agentx_argv() -> list[str]:
    ...
    3. ``sys.executable -m agentx_cli.main`` — fallback for setups where
       AgentX is launched from a venv and the ``agentx`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_agentx_bin`` for the same reason. Kept
    local (not imported from gateway) because ``agentx_cli`` sits below
    ...
    if env_bin:
        if _looks_like_path(env_bin):
            return _agentx_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _agentx_path_argv(resolved_env_bin)
        return _module_agentx_argv()

    agentx_bin = _safe_which_no_cwd("agentx") if _IS_WINDOWS else shutil.which("agentx")
    if agentx_bin:
        return _agentx_path_argv(agentx_bin)
    return _module_agentx_argv()
...
def _resolve_worker_cli_toolsets(agentx_home: Optional[str]) -> Optional[list[str]]:
    ...
    try:
        ...
        token = set_agentx_home_override(agentx_home)
        try:
            cfg = load_config()
...
def _default_spawn(
    ...
    *,
    ...
    # _apply_profile_override() runs *before* agentx_constants is imported.
    # If AGENTX_HOME is absent from the child's env, get_agentx_home() falls
    ...
    # dispatcher's. Belt-and-braces with the `get_default_agentx_root()`
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install AgentX Agent or activate its venv before running the kanban dispatcher."
        )
...
def list_profiles_on_disk() -> list[str]:
    ...
    ``agentx_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from agentx_constants import get_default_agentx_root
