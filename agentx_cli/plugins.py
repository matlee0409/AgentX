...
4. **Pip plugins**     – packages that expose the ``agentx_agent.plugins``
   entry-point group.
...
from agentx_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
...
VALID_HOOKS: Set[str] = {
    ...
    # Kanban task lifecycle hooks. Fired by agentx_cli.kanban_db when a task
...
ENTRY_POINTS_GROUP = "agentx_agent.plugins"

_NS_PARENT = "agentx_plugins"
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        ...
        :func:`agentx_cli.profiles.get_active_profile_name`, so it works in
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`agentx_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from agentx_cli.dashboard_auth import (
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        # Lazy import to avoid circular: agentx_cli.main imports plugins indirectly
        from agentx_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if self._discovered and not force:
            return
        # Safe mode (--safe-mode / AGENTX_SAFE_MODE=1): troubleshooting run
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to agentx_cli/. Two layouts are
        ...
        user_dir = get_agentx_home() / "plugins"
        ...
        for manifest in winners.values():
            ...
            # for the same reason: every platform AgentX ships must be
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``agentx_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``agentx_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
