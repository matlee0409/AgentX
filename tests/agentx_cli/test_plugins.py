"""Tests for the AgentX plugin system (agentx_cli.plugins)."""
...
from agentx_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<agentx_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<agentx_home>/plugins/``; we derive
    ``<agentx_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        ...
        import os
        agentx_home_str = os.environ.get("AGENTX_HOME")
        if agentx_home_str:
            agentx_home = Path(agentx_home_str)
        else:
            agentx_home = base.parent
        agentx_home.mkdir(parents=True, exist_ok=True)
        cfg_path = agentx_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.agentx/plugins/ are discovered."""
        plugins_dir = tmp_path / "agentx_test" / "plugins"
    ...
    def test_execution_middleware_does_not_retry_downstream_failure(self, monkeypatch):
        ...
        monkeypatch.setattr("agentx_cli.plugins.get_plugin_manager", lambda: manager)
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AGENTX_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".agentx" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        agentx_home = tmp_path / "agentx_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under agentx_plugins.<name>."""
        plugins_dir = tmp_path / "agentx_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("AGENTX_HOME", str(tmp_path / "agentx_test"))

        # Clean up any prior namespace module
        sys.modules.pop("agentx_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "agentx_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="agentx_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "agentx_cli.plugins.invoke_hook",
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import agentx_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("agentx_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        agentx_home = tmp_path / "agentx_test"
        plugins_dir = agentx_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "agentx_test" / "plugins"
        # Set AGENTX_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("agentx_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("agentx_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("agentx_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("agentx_cli.plugins.PluginContext.dispatch_tool.__module__", "agentx_cli.plugins"):
...
class TestPluginDebugLogging:
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        from agentx_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With AGENTX_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("AGENTX_PLUGINS_DEBUG", "1")
        from agentx_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    ...
    def test_works_without_cli_ref(self, tmp_path, monkeypatch):
        ...
        prof = tmp_path / ".agentx" / "profiles" / "worker1"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AGENTX_HOME", str(prof))
        ctx = self._ctx()
