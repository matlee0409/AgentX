...
sidebar_position: 4
title: "贡献指南"
description: "如何为 AgentX Agent 做贡献 — 开发环境配置、代码风格、PR 流程"
---

# 贡献指南

感谢您为 AgentX Agent 做贡献！本指南涵盖开发环境配置、代码库结构说明以及 PR 合并流程。
...
- 构建自定义/本地工具而不修改 AgentX 核心？从 [构建 AgentX 插件](../guides/build-a-agentx-plugin.md) 开始
...
只有在你明确不想使用 AgentX managed install layout 时才使用这种方式（例如容器或 CI job 里的临时 clone）。如果这样安装，请确保运行的是这个 venv 里的 `agentx` entrypoint；运行系统 `python3 -m agentx_cli.main` 可能会加载无关的系统 Python 包。
...
- **Profile 安全路径**：不得硬编码 `~/.agentx` — 代码路径使用 `agentx_constants` 中的 `get_agentx_home()`，面向用户的消息使用 `display_agentx_home()`。完整规则参见 [AGENTS.md](https://github.com/NousResearch/agentx-agent/blob/main/AGENTS.md#profiles-multi-instance-support)。
...
- 使用 [GitHub Issues](https://github.com/NousResearch/agentx-agent/issues)
...
提交贡献即表示您同意您的贡献将以 [MIT 许可证](https://github.com/NousResearch/agentx-agent/blob/main/LICENSE) 授权。