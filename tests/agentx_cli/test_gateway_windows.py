"""Tests for agentx_cli.gateway_windows."""

from pathlib import Path

import pytest

import agentx_cli.gateway as gateway
import agentx_cli.gateway_windows as gateway_windows
import agentx_cli.setup as setup
...
def test_exec_schtasks_decodes_with_replace_errors(monkeypatch):
    ...
    code, out, err = gateway_windows._exec_schtasks(["/Query", "/TN", "AgentX_Gateway"])
...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda agentx_home: "")
    monkeypatch.setattr("agentx_cli.config.get_agentx_home", lambda: str(agentx_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "agentx_cli.main"]
    assert cwd == str(agentx_home.resolve())
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_agentx_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".agentx"
        home.mkdir()
        monkeypatch.setattr("agentx_cli.config.get_agentx_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("agentx_cli.config.get_agentx_home", lambda: missing)
        assert gateway_windows._stable_gateway_working_dir(project) == str(project)


def test_write_task_script_anchors_cmd_cd_at_agentx_home(monkeypatch, tmp_path):
    project = tmp_path / "project"
    agentx_home = tmp_path / "agentx-home"
    agentx_home.mkdir()
    ...
    assert f"cd /d {gateway_windows._quote_cmd_script_arg(str(agentx_home.resolve()))}" in content
...
def test_gateway_cmd_script_uses_pythonw_without_replace_or_start_churn(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda exe: (exe.replace("python.exe", "pythonw.exe"), r"C:\\AgentX\\agentx-agent\\venv", []),
    )

    content = gateway_windows._build_gateway_cmd_script(
        r"C:\\AgentX\\agentx-agent\\venv\\Scripts\\python.exe",
        r"C:\\AgentX\\agentx-agent",
        r"C:\\AgentXHome\\profiles\\alice",
        "--profile alice",
    )
...
def test_gateway_vbs_script_is_console_less(monkeypatch):
    ...
    assert "pythonw.exe" in content
    assert "agentx_cli.main" in content
    assert "gateway run" in content
...
def test_install_scheduled_task_success_start_now_uses_direct_spawn_not_task_run(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        gateway_windows,
        "_install_scheduled_task",
        lambda task_name, script_path: (True, "Created Scheduled Task 'AgentX_Gateway_alice'"),
    )
...
def test_install_access_denied_launches_elevated_install_before_startup_fallback(monkeypatch, tmp_path, capsys):
    ...
    assert "Launched elevated AgentX gateway install prompt" in out
...
def test_install_startup_fallback_does_not_auto_spawn_when_gateway_stopped(monkeypatch, tmp_path, capsys):
    ...
    assert "agentx --profile alice gateway start" in out
...
def test_uninstall_access_denied_prompts_before_elevating(monkeypatch, tmp_path, capsys):
    ...
    assert "UAC is Windows' admin approval prompt" in out
    assert "Launched elevated AgentX gateway uninstall prompt" in out
...
def test_stop_writes_planned_stop_marker_before_killing(monkeypatch):
    ...
    monkeypatch.setattr("agentx_cli.gateway.kill_gateway_processes", fake_kill)
    monkeypatch.setattr("agentx_cli.gateway._get_restart_drain_timeout", lambda: 5.0)
...
def test_stop_escalates_to_force_kill_when_drain_times_out(monkeypatch):
    ...
    `agentx gateway start` fails with "another instance is running".
    """
    pid = 77777
    events = []
