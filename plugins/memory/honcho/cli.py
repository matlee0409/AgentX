...
def _local_config_path() -> Path:
    ...
    """
    return get_agentx_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        agentx_host.get("pinUserPeer"),
        agentx_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in agentx_host:
        aliases_src = agentx_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in agentx_host:
        prefix_src = agentx_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(agentx_host: dict) -> None:
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom agentx_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    agentx_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        agentx_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(agentx_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = agentx_host.get("userPeerAliases")
    prefix = agentx_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(agentx_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(agentx_host)
    agentx_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        agentx_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        agentx_host, current_prefix, prefix_from_root,
...
def cmd_setup(args) -> None:
    ...
    if is_local:
        ...
        current_host_key = agentx_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            agentx_host["apiKey"] = new_local_key
    ...
    if not is_local:
        ...
        existing_oauth = OAuthCredential.from_host_block(agentx_host)
        ...
        if use_oauth:
            ...
            try:
                ...
                return
            agentx_host["apiKey"] = cred.access_token
            agentx_host["oauth"] = cred.oauth_block()
            # Default the peer prompt to the name entered at consent.
            if cred.consent_peer_name:
                agentx_host["peerName"] = cred.consent_peer_name
    ...
    # --- 3. Identity ---
    current_peer = agentx_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        agentx_host["peerName"] = new_peer

    current_ai = agentx_host.get("aiPeer") or cfg.get("aiPeer", "agentx")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        agentx_host["aiPeer"] = new_ai

    current_workspace = agentx_host.get("workspace") or cfg.get("workspace", "agentx")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        agentx_host["workspace"] = new_workspace
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, agentx_host)
    ...
    if run_mapping:
        peer_target = agentx_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(agentx_host)
            agentx_host["pinUserPeer"] = False
            if prior_aliases:
                agentx_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                agentx_host, current_prefix, prefix_from_root,
            ...
            if merged:
                agentx_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                agentx_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                agentx_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = agentx_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        agentx_host["observationMode"] = new_obs
    else:
        agentx_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(agentx_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        agentx_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        agentx_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = agentx_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        agentx_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = agentx_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        agentx_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                agentx_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(agentx_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            agentx_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        agentx_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        agentx_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        agentx_host["dialecticReasoningLevel"] = new_reasoning
    else:
        agentx_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = agentx_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        agentx_host["sessionStrategy"] = new_strat

    agentx_host["enabled"] = True
    agentx_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        agentx_config = load_config()
        agentx_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(agentx_config)
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        from agentx_cli.memory_setup import cmd_setup_provider
