...
class TestIRCProtocolHelpers:
    ...
    def test_parse_numeric_reply(self):
        msg = _parse_irc_message(":server 001 agentx-bot :Welcome to IRC")
        assert msg["prefix"] == "server"
        assert msg["command"] == "001"
        assert msg["params"] == ["agentx-bot", "Welcome to IRC"]

    def test_parse_nick_collision(self):
        msg = _parse_irc_message(":server 433 * agentx-bot :Nickname is already in use")
...
class TestIRCAdapterMessageParsing:
    ...
    async def test_handle_welcome(self, adapter):
        ...
        await adapter._handle_line(":server 001 agentx :Welcome to IRC")
    ...
    async def test_handle_nick_collision(self, adapter):
        ...
        await adapter._handle_line(":server 433 * agentx :Nickname in use")
        assert adapter._current_nick == "agentx_"
        sent = writer.write.call_args[0][0]
        assert b"NICK agentx_" in sent
    ...
    async def test_handle_dm(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG agentx :private message")
        assert len(dispatched) == 1
    ...
    async def test_ignores_own_messages(self, adapter):
        ...
        await adapter._handle_line(":agentx!bot@host PRIVMSG #test :my own msg")
        assert len(dispatched) == 0
    ...
    async def test_allowed_users_case_insensitive(self, monkeypatch):
        ...
        await adapter._handle_line(":admin!u@host PRIVMSG #test :agentx: hello")
        assert len(dispatched) == 1
    ...
    async def test_nick_collision_retry(self, adapter):
        ...
        await adapter._handle_line(":server 433 * agentx :Nickname in use")
        assert adapter._current_nick == "agentx_"
        await adapter._handle_line(":server 433 * agentx_ :Nickname in use")
        assert adapter._current_nick == "agentx_1"
        await adapter._handle_line(":server 433 * agentx_1 :Nickname in use")
        assert adapter._current_nick == "agentx_2"
...
class TestIRCStandaloneSend:
    ...
    async def test_standalone_send_completes_handshake_and_sends_privmsg(self, monkeypatch):
        ...
        assert any(line.startswith("USER agentxbot-cron 0 * :AgentX Agent (cron)")
    ...
    async def test_standalone_send_strips_crlf_from_message_body(self, monkeypatch):
        ...
        assert all(line.startswith("NICK agentxbot-cron") for line in nick_lines)
    ...
    async def test_standalone_send_joins_channel_before_privmsg(self, monkeypatch):
        ...
        conn = _FakeIRCConnection([
            b":server 001 agentxbot-cron :Welcome",
            b":server 366 agentxbot-cron #cron :End of /NAMES list.",
        ])
