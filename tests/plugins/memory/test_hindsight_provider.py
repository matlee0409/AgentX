...
from agentx_cli.memory_setup import _CANCELLED
...
def _provider_for_mode(tmp_path, monkeypatch, mode: str):
    ...
    monkeypatch.setattr(
        "plugins.memory.hindsight.get_agentx_home", lambda: tmp_path
    )

    provider = HindsightMemoryProvider()
    provider.initialize(session_id="test-session", agentx_home=str(tmp_path), platform="cli")
    return provider
...
def provider(tmp_path, monkeypatch):
    ...
    p.initialize(session_id="test-session", agentx_home=str(tmp_path), platform="cli")
...
def test_normalize_retain_tags_accepts_json_array_string():
    value = json.dumps(["agent:fakeassistantname", "source_system:agentx-agent"])
    assert _normalize_retain_tags(value) == ["agent:fakeassistantname", "source_system:agentx-agent"]
...
class TestConfig:
    ...
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between AgentX Agent and the User"
    ...
    def test_config_from_env_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "plugins.memory.hindsight.get_agentx_home",
        ...
        assert cfg["banks"]["agentx"]["bankId"] == "env-bank"
        assert cfg["banks"]["agentx"]["budget"] == "high"
...
class TestPostSetup:
    def test_setup_cancel_at_mode_picker_writes_nothing(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("plugins.memory.hindsight.get_agentx_home", lambda: agentx_home)

        save_config = MagicMock()
        which = MagicMock(return_value="/usr/bin/uv")
        run = MagicMock()
        monkeypatch.setattr("agentx_cli.memory_setup._curses_select", lambda *args, **kwargs: _CANCELLED)
        ...
        monkeypatch.setattr("agentx_cli.config.save_config", save_config)

        provider = HindsightMemoryProvider()
        provider.post_setup(str(agentx_home), {"memory": {"provider": "builtin"}})

        save_config.assert_not_called()
        which.assert_not_called()
        run.assert_not_called()
        assert not (agentx_home / ".env").exists()
        assert not (agentx_home / "hindsight" / "config.json").exists()
    ...
    def test_local_embedded_setup_cancel_at_llm_picker_writes_nothing(self, tmp_path, monkeypatch):
        ...
        which = MagicMock(return_value="/usr/bin/uv")
        run = MagicMock()
        monkeypatch.setattr("agentx_cli.memory_setup._curses_select", lambda *args, **kwargs: next(selections))
        monkeypatch.setattr("shutil.which", which)
        monkeypatch.setattr("subprocess.run", run)
        monkeypatch.setattr("builtins.input", MagicMock(side_effect=AssertionError("prompt should not run")))
        monkeypatch.setattr("getpass.getpass", MagicMock(side_effect=AssertionError("prompt should not run")))
    ...
    def test_local_embedded_setup_materializes_profile_env(self, tmp_path, monkeypatch):
        ...
        saved_configs = []
        monkeypatch.setattr("agentx_cli.config.save_config", lambda cfg: saved_configs.append(cfg.copy()))

        provider = HindsightMemoryProvider()
        provider.post_setup(str(agentx_home), {"memory": {}})

        assert saved_configs[-1]["memory"]["provider"] == "hindsight"
        env_text = (agentx_home / ".env").read_text()
    ...
    def test_local_embedded_setup_respects_existing_profile_name(self, tmp_path, monkeypatch):
        ...
        provider = HindsightMemoryProvider()
        provider.save_config({"profile": "coder"}, str(agentx_home))
        provider.post_setup(str(agentx_home), {"memory": {}})

        coder_env = user_home / ".hindsight" / "profiles" / "coder.env"
        agentx_env = user_home / ".hindsight" / "profiles" / "agentx.env"
        assert coder_env.exists()
        assert not agentx_env.exists()
    ...
    def test_local_embedded_setup_blank_inputs_preserve_existing_config(self, tmp_path, monkeypatch):
        ...
        provider = HindsightMemoryProvider()
        provider.save_config(existing_config, str(agentx_home))
        ...
        monkeypatch.setattr("agentx_cli.memory_setup._curses_select", lambda *args, **kwargs: kwargs.get("default", 0))
        monkeypatch.setattr("shutil.which", lambda name: None)
        monkeypatch.setattr("builtins.input", lambda prompt="": "")
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "")
        ...
        saved = json.loads((agentx_home / "hindsight" / "config.json").read_text())
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config):
        ...
        item = call_kwargs["items"][0]
        assert item["context"] == "conversation between AgentX Agent and the User"
    ...
    def test_sync_turn_uses_aretain_batch(self, provider):
        ...
        assert call_kwargs["retain_async"] is True
        assert len(call_kwargs["items"]) == 1
        assert call_kwargs["items"][0]["context"] == "conversation between AgentX Agent and the User"
    ...
    def test_resume_creates_new_document(self, tmp_path, monkeypatch):
        ...
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        monkeypatch.setattr("plugins.memory.hindsight.get_agentx_home", lambda: tmp_path)

        p1 = HindsightMemoryProvider()
        p1.initialize(session_id="resumed-session", agentx_home=str(tmp_path), platform="cli")
        ...
        p2.initialize(session_id="resumed-session", agentx_home=str(tmp_path), platform="cli")
...
class TestBankIdTemplate:
    ...
    def test_resolve_with_profile(self):
        result = _resolve_bank_id_template(
            "agentx-{profile}", fallback="agentx",
            profile="coder", workspace="", platform="", user="", session="",
        )
        assert result == "agentx-coder"
    ...
    def test_resolve_collapses_empty_placeholders(self):
        # When user is empty, "agentx-{user}" becomes "agentx-" -> trimmed to "agentx"
        result = _resolve_bank_id_template(
            "agentx-{user}", fallback="default",
    ...
    def test_resolve_invalid_template_returns_fallback(self):
        ...
        result = _resolve_bank_id_template(
            "agentx-{unknown}", fallback="agentx",
            profile="", workspace="", platform="", user="", session="",
        )
        assert result == "agentx"
    ...
    def test_provider_template_with_missing_profile_falls_back(self, tmp_path, monkeypatch):
        ...
        p = HindsightMemoryProvider()
        # No agent_identity passed — template renders to "agentx-" which collapses to "agentx"
        p.initialize(session_id="s1", agentx_home=str(tmp_path), platform="cli")
        assert p._bank_id == "agentx"
