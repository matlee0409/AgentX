...
The web dashboard is a browser-based UI for managing your AgentX Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.
...
ExecStart=/path/to/venv/bin/python -m agentx_cli.main dashboard \
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and AgentX versions, CPU core count + utilization, memory, disk usage of the AgentX home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The AgentX version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git or pip install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `agentx update` in the background. On Docker/Nix/Homebrew installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
...
| `GET /api/agentx/update/check` | Report update availability (commits behind, install method) without applying. For git/pip installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — AgentX Agent engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Three providers ship in the box:
...
Bundled providers reported these issues:
  • nous: AGENTX_DASHBOARD_OAUTH_CLIENT_ID is not set (and
    ...
    deploys a AgentX Agent instance — set it to your provisioned
...
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `agentx_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `agentx_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `agentx_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
| `agentx_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
from agentx_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
AgentX Desktop can drive a AgentX backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateway → Remote gateway**, which asks for a **Remote URL** and a way to **Sign in**. (For the desktop app itself — install, settings, chat — see the [AgentX Desktop](/user-guide/desktop) page.)
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `agentx_cli/web_dist/` which the FastAPI server serves as a static SPA.
