#!/usr/bin/env python3
"""
SQLite State Store for AgentX Agent.
...
DEFAULT_DB_PATH = get_agentx_home() / "state.db"
...
# agentx_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``agentx_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see agentx_cli/kanban_db.py) would
...
def _db_opens_cleanly(db_path: Path) -> Optional[str]:
    ...
    try:
        ...
        probe_session_id = f"_agentx_fts_health_probe_{time.time_ns()}"
        try:
...
class SessionDB:
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``agentx_state._set_last_init_error(None)`` explicitly.
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._agentx_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._agentx_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
