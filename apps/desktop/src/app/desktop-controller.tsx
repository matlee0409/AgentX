...
import { getCronJobs, getSessionMessages, listAllProfileSessions, type SessionInfo, triggerCronJob } from '../agentx'
...
import { useAgentXConfig } from './session/hooks/use-agentx-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.agentxDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  ...
  // agentx:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.agentxDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshAgentXConfig, sttEnabled, voiceMaxRecordingSeconds } = useAgentXConfig({
    activeSessionIdRef,
  ...
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshAgentXConfig])
