...
description: "Set up AgentX Agent as a WhatsApp bot via Meta's official Business Cloud API"
---
...
Message-template support (the workaround for outside-window sends) is not yet implemented in AgentX. If you need it, please [open an issue](https://github.com/NousResearch/agentx-agent/issues) — it's planned but waiting on a clear demand signal.
...
If the model emits tool-call-shaped text instead of a structured call, it usually means the toolset was effectively empty.  See `agentx_cli/platforms.py` for the platform → default toolset mapping.
