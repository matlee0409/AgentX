...
import { getAgentXConfigRecord, type AgentXConfigRecord, saveAgentXConfig } from '@/agentx'
...
export interface I18nConfigClient {
  getConfig: () => Promise<AgentXConfigRecord>
  saveConfig: (config: AgentXConfigRecord) => Promise<{ ok: boolean }>
}
...
const defaultConfigClient: I18nConfigClient = {
  ...
  saveConfig: config => {
    ...
    return saveAgentXConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: AgentXConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: AgentXConfigRecord, locale: Locale): AgentXConfigRecord {
