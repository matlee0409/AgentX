...
#[tauri::command]
pub async fn launch_agentx_desktop(
    app: AppHandle,
    install_root: String,
) -> Result<(), String> {
    let install_root = PathBuf::from(install_root);
    let exe_path = resolve_agentx_desktop_exe(&install_root).ok_or_else(|| {
        format!(
    ...
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/AgentX
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_agentx_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    let release_dir = install_root.join("apps").join("desktop").join("release");
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "AgentX.exe"),
            ("win-arm64-unpacked", "AgentX.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/AgentX.app/Contents/MacOS", "AgentX"),
            ("mac-arm64/AgentX.app/Contents/MacOS", "AgentX"),
        ]
    } else {
        &[("linux-unpacked", "agentx")]
    };
...
pub(crate) fn resolve_agentx_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    let exe = resolve_agentx_desktop_exe(install_root)?;
    #[cfg(target_os = "macos")]
    {
        // .../AgentX.app/Contents/MacOS/AgentX -> .../AgentX.app
...
pub(crate) fn agentx_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".agentx-bootstrap-complete").exists()
        && resolve_agentx_desktop_exe(install_root).is_some()
}
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_agentx_desktop_exe(install_root).ok_or_else(|| {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // installer exits, mirroring launch_agentx_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::agentx_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    // the repo INTO at $AgentXHome\agentx-agent. Mirrors agentx_constants.
    let agentx_home = args
        .agentx_home
        .clone()
        .unwrap_or_else(|| crate::paths::agentx_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&agentx_home).join("agentx-agent");
    ...
    if let Err(err) = crate::paths::copy_self_to_agentx_home() {
...
async fn run_install_script(
    ...
    args: &[String],
    agentx_home_override: Option<&str>,
    ...
    powershell::run_script(script_path, args, sink, agentx_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("AgentX"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../AgentX.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("AgentX.exe");
    ...
    // what the updater ditto's over /Applications/AgentX.app). A regression in
    // this derivation breaks the post-update auto-relaunch, so guard it.
    #[test]
    fn resolve_agentx_desktop_app_finds_built_bundle() {
        let root = unique_tmp_dir("app-ok");
        let expected = make_release_tree(&root);

        let resolved = resolve_agentx_desktop_app(&root)
    ...
    #[test]
    fn resolve_agentx_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_agentx_desktop_app(&root).is_none(),
