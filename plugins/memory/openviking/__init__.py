...
def _load_agentx_openviking_config() -> dict:
    try:
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_agentx_openviking_config()
    ...
    def post_setup(self, agentx_home: str, config: dict) -> None:
        ...
        from agentx_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        agentx_home_path = Path(agentx_home)
        env_path = agentx_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        settings = _resolve_connection_settings(_load_agentx_openviking_config())
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this AgentX run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
