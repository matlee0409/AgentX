...
from agentx_cli.dashboard_auth.audit import audit_log, AuditEvent
...
def test_audit_write_failure_does_not_raise(monkeypatch, tmp_path):
    ...
    # Point AGENTX_HOME at a file (not a dir) so mkdir/open will fail.
    broken = tmp_path / "not-a-dir"
    broken.write_text("blocking file")
    monkeypatch.setenv("AGENTX_HOME", str(broken))
    # Should NOT raise.
