...
def scenario(name):
    ...
    def wrap(fn):
        def run():
            home = tempfile.mkdtemp(prefix=f"agentx_atyp_{name}_")
            os.environ["AGENTX_HOME"] = home
            os.environ["HOME"] = home
            for m in list(sys.modules.keys()):
                if m.startswith(("agentx_cli", "plugins", "gateway")):
                    del sys.modules[m]
            sys.path.insert(0, str(WT))
            from agentx_cli import kanban_db as kb  # noqa: F401
            print(f"\n═══ {name} ═══")
            try:
                fn(home, kb)
                print(f"  ✔ {name}")
...
def _(home, kb):
    ...
    try:
        ...
        # allow escape.
        try:
            from agentx_cli.kanban_db import resolve_workspace
            ...
            if not resolved_abs.startswith(home_abs) and resolved_abs.startswith("/tmp"):
                ...
                print(f"  ⚠ workspace resolved OUTSIDE agentx_home: {resolved}")
...
@scenario("agentx_home_with_spaces")
def _(home, kb):
...
@scenario("agentx_home_via_symlink")
def _(home, kb):
    ...
    real = tempfile.mkdtemp(prefix="agentx_real_")
    link1 = real + "_link1"
    link2 = real + "_link2"
...
def _(home, kb):
    ...
    try:
        from agentx_cli import web_server as ws  # noqa
    except Exception:
        pass
