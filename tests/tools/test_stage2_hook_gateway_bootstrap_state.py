...
AGENTX_GATEWAY_BOOTSTRAP_STATE on first boot, so a freshly-provisioned
...
AGENTX_GATEWAY_BOOTSTRAP_STATE=running; stage2-hook.sh (installed as
/etc/cont-init.d/01-agentx-setup, which runs lexicographically BEFORE
...
This mirrors the existing AGENTX_AUTH_JSON_BOOTSTRAP env-seed pattern: it seeds
...
def _seed_block(text: str) -> str:
    ...
    start = text.index('if [ ! -f "$AGENTX_HOME/gateway_state.json" ] && \\')
...
def _auth_seed_block(text: str) -> str:
    start = text.index(
        'if [ ! -f "$AGENTX_HOME/auth.json" ] && '
        '[ -n "${AGENTX_AUTH_JSON_BOOTSTRAP:-}" ]; then'
    )
...
def test_seed_block_present_and_guarded(stage2_text: str) -> None:
    ...
    assert '[ ! -f "$AGENTX_HOME/gateway_state.json" ]' in block, (
        "seed must be guarded by [ ! -f ] so persisted state wins on restart"
    )
    assert "AGENTX_GATEWAY_BOOTSTRAP_STATE" in block
...
def _run_seed(
    ...
    """Run the extracted seed block in a sandbox $AGENTX_HOME.

    ``env_value`` is the AGENTX_GATEWAY_BOOTSTRAP_STATE value (None = unset).
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        env_line = (
            f'export AGENTX_GATEWAY_BOOTSTRAP_STATE="{env_value}"\n'
            if env_value is not None
            else "unset AGENTX_GATEWAY_BOOTSTRAP_STATE\n"
        )
        script = (
            "set -e\n"
            f'AGENTX_HOME="{home}"\n'
...
def test_does_not_seed_gateway_state_through_symlink(
    ...
    script = (
        ...
        "chown() { :; }\n"
        "chmod() { :; }\n"
        'export AGENTX_GATEWAY_BOOTSTRAP_STATE="running"\n'
        + block
    )
...
def test_does_not_seed_auth_json_through_symlink(
    ...
    script = (
        ...
        "chown() { :; }\n"
        "chmod() { :; }\n"
        'export AGENTX_AUTH_JSON_BOOTSTRAP="{\\"ok\\": true}"\n'
        + block
    )
    script_path = tmp_path / "harness.sh"
    script_path.write_text(script)
...
def test_no_seed_when_env_unset(stage2_text: str) -> None:
    ...
    assert out is None, "seed must not run when AGENTX_GATEWAY_BOOTSTRAP_STATE is unset"
