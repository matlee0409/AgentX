...
# load_agentx_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_agentx_dotenv().
...
# process.  ``load_agentx_dotenv()`` is called at module-import time from
# several hot modules (cli.py, agentx_cli/main.py, run_agent.py,
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_agentx_dotenv()`` call.  Returns
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``agentx_cli.config._sanitize_env_lines`` which
    already knows all valid AgentX env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from agentx_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_agentx_dotenv(
    *,
    agentx_home: str | os.PathLike | None = None,
    ...
    home_path = Path(agentx_home or os.getenv("AGENTX_HOME", Path.home() / ".agentx"))
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    locate the access token) but BEFORE the rest of AgentX reads
    ``os.environ`` for credentials.  Any failure here is logged and
    swallowed — external secret sources must never block startup.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_agentx_dotenv()`` runs at import
    time from several hot modules (cli.py, agentx_cli/main.py,
    run_agent.py, trajectory_compressor.py, ...), so without this guard
    the Bitwarden status line would print 3-5x per CLI startup.  Use
    ``reset_secret_source_cache()`` if you need to force a re-pull
    (tests, future ``agentx secrets bitwarden sync`` from a long-running
    process).
    """
    ...
    if result.applied:
        ...
        # edited .env (see #6843).
        _sanitize_loaded_credentials()
        # Remember where these came from so the setup / `agentx model`
