...
function markerPath(agentxHome) {
  return path.join(agentxHome, '.agentx-update-in-progress')
}
 ...
 * clock for tests.
 */
function readLiveUpdateMarker(agentxHome, { kill, now = Date.now, maxAgeMs = UPDATE_MARKER_MAX_AGE_MS } = {}) {
  const file = markerPath(agentxHome)
  let raw
  try {
