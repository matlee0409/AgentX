...
def resolve_active_host() -> str:
    ...
    Resolution order:
      ...
      2. Active profile name via profiles system -> ``agentx.<profile>``
      3. Fallback: ``"agentx"`` (default profile)
    """
...
def resolve_config_path() -> Path:
    ...
    """
    local_path = get_agentx_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_agentx_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into AgentX system prompt
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    def _build() -> "Honcho":
        ...
        # the "go run agentx honcho setup" hint they used to.
        try:
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                agentx_cfg = load_config()
                honcho_cfg = agentx_cfg.get("honcho", {})
