...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`agentx_cli.plugins.invoke_hook` and its aggregators.  Python
...
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``agentx_cli/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``agentx_cli.config.load_config``
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    ...
    :func:`agentx_cli.plugins.get_pre_tool_call_block_message`.  This is
...
def allowlist_path() -> Path:
    ...
    return get_agentx_home() / ALLOWLIST_FILENAME
...
def run_once(
    ...
    ``kwargs`` is the same dict that :func:`agentx_cli.plugins.invoke_hook`
