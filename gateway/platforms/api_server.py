...
- GET  /v1/models                  — lists agentx-agent as an available model
...
def _agentx_version() -> str:
    """Return the agentx-agent version string, or "dev" if it can't be resolved.

    Tries the installed package metadata first (authoritative for a pip/uv
    install), then the in-tree ``agentx_cli.__version__`` (covers editable /
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_agentx_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        ...
        # agentx_state._WAL_INCOMPAT_MARKERS).
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "agentx-agent", "version": _agentx_version()}
        )
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — return agentx-agent as an available model."""
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-AgentX-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-AgentX-Error"] = err_msg[:200]
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = {"X-AgentX-Session-Id": session_id}
        if gateway_session_key:
            response_headers["X-AgentX-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # the June 2026 agentx-0day hardening (an 8-char key dispatching
            ...
            # that is the exact surface the agentx-0day campaign abused to write
