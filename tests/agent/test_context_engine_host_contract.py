...
engine plugins (e.g. agentx-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``agentx plugins`` picker."""
    from agentx_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "agentx_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "agentx_cli.plugins.get_plugin_context_engine",
