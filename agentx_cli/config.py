"""
Configuration management for AgentX Agent.
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "AGENTX_TOOL_PROGRESS", "AGENTX_TOOL_PROGRESS_MODE",
...
from agentx_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_agentx_home() / ".managed"
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade agentx-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``agentx_cli/`` — i.e. the git checkout for source
    installs, ``/opt/agentx`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$AGENTX_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how AgentX was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.
    ...
    3. AGENTX_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/agentx-agent`` image bakes a ``docker``
    ...
    try:
        method = (
            (get_agentx_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``agentx_constants.is_container`` (import-safe)."""
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* AgentX lives in a ``uv tool`` layout.

    ``uv tool install agentx-agent`` places the install at
    ``.../uv/tools/agentx-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``agentx-agent`` happens to be uv-tool-installed on the machine while
    the *active* AgentX is a regular pip/venv install, causing
    ``agentx update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/agentx-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade agentx-agent"
    if method == "docker":
        return "docker pull nousresearch/agentx-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade agentx-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade agentx-agent"
        return "pip install --upgrade agentx-agent"
...
# agentx_cli/main.py; lives here so the wording stays consistent and we
...
AgentX Agent runs as a published image (nousresearch/agentx-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/agentx-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate agentx-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/agentx-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/agentx-agent/tags
...
def format_managed_message(action: str = "modify this AgentX installation") -> str:
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this AgentX installation is managed by Homebrew "
            f"(AGENTX_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade agentx-agent"
        )
...
def get_container_exec_info() -> Optional[dict]:
    ...
    Returns a dict with keys: backend, container_name, exec_user, agentx_bin
    ...
    container_mode_file = get_agentx_home() / ".container-mode"
    ...
    agentx_bin = info.get("agentx_bin", "/data/current-package/bin/agentx")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "agentx_bin": agentx_bin,
    }
...
# Re-export from agentx_constants — canonical definition lives there.
from agentx_constants import get_agentx_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_agentx_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_agentx_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_agentx_uid_gid() -> tuple[Optional[int], Optional[int]]:
    ...
    subdirectories created at runtime by ``ensure_agentx_home()`` (especially
...
def _chown_to_agentx_uid(path) -> None:
    ...
    directories created by :func:`ensure_agentx_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_agentx_uid_gid()
...
def ensure_agentx_home():
    ...
    if is_managed():
        ...
        try:
            _ensure_agentx_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_agentx_home_managed(home: Path):
...
DEFAULT_CONFIG = {
    ...
    # See tools/kanban_tools.py and agentx_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``agentx dashboard`` -> Notifications).
    "kanban": {
    ...
    "display": {
        ...
        "pet": {
            ...
            # get_agentx_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *AgentX Agent*" header.
    ...
    "security": {
        ...
        # ``agentx_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://agentx-agent.nousresearch.com/docs/api/model-catalog.json",
    ...
    "sessions": {
        ...
        # default of ``agentx sessions prune``.
        "retention_days": 90,
    ...
    "updates": {
        # Run a full ``agentx backup``-style zip of AGENTX_HOME before every
        # ``agentx update``.  Backups land in ``<AGENTX_HOME>/backups/`` and
        # can be restored with ``agentx import <path>``.  Off by default:
        # zipping a large AGENTX_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``agentx update
...
OPTIONAL_ENV_VARS = {
    ...
    # AGENTX_TOOL_PROGRESS and AGENTX_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.agentx.config`` entries, then checks
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older AgentX versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``agentx_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("AGENTX_HOME", "~/.agentx")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("AGENTX_TOOL_PROGRESS")
            old_mode = get_env_value("AGENTX_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from AGENTX_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from AGENTX_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("AGENTX_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from AGENTX_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    #      on top of ensure_agentx_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_agentx_home() / "logs" / "curator"
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from agentx_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    # metadata.agentx.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _agentx_home / "config.yaml"`` local variable
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real AgentX env var names, avoiding false positives from values
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.agentx/.env or environment."""
...
def show_config():
    ...
    from agentx_cli.auth import get_anthropic_key
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: agentx config set <key> <value>")
            print()
            print("Examples:")
