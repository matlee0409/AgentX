"""Agent-construction and session-resume display methods for ``AgentXCLI``.
...
Behavior-neutral: every method is lifted verbatim from ``AgentXCLI``. ``self.*``
...
class CLIAgentSetupMixin:
    """Agent construction + session-resume display methods for ``AgentXCLI``."""
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: agentx setup")
                return False
