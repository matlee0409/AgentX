...
@pytest.fixture
def agentx_home_with_config(tmp_path, monkeypatch):
...
def test_returns_configured_external_dir(agentx_home_with_config):
    _home, external, _cfg = agentx_home_with_config
    result = get_external_skills_dirs()
    assert result == [external.resolve()]


def test_cache_reuses_result_without_reparsing(agentx_home_with_config):
    """Subsequent calls hit the cache and skip YAML parsing entirely."""
    _home, _external, _cfg = agentx_home_with_config
...
def test_cache_invalidates_on_mtime_change(agentx_home_with_config):
    """A config.yaml edit invalidates the cache on the next call."""
    _home, external, config = agentx_home_with_config
...
def test_returned_list_is_a_copy(agentx_home_with_config):
