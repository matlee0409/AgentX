...
@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_agentx_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_agentx_home", lambda: default_home)
...
class TestProfileScopedMcp:
    ...
    def test_mcp_probe_runs_inside_profile_scope(
        ...
        import agentx_cli.mcp_config as mcp_config
        from agentx_constants import get_agentx_home
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        monkeypatch.setattr(
            web_server,
            "_spawn_agentx_action",
            lambda subcommand, name: calls.append(list(subcommand)) or _FakeProc(),
        )
        monkeypatch.setattr(
            "agentx_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid():
            seen_homes.append(str(get_agentx_home()))
            return None
        ...
        assert resp.json()["agentx_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_agentx_home(self, isolated_profiles, monkeypatch):
        import agentx_cli.web_server as web_server

        monkeypatch.setattr(
            "agentx_cli.main._make_tui_argv",
    ...
    def test_chat_argv_unscoped_keeps_legacy_env(self, isolated_profiles, monkeypatch):
        ...
        assert env is not None
        assert env.get("AGENTX_HOME") != str(isolated_profiles["worker_beta"])
