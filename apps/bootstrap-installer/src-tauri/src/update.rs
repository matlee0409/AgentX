...
//!      rebuild apps/desktop by design — see cmd_update in agentx_cli/main.py),
...
//! The only OS-specific bits here are the venv shim path (resolve_agentx) and
...
/// agentx_cli/main.py (sys.exit(2)). We surface a targeted message for this.
...
pub async fn start_update(app: AppHandle) -> Result<(), String> {
        ...
        .is_err()
    {
        ...
        let mut stages = vec![
            stage_info("update", "Updating AgentX"),
...
async fn run_update(app: AppHandle) -> Result<()> {
    let agentx_home = crate::paths::agentx_home();
    let install_root = agentx_home.join("agentx-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_agentx`
    ...
    let agentx = resolve_agentx(&install_root).ok_or_else(|| {
        let msg = format!(
    ...
    // ---- stage 2: agentx desktop --build-only ----------------------------
    ...
    if let Some(target_app) = launch_target {
        ...
        crate::bootstrap::launch_agentx_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend agentx.exe (or the desktop AgentX.exe
            ...
            // force-kill every AgentX.exe except ourselves, then give the OS a
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_agentx(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("AgentX.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("AgentX.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// `startAgentX()` in the desktop gates local-backend startup on our
...
fn venv_agentx(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
...
fn resolve_agentx(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_agentx(install_root);
    if shim.exists() {
        return Some(shim);
    }
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    let mut envs = vec![(
        ...
        agentx_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        agentx_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_agentx_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no AgentX.app was found under {}",
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::agentx_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_agentx_is_under_install_root() {
        let root = Path::new("/x/agentx-agent");
        let shim = venv_agentx(root);
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_agentx(root)),
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/AgentX.app"]),
            Some(PathBuf::from("/Applications/AgentX.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("AgentX.app");
        let tmp = base.join("AgentX.app.agentx-update-new");
        let old = base.join("AgentX.app.agentx-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("AgentX.app");
        let tmp = base.join("AgentX.app.agentx-update-new"); // intentionally absent
        let old = base.join("AgentX.app.agentx-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("AgentX.app");
        let tmp = base.join("AgentX.app.agentx-update-new"); // absent
        let old = base.join("AgentX.app.agentx-update-old");
        write_marker(&target, "OLD");
