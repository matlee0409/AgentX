...
# from shadowing AgentX's own top-level modules.  ``agentx_bootstrap`` lives at
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    CLI path via ``agentx_cli.mcp_startup``); ``timeout`` overrides it.
    """
    thread = _mcp_discovery_thread
    if thread is None or not thread.is_alive():
        return
    try:
        from agentx_cli.mcp_startup import _resolve_discovery_timeout
...
def main():
    ...
    # server stalled the whole shell on "summoning agentx…" for the full
