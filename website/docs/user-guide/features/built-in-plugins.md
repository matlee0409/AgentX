...
description: "Plugins shipped with AgentX Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---
...
See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a AgentX Plugin](/guides/build-a-agentx-plugin) to write your own.
...
4. **Pip entry points** — `agentx_agent.plugins`
...
| `agentx-achievements` | dashboard tab | Steam-style collectible badges generated from your real AgentX session history |
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.agentx/cache/google_meet/<meeting_id>/`
...
**Disabling:** `agentx plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.agentx/cache/google_meet/` until you remove them.
...
- Unlock state is persisted to `$AGENTX_HOME/plugins/agentx-achievements/state.json`
...
**API** — routes mount under `/api/plugins/agentx-achievements/`:
...
**State files** — live under `$AGENTX_HOME/plugins/agentx-achievements/`:
...
**Enabling:** Nothing to enable — `agentx-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `agentx dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/agentx-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.agentx/plugins/agentx-achievements/` that ships no dashboard. The plugin's state files under `$AGENTX_HOME/plugins/agentx-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other AgentX plugin — see [Build a AgentX Plugin](/guides/build-a-agentx-plugin). The only differences are:
