"""
Tests for timezone support (agentx_time module + integration points).
...
def _reset_agentx_time_cache():
    """Reset the agentx_time module cache (replacement for removed reset_cache)."""
    agentx_time._cached_tz = None
    agentx_time._cached_tz_name = None
    agentx_time._cache_resolved = False


# =========================================================================
# agentx_time.now() — core helper
# =========================================================================

class TestAgentXTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_agentx_time_cache()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["AGENTX_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="agentx_time"):
            result = agentx_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_none_for_invalid(self):
        os.environ["AGENTX_TIMEZONE"] = "Not/A/Timezone"
        tz = agentx_time.get_timezone()
        assert tz is None
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=agentx_tz) which shifted
    ...
    def test_ensure_aware_normalizes_aware_to_agentx_tz(self):
        """Already-aware datetimes should be normalized to AgentX tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in AgentX tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of AgentX tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), AgentX is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        ...
        # of the naive timestamp exceeds _agentx_now's wall time — this would
