...
Before the s6 migration, ``docker run nousresearch/agentx-agent gateway
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``agentx gateway run`` invocation
        (not ``sleep infinity``).
...
def test_gateway_run_no_supervise_env_var(
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "AGENTX_GATEWAY_NO_SUPERVISE=1",
    ...
    if inspect.stdout.strip() == "running":
        assert not _svstat_wants_up(container_name, "gateway-default"), (
            "AGENTX_GATEWAY_NO_SUPERVISE=1: gateway-default has "
...
def test_supervised_gateway_does_not_recurse(
    ...
    # Count python processes running `agentx gateway run`. If the
    ...
    r = _sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*agentx.*gateway run' | wc -l")
    assert r.returncode == 0
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${AGENTX_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "AgentX Gateway Starting…"
    ...
    assert "⚕" in combined or "AgentX Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "AgentX Gateway Starting" in file_contents, (
