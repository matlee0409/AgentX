...
class TestShouldExclude:
    def test_excludes_agentx_agent(self):
        from agentx_cli.backup import _should_exclude
    ...
    def test_includes_nested_agentx_agent_in_skills(self):
        """skills/autonomous-ai-agents/agentx-agent/ must NOT be excluded —
        only the root-level agentx-agent/ repo is skipped."""
        from agentx_cli.backup import _should_exclude
        assert not _should_exclude(Path("skills/autonomous-ai-agents/agentx-agent/SKILL.md"))
        assert not _should_exclude(Path("skills/autonomous-ai-agents/agentx-agent/sub/item.txt"))
    ...
    def test_excludes_regeneratable_dependency_and_cache_dirs(self, rel):
        """Python dep trees and tool caches under AGENTX_HOME must be skipped —
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        agentx_home.mkdir()
        _make_agentx_tree(agentx_home)

        monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
        # get_default_agentx_root needs this
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        import agentx_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = agentx_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, agentx_home)

        assert result is not None
        assert staged_dirs, "no SQLite snapshot was staged"
        assert all(d == str(out_zip.parent) for d in staged_dirs), staged_dirs

    def test_excludes_agentx_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include agentx-agent/ directory."""
        agentx_home = tmp_path / ".agentx"
        agentx_home.mkdir()
        _make_agentx_tree(agentx_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "agentx-agent" in n]
            assert agent_files == [], f"agentx-agent files leaked into backup: {agent_files}"

    def test_excludes_dependency_and_cache_trees(self, tmp_path, monkeypatch):
        """A plugin venv / site-packages / pip cache under AGENTX_HOME must be
        ...
        venv_pkg = agentx_home / "plugins" / "heavy" / ".venv" / "lib" / "site-packages" / "dep"
        venv_pkg.mkdir(parents=True)
        (venv_pkg / "__init__.py").write_text("# dep\n")
        pip_cache = agentx_home / ".cache" / "uv" / "wheels"
    ...
    def test_includes_nested_agentx_agent_in_skills(self, tmp_path, monkeypatch):
        """Backup includes skills/.../agentx-agent/ but NOT root agentx-agent/."""
        agentx_home = tmp_path / ".agentx"
        agentx_home.mkdir()
        _make_agentx_tree(agentx_home)

        # Add a nested agentx-agent directory inside skills (like the real layout)
        nested = agentx_home / "skills" / "autonomous-ai-agents" / "agentx-agent"
        nested.mkdir(parents=True)
        (nested / "SKILL.md").write_text("# AgentX Agent Skill\n")
        (nested / "sub").mkdir()
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            # Root agentx-agent must be excluded
            root_agent = [n for n in names if n.startswith("agentx-agent/")]
            assert root_agent == [], f"root agentx-agent leaked: {root_agent}"
            # Nested skill agentx-agent must be included
            assert "skills/autonomous-ai-agents/agentx-agent/SKILL.md" in names
            assert "skills/autonomous-ai-agents/agentx-agent/sub/item.txt" in names
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/agentx-backup-*.zip."""
        agentx_home = tmp_path / ".agentx"
        agentx_home.mkdir()
        (agentx_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("agentx-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        ...
        _symlink_file_or_skip(agentx_home / "skills" / "outside-link.txt", outside)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        ...
        from agentx_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only agentx_state.db (old wrong name) is rejected."""
        from agentx_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["agentx_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into agentx home."""
        agentx_home = tmp_path / ".agentx"
        agentx_home.mkdir()
        monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (agentx_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (agentx_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (agentx_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (agentx_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_agentx_prefix(self, tmp_path, monkeypatch):
        """Import strips .agentx/ prefix if all entries share it."""
        agentx_home = tmp_path / ".agentx"
        agentx_home.mkdir()
        monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".agentx/config.yaml": "model: test\n",
            ".agentx/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (agentx_home / "config.yaml").read_text() == "model: test\n"
        assert (agentx_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_agentx_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a agentx backup."""
        agentx_home = tmp_path / ".agentx"
        agentx_home.mkdir()
        monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (agentx_home / "config.yaml").exists()
        # traversal file should NOT exist outside agentx home
    ...
    def test_preserves_live_gateway_state(self, tmp_path, monkeypatch):
        ...
        (agentx_home / "gateway_state.json").write_text(live_state)
        ...
        assert (agentx_home / "gateway_state.json").read_text() == live_state
    ...
    def test_does_not_seed_gateway_state_when_absent(self, tmp_path, monkeypatch):
        ...
        assert (agentx_home / "config.yaml").exists()
        assert not (agentx_home / "gateway_state.json").exists()
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        agentx_home = tmp_path / ".agentx"
        (agentx_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (agentx_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (agentx_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            agentx_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (agentx_home / "gateway.pid").write_text("4242")
        (agentx_home / "processes.json").write_text('{"live": true}')
        ...
        assert (agentx_home / "gateway.pid").read_text() == "4242"
        assert (agentx_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (agentx_home / "cron.pid").exists()
        assert not (agentx_home / "gateway.lock").exists()
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (agentx_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (agentx_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (agentx_home / "config.yaml").read_text() == "model: restored\n"
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (agentx_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        from agentx_cli.backup import run_backup, run_import
...
class TestFormatSize:
    def test_bytes(self):
        from agentx_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without agentx markers fails validation."""
        import io
        from agentx_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_agentx(self):
        """Detects .agentx/ prefix wrapping all entries."""
        import io
        from agentx_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".agentx/config.yaml", "test")
            zf.writestr(".agentx/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".agentx/"
...
class TestBackupEdgeCases:
    def test_nonexistent_agentx_home(self, tmp_path, monkeypatch):
        """Backup exits when agentx home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".agentx"
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("agentx-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_agentx_home(self, tmp_path, monkeypatch):
        """Backup handles empty agentx home (no files to back up)."""
        agentx_home = tmp_path / ".agentx"
        agentx_home.mkdir()
        # Only excluded dirs, no actual files
        (agentx_home / "__pycache__").mkdir()
        (agentx_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_permission_error_during_backup(self, tmp_path, monkeypatch):
        ...
        bad_file = agentx_home / "secret.db"
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = agentx_home / "ancient.txt"
    ...
    def test_skips_output_zip_inside_agentx(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside agentx root."""
        agentx_home = tmp_path / ".agentx"
        agentx_home.mkdir()
        (agentx_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside agentx home
        out_zip = agentx_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        agentx_home = tmp_path / ".agentx"
        agentx_home.mkdir()
        (agentx_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        agentx_home = tmp_path / ".agentx"
        agentx_home.mkdir()
        (agentx_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (agentx_home / "config.yaml").exists()
        assert (agentx_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (agentx_home / "profiles" / "coder" / "config.yaml").exists()
        assert (agentx_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "agentx -p coder" in coder_wrapper
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from agentx_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    ...
    def test_creates_snapshot(self, agentx_home):
        from agentx_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(agentx_home=agentx_home)
        assert snap_id is not None
        snap_dir = agentx_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, agentx_home):
        from agentx_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", agentx_home=agentx_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, agentx_home):
        from agentx_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(agentx_home=agentx_home)
        db_copy = agentx_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, agentx_home):
        from agentx_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(agentx_home=agentx_home)
        assert (agentx_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_copies_channel_aliases(self, agentx_home):
        from agentx_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(agentx_home=agentx_home)
        copied = agentx_home / "state-snapshots" / snap_id / "channel_aliases.json"
        assert copied.exists()
        assert "120363408391911677@g.us" in copied.read_text()

    def test_missing_files_skipped(self, agentx_home):
        from agentx_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(agentx_home=agentx_home)
        with open(agentx_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(agentx_home=empty) is None

    def test_list_snapshots(self, agentx_home):
        from agentx_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", agentx_home=agentx_home)
        id2 = create_quick_snapshot(label="second", agentx_home=agentx_home)

        snaps = list_quick_snapshots(agentx_home=agentx_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, agentx_home):
        from agentx_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", agentx_home=agentx_home)
        snaps = list_quick_snapshots(limit=3, agentx_home=agentx_home)
        assert len(snaps) == 3

    def test_restore_config(self, agentx_home):
        from agentx_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(agentx_home=agentx_home)

        (agentx_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (agentx_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, agentx_home=agentx_home)
        assert result is True
        assert "openrouter" in (agentx_home / "config.yaml").read_text()

    def test_restore_state_db(self, agentx_home):
        from agentx_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(agentx_home=agentx_home)

        conn = sqlite3.connect(str(agentx_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, agentx_home=agentx_home)
    ...
    def test_restore_nonexistent(self, agentx_home):
        from agentx_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", agentx_home=agentx_home) is False

    def test_auto_prune(self, agentx_home):
        from agentx_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", agentx_home=agentx_home)
        snaps = list_quick_snapshots(limit=100, agentx_home=agentx_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, agentx_home):
        from agentx_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", agentx_home=agentx_home)
        deleted = prune_quick_snapshots(keep=3, agentx_home=agentx_home)
        assert deleted == 7
        assert len(list_quick_snapshots(agentx_home=agentx_home)) == 3

    def test_snapshot_includes_pairing_directories(self, agentx_home):
        ...
        (agentx_home / "platforms" / "pairing").mkdir(parents=True)
        (agentx_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (agentx_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (agentx_home / "pairing").mkdir()
        (agentx_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (agentx_home / "feishu_comment_pairing.json").write_text(
    ...
    def test_restore_recovers_pairing_data(self, agentx_home):
        ...
        assert restore_quick_snapshot(snap_id, agentx_home=agentx_home) is True
        assert approved.exists()
        assert '"alice"' in approved.read_text()
        assert feishu.exists()
        assert '"user_xyz"' in feishu.read_text()

    def test_empty_pairing_dir_does_not_fail(self, agentx_home):
...
# ---------------------------------------------------------------------------
    ...
    def test_restore_rejects_snapshot_id_traversal(self, agentx_home):
    ...
    def test_restore_rejects_manifest_rel_traversal(self, agentx_home):
        ...
        escape_dst = agentx_home.parent.parent / "outside.txt"
        ...
        assert not escape_dst.exists(), (
            f"manifest rel traversal escaped AGENTX_HOME: {escape_dst} exists"
        )
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_in_quick_state_files(self):
        from agentx_cli.backup import _QUICK_STATE_FILES
    ...
    def test_projects_db_snapshotted(self, agentx_home):
        from agentx_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(agentx_home=agentx_home)
        copy = agentx_home / "state-snapshots" / snap_id / "projects.db"
        assert copy.exists()
    ...
    def test_kanban_db_snapshotted(self, agentx_home):
        from agentx_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(agentx_home=agentx_home)
        copy = agentx_home / "state-snapshots" / snap_id / "kanban.db"
        assert copy.exists()
        conn = sqlite3.connect(str(copy))
    ...
    def test_restore_recreates_emptied_projects_db(self, agentx_home):
        from agentx_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(agentx_home=agentx_home)

        # Simulate the upgrade wiping the store back to an empty schema.
        conn = sqlite3.connect(str(agentx_home / "projects.db"))
    ...
    def test_non_default_kanban_board_snapshotted(self, agentx_home):
        ...
        board_dir = agentx_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(agentx_home=agentx_home)
        copy = (
            agentx_home / "state-snapshots" / snap_id
    ...
    def test_additional_per_profile_dbs_round_trip(self, agentx_home):
        ...
        for name, (table, row) in seeded.items():
            conn = sqlite3.connect(str(agentx_home / name))
            conn.execute(f"CREATE TABLE {table} (id TEXT PRIMARY KEY, data TEXT)")
            conn.execute(f"INSERT INTO {table} VALUES (?, ?)", row)
            conn.commit()
            conn.close()
    ...
    def test_board_workspaces_and_attachments_are_skipped(self, agentx_home):
        ...
        board = agentx_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(agentx_home=agentx_home)
        snap = agentx_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work"
    ...
    def test_board_db_copied_wal_safely(self, agentx_home, monkeypatch):
        ...
        import agentx_cli.backup as bk
        from agentx_cli.backup import create_quick_snapshot
        ...
        copy = agentx_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
...
class TestPreUpdateBackup:
    ...
    def agentx_home(self, tmp_path):
        ...
        root.mkdir()
        _make_agentx_tree(root)
        return root

    def test_creates_backup_under_backups_dir(self, agentx_home):
        from agentx_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(agentx_home=agentx_home)
        assert out is not None
        assert out.exists()
        assert out.parent == agentx_home / "backups"
        assert out.name.startswith("pre-update-")
        assert out.suffix == ".zip"

    def test_backup_contents_match_full_backup(self, agentx_home):
    ...
    def test_does_not_recurse_into_prior_backups(self, agentx_home):
        """The ``backups/`` directory must be excluded so that each backup
        doesn't grow exponentially by including all prior backups."""
        from agentx_cli.backup import create_pre_update_backup
        # First backup
        out1 = create_pre_update_backup(agentx_home=agentx_home)
        assert out1 is not None
        # Second backup — must not include the first
        out2 = create_pre_update_backup(agentx_home=agentx_home)
    ...
    def test_rotation_keeps_only_n(self, agentx_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(agentx_home=agentx_home, keep=3)
            created.append(out)
            _t.sleep(1.05)  # ensure distinct seconds in timestamp

        remaining = sorted(
            p.name for p in (agentx_home / "backups").iterdir()
    ...
    def test_rotation_preserves_manual_files(self, agentx_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from agentx_cli.backup import create_pre_update_backup

        (agentx_home / "backups").mkdir(exist_ok=True)
        manual = agentx_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(agentx_home=agentx_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from agentx_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(agentx_home=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, agentx_home):
        ...
        """
        from agentx_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(agentx_home=agentx_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, agentx_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, agentx_home):
        ...
        first = create_pre_update_backup(agentx_home=agentx_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(agentx_home=agentx_home, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(agentx_home=agentx_home, keep=0)
    ...
    def test_skips_symlinked_files(self, agentx_home, tmp_path):
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def agentx_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".agentx"
        root.mkdir()
        _make_agentx_tree(root)
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for agentx_cli.config + agentx_constants so they pick up AGENTX_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("agentx_cli.config") or mod == "agentx_constants":
                del __import__("sys").modules[mod]
        return root

    def test_backup_flag_creates_backup(self, agentx_home, capsys):
        """--backup forces the pre-update backup for one run even when config is off."""
        from agentx_cli.main import _run_pre_update_backup
        _run_pre_update_backup(Namespace(no_backup=False, backup=True))
        out = capsys.readouterr().out
        assert "Creating pre-update backup" in out
        assert "Saved:" in out
        assert "Restore:" in out
        assert "agentx import" in out
        assert "Disable:" in out
        # Actual backup was created
        backups = list((agentx_home / "backups").glob("pre-update-*.zip"))
        assert len(backups) == 1

    def test_default_disabled_is_silent(self, agentx_home, capsys):
        """With the default (``pre_update_backup: false``), ``agentx update``
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not list((agentx_home / "backups").glob("pre-update-*.zip")) \
            if (agentx_home / "backups").exists() else True

    def test_no_backup_flag_skips(self, agentx_home, capsys):
        from agentx_cli.main import _run_pre_update_backup
        ...
        # No backup written
        assert not (agentx_home / "backups").exists() or not list(
            (agentx_home / "backups").glob("pre-update-*.zip")
        )

    def test_config_enabled_creates_backup(self, agentx_home, capsys):
        ...
        import yaml
        (agentx_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": True},
        }))
        import sys as _sys
        for mod in list(_sys.modules.keys()):
            if mod.startswith("agentx_cli.config"):
    ...
    def test_config_disabled_is_silent(self, agentx_home, capsys):
    ...
    def test_cli_flag_overrides_enabled_config(self, agentx_home, capsys):
...
class TestPreMigrationBackup:
    ...
    def test_creates_backup_under_backups_dir(self, agentx_home):
        from agentx_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(agentx_home=agentx_home)
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `agentx
        # import` and the update-backup listing both pick them up.
        assert out.parent == agentx_home / "backups"
        assert out.name.startswith("pre-migration-")
        assert out.suffix == ".zip"

    def test_backup_uses_shared_exclusion_rules(self, agentx_home):
        """Pre-migration backup reuses the same exclusion rules as
        ``agentx backup`` / ``create_pre_update_backup`` — no drift."""
        from agentx_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(agentx_home=agentx_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_agentx_import(self, agentx_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid AgentX
        backup — `agentx import` should accept it."""
        from agentx_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(agentx_home=agentx_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_recurse_into_prior_backups(self, agentx_home):
        from agentx_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(agentx_home=agentx_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(agentx_home=agentx_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, agentx_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(agentx_home=agentx_home, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((agentx_home / "backups").glob("pre-migration-*.zip"))
        assert len(remaining) <= 3, f"expected <=3 backups retained, got {len(remaining)}"

    def test_missing_agentx_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.agentx yet — nothing to back up."""
        from agentx_cli.backup import create_pre_migration_backup
        missing = tmp_path / "does-not-exist"
        out = create_pre_migration_backup(agentx_home=missing)
        assert out is None

    def test_does_not_touch_pre_update_backups(self, agentx_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from agentx_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(agentx_home=agentx_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        import time as _t
        for _ in range(3):
            out = create_pre_migration_backup(agentx_home=agentx_home, keep=1)
            assert out is not None
            _t.sleep(1.05)
...
class TestRestoreCronJobsIfEmptied:
    ...
    def _make_snapshot(self, agentx_home: Path, label="pre-update"):
        from agentx_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, agentx_home=agentx_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from agentx_cli.backup import restore_cron_jobs_if_emptied
        agentx_home = tmp_path / ".agentx"
        jobs_path = agentx_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(agentx_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, agentx_home=agentx_home)
        assert result is not None
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = agentx_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, agentx_home=agentx_home) is None
        assert restore_cron_jobs_if_emptied("", agentx_home=agentx_home) is None
...
class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, agentx_home: Path) -> None:
        agentx_home.mkdir(parents=True, exist_ok=True)
        (agentx_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (agentx_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (agentx_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_captures_external_paths_under_external_prefix(self, tmp_path, monkeypatch):
        ...
        agentx_home = tmp_path / ".agentx"
        self._make_min_tree(agentx_home)
        # External provider state living OUTSIDE AGENTX_HOME.
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        ...
        assert not (agentx_home / "_external").exists()
