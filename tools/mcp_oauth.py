...
This module provides the glue:
    - ``AgentXTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "AgentX Agent"
"""
...
from agentx_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__agentx_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    try:
        ...
        base = Path(get_agentx_home())
    except ImportError:
        base = Path(os.environ.get("AGENTX_HOME", str(Path.home() / ".agentx")))
...
# ---------------------------------------------------------------------------
# AgentXTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class AgentXTokenStorage:
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://agentx-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "AgentX Agent")
    scope = cfg.get("scope")
