...
See: https://github.com/NousResearch/agentx-agent/issues/25114
See: https://github.com/NousResearch/agentx-agent/issues/36144
See: https://github.com/NousResearch/agentx-agent/issues/29015
"""
...
class TestGetSubprocessHome:
    """Unit tests for agentx_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(agentx_constants, "is_container", lambda: False)
    ...
    def test_returns_none_when_agentx_home_unset(self, monkeypatch):
        monkeypatch.delenv("AGENTX_HOME", raising=False)
        from agentx_constants import get_subprocess_home
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        agentx_home = real_home / ".agentx" / "profiles" / "coder"
        profile_home = agentx_home / "home"
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        from agentx_constants import get_subprocess_home, get_real_home
    ...
    def test_real_home_falls_back_to_os_account_when_home_is_profile(self, tmp_path, monkeypatch):
        ...
        from agentx_constants import get_real_home
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_agentx_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_agentx_home_override(profile)
        try:
            assert get_agentx_home() == profile
            release.set()
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(agentx_home / "home")
        assert result["AGENTX_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_agentx_home_unset(self, monkeypatch):
        monkeypatch.delenv("AGENTX_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["AGENTX_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from agentx_cli.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from agentx_cli.profiles import create_profile
...
class TestPythonProcessUnchanged:
    ...
    def test_path_home_unchanged_after_subprocess_home_resolved(
        ...
        assert sub_home in (None, str(agentx_home / "home"), original_home)
