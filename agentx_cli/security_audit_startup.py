...
def _container_no_volume_mount(agentx_home: Optional[Path]) -> Optional[str]:
    if not _in_container():
        return None
    home = agentx_home or Path(
        os.environ.get("AGENTX_HOME", os.path.expanduser("~/.agentx"))
    )
    try:
...
def run_security_audit(
    *, agentx_home: Optional[Path] = None, config: Optional[dict] = None
) -> list[str]:
    ...
    for check in (
        ...
        try:
            ...
            continue
    try:
        r = _container_no_volume_mount(agentx_home)
        if r:
            findings.append(r)
    except Exception:
        pass
...
def log_startup_security_warnings(
    *,
    ...
    _AUDIT_RAN = True
    try:
        findings = run_security_audit(agentx_home=agentx_home, config=config)
    except Exception:
        return []
    if findings:
        logger.warning(
