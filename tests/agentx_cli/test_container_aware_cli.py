...
def container_env(tmp_path, monkeypatch):
    ...
    container_mode = agentx_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=agentx-agent\n"
        "exec_user=agentx\n"
        "agentx_bin=/data/current-package/bin/agentx\n"
    )
    return agentx_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("agentx_constants.is_container", return_value=False):
...
def test_get_container_exec_info_skipped_when_agentx_dev(container_env, monkeypatch):
    """Returns None when AGENTX_DEV=1 is set (dev mode bypass)."""
...
def test_get_container_exec_info_not_skipped_when_agentx_dev_zero(container_env, monkeypatch):
    """AGENTX_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("AGENTX_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        agentx_home = Path(tmpdir) / ".agentx"
        agentx_home.mkdir()
        (agentx_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("agentx_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_agentx_home": lambda: agentx_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("AGENTX_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=agentx-custom\n"
        "exec_user=myuser\n"
        "agentx_bin=/opt/agentx/bin/agentx\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "agentx-custom"
    assert info["exec_user"] == "myuser"
    assert info["agentx_bin"] == "/opt/agentx/bin/agentx"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from agentx_cli.main import _exec_in_container
