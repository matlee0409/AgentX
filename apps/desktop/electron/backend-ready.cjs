...
const _READY_RE = /^AGENTX_DASHBOARD_READY port=(\d+)/m

// The announcement clock starts the instant the backend process is spawned —
// before uvicorn binds its socket. On a cold install the child must first
// compile and import the whole `agentx_cli.main` → `web_server` → FastAPI/
...
/**
 * Watch a child process's stdout for the `AGENTX_DASHBOARD_READY port=<N>`
