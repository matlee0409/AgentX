# ============================================================================
# AgentX Agent Installer for Windows
# ============================================================================
...
$RepoUrlSsh = "git@github.com:NousResearch/agentx-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/agentx-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * AgentX Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/agentx-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/agentx-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/agentx-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
...
function Install-Venv {
    ...
    if (Test-Path "venv") {
        ...
        if ($env:OS -eq "Windows_NT") {
            ...
            # `pythonw.exe -m agentx_cli.main gateway run` straight out of
            ...
            # user (see agentx_cli/gateway_windows.py), so the installer always
...
function Copy-ConfigTemplates {
    ...
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in agentx_cli/default_soul.py. The runtime
...
function Install-Desktop {
    ...
    #
    # The Tauri bootstrap installer's launch_agentx_desktop command
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the AgentX-managed
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m agentx_cli.main setup
    } else {
...
function Start-GatewayIfConfigured {
    ...
    $agentxCmd = "$InstallDir\venv\Scripts\agentx.exe"
    if (-not (Test-Path $agentxCmd)) {
        $agentxCmd = "agentx"
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            Start-Process -FilePath $agentxCmd -ArgumentList "gateway" `
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://agentx-agent.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
