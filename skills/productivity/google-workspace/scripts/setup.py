#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for AgentX Agent.
...
# Ensure sibling modules (_agentx_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _agentx_home import display_agentx_home, get_agentx_home
...
def install_deps():
    ...
    print("  pip install 'agentx-agent[google]'")
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_agentx_home()}/google_token.json")
