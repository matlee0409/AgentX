...
agentx cron create "0 2 * * *" \
  "You are a project manager triaging the matlek0409/AgentX GitHub repo.

1. Run: gh issue list --repo matlek0409/AgentX --state open --json number,title,labels,author,createdAt --limit 30
...
agentx cron create "0 9 * * 1" \
  "Scan the matlek0409/AgentX repo for documentation drift.

1. Run: gh pr list --repo matlek0409/AgentX --state merged --json number,title,files,mergedAt --limit 30
