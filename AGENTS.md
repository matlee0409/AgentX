# AgentX Agent - Development Guide
...
├── cli.py                # AgentXCLI class — interactive CLI orchestrator (~11k LOC)
├── agentx_state.py       # SessionDB — SQLite session store (FTS5 search)
├── agentx_constants.py   # get_agentx_home(), display_agentx_home() — profile-aware paths
├── agentx_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── agentx_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── agentx-achievements/  # Gamified achievement tracking
...
`gateway.log` when running the gateway. Profile-aware via `get_agentx_home()`.
...
- **Skin engine** (`agentx_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `AgentXCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
...
### Slash Command Registry (`agentx_cli/commands.py`)
...
1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `agentx_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `AgentXCLI.process_command()` in `cli.py`:
```python
...
npm run dev       # watch mode (rebuilds agentx-ink + tsx --watch)
npm start         # production
npm run build     # full build (agentx-ink + tsc)
...
The dashboard embeds the real `agentx --tui` — **not** a rewrite.  See `agentx_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `agentx_cli/web_server.py`.
...
A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). It does NOT embed `agentx --tui` — it has its own composer, transcript, and slash-command pipeline. Route desktop bugs to the `agentx-desktop-app-work` skill, not `agentx-dashboard-work`.
...
**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_agentx_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `AGENTX_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_agentx_home()` for the base directory — never `Path.home() / ".agentx"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `agentx_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `agentx_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `agentx tools`, `agentx setup`, most CLI subcommands | `agentx_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`agentx_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
agentx_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
...
Add to `_BUILTIN_SKINS` dict in `agentx_cli/skin_engine.py`:
...
### General plugins (`agentx_cli/plugins.py` + `plugins/<name>/`)
...
`post_setup(agentx_home, config)` for setup-wizard integration.
...
(`run_agent.py`, `cli.py`, `gateway/run.py`, `agentx_cli/main.py`, etc.).
...
`metadata.agentx.tags`, `metadata.agentx.category`,
`metadata.agentx.related_skills`, `metadata.agentx.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.agentx.*` by the loader.
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "AgentX Agent" is the secondary collaborator. If the
   contributor's commit shows "AgentX Agent" as author (because they
...
- **CLI:** `agentx_cli/curator.py` wires `agentx curator <verb>` where
...
- **CLI:** `agentx_cli/kanban.py` wires `agentx kanban` with verbs
...
The core mechanism: `_apply_profile_override()` in `agentx_cli/main.py` sets
`AGENTX_HOME` before any module imports. All `get_agentx_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_agentx_home()` for all AGENTX_HOME paths.** Import from `agentx_constants`.
   NEVER hardcode `~/.agentx` or `Path.home() / ".agentx"` in code that reads/writes state.
   ```python
   # GOOD
   from agentx_constants import get_agentx_home
   config_path = get_agentx_home() / "config.yaml"

   # BAD — breaks profiles
   config_path = Path.home() / ".agentx" / "config.yaml"
   ```

2. **Use `display_agentx_home()` for user-facing messages.** Import from `agentx_constants`.
   This returns `~/.agentx` for default or `~/.agentx/profiles/<name>` for profiles.
   ```python
   # GOOD
   from agentx_constants import display_agentx_home
   print(f"Config saved to {display_agentx_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.agentx/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_agentx_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_agentx_home()`,
   not `Path.home() / ".agentx"`.

4. **Tests that mock `Path.home()` must also set `AGENTX_HOME`** — since code now uses
   `get_agentx_home()` (reads env var), not `Path.home() / ".agentx"`:
   ```python
...
6. **Profile operations are HOME-anchored, not AGENTX_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".agentx" / "profiles"`, NOT `get_agentx_home() / "profiles"`.
...
Use `get_agentx_home()` from `agentx_constants` for code paths. Use `display_agentx_home()`
...
Existing call sites in `agentx_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `agentx_cli/curses_ui.py` — see
`agentx_cli/tools_config.py` for the canonical pattern.
...
The `_isolate_agentx_home` autouse fixture in `tests/conftest.py` redirects `AGENTX_HOME` to a temp dir. Never hardcode `~/.agentx/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_agentx_home()` resolve within the temp dir.
Use the pattern from `tests/agentx_cli/test_profiles.py`:
```python
@pytest.fixture
...
- The plugin disables itself in child processes (sentinel envvar
  `AGENTX_ISOLATE_CHILD=1`), so there's no fork-bomb risk.
