...
declare global {
  interface Window {
    agentxDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<AgentXConnection>
      ...
      api: <T>(request: AgentXApiRequest) => Promise<T>
      notify: (payload: AgentXNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<AgentXReadFileTextResult>
      selectPaths: (options?: AgentXSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<AgentXPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<AgentXPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: AgentXTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<AgentXReadDirResult>
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<AgentXGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<AgentXGitBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<AgentXRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: AgentXReviewScope, baseRef?: null | string) => Promise<AgentXReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: AgentXReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<AgentXReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: AgentXTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<AgentXTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: AgentXWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: AgentXPreviewFileChanged) => void) => () => void
...
export interface AgentXTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface AgentXTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface AgentXConnection {
  baseUrl: string
...
export interface AgentXTitleBarTheme {
  background: string
  foreground: string
}

export interface AgentXWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface AgentXApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface AgentXNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface AgentXPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface AgentXReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface AgentXPreviewWatch {
  id: string
  path: string
}
...
export interface AgentXGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface AgentXGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
...
export interface AgentXRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface AgentXRepoStatus {
  branch: null | string
...
// turn began.
export type AgentXReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface AgentXReviewFile {
  path: string
  added: number
  removed: number
...
export interface AgentXReviewList {
  files: AgentXReviewFile[]
...
export interface AgentXReviewPr {
  url: string
  state: string
  number: number
}
...
export interface AgentXReviewShipInfo {
  ghReady: boolean
  pr: AgentXReviewPr | null
}

export interface AgentXReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface AgentXReadDirResult {
  entries: AgentXReadDirEntry[]
  error?: string
}

export interface AgentXPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface AgentXSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
