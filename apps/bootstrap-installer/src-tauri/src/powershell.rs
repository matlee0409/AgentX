...
///
/// `agentx_home_override` propagates to the child as $AGENTX_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, agentx_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = agentx_home_override {
...
fn stable_script_cwd<'a>(script_path: &'a Path, agentx_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = agentx_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_agentx_home() {
