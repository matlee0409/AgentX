"""Tests for agentx_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
class TestGenerateBotUsername:
    def test_secure_default_format(self):
        ...
        assert len(name) == len("agentx_") + 16 + len("_bot")
        assert len(name) <= 32
    ...
    def test_slug_uses_telegram_safe_base32_chars(self):
        name = generate_bot_username()
        slug = name.removeprefix("agentx_").removesuffix("_bot")
        assert len(slug) == 16
...
class TestCreatePairing:
    def test_success(self):
        ...
        mock_resp.json.return_value = {
            ...
            "suggested_username": "agentx_abcdefghijklmnop_bot",
            "deep_link": "https://t.me/newbot/AgentXSetupBot/agentx_abcdefghijklmnop_bot?name=AgentX+Agent",
            "qr_payload": "https://t.me/newbot/AgentXSetupBot/agentx_abcdefghijklmnop_bot?name=AgentX+Agent",
            "expires_at": "2026-05-18T00:00:00.000Z",
        }

        with patch(
            "agentx_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
            pairing = create_pairing("https://api.example.com", bot_name="AgentX Agent")
...
class TestPollForToken:
    def pairing(self):
        return TelegramPairing(
            ...
            poll_token="secret-token",
            suggested_username="agentx_abcdefghijklmnop_bot",
            deep_link="https://t.me/newbot/AgentXSetupBot/agentx_abcdefghijklmnop_bot",
            qr_payload="https://t.me/newbot/AgentXSetupBot/agentx_abcdefghijklmnop_bot",
        )
    ...
    def test_immediate_success(self):
        ...
        mock_resp.json.return_value = {
            "bot_username": "agentx_abcdefghijklmnop_bot",
            "owner_user_id": 42,
            "status": "ready",
            "token": VALID_TOKEN,
        }
        ...
        with patch(
            "agentx_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("agentx_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_setup_result_includes_owner_user_id(self):
        ...
        with patch("agentx_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("agentx_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_invalid_ready_token_returns_none(self):
        ...
        with patch("agentx_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("agentx_cli.telegram_managed_bot.time.sleep"):
                with patch(
                    "agentx_cli.telegram_managed_bot.time.monotonic"
                ) as mock_time:
    ...
    def test_eventual_success(self):
        ...
        with patch("agentx_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("agentx_cli.telegram_managed_bot.time.sleep"):
                token = poll_for_token(
...
class TestSetupTelegramAuto:
    ...
    def test_setup_result_passes_profile_name_for_profile_home(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "agentx_cli.telegram_managed_bot.auto_setup_telegram_bot_result",
    ...
    def test_profile_name_from_home_path_handles_windows_separators(self):
        from agentx_cli.setup import _profile_name_from_agentx_home

        assert (
            _profile_name_from_agentx_home(
