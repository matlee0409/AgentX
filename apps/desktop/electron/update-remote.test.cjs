 ...
 * origin=git@github.com:matlek0409/AgentX.git. A background
...
test('canonicalGitHubRemote normalizes SSH and HTTPS forms to the same value', () => {
  assert.equal(canonicalGitHubRemote('git@github.com:matlek0409/AgentX.git'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('git@github.com:matlek0409/AgentX'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('ssh://git@github.com/matlek0409/AgentX.git'), OFFICIAL_REPO_CANONICAL)
  assert.equal(canonicalGitHubRemote('https://github.com/matlek0409/AgentX.git'), OFFICIAL_REPO_CANONICAL)
  // Case-insensitive: an uppercased owner still canonicalizes to the same repo.
  assert.equal(canonicalGitHubRemote('git@github.com:nousresearch/agentx-agent.git'), OFFICIAL_REPO_CANONICAL)
  // Trailing slashes are stripped.
  assert.equal(canonicalGitHubRemote('https://github.com/matlek0409/AgentX/'), OFFICIAL_REPO_CANONICAL)
})
...
test('isSshRemote detects scp-like and ssh:// forms only', () => {
  assert.equal(isSshRemote('git@github.com:matlek0409/AgentX.git'), true)
  assert.equal(isSshRemote('ssh://git@github.com/matlek0409/AgentX.git'), true)
  assert.equal(isSshRemote('https://github.com/matlek0409/AgentX.git'), false)
...
test('isOfficialSshRemote is true only for the official repo over SSH', () => {
  assert.equal(isOfficialSshRemote('git@github.com:matlek0409/AgentX.git'), true)
  assert.equal(isOfficialSshRemote('git@github.com:matlek0409/AgentX'), true)
  assert.equal(isOfficialSshRemote('ssh://git@github.com/matlek0409/AgentX.git'), true)
  // Case-insensitive owner/repo match.
  assert.equal(isOfficialSshRemote('git@github.com:nousresearch/agentx-agent.git'), true)
})
...
test('isOfficialSshRemote does NOT match forks, other hosts, or HTTPS', () => {
  ...
  assert.equal(isOfficialSshRemote('git@gitlab.com:matlek0409/AgentX.git'), false)
  ...
  assert.equal(isOfficialSshRemote('https://github.com/matlek0409/AgentX.git'), false)
