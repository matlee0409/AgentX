...
def server():
    with patch.dict("sys.modules", {
        "agentx_constants": MagicMock(get_agentx_home=MagicMock(return_value="/tmp/agentx_test")),
        "agentx_cli.env_loader": MagicMock(),
        "agentx_cli.banner": MagicMock(),
        "agentx_state": MagicMock(),
    }):
        import importlib
...
def test_sync_session_key_after_compress_reanchors_active_session_lease(
    ...
    from agentx_cli.active_sessions import (
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        "agentx_cli.plugins.get_plugin_command_handler",
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "agentx-agent-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "agentx-agent-dev"
