...
Browser-based dashboard for managing AgentX Agent configuration, API keys, and monitoring active sessions.
...
cd ../
python -m agentx_cli.main web --no-open
...
`agentx dashboard` on port 9119 serves the **built** bundle from `agentx_cli/web_dist/`, not the Vite dev server — changes in `web/src/` will not appear there until you run `npm run build` and restart the dashboard (or use `web --no-open` + Vite as above).
...
This outputs to `../agentx_cli/web_dist/`, which the FastAPI server serves as a static SPA. The built assets are included in the Python package via `pyproject.toml` package-data.
...
| Wordmark | `Typography` + size/tracking only | Sidebar/mobile “AgentX Agent” — mixed case, no Mondwest, no `text-display` |
