...
sidebar_position: 2
title: "Configuration"
description: "Configure AgentX Agent — config.yaml, providers, models, API keys, and more"
---
...
Leaving these unset keeps the legacy defaults (`AGENTX_API_TIMEOUT=1800`s, `AGENTX_API_CALL_STALE_TIMEOUT=90`s, native Anthropic 900s). The non-streaming stale detector is auto-disabled for local endpoints when left implicit and can scale upward for very large contexts. Not currently wired for AWS Bedrock (both `bedrock_converse` and AnthropicBedrock SDK paths use boto3 with its own timeout configuration). See the commented example in [`cli-config.yaml.example`](https://github.com/NousResearch/agentx-agent/blob/main/cli-config.yaml.example).
...
```yaml
updates:
  pre_update_backup: false       # Create a full AGENTX_HOME zip before every update
...
AgentX supports six terminal backends. Each determines where the agent's shell commands actually execute — your local machine, a Docker container, a remote server via SSH, a Modal cloud sandbox (direct or via the Nous-managed gateway), a Daytona workspace, or a Singularity/Apptainer container.
...
For cloud sandboxes such as Modal and Daytona, `container_persistent: true` means AgentX will try to preserve filesystem state across sandbox recreation. It does not promise that the same live sandbox, PID space, or background processes will still be running later.
...
agentx_home = Path(os.environ["AGENTX_HOME"])
...
For the **SSH**, **Modal**, and **Daytona** backends (anywhere the agent's working tree lives on a different machine than the host running AgentX), AgentX tracks files the agent touched inside the remote sandbox and, on session teardown / sandbox cleanup, **syncs the modified files back to the host** under `~/.agentx/cache/remote-syncs/<session-id>/`.
...
- The remote sandbox may have been torn down by the time you go looking; the local `~/.agentx/cache/remote-syncs/…` copy is the authoritative record of what the agent changed.
...
| Stale stream detection | 180s | Auto-disabled | `AGENTX_STREAM_STALE_TIMEOUT` |
| Stale non-stream detection | 300s | Auto-disabled when left implicit | `providers.<id>.stale_timeout_seconds` or `AGENTX_API_CALL_STALE_TIMEOUT` |
...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="AgentX Agent — Auxiliary Models Tutorial"
...
When the cap is reached, AgentX returns a direct limit message for new sessions.
...
quick_commands:
  ...
  update:
    type: exec
    command: cd ~/.agentx/agentx-agent && git pull && pip install -e .
  gpu:
    type: exec
