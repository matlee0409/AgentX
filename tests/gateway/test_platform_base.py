...
class TestMediaDeliveryPathValidation:
    ...
    def test_allows_operator_configured_extra_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AGENTX_MEDIA_ALLOW_DIRS", str(extra_root))
    ...
    def test_recency_trust_rejects_old_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("AGENTX_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("AGENTX_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("AGENTX_MEDIA_TRUST_RECENT_SECONDS", "60")
    ...
    def test_recency_trust_allows_pdf_in_project_dir(self, tmp_path, monkeypatch):
        ...
        files outside ~/.agentx/cache/* were silently dropped, leaving the
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_agentx_credentials(self, tmp_path, monkeypatch):
        """~/.agentx/.env and ~/.agentx/auth.json stay blocked even in
        ...
        fake_home = tmp_path / "home"
        agentx_dir = fake_home / ".agentx"
        agentx_dir.mkdir(parents=True)
        env_file = agentx_dir / ".env"
    ...
    def test_denylist_blocks_agentx_config_in_active_profile(self, tmp_path, monkeypatch):
        ...
        agentx_dir = fake_home / ".agentx"
        agentx_dir.mkdir(parents=True)
        config_file = agentx_dir / "config.yaml"
    ...
    def test_denylist_blocks_shared_agentx_root_config_for_profiles(self, tmp_path, monkeypatch):
        """Profile-mode gateways must still block the shared AgentX root config."""
        self._patch_roots(monkeypatch)

        fake_home = tmp_path / "home"
        profile_home = fake_home / ".agentx" / "profiles" / "work"
        profile_home.mkdir(parents=True)
        agentx_root = fake_home / ".agentx"
        config_file = agentx_root / "config.yaml"
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        ...
        agentx_dir = fake_home / ".agentx"
        agentx_dir.mkdir(parents=True)
        token = agentx_dir / "google_token.json"
        token.write_text('{"access_token": "***", "refresh_token": "***"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._AGENTX_HOME", agentx_dir)
        monkeypatch.setattr("gateway.platforms.base._AGENTX_ROOT", agentx_dir)
    ...
    def test_denylist_blocks_pairing_directory_contents(self, tmp_path, monkeypatch):
        """Files under ~/.agentx/pairing/ (platform pairing tokens) are
    ...
    def test_agentx_cache_still_delivers_under_denied_home(self, tmp_path, monkeypatch):
        ...
        fake_home = tmp_path / "home"
        agentx_dir = fake_home / ".agentx"
        cache_dir = agentx_dir / "cache" / "documents"
    ...
    def test_denylist_blocks_non_cache_file_under_agentx_home(self, tmp_path, monkeypatch):
        ...
        agentx_dir = fake_home / ".agentx"
        agentx_dir.mkdir(parents=True)
        artifact = agentx_dir / "adhoc_report.pdf"
    ...
    def test_strict_mode_truthy_aliases(self, monkeypatch, tmp_path):
        """``AGENTX_MEDIA_DELIVERY_STRICT=true|yes|on|1`` all enable strict mode."""
        self._patch_roots(monkeypatch)
        from gateway.platforms.base import _media_delivery_strict_mode

        for raw in ("1", "true", "TRUE", "yes", "on"):
            monkeypatch.setenv("AGENTX_MEDIA_DELIVERY_STRICT", raw)
    ...
    def test_root_home_agentx_env_still_blocked(self, tmp_path, monkeypatch):
        """``~/.agentx/.env`` stays blocked under the $HOME exception — it is a
...
class TestGetHumanDelay:
    def test_off_mode(self):
        with patch.dict(os.environ, {"AGENTX_HUMAN_DELAY_MODE": "off"}):
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_default_is_off(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("AGENTX_HUMAN_DELAY_MODE", None)
            assert BasePlatformAdapter._get_human_delay() == 0.0

    def test_natural_mode_range(self):
        with patch.dict(os.environ, {"AGENTX_HUMAN_DELAY_MODE": "natural"}):
    ...
    def test_custom_mode_uses_env_vars(self):
        env = {
            ...
            "AGENTX_HUMAN_DELAY_MIN_MS": "100",
            "AGENTX_HUMAN_DELAY_MAX_MS": "200",
        }
        with patch.dict(os.environ, env):
            delay = BasePlatformAdapter._get_human_delay()
