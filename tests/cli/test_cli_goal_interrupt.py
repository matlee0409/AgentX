...
minimal ``AgentXCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal AgentXCLI stub with an active goal wired in."""
...
class TestInterruptAutoPause:
    def test_interrupted_turn_pauses_goal_and_skips_continuation(self, agentx_home):
        ...
        with patch("agentx_cli.goals.judge_goal") as judge_mock:
    ...
    def test_interrupted_turn_is_resumable(self, agentx_home):
        ...
        with patch("agentx_cli.goals.judge_goal"):
            cli._maybe_continue_goal_after_turn()
...
class TestEmptyResponseSkip:
    def test_empty_response_does_not_invoke_judge(self, agentx_home):
    ...
    def test_no_assistant_message_skipped(self, agentx_home):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, agentx_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, agentx_home):
        ...
        src = inspect.getsource(AgentXCLI.chat)
