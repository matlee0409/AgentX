...
sidebar_position: 7
title: "Docker"
description: "在 Docker 中运行 AgentX Agent 以及将 Docker 用作终端后端"
---

# AgentX Agent — Docker

Docker 与 AgentX Agent 的交集有两种截然不同的方式：
...
如果这是你第一次运行 AgentX Agent，请在宿主机上创建一个数据目录，并以交互方式启动容器以运行设置向导：
...
2. 运行 `/etc/cont-init.d/02-reconcile-profiles`（即 `agentx_cli.container_boot`）：遍历 `$AGENTX_HOME/profiles/<name>/`，在 `/run/service/gateway-<profile>/` 下重建各 profile 的 gateway s6 服务槽，并仅自动启动上次记录状态为 `running` 的 profile（参见 [Per-profile gateway 监管](#per-profile-gateway-supervision)）。
...
当工具必须在每次容器启动时立即可用且无需重新安装延迟时，构建一个继承自 `nousresearch/agentx-agent` 并在层中安装该工具的新镜像：
...
入口点脚本和 `/opt/data` 语义原样继承，本页其余内容仍然适用。拉取更新的上游 `nousresearch/agentx-agent` 时记得重新构建镜像。
...
如果某个工具可能对大多数 AgentX Agent 用户有用，考虑将其贡献到上游，而不是在私有派生镜像中维护。在 [agentx-agent 仓库](https://github.com/NousResearch/agentx-agent)提交 issue 或 pull request，描述该工具及其使用场景。被纳入官方镜像的工具惠及所有用户，并避免了维护下游 fork 的开销。
...
docker run -it --rm nousresearch/agentx-agent:latest version     # 验证版本
