...
"state": …}`` and the route serializes those into the ``agentx_session_pkce``
cookie.
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"agentx_session_pkce\"]``
        ...
        cookie_payload = {
            "agentx_session_pkce": f"state={state};verifier={code_verifier}",
        }
...
def register(ctx) -> None:
    ...
    engagement layer (``agentx_cli.web_server.should_require_auth`` +
    ...
    if not client_id:
        LAST_SKIP_REASON = (
            ...
            "'agent:{instance_id}') when it deploys a AgentX Agent "
