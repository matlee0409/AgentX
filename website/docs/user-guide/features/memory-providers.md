...
AgentX Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.
...
**Config:** `$AGENTX_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$AGENTX_HOME/honcho.json` > `~/.agentx/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/NousResearch/agentx-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/agentx).
...
See the [config reference](https://github.com/NousResearch/agentx-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/agentx).
...
echo "OPENVIKING_API_KEY=..." >> ~/.agentx/.env
```
...
**Switching modes:** Re-run `agentx memory setup mem0 --mode <platform|oss>` or edit `mem0.json` directly.
...
| `retain_context` | `conversation between AgentX Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/NousResearch/agentx-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
