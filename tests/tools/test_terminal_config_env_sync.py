...
at startup, by THREE separate code paths:
  ...
  2. gateway/run.py    -> ``_terminal_env_map`` dict (gateway / messaging
                          platforms)
  3. agentx_cli/config.py:set_config_value
...
mirrors the pattern used in tests/agentx_cli/test_config_drift.py.
"""
...
def _save_config_env_sync_keys() -> set[str]:
    ...
    """
    from agentx_cli import config as hc_config
...
def test_save_config_set_supports_critical_bridged_keys():
    ...
    assert not missing, (
        ...
        f"in agentx_cli/config.py (set_config_value bridges through it)."
    )
