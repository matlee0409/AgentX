"""
Interactive setup wizard for AgentX Agent.
...
_DOCS_BASE = "https://agentx-agent.nousresearch.com/docs"
...
# display_agentx_home imported lazily at call sites (stale-module safety during agentx update)
...
from agentx_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from agentx_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def is_noninteractive() -> bool:
    ...
    ``AGENTX_NONINTERACTIVE=1`` (see ``agentx_cli/web_server.py``). In that
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from agentx_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, agentx_home):
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from agentx_cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from agentx_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {agentx_home}/cron/, sessions/, logs/"
    )
    print()
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from agentx_cli.main import select_provider_and_model
    try:
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    """
    try:
        from agentx_cli.auth import get_xai_oauth_auth_status
...
def _setup_telegram_auto_result():
    ...
    try:
        from agentx_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_agentx_home(Path(get_agentx_home()))
    except Exception:
        pass
...
def _profile_name_from_agentx_home(agentx_home) -> str | None:
    """Return the active profile name when AGENTX_HOME is a profile dir."""
    if agentx_home.parent.name == "profiles":
        return agentx_home.name
    return None
...
def _setup_qqbot():
    ...
    from agentx_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://agentx-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://agentx-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
...
def setup_gateway(config: dict):
    ...
    from agentx_cli.gateway import _all_platforms, _platform_status, _configure_platform
    ...
    if any_messaging:
        ...
        if supports_systemd and has_legacy_agentx_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo agentx gateway install --system")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``agentx_cli.auth`` — lists every supported
    ...
    """
    try:
        from agentx_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from agentx_cli.config
    ...
    if section_key == "model":
        ...
        from agentx_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_agentx.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_agentx migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_agentx", _OPENCLAW_SCRIPT
    )
...
def _offer_openclaw_migration(agentx_home: Path) -> bool:
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=agentx_home.resolve(),
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    try:
        from agentx_cli.main import _model_flow_nous
...
def run_setup_wizard(args):
    ...
    """
    from agentx_cli.config import is_managed, managed_error
    if is_managed():
    ...
    print(
        color(
            "│             ⚕ AgentX Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your AgentX Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, agentx_home)
            return
        ...
        migration_ran = _offer_openclaw_migration(agentx_home)
        if migration_ran:
            config = load_config()
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, agentx_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, agentx_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {agentx_home}")
    ...
    _print_setup_summary(config, agentx_home)


def _run_first_time_quick_setup(config: dict, agentx_home, is_existing: bool):
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    try:
        ...
        from agentx_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _run_blank_slate_setup(config: dict, agentx_home, is_existing: bool):
    ...
    _blank_slate_walkthrough(config, agentx_home)


def _blank_slate_walkthrough(config: dict, agentx_home):
...
def _run_quick_setup(config: dict, agentx_home):
