...
**→ [Build a AgentX Plugin](/guides/build-a-agentx-plugin)** — step-by-step guide with a complete working example.
...
| Distribute via pip | `[project.entry-points."agentx_agent.plugins"]` |
...
| pip | `agentx_agent.plugins` entry_points | Distributed packages |
...
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a AgentX Plugin](/guides/build-a-agentx-plugin) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a AgentX Plugin](/guides/build-a-agentx-plugin) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a AgentX Plugin](/guides/build-a-agentx-plugin) · [Extending the CLI](/developer-guide/extending-the-cli) |
...
See the **[full guide](/guides/build-a-agentx-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
