...
sidebar_position: 3
title: "常见问题与故障排查"
description: "AgentX Agent 常见问题解答及常见问题解决方案"
---
...
AgentX Agent 可与任何兼容 OpenAI 的 API 配合使用。支持的提供商包括：
...
**原生不支持。** AgentX Agent 需要类 Unix 环境。在 Windows 上，请安装 [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) 并在其中运行 AgentX。标准安装命令在 WSL2 中可完美运行：
...
API 调用**仅发送至您配置的 LLM 提供商**（例如 OpenRouter、您本地的 Ollama 实例）。AgentX Agent 不收集遥测数据、使用数据或分析数据。您的对话、记忆和技能均存储在本地 `~/.agentx/` 目录中。
...
AgentX Agent 本身**免费且开源**（MIT 许可证）。您只需为所选提供商的 LLM API 用量付费。本地模型完全免费运行。

### 多人可以使用同一个实例吗？

可以。[消息网关](../user-guide/messaging/index.md)允许多个用户通过 Telegram、Discord、Slack、WhatsApp 或 Home Assistant 与同一个 AgentX Agent 实例交互。访问权限通过白名单（特定用户 ID）和私信配对（第一个发消息的用户获得访问权）来控制。
...
1. **搜索现有 issue：** [GitHub Issues](https://github.com/NousResearch/agentx-agent/issues)
