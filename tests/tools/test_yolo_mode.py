...
class TestYoloMode:
    ...
    def test_yolo_mode_empty_string_does_not_bypass(self, monkeypatch):
        """Empty string for AGENTX_YOLO_MODE should not trigger bypass."""
        monkeypatch.setenv("AGENTX_YOLO_MODE", "")
        monkeypatch.setenv("AGENTX_INTERACTIVE", "1")
        monkeypatch.setenv("AGENTX_SESSION_KEY", "test-session")

        # Empty string is falsy in Python, so getenv("AGENTX_YOLO_MODE") returns ""
