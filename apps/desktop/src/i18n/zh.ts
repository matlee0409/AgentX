...
export const zh: Translations = {
  ...
  notifications: {
    ...
    errors: {
      ...
      methodNotAllowed: '桌面后端拒绝了该请求 (405 Method Not Allowed)。请尝试重启 AgentX Desktop。',
  ...
  settings: {
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'AgentX Desktop 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 AgentX 后端时，可以使用远程网关。下面可按 profile 指定各自的远程主机。',
      appliesTo: '应用于',
      ...
      remoteTitle: '远程网关',
      remoteDesc:
        '将此桌面外壳连接到远程 AgentX 后端。托管网关使用 OAuth 或用户名密码；自托管网关也可能使用会话 token。',
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: 'AgentX Desktop 将使用已保存设置重新连接。',
  ...
  commandCenter: {
    ...
    agentxActiveSessions: (version, count) => `AgentX ${version} · 活跃会话 ${count}`,
    restartGateway: '重启网关',
  ...
  install: {
    ...
    failedTitle: '安装失败',
    settingUpTitle: '正在设置 AgentX Agent',
    finishingTitle: '正在收尾',
    failedDesc:
