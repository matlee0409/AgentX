...
Diagnoses issues with AgentX Agent setup.
"""
...
from agentx_cli.config import get_project_root, get_agentx_home, get_env_path
from agentx_cli.env_loader import load_agentx_dotenv
from agentx_constants import display_agentx_home
from agentx_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
AGENTX_HOME = get_agentx_home()
_DHH = display_agentx_home()  # user-facing display path (e.g. ~/.agentx or ~/.agentx/profiles/coder)

# Load environment variables from ~/.agentx/.env so API key checks work
_env_path = get_env_path()
load_agentx_dotenv(agentx_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from agentx_cli.colors import Colors, color
from agentx_cli.models import _AGENTX_USER_AGENT
from agentx_constants import OPENROUTER_MODELS_URL
...
from agentx_constants import is_termux as _is_termux
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from agentx_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches agentx_cli.__version__.
    ...
    """
    try:
        from agentx_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != agentx_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'agentx update', or set "
            "agentx_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from agentx_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # Detect drift between pyproject.toml and agentx_cli/__init__.py versions
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from agentx_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from agentx_cli.auth import PROVIDER_REGISTRY, get_auth_status
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from agentx_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from agentx_cli.config import check_config_version, migrate_config
        ...
        try:
            import yaml
            from agentx_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
    ...
    for subdir_name in expected_subdirs:
        subdir_path = agentx_home / subdir_name
    ...
    memories_dir = agentx_home / "memories"
    ...
    state_db_path = agentx_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from agentx_state import _db_opens_cleanly, repair_state_db_schema
            ...
            from agentx_state import is_malformed_db_error, repair_state_db_schema
    ...
    wal_path = agentx_home / "state.db-wal"
    if wal_path.exists():
        try:
    ...
    try:
        from agentx_constants import is_container as _is_container
    ...
    try:
        from agentx_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
