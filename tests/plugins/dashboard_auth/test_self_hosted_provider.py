...
class TestStartLogin:
    ...
    def test_two_calls_differ(self, provider):
        a = provider.start_login(redirect_uri="https://agentx.example/auth/callback")
        b = provider.start_login(redirect_uri="https://agentx.example/auth/callback")
        assert (
            a.cookie_payload["agentx_session_pkce"]
            != b.cookie_payload["agentx_session_pkce"]
        )
...
class TestPluginRegister:
    ...
    def patch_config(self, monkeypatch):
        def _set(oauth_block):
            ...
            monkeypatch.setattr("agentx_cli.config.load_config", lambda: cfg)
    ...
    def test_empty_env_does_not_shadow_config(self, patch_config, monkeypatch):
        ...
        monkeypatch.setenv("AGENTX_DASHBOARD_OIDC_ISSUER", "")
        monkeypatch.setenv("AGENTX_DASHBOARD_OIDC_CLIENT_ID", "")
        ctx = MagicMock()
        oidc_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
    ...
    def test_config_load_failure_falls_through(self, monkeypatch):
        ...
        monkeypatch.setattr("agentx_cli.config.load_config", _broken)
        ctx = MagicMock()
    ...
    def test_non_https_issuer_skips_with_reason(self, patch_config, monkeypatch):
        patch_config(None)
        monkeypatch.setenv(
            "AGENTX_DASHBOARD_OIDC_ISSUER", "http://insecure.example"
        )
        monkeypatch.setenv("AGENTX_DASHBOARD_OIDC_CLIENT_ID", _CLIENT_ID)
        ctx = MagicMock()
        oidc_plugin.register(ctx)
