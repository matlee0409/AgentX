...
try:
    from agentx_constants import display_agentx_home, get_agentx_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where agentx_constants isn't importable
    # (mirrors the same fallback used by the google-workspace skill's
    # _agentx_home.py shim).
...
def _legacy_token_path() -> Path:
    return _agentx_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _agentx_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _agentx_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _agentx_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _agentx_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install agentx-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'agentx-agent[google_chat]'")
        return False
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_agentx_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_agentx_home()}/google_chat_user_token.json"
    )
