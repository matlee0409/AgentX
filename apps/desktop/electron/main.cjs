...
const { canImportAgentXCli, verifyAgentXCli } = require('./backend-probes.cjs')
...
const { buildDesktopBackendEnv, normalizeAgentXHomeRoot } = require('./backend-env.cjs')
...
function resolveAgentXHome() {
  if (process.env.AGENTX_HOME) return normalizeAgentXHomeRoot(process.env.AGENTX_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'agentx-home')
  if (IS_WINDOWS) {
    ...
    if (fromRegistry) return normalizeAgentXHomeRoot(fromRegistry)
  }
...
const AGENTX_HOME = resolveAgentXHome()

function agentxManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_agentx_node_dirs() in
  // agentx_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(AGENTX_HOME, 'node')
  const bin = path.join(root, 'bin')
  const entries = IS_WINDOWS ? [root, bin] : [bin, root]
  return entries.filter(directoryExists)
}

function pathWithAgentXManagedNode(...entries) {
  return [...agentxManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}
...
// boots, going straight to `resolveAgentXBackend()`. Missing or stale marker
...
// local backend as. When set, startAgentX() passes `agentx --profile <name>
// dashboard …`, which deterministically pins AGENTX_HOME (see
// _apply_profile_override in agentx_cli/main.py) and bypasses the sticky
// ~/.agentx/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors agentx_cli.profiles._PROFILE_ID_RE so we never hand the backend a
...
// errors.log, gateway.log produced by agentx_logging.setup_logging — one log
...
//
// Mirror the Python logs (agentx_logging.py RotatingFileHandler, maxBytes x
...
// electron-builder derives from the build `appId` (com.nousresearch.agentx) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.agentx')
}
...
app.setAboutPanelOptions({
  ...
  applicationVersion: resolveAgentXVersion(),
...
let mainWindow = null
let agentxProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by agentxProcess +
// connectionPromise + startAgentX(); this pool only holds EXTRA profile
...
// onto the error so subsequent startAgentX() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning agentx dashboard children in a tight loop while boot is broken.
...
function rememberLog(chunk) {
  ...
  agentxLog.push(...lines)
  if (agentxLog.length > 300) {
    agentxLog.splice(0, agentxLog.length - 300)
  }
...
// (`force_kill_other_agentx`, taskkill /IM agentx.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(AGENTX_HOME)
  ...
  while (marker && Date.now() < deadline) {
    ...
    marker = readLiveUpdateMarker(AGENTX_HOME)
  }
  if (marker) {
...
function unwrapWindowsVenvAgentXCommand(command, dashboardArgs) {
  if (!IS_WINDOWS || !command || isCommandScript(command)) return null

  const resolved = path.resolve(String(command))
  if (!/^agentx(?:\.exe)?$/i.test(path.basename(resolved))) return null
  ...
  return {
    label: `existing AgentX no-console Python at ${python}`,
    command: python,
    args: ['-m', 'agentx_cli.main', ...dashboardArgs],
    bootstrap: false,
...
function isAgentXSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'agentx_cli', 'main.py'))
}
...
// findGitBash — locate bash.exe on Windows. AgentX' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\agentx\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'agentx', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'agentx', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
function getNoConsoleVenvPython(venvRoot) {
  ...
  // and breaks imports (yaml, agentx_cli, …) even when PYTHONPATH is patched.
...
function applyWindowsNoConsoleSpawnHints(backend) {
  ...
  const usesAgentXModule =
    backend.kind === 'python' ||
    (Array.isArray(backend.args) && backend.args[0] === '-m' && backend.args[1] === 'agentx_cli.main')

  if (!usesAgentXModule) return backend
...
function recentAgentXLog() {
  return agentxLog.slice(-20).join('\n')
}
...
function resolveUpdateRoot() {
  const candidates = [
    ...
    !IS_PACKAGED && isAgentXSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isAgentXSourceRoot(ACTIVE_AGENTX_ROOT) ? ACTIVE_AGENTX_ROOT : null
  ].filter(Boolean)
...
// AGENTX_HOME/agentx-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_agentx_home). That binary owns
// ALL repo mutation — running `agentx update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'agentx-setup.exe' : 'agentx-setup'
  const candidate = path.join(AGENTX_HOME, name)
...
function venvAgentXShimPath(updateRoot) {
  return IS_WINDOWS
...
// `agentxProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (agentxProcess && Number.isInteger(agentxProcess.pid)) pids.push(agentxProcess.pid)
  ...
  if (agentxProcess && !agentxProcess.killed) {
    try {
      agentxProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvAgentXShimPath(updateRoot)
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri agentx-setup. Unlike Windows
      ...
      // whole update itself: `agentx update` (backend) + `agentx desktop
    ...
    if (!updater) {
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') command = `agentx update --branch ${branch}`
        }
      } catch {
      ...
      return { ok: true, manual: true, command, agentxRoot: updateRoot }
    }
    ...
    const child = spawn(updater, updaterArgs, {
      ...
      env: {
        ...process.env,
        AGENTX_HOME,
        PATH: pathWithAgentXManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvAgentX = path.join(venvBin, IS_WINDOWS ? 'agentx.exe' : 'agentx')
  const updaterArgs = fileExists(venvAgentX) ? ['--update', '--branch', branch] : ['--repair', '--branch', branch]
...
function resolveAgentXCliBinary(updateRoot) {
  const venvAgentX = path.join(updateRoot, 'venv', 'bin', 'agentx')
  if (fileExists(venvAgentX)) return venvAgentX
...
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const agentx = resolveAgentXCliBinary(updateRoot)
  if (!agentx) {
    emitUpdateProgress({ stage: 'manual', message: 'agentx update', percent: null })
    return { ok: true, manual: true, command: 'agentx update', agentxRoot: updateRoot }
  }
  ...
  const env = {
    AGENTX_HOME,
    PATH: pathWithAgentXManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `agentx update` reaps stale `agentx dashboard` backends (a code update
  ...
  if (agentxProcess && Number.isInteger(agentxProcess.pid)) {
    desktopChildPids.push(agentxProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'AgentX.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'AgentX.app')
...
function isBootstrapComplete() {
  ...
  return isAgentXSourceRoot(ACTIVE_AGENTX_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function resolveAgentXCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/AgentX.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveAgentXCwd(), sanitized: Boolean(trimmed) }
  }
...
function createActiveBackend(dashboardArgs) {
  ...
  return applyWindowsNoConsoleSpawnHints({
    ...
    env: buildDesktopBackendEnv({
      agentxHome: AGENTX_HOME,
      pythonPathEntries: [ACTIVE_AGENTX_ROOT],
      venvRoot: VENV_ROOT
    }),
...
function resolveAgentXBackend(dashboardArgs) {
  ...
  if (overrideRoot && isAgentXSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `AgentX source at ${overrideRoot}`, dashboardArgs)
  ...
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isAgentXSourceRoot.)
  if (!IS_PACKAGED && isAgentXSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `AgentX source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_AGENTX_ROOT -- the canonical install at
  ...
  //    to spawning agentx. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `agentx update` from the CLI.
  ...
  if (process.env.AGENTX_DESKTOP_IGNORE_EXISTING !== '1') {
    let agentxCommand = null
    const agentxOverride = process.env.AGENTX_DESKTOP_AGENTX

    if (agentxOverride) {
      const resolvedOverride = findOnPath(agentxOverride)
      if (resolvedOverride) {
        agentxCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(agentxOverride, { isWsl: IS_WSL })) {
        agentxCommand = agentxOverride
      } else {
        rememberLog(`Ignoring Windows AgentX override under WSL: ${agentxOverride}`)
      }
    } else {
      agentxCommand = findOnPath('agentx')
    }

    if (agentxCommand) {
      if (looksLikeDesktopAppBinary(agentxCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving AgentX CLI: ${agentxCommand}`)
        agentxCommand = null
      }
    }

    if (agentxCommand) {
      const unwrapped = unwrapWindowsVenvAgentXCommand(agentxCommand, dashboardArgs)
      if (unwrapped) {
        return unwrapped
      }
      ...
      const shellForProbe = isCommandScript(agentxCommand)
      if (verifyAgentXCli(agentxCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvAgentXCommand(agentxCommand, dashboardArgs) || {
            label: `existing AgentX CLI at ${agentxCommand}`,
      ...
      rememberLog(
        `Ignoring existing AgentX CLI at ${agentxCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed agentx_cli module via system Python.
  ...
  if (python) {
    ...
    // having agentx_cli installed -- common on dev boxes that have
    ...
    if (canImportAgentXCli(python)) {
      return applyWindowsNoConsoleSpawnHints({
        kind: 'python',
        label: `installed agentx_cli module via ${python}`,
    ...
    rememberLog(`Ignoring system Python ${python}: agentx_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveAgentXBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'AgentX Agent not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveAgentXBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (!bootstrapResult.ok) {
      ...
      // Latch the failure so subsequent startAgentX() calls return this
    ...
    return ensureRuntime(resolveAgentXBackend(backend.args))
  }
  ...
  if (!isAgentXSourceRoot(ACTIVE_AGENTX_ROOT)) {
    throw new Error(
  ...
  if (!fileExists(venvPython)) {
    ...
    // isAgentXSourceRoot() and the bootstrap writes the marker only after
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveAgentXCwd()
...
async function waitForAgentX(baseUrl, token) {
  ...
  throw new Error(`AgentX backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
const ZOOM_STORAGE_KEY = 'agentx:desktop:zoomLevel'
...
//
//   * REST is authed by HttpOnly session cookies (``agentx_session_at``),
...
//     Both are set as HttpOnly cookies (``agentx_session_at`` ~15 min,
//     ``agentx_session_rt`` 24h). When the AT cookie lapses but the RT cookie
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to AgentX gateway',
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (agentx #37247), and the
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startAgentX())
...
function resetAgentXConnection() {
...
// startAgentX() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetAgentXConnection() nulls agentxProcess.
  const dying = agentxProcess && !agentxProcess.killed ? agentxProcess : null
  resetAgentXConnection()
...
// profile to startAgentX() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startAgentX() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForAgentX(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
      logs: agentxLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  // step 3 in agentx_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const dashboardArgs = ['--profile', profile, 'dashboard', '--no-open', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveAgentXBackend(dashboardArgs))
  const agentxCwd = resolveAgentXCwd()
  ...
  await Promise.race([waitForAgentX(baseUrl, token), startFailed])
  ready = true
...
async function startAgentX() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startAgentX() call re-throws the same error
  ...
  connectionPromise = (async () => {
    ...
    agentxProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin AGENTX_HOME for the child so Python's get_agentx_home()
          // resolves to the SAME location our resolveAgentXHome() picked. Without
    ...
    agentxProcess.stdout.on('data', rememberLog)
    agentxProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    agentxProcess.once('error', error => {
    ...
    agentxProcess.once('exit', (code, signal) => {
    ...
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(agentxProcess, { readyFile }),
      backendStartFailed
    ])
    if (readyFile) {
      fs.unlink(readyFile, () => {})
    ...
    await Promise.race([waitForAgentX(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null agentxProcess when the child dies.
      childAlive: () => agentxProcess !== null && agentxProcess.exitCode === null && !agentxProcess.killed,
      rememberLog
    })
    updateBootProgress({
      phase: 'backend.ready',
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startAgentX().catch(error => rememberLog(error.stack || error.message))
  })
}
...
ipcMain.handle('agentx:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetAgentXConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote AgentX backend failed liveness probe; dropping stale connection.')
    resetAgentXConnection()
...
ipcMain.handle('agentx:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
...
ipcMain.handle('agentx:window:openNewSession', async () => {
...
ipcMain.handle('agentx:bootstrap:reset', async () => {
  ...
  // reset connection state so the next startAgentX() call restarts the
...
ipcMain.handle('agentx:bootstrap:repair', async () => {
  ...
  // startAgentX() re-runs the full installer (refreshing a broken/partial
...
// it via writeDefaultProjectDir so resolveAgentXCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('agentx:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveAgentXCwd()
}))
...
ipcMain.handle('agentx:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: agentxLog.slice(-200) }))
...
function terminalShellEnv() {
  ...
  // Let a agentx/--tui launched in this pane know it's embedded in the desktop
...
ipcMain.handle('agentx:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart AgentX.')
  }
...
// agentx_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real AgentX version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveAgentXVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'agentx_cli', '__init__.py')
...
ipcMain.handle('agentx:version', async () => ({
  appVersion: resolveAgentXVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  agentxRoot: resolveUpdateRoot()
}))
...
// (agentx_cli/uninstall.py + agentx_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const fallback = () => ({
    ...
    agent_installed: isAgentXSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'agentx_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
...
async function runDesktopUninstall(mode) {
  ...
  // Python with PYTHONPATH=<agentRoot> so `import agentx_cli` resolves from
