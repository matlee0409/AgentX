...
from agentx_constants import agent_browser_runnable, get_agentx_home
from utils import env_int, is_truthy_value
from agentx_cli.config import DEFAULT_CONFIG, cfg_get
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    agentx_node_bin = str(agentx_home / "node" / "bin")
    agentx_node_root = str(agentx_home / "node")
    agentx_nm_bin = str(agentx_home / "node_modules" / ".bin")
    return [agentx_node_bin, agentx_node_root, agentx_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_agentx_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_agentx_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_agentx_home() / "node")),
            ]
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = agentx_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    from agentx_constants import get_agentx_dir
    screenshots_dir = get_agentx_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/agentx-agent:latest")
                else:
