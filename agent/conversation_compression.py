...
def compress_context(
    ...
    *,
    ...
    # long-lived ``agentx_state.SessionDB`` class still bound to the
        ...
        try:
            ...
            if getattr(agent, "_last_compression_lock_error_sid", None) != _lock_sid:
                ...
                logger.warning(
                    ...
                    "process (or `agentx update`) to resync.",
    ...
    if agent._session_db:
        try:
            ...
            if in_place:
                ...
                # from ``agentx_logging._session_context`` (set once per turn in
                ...
                try:
                    agent._session_db.create_session(
                        ...
                        source=agent.platform or os.environ.get("AGENTX_SESSION_SOURCE", "cli"),
                        model=agent.model,
                ...
                try:
                    from agentx_cli.goals import migrate_goal_to_session
    ...
    # engines (e.g. agentx-lcm) use boundary_reason="compression" to preserve
    # DAG lineage / checkpoint per-session state across the boundary instead of
    # re-initializing fresh. See agentx-lcm#68. Built-in ContextCompressor
    ...
    def _shrink_data_url(url: str) -> tuple:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="agentx_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
