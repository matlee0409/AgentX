"""
Microsoft Teams platform adapter for AgentX Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def _fetch_attachment_bytes(self, url: str, timeout: float = 30.0) -> bytes:
        ...
        async with httpx.AsyncClient(
            ...
            response = await client.get(
                url,
                headers={"User-Agent": "Mozilla/5.0 (compatible; AgentXAgent/1.0)"},
            )
            response.raise_for_status()
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        agentx_action = data.get("agentx_action", "")
        session_key = data.get("session_key", "")

        if not agentx_action or not session_key:
        ...
        choice = choice_map.get(agentx_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="agentx_approve",
                    data={**btn_data_base, "agentx_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="agentx_approve",
                    data={**btn_data_base, "agentx_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="agentx_approve",
                    data={**btn_data_base, "agentx_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="agentx_approve",
                    data={**btn_data_base, "agentx_action": "deny"},
                    style="destructive",
                ),
            ])
        )
