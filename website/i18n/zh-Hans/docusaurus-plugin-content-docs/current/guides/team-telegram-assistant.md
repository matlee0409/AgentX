...
本教程将引导你搭建一个由 AgentX Agent 驱动的 Telegram 机器人，供多名团队成员使用。完成后，你的团队将拥有一个共享 AI 助手，可以向它发消息寻求代码、研究、系统管理等方面的帮助——并通过按用户授权保障安全。
...
- **已在服务器或 VPS 上安装 AgentX Agent**（不是你的笔记本——机器人需要持续运行）。如尚未安装，请参阅[安装指南](/getting-started/installation)。
...
2. **发送 `/newbot`**——BotFather 会询问两件事：
   ...
   - **用户名**——必须以 `bot` 结尾（例如 `myteam_agentx_bot`）
