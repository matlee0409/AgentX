...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(agentx_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = agentx_lsp_bin_dir().parent  # <AGENTX_HOME>/lsp/
    ...
    for c in _native_binary_candidates(nm_bin):
        if c.exists():
            ...
            link = agentx_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    ...
    console script.
    """
    pip_target = agentx_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = agentx_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
