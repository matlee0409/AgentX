...
1. `resolve_command()` from `agentx_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
- **Explicit target delivery** — `send_message` tool specifying `telegram:-1001234567890`, or the [`agentx send` CLI](/guides/pipe-script-output) wrapping the same tool for shell scripts
