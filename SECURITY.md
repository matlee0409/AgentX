# AgentX Agent Security Policy

This document describes AgentX Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/NousResearch/agentx-agent/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **AgentX Agent does not operate a bug
bounty program.**
...
AgentX Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running AgentX Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted AgentX Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in AgentX Agent's documentation
...
AgentX Agent supports two OS-level isolation postures. They address
...
AgentX Agent supports this in two ways:

- **AgentX Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, AgentX Agent's in-process heuristics
...
AgentX Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in AgentX Agent
itself. Bugs in AgentX Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   AgentX Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, AgentX Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  AgentX Agent has documented a stance about how its output should
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not AgentX Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in AgentX Agent. Bugs in AgentX
...
- AgentX Agent includes supply-chain guards for MCP server
