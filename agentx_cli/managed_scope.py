...
This is DISTINCT from ``agentx_cli.config.is_managed()`` / ``AGENTX_MANAGED``,
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through agentx_cli.config.load_config) to honor managed
    scope. Mirrors agentx_cli.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from agentx_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
