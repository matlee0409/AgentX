...
Let the agentx agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where agentx runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `agentx meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `agentx meet` CLI |
...
| `cli.py` | `agentx meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/cli.py` | v3: `agentx meet node {run,list,approve,remove,status,ping}` |
...
agentx meet install                                      # pip + Chromium
agentx meet setup                                        # preflight
agentx meet auth                                         # optional
agentx meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
agentx meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.agentx/.env
agentx meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
agentx meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
agentx meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.agentx/.env
agentx meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, agentx will **not** switch your system audio input automatically — the
...
agentx plugins enable google_meet
agentx meet node run --display-name my-mac --host 0.0.0.0 --port 18789
