"""Tests for agentx_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
import agentx_logging
# Use whatever RotatingFileHandler class agentx_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. agentx_logging
...
from agentx_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    agentx_logging._logging_initialized = False
    ...
    agentx_logging._install_session_record_factory()
    yield
    ...
    agentx_logging._logging_initialized = False
    agentx_logging.clear_session_context()
...
def agentx_home(tmp_path, monkeypatch):
    ...
    Uses the same tmp_path as the autouse _isolate_agentx_home from conftest,
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, agentx_home):
        log_dir = agentx_logging.setup_logging(agentx_home=agentx_home)
        assert log_dir == agentx_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, agentx_home):
        agentx_logging.setup_logging(agentx_home=agentx_home)
        root = logging.getLogger()
    ...
    def test_creates_errors_log_handler(self, agentx_home):
        agentx_logging.setup_logging(agentx_home=agentx_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, agentx_home):
        agentx_logging.setup_logging(agentx_home=agentx_home)
        agentx_logging.setup_logging(agentx_home=agentx_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, agentx_home):
        agentx_logging.setup_logging(agentx_home=agentx_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        agentx_logging.setup_logging(agentx_home=agentx_home, force=True)
    ...
    def test_custom_log_level(self, agentx_home):
        agentx_logging.setup_logging(agentx_home=agentx_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, agentx_home):
        agentx_logging.setup_logging(
            agentx_home=agentx_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, agentx_home):
        agentx_logging.setup_logging(agentx_home=agentx_home)
    ...
    def test_writes_to_agent_log(self, agentx_home):
        agentx_logging.setup_logging(agentx_home=agentx_home)

        test_logger = logging.getLogger("test_agentx_logging.write_test")
        ...
        agent_log = agentx_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_warnings_appear_in_both_logs(self, agentx_home):
        agentx_logging.setup_logging(agentx_home=agentx_home)

        test_logger = logging.getLogger("test_agentx_logging.warning_test")
        ...
        agent_log = agentx_home / "logs" / "agent.log"
        errors_log = agentx_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, agentx_home):
        agentx_logging.setup_logging(agentx_home=agentx_home)

        test_logger = logging.getLogger("test_agentx_logging.info_test")
    ...
    def test_reads_config_yaml(self, agentx_home):
    ...
    def test_explicit_params_override_config(self, agentx_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (agentx_home / "config.yaml").write_text(yaml.dump(config))

        agentx_logging.setup_logging(agentx_home=agentx_home, log_level="WARNING")
    ...
    def test_record_factory_installed(self, agentx_home):
        """The custom record factory injects session_tag on all records."""
        agentx_logging.setup_logging(agentx_home=agentx_home)
        factory = logging.getLogRecordFactory()
        assert getattr(factory, "_agentx_session_injector", False), (
            "Record factory should have _agentx_session_injector marker"
        )
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, agentx_home):
        agentx_logging.setup_logging(agentx_home=agentx_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, agentx_home):
        agentx_logging.setup_logging(agentx_home=agentx_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_created_after_cli_init(self, agentx_home):
        ...
        gw_log = agentx_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "gateway connected after cli init" in gw_log.read_text()

    def test_gateway_log_created_after_cli_init_without_duplicate_handlers(self, agentx_home):
    ...
    def test_gateway_log_receives_gateway_records(self, agentx_home):
    ...
    def test_gateway_log_rejects_non_gateway_records(self, agentx_home):
    ...
    def test_agent_log_still_receives_all(self, agentx_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, agentx_home):
        agentx_logging.setup_logging(agentx_home=agentx_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_created_after_cli_init(self, agentx_home):
        agentx_logging.setup_logging(agentx_home=agentx_home, mode="cli")
        agentx_logging.setup_logging(agentx_home=agentx_home, mode="gui")
    ...
    def test_gui_log_receives_only_gui_components(self, agentx_home):
        agentx_logging.setup_logging(agentx_home=agentx_home, mode="gui")

        logging.getLogger("agentx_cli.web_server").info("dashboard online")
        ...
        gui_log = agentx_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, agentx_home):
        """When session context is set, log lines include [session_id]."""
        agentx_logging.setup_logging(agentx_home=agentx_home)
        agentx_logging.set_session_context("abc123")
    ...
    def test_no_session_tag_without_context(self, agentx_home):
    ...
    def test_clear_session_context(self, agentx_home):
        """After clearing, session tag disappears."""
        agentx_logging.setup_logging(agentx_home=agentx_home)
        agentx_logging.set_session_context("xyz789")
        agentx_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, agentx_home):
        ...
        def thread_a():
            agentx_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            for h in logging.getLogger().handlers:
                h.flush()

        def thread_b():
            agentx_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        agentx_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        agentx_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = agentx_logging._ComponentFilter(("gateway",))
    ...
    def test_passes_nested_matching_prefix(self):
        ...
        f = agentx_logging._ComponentFilter(
            agentx_logging.COMPONENT_PREFIXES["gateway"]
        )
        record = logging.LogRecord(
    ...
    def test_multiple_prefixes(self):
        f = agentx_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in agentx_logging.COMPONENT_PREFIXES
        # The gateway component captures core gateway logs, the agentx_plugins
        ...
        # break this test.
        gateway_prefixes = agentx_logging.COMPONENT_PREFIXES["gateway"]
        assert "gateway" in gateway_prefixes
        assert "agentx_plugins" in gateway_prefixes
        assert "plugins.platforms" in gateway_prefixes

    def test_agent_prefix(self):
        prefixes = agentx_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == agentx_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = agentx_logging.COMPONENT_PREFIXES["cli"]
        assert "agentx_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == agentx_logging.COMPONENT_PREFIXES["cron"]

    def test_gui_prefix(self):
        prefixes = agentx_logging.COMPONENT_PREFIXES["gui"]
        assert "agentx_cli.web_server" in prefixes
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, agentx_home):
        agentx_logging.setup_logging(agentx_home=agentx_home)
        agentx_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_agentx_verbose", False)
        ]
        assert len(verbose_handlers) == 1
        assert verbose_handlers[0].level == logging.DEBUG

    def test_idempotent(self, agentx_home):
        agentx_logging.setup_logging(agentx_home=agentx_home)
        agentx_logging.setup_verbose_logging()
        agentx_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        agentx_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = agentx_logging._ComponentFilter(("test",))
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        agentx_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("agentx_cli.config.is_managed", return_value=True):
                agentx_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, agentx_home):
        level, max_size, backup = agentx_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, agentx_home):
        import yaml
    ...
    def test_handles_missing_logging_section(self, agentx_home):
        import yaml
        config = {"model": "test"}
        (agentx_home / "config.yaml").write_text(yaml.dump(config))
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> agentx_logging._ManagedRotatingFileHandler:
        handler = agentx_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that agentx_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        handler.flush()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        agentx_logging.setup_logging(agentx_home=agentx_home, mode="gateway")
        gw_path = agentx_home / "logs" / "gateway.log"
        rotated = agentx_home / "logs" / "gateway.log.1"
...
class TestSafeStderr:
    ...
    def test_returns_stderr_on_utf8_system(self, monkeypatch):
        ...
        result = agentx_logging._safe_stderr()
