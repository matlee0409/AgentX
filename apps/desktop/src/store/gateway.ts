import type { ConnectionState, GatewayEvent } from '@agentx/shared'
import { atom } from 'nanostores'

import { AgentXGateway } from '@/agentx'
...
const isOpen = (gateway: AgentXGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<AgentXGateway | null>(null)
...
let primaryGateway: AgentXGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: AgentXGateway | null, profile = 'default'): void {
...
export function activeGateway(): AgentXGateway | null {
...
function createSecondary(profile: string): Secondary {
  const gateway = new AgentXGateway()
...
export async function ensureActiveGatewayOpen(): Promise<AgentXGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
