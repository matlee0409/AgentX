#!/usr/bin/env python3
"""Canvas LMS API CLI for AgentX Agent.
...
def _check_config():
    ...
    if missing:
        agentx_env = os.path.join(
            os.environ.get("AGENTX_HOME", os.path.expanduser("~/.agentx")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {agentx_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for AgentX Agent"
    )
