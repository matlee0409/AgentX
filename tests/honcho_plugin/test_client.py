...
from agentx_cli.profiles import _get_default_agentx_home
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.agentx/honcho.json
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"agentx": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"agentx": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveConfigPath:
    def test_prefers_agentx_home_when_exists(self, tmp_path):
        agentx_home = tmp_path / "agentx"
        agentx_home.mkdir()
        local_cfg = agentx_home / "honcho.json"
        local_cfg.write_text('{"apiKey": "local"}')

        with patch.dict(os.environ, {"AGENTX_HOME": str(agentx_home)}):
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        ...
        # _get_default_agentx_home() must resolve back to ~/.agentx — that's
        ...
        assert _get_default_agentx_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_agentx_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".agentx".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".agentx"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    ...
    def test_default_returns_agentx(self):
        with patch.dict(os.environ, {}, clear=True):
    ...
    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("AGENTX_HONCHO_HOST", None)
            with patch("agentx_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "agentx_coder"

    def test_default_profile_returns_agentx(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("AGENTX_HONCHO_HOST", None)
            with patch("agentx_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "agentx"

    def test_custom_profile_returns_agentx(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("AGENTX_HONCHO_HOST", None)
            with patch("agentx_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "agentx"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("AGENTX_HONCHO_HOST", None)
            # Temporarily remove agentx_cli.profiles to simulate import failure
            saved = sys.modules.get("agentx_cli.profiles")
            sys.modules["agentx_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "agentx"
            finally:
                if saved is not None:
                    sys.modules["agentx_cli.profiles"] = saved
                else:
                    sys.modules.pop("agentx_cli.profiles", None)
...
class TestProfileScopedConfig:
    ...
    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="agentx")
        assert config.host == "agentx"
        assert config.workspace_id == "agentx"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "agentx": {"aiPeer": "agentx", "peerName": "alice"},
                "agentx_coder": {
                    "aiPeer": "agentx_coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="agentx_coder", config_path=config_file,
        )
        assert config.host == "agentx_coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "agentx_dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="agentx_dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "agentx_dreamer"
    ...
    def test_from_global_config_reads_legacy_dot_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "agentx.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        config = HonchoClientConfig.from_global_config(
            host="agentx_dreamer",
        ...
        assert config.host == "agentx_dreamer"
        assert config.peer_name == "dreamer-user"
        assert config.workspace_id == "agentx_dreamer"
...
class TestObservationModeMigration:
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"agentx": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_agentx_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="agentx",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("agentx_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("agentx_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_agentx_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="agentx",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("agentx_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
