...
sidebar_position: 2
title: "环境变量"
description: "AgentX Agent 使用的所有环境变量完整参考"
---
...
| `AGENTX_KANBAN_HOME` | 覆盖锚定 kanban 看板（数据库 + 工作区 + 工作日志）的共享 AgentX 根目录。回退到 `get_default_agentx_root()`（任意活动 profile 的父目录）。适用于测试和非常规部署 |
...
| 变量 | 描述 |
|----------|-------------|
| `AGENTX_MAX_ITERATIONS` | 每次对话的最大工具调用迭代次数（默认：90） |
...
| `AGENTX_SAFE_MODE` | 故障排查模式：禁用**所有**自定义项——跳过插件发现和 MCP 服务器加载。由 `--safe-mode` 自动设置（同时也会设置上面两个 flag）。 |
| `AGENTX_MD_NAMES` | 自动注入的规则文件名逗号分隔列表（默认：`AGENTS.md,CLAUDE.md,.cursorrules,SOUL.md`）。 |
| `AGENTX_TOOL_PROGRESS` | 工具进度显示的已弃用兼容变量。优先使用 `config.yaml` 中的 `display.tool_progress`。 |
| `AGENTX_TOOL_PROGRESS_MODE` | 工具进度模式的已弃用兼容变量。优先使用 `config.yaml` 中的 `display.tool_progress`。 |
...
| `AGENTX_KANBAN_TASK` | kanban 调度器生成工作进程时设置（任务 UUID）。工作进程和生成的 `agentx-tools` MCP 子进程继承它，以便 kanban 工具正确门控。请勿手动设置。 |
...
| `AGENTX_WRITE_SAFE_ROOT` | 可选目录前缀，限制 `write_file`/`patch` 写入；超出范围的路径需要审批。 |
