...
description: "使用 Nix 安装和部署 AgentX Agent——从快速 `nix run` 到完全声明式的 NixOS 模块（含容器模式）"
---

# Nix & NixOS 安装配置

AgentX Agent 提供了一个 Nix flake，支持三个层级的集成：
...
nix run github:NousResearch/agentx-agent -- chat
...
对于只包含 `plugin.yaml` + `__init__.py` 的源码树插件（例如 [agentx-lcm](https://github.com/stephenschoettler/agentx-lcm)）：
...
对于通过 `[project.entry-points."agentx_agent.plugins"]` 注册的 pip 打包插件（例如 [rtk-agentx](https://github.com/ogallotti/rtk-agentx)）：
