...
- Input history is stored in `~/.agentx/.agentx_history` or under `AGENTX_HOME`.
...
```text
ui-tui/
  packages/agentx-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      agentx-ink.d.ts            type declarations for @agentx/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent AgentXCLI subprocess for slash commands
```