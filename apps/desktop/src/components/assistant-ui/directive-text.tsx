...
const AGENTX_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type AgentXRefType = (typeof AGENTX_REF_TYPES)[number]
...
const ICON_PATHS: Record<AgentXRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as AgentXRefType] ?? ICON_FALLBACK
...
const AGENTX_DIRECTIVE_RE = new RegExp(
...
export const agentxDirectiveFormatter: Unstable_DirectiveFormatter = {
  ...
  const matches = [
    ...
    ...Array.from(text.matchAll(AGENTX_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as AgentXRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: AgentXRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...agentxDirectiveFormatter.parse(text)]
  } catch {
