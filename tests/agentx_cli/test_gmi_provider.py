...
from agentx_cli.auth import resolve_provider
...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from agentx_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        from agentx_cli.config import OPTIONAL_ENV_VARS
...
class TestGmiModelCatalog:
    ...
    def test_provider_model_ids_prefers_live_api(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "agentx_cli.models.fetch_api_models",
    ...
    def test_provider_model_ids_falls_back_to_static_models(self, monkeypatch):
        ...
        monkeypatch.setattr("agentx_cli.models.fetch_api_models", lambda api_key, base_url: None)
...
class TestGmiDoctor:
    ...
    def test_run_doctor_checks_gmi_models_endpoint(self, monkeypatch, tmp_path):
        ...
        try:
            from agentx_cli import auth as _auth_mod
...
class TestGmiAuxiliary:
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        # GMI profile declares default_headers with a AgentXAgent User-Agent
        ...
        assert headers.get("User-Agent", "").startswith("AgentXAgent/")

    def test_gmi_profile_declares_agentx_user_agent(self):
        """The GMI plugin sets a AgentXAgent/<ver> User-Agent on its profile."""
        from providers import get_provider_profile

        profile = get_provider_profile("gmi")
        assert profile is not None
        ua = profile.default_headers.get("User-Agent", "")
        assert ua.startswith("AgentXAgent/"), (
            f"expected GMI profile User-Agent to start with 'AgentXAgent/', got {ua!r}"
        )
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("agentx_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "agentx_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["agentx", "chat", "--provider", "gmi"])

        from agentx_cli.main import main
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("agentx_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("agentx_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
        monkeypatch.setattr("agentx_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
    ...
    def test_model_flow_api_key_provider_persists_gmi_selection(self, monkeypatch):
        ...
        config = yaml.safe_load((get_agentx_home() / "config.yaml").read_text()) or {}
