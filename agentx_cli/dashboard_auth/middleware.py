...
from agentx_cli.dashboard_auth.base import ProviderError, RefreshExpiredError
from agentx_cli.dashboard_auth.cookies import read_session_cookies
from agentx_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    """
    from agentx_cli.dashboard_auth.prefix import prefix_from_request
...
async def gated_auth_middleware(
    ...
    # sends only ``agentx_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if session is None:
        ...
        from agentx_cli.dashboard_auth.cookies import clear_session_cookies
        from agentx_cli.dashboard_auth.prefix import prefix_from_request
