...
Three cookies in play:
  - agentx_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL, ~15 min)
  - agentx_session_rt:   the OAuth refresh token
  ...
  - agentx_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
setter wrote ``__Secure-agentx_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "agentx_session_at"
SESSION_RT_COOKIE = "agentx_session_rt"
PKCE_COOKIE = "agentx_session_pkce"
