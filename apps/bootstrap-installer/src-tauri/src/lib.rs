...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // startup-grace handling used by launch_agentx_desktop — out of
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::agentx_home().join("agentx-agent");
                if bootstrap::agentx_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            // Brief grace so the spawned app is registered
                            // before we exit (mirrors launch_agentx_desktop).
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            update::start_update,
            // Hand-off
            bootstrap::launch_agentx_desktop,
            // Diagnostics
            paths::get_log_path,
            paths::get_agentx_home,
            paths::open_log_dir,
        ])
