"""Persistent slash-command worker — one AgentXCLI per TUI session.
...
def _run(cli: AgentXCLI, command: str) -> str:
...
def main():
    ...
    # Start before the (hundreds-of-ms) AgentXCLI build — that window is itself
    ...
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = AgentXCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
