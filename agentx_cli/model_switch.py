...
- ``agentx_cli.providers``        -- canonical provider identity + overlays
- ``agentx_cli.model_normalize``  -- per-provider name formatting
...
from agentx_cli.providers import (
    ProviderDef,
...
_AGENTX_MODEL_WARNING = (
    "Nous Research AgentX 3 & 4 models are NOT agentic and are not designed "
    "for use with AgentX Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research AgentX 3 / AgentX 4 chat families.
# The previous substring check (`"agentx" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``agentx-brain:qwen3-14b-ctx16k`` that just
# happen to carry "agentx" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/AgentX-3-Llama-3.1-70B, agentx-4-405b, openrouter/agentx3:70b
# Negative examples it must NOT match:
#   agentx-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
...
def is_nous_agentx_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous AgentX 3/4 chat model.
...
def _check_agentx_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous AgentX 3/4 chat model."""
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from agentx_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            ...
            try:
                from agentx_cli.config import validate_config_structure
        ...
        from agentx_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from agentx_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from agentx_cli.config import load_config
            from agentx_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from agentx_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in agentx_cli.runtime_provider.resolve_runtime_provider;
    ...
    agentx_warn = _check_agentx_model_warning(new_model)
    if agentx_warn:
        warnings.append(agentx_warn)
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from agentx_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from agentx_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from agentx_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # https://agentx-agent.nousresearch.com/docs/api/model-catalog.json so
    ...
    if "ollama-cloud" not in curated:
        from agentx_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from agentx_cli.models import fetch_lmstudio_models
    ...
    from agentx_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from agentx_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for agentx_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(agentx_id)
        if (
            _alias_target
            and _alias_target != agentx_id
        ...
        pconfig = PROVIDER_REGISTRY.get(agentx_id)
        ...
        if not has_creds:
            try:
                from agentx_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(agentx_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        model_ids = cached_provider_model_ids(agentx_id)
        if not model_ids:
            model_ids = curated.get(agentx_id, [])
            if agentx_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(agentx_id, model_ids)
        total = len(model_ids)
    ...
    from agentx_cli.providers import AGENTX_OVERLAYS
    from agentx_cli.auth import PROVIDER_REGISTRY as _auth_registry
    ...
    _mdev_to_agentx = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    ...
    for pid, overlay in AGENTX_OVERLAYS.items():
        ...
        agentx_slug = _mdev_to_agentx.get(pid, pid)
        if agentx_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(agentx_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, agentx_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or agentx_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(agentx_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", agentx_slug, exc)
        ...
        # configured.
        if not has_creds and agentx_slug == "anthropic":
            try:
                ...
                if (agentx_creds and agentx_creds.get("accessToken")) or \
        ...
        if agentx_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(agentx_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(agentx_slug)
                model_ids = _ids if _ids else (curated.get(agentx_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(agentx_slug, []) or curated.get(pid, [])
        elif agentx_slug == "nous":
            ...
            try:
                ...
                from agentx_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(agentx_slug, []) or curated.get(pid, [])
                if agentx_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(agentx_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models] if max_models is not None else model_ids

        results.append({
            "slug": agentx_slug,
            "name": get_label(agentx_slug),
            "is_current": agentx_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "agentx",
        ...
        seen_slugs.add(agentx_slug.lower())
        _record_builtin_endpoint(agentx_slug)
    ...
    # with `agentx model`).
    try:
        from agentx_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is AgentX's canonical write key (matches
            ...
            # AgentX writes ``models:`` as a dict keyed by model id
            # (see agentx_cli/main.py::_save_custom_provider); older
            ...
            if should_probe:
                try:
                    from agentx_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # AgentX's own writer uses; a hyphen variant is accepted
            ...
            # run_agent.py, agentx_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`agentx_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from agentx_cli.models import fetch_openrouter_models
