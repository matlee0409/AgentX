...
def memory_env(tmp_path, monkeypatch):
    ...
    memories = agentx_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("AGENTX_HOME", str(agentx_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nAgentX repo is at ~/.agentx/agentx-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
class TestMemoryReset:
    ...
    def test_reset_all_with_yes_flag(self, memory_env):
        ...
        agentx_home, memories = memory_env
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        agentx_home = tmp_path / ".agentx"
        (agentx_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's AGENTX_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_agentx_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
