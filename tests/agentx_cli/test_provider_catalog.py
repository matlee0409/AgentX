"""Tests for the unified provider catalog (agentx_cli.provider_catalog).
...
from agentx_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
...
def test_catalog_covers_every_agentx_model_provider():
    """PARITY CONTRACT: the catalog == the `agentx model` universe."""
    slugs = {d.slug for d in provider_catalog()}
    for entry in CANONICAL_PROVIDERS:
        assert entry.slug in slugs, (
            f"{entry.slug} is shown in `agentx model` but missing from provider_catalog()"
        )


def test_catalog_has_no_providers_outside_agentx_model():
    """The catalog must not invent providers `agentx model` doesn't show."""
