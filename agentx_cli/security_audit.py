"""On-demand supply-chain audit for AgentX Agent installs.
...
def _discover_plugins(agentx_home: Path) -> list[Component]:
...
def run_audit(
    *,
    ...
    home = agentx_home or Path(get_agentx_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, agentx_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(agentx_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, agentx_home=home
    )
    if total == 0:
