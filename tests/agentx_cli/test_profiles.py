"""Comprehensive tests for agentx_cli.profiles module.
...
def profile_env(tmp_path, monkeypatch):
    ...
    * AGENTX_HOME  -> tmp_path/.agentx  (so get_agentx_home() agrees)
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["agentx", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the AgentX install itself or with
...
class TestGetProfileDir:
    ...
    def test_default_returns_agentx_home(self, profile_env):
    ...
    def test_named_profile_returns_profiles_subdir(self, profile_env):
        ...
        assert result == tmp_path / ".agentx" / "profiles" / "coder"

    def test_named_profile_matching_is_case_insensitive(self, profile_env):
        tmp_path = profile_env
        assert get_profile_dir("Coder") == tmp_path / ".agentx" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.agentx must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".agentx"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes agentx-agent, .worktrees,
        ...
        (default_home / "agentx-agent" / ".git").mkdir(parents=True)
        (default_home / "agentx-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "agentx-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "agentx-agent").exists()
...
class TestNoSkillsOptOut:
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `agentx update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestBackfillProfileEnvs:
    ...
    def test_never_overwrites_existing_profile_env(self, profile_env):
        tmp_path = profile_env
        (tmp_path / ".agentx" / ".env").write_text("KEY=root\n")
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("agentx_cli.profiles._cleanup_gateway_service"):
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("agentx_cli.profiles._cleanup_gateway_service"), \
             patch("agentx_cli.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
...
class TestGetActiveProfileName:
    ...
    def test_default_agentx_home_returns_default(self, profile_env):
        # AGENTX_HOME points to tmp_path/.agentx which is the default
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("agentx")
        assert result is not None
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        monkeypatch.setattr("agentx_cli.profiles.shutil.which", lambda name: "/opt/agentx/bin/agentx")
        from agentx_cli.profiles import create_wrapper_script
    ...
    def test_creates_bat_on_windows(self, profile_env, monkeypatch):
        ...
        assert "agentx -p mybot" in content
        assert "%*" in content

    def test_remove_finds_bat_on_windows(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "win32")
        from agentx_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
    ...
    def test_remove_returns_false_when_absent(self, profile_env):
        from agentx_cli.profiles import remove_wrapper_script
    ...
    def test_custom_alias_target_on_posix(self, profile_env, monkeypatch):
        ...
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "agentx -p redqueen" in content
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from agentx_cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_no_alias_returns_none(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from agentx_cli.profiles import find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr("sys.platform", "darwin")
        from agentx_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        with patch("agentx_cli.profiles.check_alias_collision", return_value="skip"):
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "agentx.ssi_health": {"workspace": "agentx", "enabled": True}
            }
        }))
        ...
        assert "agentx.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["agentx_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["agentx_heimdall"]["workspace"] == "agentx"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "agentx.ssi_health": {"aiPeer": "ssi_health"},
                "agentx_heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["agentx.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["agentx_heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("agentx-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".agentx_history",
        ...
        excluded_prefixes = [
            "default/agentx-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.agentx_history",
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultAgentXHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_agentx_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".agentx" / "profiles"

    def test_default_agentx_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_agentx_home()
        assert home == tmp_path / ".agentx"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (AGENTX_HOME outside ~/.agentx), profiles go under AGENTX_HOME."""
    ...
    def test_default_agentx_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_agentx_home() returns AGENTX_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AGENTX_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (AGENTX_HOME under ~/.agentx), profiles root is still ~/.agentx/profiles."""
        native = tmp_path / ".agentx"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under AGENTX_HOME."""
        from agentx_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AGENTX_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".agentx" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from agentx_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".agentx"
    ...
    def test_gateway_running_check_rejects_pid_reused_by_other_profile(self, profile_env):
        ...
        gateway green while ``agentx -p <name> gateway status`` showed it
        stopped.
        ...
        tmp_path = profile_env
        coder_home = tmp_path / ".agentx" / "profiles" / "coder"
    ...
    def test_gateway_running_check_detects_matching_named_profile(self, profile_env):
        ...
        with patch("gateway.status.get_running_pid", return_value=None), patch(
            ...
            "gateway.status._read_process_cmdline",
            return_value="agentx -p coder gateway run --replace",
        ):
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_default(self, profile_env):
        ...
        name, home = serve[0]
        assert name == "default"
        assert home == _get_default_agentx_home()
    ...
    def test_on_returns_default_plus_all_named(self, profile_env):
        ...
        assert serve["default"] == _get_default_agentx_home()
