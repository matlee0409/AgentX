...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_agentx_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# AgentX Agent bash completion
# Add to ~/.bashrc:
...
_agentx_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_agentx_profiles)" -- "$cur"))
        return
    fi
...
_agentx() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_agentx_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# AgentX Agent fish completion",
        "# Add to your config:",
        "#   agentx completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __agentx_profiles",
        "    echo default",
        ...
        " -d 'Profile name' -xa '(__agentx_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__agentx_profiles)' -d 'Profile name'"
                )
