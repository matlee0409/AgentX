...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    try:
        from agentx_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "Try `agentx auth login nous` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def _print_post_register_hint(
    *,
    ...
    from agentx_cli.config import get_env_path
...
def cmd_dashboard_register(args) -> None:
    ...
    from agentx_cli.auth import AuthError, resolve_nous_access_token
    from agentx_cli.config import get_env_value, is_managed, save_env_value
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            ...
            print("  Run `agentx setup` (or `agentx auth login nous`) first, then retry.")
        else:
