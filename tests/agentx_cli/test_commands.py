...
class TestSlackNativeSlashes:
    ...
    def test_agentx_catchall_is_first(self):
        """``/agentx`` must be reserved as the first slot so the legacy
        ``/agentx <subcommand>`` form keeps working after we add new
        commands and hit the 50-slash cap."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "agentx"
    ...
    def test_excludes_slack_reserved_commands(self):
        ...
        Users can still reach them via /agentx <command>."""
        names = {n for n, _d, _h in slack_native_slashes()}
    ...
    def test_includes_aliases_as_first_class_slashes(self):
        ...
        aliases stay reachable via ``/agentx <alias>``.
        """
        slashes = slack_native_slashes()
    ...
    def test_telegram_parity(self):
        ...
        via_agentx_norm = {_norm(n) for n in _SLACK_VIA_AGENTX_ONLY}
        missing = (tg_norm - slack_norm) - reserved_norm - via_agentx_norm
        assert not missing, (
...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_completes_toolset_names(self, monkeypatch):
        ...
        from agentx_cli import commands as commands_mod
        ...
        monkeypatch.setattr(
            "agentx_cli.tools_config._get_plugin_toolset_keys",
            lambda: set(),
        )
...
from agentx_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch agentx_cli.plugins.get_plugin_commands() to a fixed dict."""
        from agentx_cli import plugins as _plugins_mod
    ...
    def test_plugin_command_appears_in_slack_subcommand_map(self, monkeypatch):
        """/agentx metricas must route through the Slack subcommand map."""
        self._patch_plugin_commands(monkeypatch, {
            "metricas": {
                "handler": lambda _a: "ok",
                "description": "Metrics",
