...
import type { AgentXReadDirResult } from '@/global'
import { $connection } from '@/store/session'

import { clearProjectDirCache, readProjectDir } from './ipc'
import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<AgentXReadDirResult>>()
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): AgentXReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: AgentXReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<AgentXReadDirResult>(resolve => {
