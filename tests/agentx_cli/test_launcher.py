...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./agentx` should use `agentx_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "agentx"
    called = []

    fake_main_module = types.ModuleType("agentx_cli.main")

    def fake_main():
        called.append("agentx_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "agentx_cli.main", fake_main_module)
