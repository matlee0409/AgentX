"""
Baked-in build metadata for AgentX Agent.

Source installs report their git revision live via ``git rev-parse`` (see
``agentx_cli/dump.py`` and ``agentx_cli/banner.py``).  That doesn't work inside
...
arg into ``<project_root>/.agentx_build_sha``.  This module is the single
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".agentx_build_sha"


def get_build_sha(short: int = 8) -> Optional[str]:
    """Return the baked-in build SHA, truncated to ``short`` chars, or None.

    Reads ``<project_root>/.agentx_build_sha`` if present.  The file is
