...
sidebar_position: 8
title: "Open WebUI"
description: "Connect Open WebUI to AgentX Agent via the OpenAI-compatible API server"
---

# Open WebUI Integration

[Open WebUI](https://github.com/open-webui/open-webui) (126k★) is the most popular self-hosted chat interface for AI. With AgentX Agent's built-in API server, you can use Open WebUI as a polished web frontend for your agent — complete with conversation management, user accounts, and a modern chat interface.
...
Open WebUI connects to AgentX Agent's API server just like it would connect to OpenAI. AgentX handles the requests with its full toolset — terminal, file operations, web search, memory, skills — and returns the final response.
...
If you want AgentX + Open WebUI wired together locally with a reusable launcher, run:
...
- ensures `~/.agentx/.env` contains `API_SERVER_ENABLED`, `API_SERVER_HOST`, `API_SERVER_KEY`, `API_SERVER_PORT`, and `API_SERVER_MODEL_NAME`
- restarts the AgentX gateway so the API server comes up
- installs Open WebUI into `~/.local/open-webui-venv`
- writes a launcher at `~/.local/bin/start-open-webui-agentx.sh`
- on macOS, installs a `launchd` user service; on Linux with `systemd --user`, installs a user service there

Defaults:

- AgentX API: `http://127.0.0.1:8642/v1`
- Open WebUI: `http://127.0.0.1:8080`
- model name advertised to Open WebUI: `AgentX Agent`

Useful overrides:

```bash
OPEN_WEBUI_NAME='My AgentX UI' \
OPEN_WEBUI_ENABLE_SIGNUP=true \
AGENTX_API_MODEL_NAME='My AgentX Agent' \
bash scripts/setup_open_webui.sh
```
...
6. Enter:
   ...
   - **API Key**: the exact same value as `API_SERVER_KEY` in AgentX
...
This is the default and requires no extra configuration. Open WebUI sends standard OpenAI-format requests and AgentX Agent responds accordingly. Each request includes the full conversation history.
...
With the Responses API, Open WebUI sends requests in the Responses format (`input` array + `instructions`), and AgentX Agent can preserve full tool call history across turns via `previous_response_id`. When `stream: true`, AgentX also streams spec-native `function_call` and `function_call_output` items, which enables custom structured tool-call UI in clients that render Responses events.
...
2. AgentX Agent creates a server-side `AIAgent` instance using the API server's profile, model/provider config, memory, skills, and configured API-server toolsets
...
|----------|-------------|
| `OPENAI_API_BASE_URL` | AgentX Agent's API URL (include `/v1`) |
...
AgentX Agent may be executing multiple tool calls (reading files, running commands, searching the web) before producing its final response. This is normal for complex queries. The response appears all at once when the agent finishes.

### "Invalid API key" errors

Make sure your `OPENAI_API_KEY` in Open WebUI matches the `API_SERVER_KEY` in AgentX Agent.
