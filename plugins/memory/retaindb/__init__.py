...
class RetainDBMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Project resolution: RETAINDB_PROJECT > agentx-<profile> > "default"
        # If unset, the API auto-creates and uses the "default" project — no config required.
        explicit = os.environ.get("RETAINDB_PROJECT")
        if explicit:
            project = explicit
        else:
            agentx_home = str(kwargs.get("agentx_home", ""))
            profile_name = os.path.basename(agentx_home) if agentx_home else ""
        ...
        agentx_home_path = get_agentx_home()
        db_path = agentx_home_path / "retaindb_queue.db"
        self._queue = _WriteQueue(self._client, db_path)

        # Seed agent identity from SOUL.md in background
        soul_path = agentx_home_path / "SOUL.md"
        if soul_path.exists():
