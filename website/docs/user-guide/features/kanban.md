...
Every `agentx kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `agentx chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `agentx_cli.kanban.run_slash()` entry point that reuses the `agentx kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `agentx kanban`. You don't have to leave the chat to drive the board.
