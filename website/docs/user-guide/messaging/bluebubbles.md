...
Connect AgentX to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between AgentX and the BlueBubbles server
...
By default, AgentX responds to every authorized BlueBubbles/iMessage DM or group message. To make group chats opt-in, enable mention gating:
...
With `require_mention: true`, DMs still work normally, but group-chat messages are ignored unless they match a mention pattern. If you do not configure custom patterns, AgentX uses conservative defaults for `AgentX` and `@AgentX agent` variants.
...
When someone messages your iMessage, AgentX automatically sends them a pairing code. Approve it with:
```bash
agentx pairing approve bluebubbles <CODE>
```
...
AgentX will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → AgentX
AgentX → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** AgentX sends messages via the BlueBubbles REST API.
...
| `BLUEBUBBLES_MENTION_PATTERNS` | No | AgentX wake words | JSON array, newline-separated, or comma-separated regex patterns for group mention matching |
...
### Chat Addressing
You can address chats by email or phone number — AgentX resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `agentx logs gateway` for webhook errors (or `agentx logs -f` to follow in real-time)
