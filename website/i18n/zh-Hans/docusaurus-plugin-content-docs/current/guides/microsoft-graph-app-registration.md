...
4. 填写以下内容：
   - **Name：**`AgentX Teams Meeting Pipeline`（或任何你能识别的名称）。
...
3. **Description：**`agentx-graph-secret`。**Expires：**根据你的轮换策略选择合适的值（通常为 6-24 个月）。
4. 点击 **Add**。
...
```powershell
# Create a policy scoped to the AgentX app
New-CsApplicationAccessPolicy `
  -Identity "AgentX-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict AgentX meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "AgentX-Meeting-Pipeline-Policy" `
...
AgentX 内置了 Graph 身份验证冒烟测试。在 AgentX 安装目录下执行：
...
3. 重启 gateway 以使新密钥生效：`agentx gateway restart`。
4. 使用上述冒烟测试进行验证。
5. 在 Azure 门户中删除旧密钥。
