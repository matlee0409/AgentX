...
from agentx_cli.nous_account import NousPortalAccountInfo, NousToolAccessInfo
from agentx_cli import nous_subscription as ns
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    import agentx_cli.setup as setup_mod

    monkeypatch.setattr(setup_mod, "prompt_checklist", _fake_checklist, raising=False)
    monkeypatch.setattr(
        "agentx_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
