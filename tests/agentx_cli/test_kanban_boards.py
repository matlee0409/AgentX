...
def fresh_home(tmp_path, monkeypatch):
    ...
    # Also reset agentx_constants cache so get_default_agentx_root() re-reads.
    try:
        import agentx_constants
        agentx_constants._cached_default_agentx_root = None  # type: ignore[attr-defined]
    except Exception:
        pass
...
class TestPathResolution:
    ...
    def test_env_var_workspaces_override(self, fresh_home, tmp_path, monkeypatch):
        forced = tmp_path / "ws"
        monkeypatch.setenv("AGENTX_KANBAN_WORKSPACES_ROOT", str(forced))
...
class TestCurrentBoard:
    ...
    def test_env_var_takes_precedence(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("AGENTX_KANBAN_BOARD", "envboard")
    ...
    def test_env_beats_file(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("AGENTX_KANBAN_BOARD", "b")
...
class TestConnectionIsolation:
    ...
    def test_connect_stale_env_uses_fallback_board_without_recreating_it(
        ...
        kb.create_board("persist")
        kb.set_current_board("persist")
        monkeypatch.setenv("AGENTX_KANBAN_BOARD", "ephemeral")
...
class TestWorkerSpawnEnv:
    ...
    def test_default_board_spawn_keeps_legacy_paths(self, fresh_home, monkeypatch):
        ...
        env = captured["env"]
        assert env["AGENTX_KANBAN_BOARD"] == "default"
        assert env["AGENTX_KANBAN_DB"] == str(fresh_home / "kanban.db")
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [sys.executable, "-m", "agentx_cli.main", "kanban"] + args,
        env=env,
        capture_output=True,
        text=True,
        cwd=str(_WORKTREE),
