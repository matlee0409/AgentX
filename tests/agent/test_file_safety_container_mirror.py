...
class TestClassifyContainerMirrorTarget:
    def test_returns_none_without_context(self):
        """No Docker context — /root/.agentx/… must not be flagged."""
        from agent.file_safety import classify_container_mirror_target

        assert classify_container_mirror_target("/root/.agentx/profiles/group1/SOUL.md") is None
    ...
    def test_non_agentx_path_not_flagged(self):
        """/root/workspace/… is not .agentx state and must not be blocked."""
        from agent.file_safety import classify_container_mirror_target
