...
sidebar_position: 4
title: "Slack"
description: "Set up AgentX Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect AgentX Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
...
4. Enter an app name (e.g., "AgentX Agent") and select your workspace
5. Click **Create App**
...
[`COMMAND_REGISTRY`](https://github.com/matlee0409/AgentX/blob/main/agentx_cli/commands.py)
...
| **Channels** | Bot **only responds when @mentioned** (e.g., `@AgentX Agent what time is it?`). In channels, AgentX replies in a thread attached to that message. |
...
Make sure the bot has been **invited to the channel** (`/invite @AgentX Agent`).
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @AgentX Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @AgentX Agent` |
...
7. ✅ Bot was **invited** to the channel (`/invite @AgentX Agent`)
