...
import agentx_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `agentx gateway restart`)."""
        unit_path = tmp_path / "agentx-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``/tmp/pytest-of-.../agentx_test`` AGENTX_HOME — silently breaking
    ...
    def test_refresh_refuses_to_bake_any_tempdir_home_into_real_user_unit(
        ...
        """Structural guard: a manual E2E AGENTX_HOME like
        ``/tmp/agentx-e2e-41264`` carries none of the pytest markers but
        poisons the unit identically (seen live 2026-06-11 — an E2E probe ran
        ``agentx gateway restart`` with a /tmp AGENTX_HOME exported; the
        ...
        must refuse ANY temp-dir AGENTX_HOME, not just pytest-shaped ones.
        """
        unit_path = tmp_path / "agentx-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
        ...
        polluted_unit = (
            ...
            "WorkingDirectory=/tmp/agentx-e2e-41264\n"
        )
        monkeypatch.setattr(
            gateway_cli,
            "generate_systemd_unit",
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_var_tmp_home(self):
        unit = '[Service]\nEnvironment="AGENTX_HOME=/var/tmp/agentx-x"\n'
    ...
    def test_detects_tmp_home_in_launchd_plist(self):
        plist = (
            "<dict>\n  <key>AGENTX_HOME</key>\n"
            "  <string>/tmp/agentx-e2e-99999</string>\n</dict>\n"
        )
        assert (
            gateway_cli._temp_home_in_service_definition(plist)
            == "/tmp/agentx-e2e-99999"
        )
    ...
    def test_accepts_macos_real_home_plist(self):
        plist = (
            "<dict>\n  <key>AGENTX_HOME</key>\n"
            "  <string>/Users/alice/.agentx</string>\n</dict>\n"
        )
        assert gateway_cli._temp_home_in_service_definition(plist) is None

    def test_accepts_unit_without_agentx_home(self):
        unit = "[Service]\nExecStart=/usr/bin/python -m agentx_cli.main gateway run\n"
        assert gateway_cli._temp_home_in_service_definition(unit) is None
    ...
    def test_tmp_prefixed_non_temp_path_is_accepted(self):
        ...
        unit = '[Service]\nEnvironment="AGENTX_HOME=/tmpfs-data/.agentx"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is None
...
class TestGeneratedSystemdUnits:
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_agentx_home_for_target_user", lambda home: "/home/alice/.agentx")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        ...
        monkeypatch.setenv("AGENTX_RESTART_DRAIN_TIMEOUT", "9")
        assert gateway_cli._get_restart_drain_timeout() == 9.0

        monkeypatch.setenv("AGENTX_RESTART_DRAIN_TIMEOUT", "invalid")
        assert (
            gateway_cli._get_restart_drain_timeout()
            == DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
        )
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        ...
        # AGENTX_HOME resolves under the (pytest tmp) test AGENTX_HOME.
        monkeypatch.setattr(
            gateway_cli,
    ...
    def test_launchd_install_falls_back_to_detached_on_bootstrap_5(self, tmp_path, monkeypatch, capsys):
        ...
        # (which would trip on the pytest-tmp test AGENTX_HOME) stays out of
        # the way — this test exercises the bootstrap-error fallback.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist><key>AGENTX_HOME</key>"
                "<string>/Users/alice/.agentx</string></plist>"
            ),
        )
    ...
    def test_launchd_fallback_exits_when_spawn_fails(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "nohup agentx gateway run" in out
    ...
    def test_launchd_unsupported_marker_write_and_clear(self, tmp_path, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_agentx_home", lambda: tmp_path)
...
class TestGatewaySystemServiceRouting:
    ...
    def test_systemd_status_surfaces_planned_restart_failure(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_agentx_units", lambda: False)
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: agentx gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup agentx gateway" in out
...
class TestSystemUnitAgentXHome:
    ...
    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: AGENTX_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("AGENTX_HOME", "/root/.agentx/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'AGENTX_HOME=/home/alice/.agentx/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_agentx_home(self, monkeypatch):
        # Custom AGENTX_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("AGENTX_HOME", "/opt/agentx-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'AGENTX_HOME=/opt/agentx-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's AGENTX_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        agentx_home = str(gateway_cli.get_agentx_home().resolve())
        assert f'AGENTX_HOME={agentx_home}' in unit


class TestAgentXHomeForTargetUser:
    """Unit tests for _agentx_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("AGENTX_HOME", raising=False)

        result = gateway_cli._agentx_home_for_target_user("/home/alice")
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._agentx_home_for_target_user("/home/alice")
        assert result == "/home/alice/.agentx/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("AGENTX_HOME", "/opt/agentx")

        result = gateway_cli._agentx_home_for_target_user("/home/alice")
        assert result == "/opt/agentx"
...
class TestProfileArg:
    ...
    def test_default_agentx_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.agentx should not produce a --profile flag."""
        agentx_home = tmp_path / ".agentx"
        agentx_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
        result = gateway_cli._profile_arg(str(agentx_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.agentx/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".agentx" / "profiles" / "mybot"
    ...
    def test_named_profile_under_target_user_root_returns_flag(self, tmp_path):
        ...
        target_root = tmp_path / "home" / "alice" / ".agentx"
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile AGENTX_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "agentx"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.agentx/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".agentx" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".agentx" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_agentx_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_agentx_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
...
class TestRemapPathForUser:
    ...
    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/agentx", str(tmp_path / "alice"))
        assert result == "/opt/agentx"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".agentx" / "agentx-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(gateway_cli, "get_agentx_home", lambda: root_home / ".agentx")
...
class TestDockerAwareGateway:
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'agentx gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'agentx gateway start' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacyAgentXUnitDetection:
    """Tests for _find_legacy_agentx_units / has_legacy_agentx_units.

    These guard against the scenario that tripped Luis in April 2026: an
    older install left a ``agentx.service`` unit behind when the service was
    ...
    The detector must flag ``agentx.service`` ONLY when it actually runs our
    ...
    _OUR_UNIT_TEXT = (
        ...
        "ExecStart=/usr/bin/python -m agentx_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_legacy_agentx_service_in_user_scope(self, tmp_path, monkeypatch):
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = user_dir / "agentx.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        results = gateway_cli._find_legacy_agentx_units()

        assert len(results) == 1
        name, path, is_system = results[0]
        assert name == "agentx.service"
        assert path == legacy
        assert is_system is False
        assert gateway_cli.has_legacy_agentx_units() is True

    def test_detects_legacy_agentx_service_in_system_scope(self, tmp_path, monkeypatch):
        _, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = system_dir / "agentx.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_ignores_profile_unit_agentx_gateway_coder(self, tmp_path, monkeypatch):
        """CRITICAL: profile units must NOT be flagged as legacy.

        Teknium's concern — ``agentx-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "agentx-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "agentx-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "agentx-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
        ...
        assert results == []
        assert gateway_cli.has_legacy_agentx_units() is False

    def test_ignores_unrelated_agentx_service(self, tmp_path, monkeypatch):
        """Third-party ``agentx.service`` that isn't ours stays untouched.

        If a user has some other package named ``agentx`` installed as a
        service, we must not flag it.
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "agentx.service").write_text(
            "[Unit]\nDescription=Some Other AgentX\n[Service]\n"
            "ExecStart=/opt/other-agentx/bin/daemon --foreground\n",
            encoding="utf-8",
        )
    ...
    def test_returns_empty_when_no_legacy_files_exist(self, tmp_path, monkeypatch):
        ...
        assert gateway_cli._find_legacy_agentx_units() == []
        assert gateway_cli.has_legacy_agentx_units() is False
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        user_dir, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "agentx.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "agentx.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m agentx_cli.main gateway run
          - python path/to/agentx_cli/main.py gateway run
          - agentx gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m agentx_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/agentx/agentx_cli/main.py gateway run",
        ...
        for i, execstart in enumerate(variants):
            name = f"agentx.service" if i == 0 else f"agentx.service"  # same name
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "agentx.service"
...
class TestRemoveLegacyAgentXUnits:
    """Tests for remove_legacy_agentx_units (the migration action)."""
    ...
    def test_returns_zero_when_no_legacy_units(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_agentx_units(interactive=False)
    ...
    def test_dry_run_lists_without_removing(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_agentx_units(
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        assert any("--user stop agentx.service" in c for c in cmds_joined)
        assert any("--user disable agentx.service" in c for c in cmds_joined)
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo agentx gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop agentx.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable agentx.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "agentx.service"
        system_legacy = system_dir / "agentx.service"
    ...
    def test_interactive_prompt_no_skips_removal(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_agentx_units(interactive=True)
...
class TestMigrateLegacyCommand:
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        if project_root is None:
            import agentx_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "agentx_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_agentx_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "agentx_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_agentx_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_agentx_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_agentx_units", fake_remove)
...
class TestSystemScopeRemediationOutput:
    ...
    def test_restart_remediation_uses_systemctl_restart(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl restart agentx-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop agentx-gateway" in out
...
class TestServiceWorkingDirIsStable:
    ...
    def test_stable_working_dir_uses_agentx_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".agentx"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_agentx_home", lambda: home)
    ...
    def test_stable_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        # AGENTX_HOME points somewhere that does not exist -> fall back.
        missing = tmp_path / "does-not-exist" / ".agentx"
        monkeypatch.setattr(gateway_cli, "get_agentx_home", lambda: missing)
        assert gateway_cli._stable_service_working_dir() == str(gateway_cli.PROJECT_ROOT)

    def test_user_unit_workingdirectory_is_agentx_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".agentx"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_agentx_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_agentx_home(self, tmp_path, monkeypatch):
        import re
