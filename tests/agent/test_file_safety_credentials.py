...
Regression for https://github.com/NousResearch/agentx-agent/issues/17656 —
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_agentx_home_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "agentx_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_agentx_home_path", lambda: home)
    return home
...
def test_arbitrary_agentx_home_file_not_blocked(fake_home):
...
def test_skills_hub_block_still_applies(fake_home):
    ...
    assert err is not None
    assert "internal AgentX cache file" in err


def test_path_traversal_resolves_to_blocked(fake_home, tmp_path):
    """A path that traverses through a sibling dir back into AGENTX_HOME's
    ...
    sibling.mkdir()
    traversal = sibling / ".." / "agentx_home" / "auth.json"
...
def test_symlink_to_auth_json_blocked(fake_home, tmp_path):
    """A symlink pointing at AGENTX_HOME/auth.json from outside the home
...
def test_read_file_tool_blocks_relative_path_under_terminal_cwd(
    ...
    ``read_file_tool`` against ``TERMINAL_CWD == AGENTX_HOME`` must still
    ...
    _create(fake_home, "auth.json")
    # Force the file_tools resolver to anchor relative paths at AGENTX_HOME
...
def test_dotenv_blocked(fake_home):
    """.env in AGENTX_HOME holds API keys — blocked."""
    from agent.file_safety import get_read_block_error
...
def test_identically_named_agentx_files_outside_home_not_blocked(
    fake_home, tmp_path
):
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(fs, "_agentx_home_path", lambda: profile)
