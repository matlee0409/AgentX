"""Tests for the kanban CLI surface (agentx_cli.kanban)."""
...
from agentx_cli import kanban as kc
...
def test_run_slash_session_filter(kanban_home):
    """`agentx kanban list --session <id>` filters by the originating
...
def test_kanban_in_autocomplete_table():
    from agentx_cli.commands import COMMANDS, SUBCOMMANDS
...
def test_run_slash_board_override_restores_prior_env(kanban_home, monkeypatch):
    kb.create_board("alpha")
    kb.create_board("beta")
    monkeypatch.setenv("AGENTX_KANBAN_BOARD", "beta")

    kc.run_slash("--board alpha list")

    assert os.environ.get("AGENTX_KANBAN_BOARD") == "beta"
