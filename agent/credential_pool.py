...
def _write_through_provider_state_to_global_root(
    ...
    ``agentx_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        try:
            with _auth_store_lock():
                ...
                # agentx_cli.auth._save_xai_oauth_tokens (#43589); the pool
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("agentx_pkce"),
                )
                updated = replace(
                    entry,
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("agentx_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    try:
        from agentx_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
    ...
    if provider == "anthropic":
        ...
        # Claude Code CLI, agentx_pkce creds from a previous OAuth login)
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"agentx_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_agentx_oauth_credentials

        for source_name, creds in (
            ("agentx_pkce", read_agentx_oauth_credentials()),
        ...
        try:
            from agentx_cli.auth import resolve_qwen_runtime_credentials
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    try:
        from agentx_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from agentx_cli.env_loader import get_secret_source
