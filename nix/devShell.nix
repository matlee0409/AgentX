...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      agentxNpmLib = self'.packages.default.passthru.agentxNpmLib;
      ...
      # Non-npm packages may have their own devShellHook (e.g. agentx-agent
    ...
    in
    {
      devShells.default = pkgs.mkShell {
        ...
        shellHook = ''
          echo "AgentX Agent dev shell"
          ${combinedNonNpm}
          ${agentxNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          echo "Ready. Run 'agentx' to start."
        '';
      };
    };
}
