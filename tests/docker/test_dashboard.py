...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    """Without AGENTX_DASHBOARD, s6-svstat should report the dashboard
    slot as DOWN (not up-with-sleep-infinity, which would
    false-positive `agentx doctor` and any other health check).
    ...
    AGENTX_DASHBOARD is unset, so the slot reflects reality.
    """
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name, built_image,
         "sleep", "60"],
    ...
    assert "down" in r.stdout, (
        f"Dashboard slot should be 'down' without AGENTX_DASHBOARD; "
...
def test_dashboard_slot_reports_up_when_enabled(
    ...
    """Symmetry: with AGENTX_DASHBOARD=1, s6-svstat reports the slot as up."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         ...
         # AGENTX_DASHBOARD_OAUTH_CLIENT_ID in this test env), start_server
         ...
         "-e", "AGENTX_DASHBOARD_BASIC_AUTH_USERNAME=admin",
         "-e", "AGENTX_DASHBOARD_BASIC_AUTH_PASSWORD=test-dashboard-pw",
...
def test_dashboard_opt_in_starts(
    ...
    """With AGENTX_DASHBOARD=1, a dashboard process should be visible."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "AGENTX_DASHBOARD=1",
    ...
    ok, _ = _poll(
        container_name, "pgrep -f 'agentx dashboard'", deadline_s=30.0,
    )
    assert ok, "Dashboard should be running with AGENTX_DASHBOARD=1"


def test_dashboard_port_override(
    built_image: str, container_name: str,
) -> None:
    """AGENTX_DASHBOARD_PORT changes the dashboard's listen port."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "AGENTX_DASHBOARD=1", "-e", "AGENTX_DASHBOARD_PORT=9120",
...
def test_dashboard_restarts_after_crash(
    ...
    for _attempt in range(10):
        first_pid_result = docker_exec(
            container_name, "pgrep", "-f", "agentx dashboard",
        )
    ...
    # Kill the dashboard. The dashboard process runs as agentx, so the
    # agentx user can kill it (same UID).
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "AGENTX_DASHBOARD=1",
         "-e", "AGENTX_DASHBOARD_HOST=0.0.0.0",
         "-e", "AGENTX_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
         built_image, "sleep", "120"],
        check=True, capture_output=True, timeout=30,
    )
