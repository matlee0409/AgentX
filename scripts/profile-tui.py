...
bypasses the agentx_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.
...
DEFAULT_LOG = Path(os.environ.get("AGENTX_PERF_LOG", str(get_agentx_home() / "perf.log")))
DEFAULT_STATE_DB = get_agentx_home() / "state.db"
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "agentx-ink" / "src"
