...
as_agentx() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid agentx "$@"; }
...
needs_chown=false
if [ "$(stat -c %u "$AGENTX_HOME" 2>/dev/null)" != "$actual_agentx_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $AGENTX_HOME (targeted) to agentx ($actual_agentx_uid)"
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$AGENTX_HOME/$sub" ]; then
            chown_agentx_tree "$AGENTX_HOME/$sub"
        fi
    done
fi
...
# chown would fail.
if [ -d "$AGENTX_HOME/profiles" ]; then
    chown_agentx_tree "$AGENTX_HOME/profiles"
fi
...
if [ -d "$AGENTX_HOME/cron" ]; then
    chown_agentx_tree "$AGENTX_HOME/cron"
fi
...
# entries of agentx_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
seed_one() {
    ...
    if [ ! -f "$AGENTX_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$AGENTX_HOME/$dest"; then
            ...
            as_agentx cp "$INSTALL_DIR/$src" "$AGENTX_HOME/$dest"
        fi
    fi
}
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_agentx "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
