...
// agentx_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - agentx_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - agentx_session_rt: the OAuth refresh token. Long-lived (24h rotating,
...
const AT_COOKIE_VARIANTS = ['__Host-agentx_session_at', '__Secure-agentx_session_at', 'agentx_session_at']
const RT_COOKIE_VARIANTS = ['__Host-agentx_session_rt', '__Secure-agentx_session_rt', 'agentx_session_rt']
