...
def _failure_marker_path() -> str:
    ...
    return os.path.join(_get_agentx_home(), ".tirith-install-failed")
...
def _agentx_bin_dir() -> str:
    """Return $AGENTX_HOME/bin, creating it if needed."""
    d = os.path.join(_get_agentx_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    ...
    try:
        ...
        dest = os.path.join(_agentx_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    agentx_bin = os.path.join(_agentx_bin_dir(), "tirith")
    if os.path.isfile(agentx_bin) and os.access(agentx_bin, os.X_OK):
        _resolved_path = agentx_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return agentx_bin
