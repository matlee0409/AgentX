...
import { getAgentXConfig, getAgentXConfigDefaults } from '@/agentx'
...
interface AgentXConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useAgentXConfig({ activeSessionIdRef, refreshProjectBranch }: AgentXConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshAgentXConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getAgentXConfig(), getAgentXConfigDefaults().catch(() => ({}))])
  ...
  return { refreshAgentXConfig, sttEnabled, voiceMaxRecordingSeconds }
}
