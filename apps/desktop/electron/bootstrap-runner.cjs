 ...
 *
 * Drives apps/desktop's first-launch install of AgentX Agent by spawning
...
function bootstrapCacheDir(agentxHome) {
  return path.join(agentxHome, 'bootstrap-cache')
}
...
function installedAgentInstallScript(agentxHome) {
  if (!agentxHome) return null
  const candidate = path.join(agentxHome, 'agentx-agent', 'scripts', installScriptName())
  try {
    fs.accessSync(candidate, fs.constants.R_OK)
    return candidate
  } catch {
...
function cachedScriptPath(agentxHome, commit) {
  return path.join(bootstrapCacheDir(agentxHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/NousResearch/agentx-agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(agentxHome, installStamp.commit)
  try {
  ...
  try {
    ...
    const installed = installedAgentInstallScript(agentxHome)
    if (installed) {
      emit({
        type: 'log',
        line:
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, agentxHome } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...
          AGENTX_HOME: agentxHome || process.env.AGENTX_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, agentxHome } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, agentxHome }) {
  const args = ['--dir', activeRoot, '--agentx-home', agentxHome]
  if (installStamp && installStamp.branch) {
    args.push('--branch', installStamp.branch)
  }
  if (installStamp && installStamp.commit) {
...
async function fetchManifest({ scriptPath, installerKind, emit, agentxHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, agentxHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, agentxHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, agentxHome })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(agentxHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, agentxHome, emit })
