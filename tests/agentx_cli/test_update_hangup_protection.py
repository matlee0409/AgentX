...
``_finalize_update_output`` in ``agentx_cli/main.py``.  These exist so
...
class TestInstallHangupProtection:
    ...
    def test_installs_sighup_ignore(self, tmp_path, monkeypatch):
        ...
        # Clear cached get_agentx_home if present
        import agentx_cli.config as _cfg
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_agentx_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "agentx_cli.config.get_agentx_home", _boom, raising=True
        )
