...
import { getProfiles, setApiRequestProfile } from '@/agentx'
...
import type { ProfileInfo } from '@/types/agentx'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.agentxDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
