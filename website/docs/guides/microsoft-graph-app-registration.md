...
4. Fill in:
   - **Name:** `AgentX Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `agentx-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
AgentX ships a Graph auth smoke-test. From your AgentX install:
...
3. Restart the gateway so the new secret is picked up: `agentx gateway restart`.
