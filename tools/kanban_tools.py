...
def _handle_create(args: dict, **kw) -> str:
    ...
    session_id = args.get("session_id") or os.environ.get("AGENTX_SESSION_ID")
    priority = args.get("priority")
    # Resolve workspace. If the caller passed one explicitly, honor it.
    # Otherwise, a dispatcher-spawned worker (AGENTX_KANBAN_TASK set)
    ...
    # (kanban toolset, no AGENTX_KANBAN_TASK) and CLI/dashboard callers
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    - **Gateway** (telegram/discord/slack/etc): ``AGENTX_SESSION_PLATFORM``
      and ``AGENTX_SESSION_CHAT_ID`` are set in ContextVars by the
    ...
    try:
        ...
        notifier_profile = os.environ.get("AGENTX_PROFILE")
