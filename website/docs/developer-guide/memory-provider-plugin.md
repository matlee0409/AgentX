...
description: "How to build a memory provider plugin for AgentX Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give AgentX Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          agentx_home (str): Active AGENTX_HOME path. Use for storage.
        """
...
| `save_config(values, agentx_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
```python
def save_config(self, values: dict, agentx_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(agentx_home) / "my-provider.json"
...
All storage paths **must** use the `agentx_home` kwarg from `initialize()`, not hardcoded `~/.agentx`:

```python
# CORRECT — profile-scoped
from agentx_constants import get_agentx_home
data_dir = get_agentx_home() / "my-provider"
