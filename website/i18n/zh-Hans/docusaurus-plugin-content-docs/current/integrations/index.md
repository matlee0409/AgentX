...
AgentX Agent 可连接外部系统，用于 AI 推理、工具服务器、IDE 工作流、程序化访问等。这些集成扩展了 AgentX 的能力边界与运行环境。
...
- **[IDE 集成（ACP）](/user-guide/features/acp)** — 在兼容 ACP 的编辑器（如 VS Code、Zed 和 JetBrains）中使用 AgentX Agent。AgentX 作为 ACP 服务器运行，在编辑器内渲染聊天消息、工具活动、文件差异和终端命令。
...
- **[构建插件](/guides/build-a-agentx-plugin)** — 创建包含工具、hook 和 CLI 命令的 AgentX 插件的分步指南。
