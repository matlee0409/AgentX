...
from agentx_cli.config import get_agentx_home, get_config_path, load_config, save_config
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_agentx.py script in known locations."""
...
def _cmd_migrate(args):
    ...
    # migrate-agentx posture (two-phase: run once without secrets, rerun with
    ...
    print_info(f"Target:      {agentx_home}")
    ...
    # Delegates to agentx_cli.backup.create_pre_migration_backup(), which
    ...
    if not no_backup:
        try:
            from agentx_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(agentx_home=agentx_home)
            if backup_archive:
