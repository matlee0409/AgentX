...
def _save_mem0_json(agentx_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(agentx_home) / "mem0.json"
    existing = {}
...
def _setup_platform(agentx_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    provider.save_config(provider_config, agentx_home)

    if env_writes:
        _write_env(Path(agentx_home) / ".env", env_writes)
...
def _setup_oss(agentx_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(agentx_home, config)
        return
    ...
    if env_writes:
        _write_env(Path(agentx_home) / ".env", env_writes)
    _save_mem0_json(agentx_home, {"mode": "oss", "user_id": user_id, "agent_id": "agentx", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, agentx_home: str) -> str:
    """Prompt for API key, showing masked existing value if found."""
    existing = os.environ.get(env_var, "")
    if not existing:
        env_path = Path(agentx_home) / ".env"
        if env_path.exists():
...
def _setup_oss_interactive(agentx_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], agentx_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], agentx_home)
        if key:
    ...
    if env_writes:
        _write_env(Path(agentx_home) / ".env", env_writes)
    _save_mem0_json(agentx_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(agentx_home: str, config: dict) -> None:
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(agentx_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(agentx_home, config, flags)
        return
