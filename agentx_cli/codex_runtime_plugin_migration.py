...
def _translate_one_server(
    ...
    if not isinstance(agentx_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(agentx_cfg.get("command"))
    has_url = bool(agentx_cfg.get("url"))
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    codex-routed agentx-tools call fails silently once the directory is GC'd.
...
def _build_agentx_tools_mcp_entry() -> dict:
    ...
    # (not get_agentx_home()) on purpose: when the env var is unset we want
    ...
    agentx_home = os.environ.get("AGENTX_HOME") or ""
    if agentx_home and _looks_like_test_tempdir(agentx_home):
        agentx_home = ""
    if agentx_home:
...
def migrate(
    ...
    *,
    ...
    expose_agentx_tools: bool = True,
) -> MigrationReport:
    """Translate AgentX mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        agentx_config: full ~/.agentx/config.yaml dict
        ...
        expose_agentx_tools: when True (default), register AgentX' own
    ...
    agentx_servers = (agentx_config or {}).get("mcp_servers") or {}
    if not isinstance(agentx_servers, dict):
    ...
    for name, cfg in agentx_servers.items():
    ...
    # The server itself is agent/transports/agentx_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_agentx_tools:
        translated["agentx-tools"] = _build_agentx_tools_mcp_entry()
        if "agentx-tools" not in report.migrated:
            report.migrated.append("agentx-tools")
