#!/usr/bin/env python3
"""Live test harness for AgentX Agent's Tool Search feature.
...
def setup_isolated_home(enabled: bool) -> Path:
    ...
    """
    home_dir = Path(tempfile.mkdtemp(prefix="agentx_ts_live_"))
    agentx_home = home_dir / ".agentx"
    agentx_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, agentx_home / "auth.json")
    ...
    if real_env_file.exists():
        shutil.copy(real_env_file, agentx_home / ".env")
        ...
        load_agentx_dotenv(agentx_home=str(Path.home() / ".agentx"))
    ...
    (agentx_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return agentx_home
