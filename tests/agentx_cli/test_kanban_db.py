"""Tests for the Kanban DB layer (agentx_cli.kanban_db)."""
...
from agentx_cli import kanban_db as kb
...
def test_connect_honors_kanban_busy_timeout_env(kanban_home, monkeypatch):
    ...
    """
    monkeypatch.setenv("AGENTX_KANBAN_BUSY_TIMEOUT_MS", "123456")
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("AGENTX_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import agentx_cli.kanban_db as _kb
...
def test_detect_crashed_workers_skips_freshly_claimed_tasks(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.delenv("AGENTX_KANBAN_CRASH_GRACE_SECONDS", raising=False)
...
def test_detect_crashed_workers_grace_period_env_override(
    ...
    """AGENTX_KANBAN_CRASH_GRACE_SECONDS env var adjusts the window."""
    import agentx_cli.kanban_db as _kb

    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("AGENTX_KANBAN_CRASH_GRACE_SECONDS", "5")
...
def test_resolve_crash_grace_seconds_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv("AGENTX_KANBAN_CRASH_GRACE_SECONDS", bad_val)
...
def test_respawn_guard_rate_limit_cooldown_zero_allows_immediately(
    ...
    monkeypatch.setenv("AGENTX_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "0")
    now = 6_000_000
...
def test_resolve_rate_limit_cooldown_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv(
            "AGENTX_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", bad_val
        )
        assert (
...
def test_dispatch_skips_nonspawnable_into_separate_bucket(kanban_home, monkeypatch):
    ...
    from agentx_cli import profiles
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real AgentX profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="agentx-task", assignee="daily")
...
def test_scratch_workspace_created_under_agentx_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
        assert task is not None
...
def test_dispatch_worktree_task_persists_materialized_workspace_and_branch(kanban_home, tmp_path, monkeypatch):
    ...
    import agentx_cli.profiles as profiles
...
def test_cleanup_workspace_removes_managed_scratch_dir(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        assert ws.is_dir()
        kb.complete_task(conn, t, result="ok")
    assert not ws.exists(), "AgentX-managed scratch dir should be cleaned up"
...
def test_cleanup_workspace_honors_workspaces_root_env_override(tmp_path, monkeypatch):
    """``AGENTX_KANBAN_WORKSPACES_ROOT`` extends the managed-scratch set.
    ...
    monkeypatch.setenv("AGENTX_KANBAN_WORKSPACES_ROOT", str(workspaces_override))
    kb.init_db()
...
class TestSharedBoardPaths:
    ...
    def _set_home(self, monkeypatch, tmp_path, agentx_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AGENTX_HOME", str(agentx_home))
        monkeypatch.delenv("AGENTX_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_agentx(
        self, tmp_path, monkeypatch
    ):
        # Standard install: AGENTX_HOME == ~/.agentx, no profile active.
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # AGENTX_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `agentx -p coder`).
    ...
    def test_docker_custom_agentx_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: AGENTX_HOME points outside ~/.agentx.
        # `get_default_agentx_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".agentx"`.
        custom_root = tmp_path / "opt" / "agentx"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: AGENTX_HOME=/opt/agentx/profiles/coder;
        # `get_default_agentx_root()` walks up to /opt/agentx because
    ...
    def test_explicit_override_via_agentx_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: AGENTX_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".agentx"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AGENTX_HOME", str(profile_home))
        monkeypatch.setenv("AGENTX_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AGENTX_KANBAN_HOME", "   ")
    ...
    def test_agentx_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # AGENTX_KANBAN_DB pins the file path directly and beats both
        # AGENTX_KANBAN_HOME and the `get_default_agentx_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AGENTX_HOME", str(default_home))
        monkeypatch.setenv("AGENTX_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("AGENTX_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows AGENTX_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_agentx_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # AGENTX_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".agentx"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("AGENTX_HOME", str(default_home))
        monkeypatch.setenv("AGENTX_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("AGENTX_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows AGENTX_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AGENTX_HOME", str(default_home))
        monkeypatch.setenv("AGENTX_KANBAN_DB", "   ")
        monkeypatch.setenv("AGENTX_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject AGENTX_KANBAN_DB
        # and AGENTX_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites AGENTX_HOME.
        default_home = tmp_path / ".agentx"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see agentx_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    with _patch("agentx_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="agentx_state"):
            conn = kb.connect()
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_agentx_argv()
#
...
def test_resolve_agentx_argv_prefers_path_shim(monkeypatch):
    """When `agentx` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import agentx_cli.kanban_db as kb

    monkeypatch.delenv("AGENTX_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/agentx")
    argv = kb._resolve_agentx_argv()
    assert argv == ["/usr/local/bin/agentx"]


def test_resolve_agentx_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import agentx_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("AGENTX_BIN", ".\\agentx.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_agentx_argv() == [os.path.abspath(".\\agentx.exe")]


def test_resolve_agentx_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "agentx.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("AGENTX_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_agentx_argv() == [sys.executable, "-m", "agentx_cli.main"]


def test_resolve_agentx_argv_honors_agentx_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like AGENTX_BIN lets service managers pin the executable."""
    import shutil
    import agentx_cli.kanban_db as kb

    shim = tmp_path / "bin" / "agentx"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("AGENTX_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_agentx_argv() == [str(shim)]


def test_resolve_agentx_argv_agentx_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare AGENTX_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import agentx_cli.kanban_db as kb

    cwd_agentx = tmp_path / "agentx"
    cwd_agentx.write_text("wrong\n", encoding="utf-8")
    cwd_agentx.chmod(cwd_agentx.stat().st_mode | stat.S_IXUSR)
    path_agentx = tmp_path / "bin" / "agentx"
    path_agentx.parent.mkdir()
    path_agentx.write_text("right\n", encoding="utf-8")
    path_agentx.chmod(path_agentx.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_agentx.parent))
    monkeypatch.setenv("AGENTX_BIN", "agentx")

    assert kb._resolve_agentx_argv() == [str(path_agentx)]


def test_resolve_agentx_argv_agentx_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare AGENTX_BIN does not accept current-directory shadow executables."""
    import sys
    import agentx_cli.kanban_db as kb

    (tmp_path / "agentx.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_agentx_argv_agentx_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved AGENTX_BIN batch shim is not used as worker argv[0]."""
    import sys
    import agentx_cli.kanban_db as kb
...
def test_resolve_agentx_argv_agentx_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved AGENTX_BIN command names do not delegate cwd search to Popen."""
    import sys
    import agentx_cli.kanban_db as kb
...
def test_resolve_agentx_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m agentx_cli.main`.
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_agentx_argv()
    assert argv == [sys.executable, "-m", "agentx_cli.main"]


def test_resolve_agentx_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `agentx_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m agentx_cli.main --version`
    ...
    assert "AgentX Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="agentx_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_healthy_commit_no_exception(tmp_path):
    ...
    from agentx_cli.kanban_db import connect, write_txn
    db = tmp_path / "test.db"
...
def test_write_txn_raises_on_truncated_file(tmp_path):
    ...
    with pytest.raises(sqlite3.DatabaseError, match="torn-extend|page count mismatch"):
        with unittest.mock.patch("agentx_cli.kanban_db.os.path.getsize", side_effect=fake_getsize):
            with write_txn(conn) as c:
                c.execute(
                    "INSERT INTO tasks (id, title, assignee, status, priority, created_at) "
...
def test_write_txn_post_commit_check_fires_every_call(tmp_path):
    ...
    from agentx_cli.kanban_db import connect, write_txn
    import agentx_cli.kanban_db as kanban_db_module
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
    call_count = 0
...
def test_connect_sets_wal_autocheckpoint_100(tmp_path):
    ...
    from agentx_cli.kanban_db import connect
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    import struct
    from agentx_cli.kanban_db import connect, _check_file_length_invariant
...
def test_reap_worker_zombies_returns_count():
    ...
    with patch("agentx_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("agentx_cli.kanban_db._record_worker_exit"):
...
def test_reap_worker_zombies_noop_on_windows(monkeypatch):
    ...
    monkeypatch.setattr("agentx_cli.kanban_db.os.name", "nt")
    with patch("agentx_cli.kanban_db.os.waitpid") as mock_waitpid:
...
def test_reap_worker_zombies_noop_no_children():
    ...
    with patch("agentx_cli.kanban_db.os.waitpid", side_effect=ChildProcessError):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_reap_worker_zombies_records_exit_status():
    ...
    with patch("agentx_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch(
            "agentx_cli.kanban_db._record_worker_exit",
...
def test_reap_worker_zombies_handles_waitpid_os_error():
    ...
    with patch("agentx_cli.kanban_db.os.waitpid", side_effect=OSError("test error")):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_zombie_reaper_survives_all_boards_failing():
    ...
    for tick in range(5):
        ...
        with patch(
            "agentx_cli.kanban_db.os.waitpid", side_effect=make_fake_waitpid(pids)
        ):
            with patch("agentx_cli.kanban_db._record_worker_exit"):
                pids = kb.reap_worker_zombies()
...
def test_dispatch_once_still_reaps_via_extracted_fn(kanban_home):
    ...
    with patch("agentx_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("agentx_cli.kanban_db._record_worker_exit"):
            with patch("agentx_cli.kanban_db.os.name", "posix"):
                pids = kb.reap_worker_zombies()
