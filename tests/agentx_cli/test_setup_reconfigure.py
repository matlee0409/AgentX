...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("agentx_cli.setup.ensure_agentx_home", {}),
        ("agentx_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("agentx_cli.config.is_managed", {"return_value": False}),
        ("agentx_cli.setup.load_config", {"return_value": {}}),
        ("agentx_cli.setup.save_config", {}),
        ("agentx_cli.setup.get_env_value", {"return_value": None}),
        ("agentx_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("agentx_cli.setup._print_setup_summary", {}),
        ("agentx_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("agentx_cli.setup.load_config", {"return_value": {}}),
        ("agentx_cli.setup.save_config", {}),
        ("agentx_cli.auth.get_active_provider", {"return_value": None}),
        ("agentx_cli.setup.get_env_value", {"return_value": None}),
        ("agentx_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="agentx_cli.setup.prompt_choice",
                quick="agentx_cli.setup._run_quick_setup",
                model="agentx_cli.setup.setup_model_provider",
                terminal="agentx_cli.setup.setup_terminal_backend",
                agent="agentx_cli.setup.setup_agent_settings",
                gateway="agentx_cli.setup.setup_gateway",
                tools="agentx_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`agentx setup --reconfigure` behaves the same as bare `agentx setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("agentx_cli.setup.prompt_choice", {"return_value": 0}),
                first="agentx_cli.setup._run_first_time_quick_setup",
            )
            from agentx_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "agentx_cli.setup.run_setup_wizard",
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["agentx", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["agentx", "setup"])
        try:
            main()
        except SystemExit:
            pass
