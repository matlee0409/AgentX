"""Unit tests for agentx_cli.managed_scope (resolver + loaders + key helpers)."""
import textwrap
...
def test_get_managed_dir_empty_override_falls_through(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("AGENTX_MANAGED_DIR", "   ")  # whitespace = unset
    # Under pytest the /etc/agentx default is ignored, so this is None; the
...
def test_get_managed_dir_default_ignored_under_pytest(monkeypatch):
    ...
    monkeypatch.delenv("AGENTX_MANAGED_DIR", raising=False)
    assert managed_scope.get_managed_dir() is None
