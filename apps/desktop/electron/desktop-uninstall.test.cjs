...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'agentx_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'agentx_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'agentx_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/AgentX.app/Contents/MacOS/AgentX', 'darwin'),
    '/Applications/AgentX.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/AgentX.app/Contents/MacOS/AgentX', 'darwin'),
    '/Users/x/Applications/AgentX.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\AgentX\\AgentX.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\AgentX'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\agentx-desktop\\AgentX.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\agentx-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\AgentX.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_AgentXXXXX/agentx', 'linux', { APPIMAGE: '/home/x/Apps/AgentX.AppImage' }),
    '/home/x/Apps/AgentX.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(resolveRemovableAppPath('/opt/agentx/linux-unpacked/agentx', 'linux', {}), '/opt/agentx/linux-unpacked')
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/AgentX.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/AgentX.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    ...
    uninstallArgs: ['-m', 'agentx_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/agentx/linux-unpacked',
    agentxHome: '/home/x/.agentx'
  })
  ...
  assert.match(script, /'-m' 'agentx_cli\.uninstall' '--mode' 'gui'/)
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    agentRoot: '/home/x/.agentx/agentx-agent',
    uninstallArgs: ['-m', 'agentx_cli.uninstall', '--mode', 'full'],
    appPath: null,
    agentxHome: '/home/x/.agentx'
  })
  // System python + source on PYTHONPATH so import agentx_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.agentx\/agentx-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'agentx_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'agentx_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    agentxHome: '/h'
  })
  assert.doesNotMatch(script, /rm -rf '\//)
  // Still runs the uninstall.
  assert.match(script, /'-m' 'agentx_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\agentx',
    uninstallArgs: ['-m', 'agentx_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\AgentX',
    agentxHome: 'C:\\Users\\x\\AppData\\Local\\agentx'
  })
  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import agentx_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\agentx;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "agentx_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\AgentX" >nul 2>&1/)
