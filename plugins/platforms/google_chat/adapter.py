...
# ``agentx_plugins.platforms__google_chat.adapter`` once the plugin
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        try:
            ...
            _agentx_home = _get_agentx_home()
        except (ModuleNotFoundError, ImportError):
            _agentx_home = _Path.home() / ".agentx"
        self._thread_count_store = _ThreadCountStore(
            _agentx_home / "google_chat_thread_counts.json"
        )
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        ...
        body: Dict[str, Any] = {"text": "AgentX is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
...
def interactive_setup() -> None:
    ...
    The setup wizard at ``agentx_cli/gateway.py`` calls this for plugin
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'agentx-agent[google_chat]'",
