...
**→ [构建 AgentX Plugin](/guides/build-a-agentx-plugin)** — 包含完整可运行示例的分步指南。
...
| 通过 pip 分发 | `[project.entry-points."agentx_agent.plugins"]` |
...
| pip | `agentx_agent.plugins` entry_points | 分发包 |
...
| 想要添加… | 方式 | 编写指南 |
|---|---|---|
| LLM 可调用的**工具** | Python 插件 — `ctx.register_tool()` | [Build a AgentX Plugin](/guides/build-a-agentx-plugin) · [Adding Tools](/developer-guide/adding-tools) |
| **生命周期 hook**（LLM 前后、会话开始/结束、工具过滤） | Python 插件 — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a AgentX Plugin](/guides/build-a-agentx-plugin) |
| CLI / gateway 的**斜杠命令** | Python 插件 — `ctx.register_command()` | [Build a AgentX Plugin](/guides/build-a-agentx-plugin) · [Extending the CLI](/developer-guide/extending-the-cli) |
...
完整的处理器约定、schema 格式、hook 行为、错误处理和常见错误请参见 **[完整指南](/guides/build-a-agentx-plugin)**。