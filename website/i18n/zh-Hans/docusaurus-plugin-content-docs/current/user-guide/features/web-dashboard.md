...
Web Dashboard 是一个基于浏览器的 UI，用于管理你的 AgentX Agent 安装。无需编辑 YAML 文件或运行 CLI 命令，即可通过简洁的 Web 界面配置设置、管理 API 密钥并监控会话。
...
前端使用 React 19、TypeScript、Tailwind CSS v4 和 shadcn/ui 风格组件构建。生产构建输出到 `agentx_cli/web_dist/`，由 FastAPI 服务器作为静态 SPA 提供服务。
